...
This document is the execution guide for porting the upstream `firstmate` project (vendored at `firstmate/`) into **Multplx** — a standalone, customized orchestrator per the vision in `CLAUDE.md`. It sequences the fourteen plan artifacts in `plans/` (plans 01–11 are the port; plans 12–14 are post-port improvements that don't gate it), explains how to port safely, and defines the testing discipline for every phase.
...
Parallelization notes: 03/04/05 are a strict chain; 6.5 lands after 06 and before any new plan-07 coverage; 07 and 08 are independent of each other after that test-infrastructure boundary; 08 can otherwise be built any time after 02; 11 is strictly last within the port. Post-port plans 12/13 are independent of each other; 14 needs 11. Everything else respects the numeric order.
...
- All twelve port steps (01–11 plus 6.5) landed; `plans/` updated with any deviations. (Plans 12–14 are post-port improvements — tracked here, but they do not gate the port.)
