...
4. Keep the `firstmate/` folder as a read-only, test-excluded reference until all fifteen planned phases are complete, then remove it (maintainer direction 2026-07-30; see the Decisions record, §5.8). The end state is a single Multplx tree at the repo root with no vendored upstream copy.
...
- After all fifteen planned phases are complete, `firstmate/` is deleted from the repo and the Multplx tree at the root is the only copy. **Precondition:** the upstream fork-point SHA is recorded in `docs/upstream.md` (Phase 0 step, required by plan 14) before the folder goes.
...
8. **End state: `firstmate/` is removed after all fifteen planned phases** (timing updated by the maintainer 2026-07-30). All renamed/rebranded files live at the repo root, while the vendored reference stays read-only and excluded from Multplx tests until that final boundary. ("The Multplx tree" always means the repo root, not a subfolder.)
