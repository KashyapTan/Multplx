...
Codex App is not a selectable Multplx runtime backend.
Codex Desktop host tools can create and coordinate visible threads and those threads can write Multplx status files when given an authorized path, but Multplx has no supported shell-callable bridge to those host tools.
...
3. Read enough live state or bounded transcript to coordinate the task.
4. Archive, kill, or otherwise stop the exact endpoint.
5. Let the thread append Multplx's normal lifecycle lines to `state/<id>.status`.

The status return channel is mandatory.
A visible thread that cannot report into Multplx's normal lifecycle is not a complete backend.

## Current blocker

Multplx backend scripts are shell entry points and can call tmux, Herdr, and cmux directly.
Codex Desktop host tools are available to a Desktop conversation, not to arbitrary Multplx subprocesses.
...
A one-process probe could create and archive a thread record, but no supported bridge was found that lets Multplx create, continue, read, and archive the same visible Desktop-owned endpoint over its full lifetime.
...
Once available, Multplx should add a real `bin/backends/codex-app.sh`, persist `backend=codex-app` and `codex_app_thread_id=`, and route spawn, send, peek, watch, and cleanup through the shared dispatcher.
