...
It is off by default, and the last `/calm` choice persists for the effective Multplx home across Pi session starts and resumes.

While Calm is active, Pi's built-in `Working...` activity remains visible and no separate Calm status row is added.
Calm hides collapsed thinking labels, the shells for Pi's seven built-in tools, the `mx_watch_arm_pi` tool shell, and canonically classified Multplx operational user rows.
...
Every hidden Multplx input remains available to the model and in serialized session data and exported artifacts.
...
`.pi/extensions/lib/mx-calm-visibility.ts` owns the visibility policy, and `.pi/extensions/lib/mx-calm-operational-user-layout.ts` owns the zero-height operational-user row adapter.
