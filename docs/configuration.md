...
The shared orchestrator behavior lives in [`AGENTS.md`](../AGENTS.md) - edit it like any prompt when the system is empty, or dispatch shared-repo edits to an actor while tasks are in flight.
...
`config/` holds local gitignored operating choices, and `projects/` holds the local project clones that Multplx reads but changes only through the guarded exceptions in `AGENTS.md`.
...
That delta is owned in code by `mx_backend_required_tools` in `bin/mx-backend.sh`: the resolved backend's own session-provider CLI (`tmux`, `herdr`, or `cmux`), `jq` for the JSON-emitting experimental adapters (`herdr`, `cmux`) whose spawn and liveness paths parse the backend's JSON output, and the `treehouse` worktree provider for every session-provider-only backend (`tmux`, `herdr`, `cmux`).
...
MX_CLASSIFY_PAUSED_VERB=paused     # leading status verb for a declared external wait; excluded from MX_MAINTAINER_RE and distinct from blocked
