...
`bin/mx-guard.sh`, `bin/mx-turnend-guard.sh`, the watcher lock, and the watcher beacon still prove whether supervision is healthy after an allowed call.
...
```text
bin/mx-watch-arm.sh          (arm; blessed entry point)
bin/mx-watch-checkpoint.sh   (checkpoint; blessed entry point)
bin/mx-watch.sh              (watch; protected but never blessed)
```

The relative form, the `<code-root>`-anchored absolute form, and any word ending in `/bin/<script>` all resolve to that identity.
Suffix matching recognizes an expanded-path prefix statically, so `$MX_HOME/bin/mx-watch-arm.sh`, `$HOME/broker/bin/mx-watch-arm.sh`, and `~/broker/bin/mx-watch-arm.sh` are the arm identity.
The classifier never expands the variable or tilde; it matches the literal bytes only.
Static quote forms are cooked before the suffix match, so a command word split by ordinary quotes (`mx-"watch"-arm.sh`), ANSI-C quoting (`mx-$'\x77'atch-arm.sh`), or a bash locale string (`mx-$"watch"-arm.sh`) all resolve to the same identity; this reads the fixed literal bytes as the shell would cook them and never runs an expansion or a command.
...
`bin/mx-watch.sh` is protected but is not a blessed entry point.
A direct `bin/mx-watch.sh` execution - relative, `<code-root>`-anchored, `$VAR`-prefixed, or `~`-prefixed - always denies with `watcher-direct`, whose reason points the caller at `bin/mx-watch-arm.sh` and `bin/mx-watch-checkpoint.sh`.
...
`bin/mx-watch-arm.sh` and `bin/mx-watch-checkpoint.sh` are the only blessed final nodes, including their expanded-path forms; a `bin/mx-watch.sh` final node is never blessed and denies with `watcher-direct`.
...
An actually executed `pkill` command is denied when its parsed pattern arguments target `mx-watch`.
Path-qualified `pkill`, `command pkill`, and `sudo pkill` are recognized.

`kill "$(pgrep -f '/bin/mx-watch.sh')"` is also denied because the executed `kill` consumes an executed watcher-wide `pgrep` substitution.
A standalone read-only `pgrep` is allowed.
Quoted text such as `echo 'pkill -f mx-watch'` is data and is allowed.
...
When the command carries such grammar and its raw bytes reference both a `mx-watch` target and a `pkill` or `kill` verb, the classifier cannot prove which command position the kill occupies, so it denies with `broad-watcher-kill` rather than allowing.
This backstop mirrors the protected-execution fail-closed rule and covers forms like `while true; do pkill -f mx-watch; done`, `for x in 1; do pkill -f mx-watch; done`, `case x in x) pkill -f mx-watch ;; esac`, and `until false; do kill $(pgrep -f mx-watch); done`.
It is gated on the grammar being unsupported: in grammar the classifier does model, command-position analysis is authoritative, so data mentions such as `echo 'pkill -f mx-watch'` and a loop that only names the watcher without a kill verb such as `for f in 1; do echo mx-watch; done` remain allowed.
...
| `watcher-direct` | A direct `bin/mx-watch.sh` execution; the watcher must be reached through `bin/mx-watch-arm.sh` or `bin/mx-watch-checkpoint.sh`. |
...
`tests/mx-arm-pretool-check.test.sh` owns the adversarial acceptance matrix.
Every row runs through Codex-shaped stdin, Claude-shaped stdin, and Pi-shaped CLI entry forms.
