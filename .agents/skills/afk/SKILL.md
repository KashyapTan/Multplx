...
name: afk
description: >-
  Enter away-mode supervision when the maintainer invokes /afk, says they are going afk, `state/.afk` exists, an incoming message starts with `MX_INJECT_MARK`, or any `state/.subsuper-*` marker is involved.
  It sets a durable away-mode flag so the sub-supervisor daemon can self-handle routine wakes and escalate maintainer-relevant events plus bounded declared-external-wait rechecks as batched digests during walk-away stretches, then exits automatically when any real unmarked message returns broker to full per-wake responsiveness.
user-invocable: true
metadata:
  internal: true
---
...
tradeoff **consented** and **explicit**: the maintainer is stepping away, so the
sub-supervisor may triage routine wakes in bash instead of waking broker's
LLM for each one. Escalations still reach the maintainer, but as one pre-read,
...
2. **Ensure the sub-supervisor daemon is running as a tracked background process.**
   ...
   - **Harness WITH a native in-pane tracked-background tool** (e.g. claude's
     ...
     `bin/mx-afk-launch.sh start-native`, then run
     `MX_AFK_STATE_PREPARED=1 bin/mx-afk-start.sh` through that native tool.
     This is a deliberate no-separate-terminal exception because the harness-hosted job creates no terminal or layout mutation, and a shell launcher cannot invoke a harness-native background tool.
     The launcher still owns lifecycle state and records the no-terminal mode, while the daemon inherits and auto-discovers the maintainer pane.
     If the native launch fails, run `bin/mx-afk-launch.sh stop` to roll back the prepared lifecycle.
     Do not wrap it in `nohup ... &` (Codex/herdr can reap fire-and-forget shell children after a tool call returns).
   - **Harness WITHOUT one** (e.g. pi): run `bin/mx-afk-launch.sh start`. It is
     ...
     a detached tmux session), records its exact id, and passes the maintainer pane
     in as `MX_SUPERVISOR_TARGET` so the daemon injects into the maintainer, not its
     own new pane. **Never manufacture a terminal by splitting the maintainer's
     active pane** (`herdr pane split`): a split co-tenants the tab and visibly
     shrinks the maintainer's pane (docs/herdr-backend.md "Away-mode supervisor
     support").
   Both paths share `bin/mx-afk-start.sh` as the daemon entry.
   ...
   It exits immediately if the identity-backed daemon lock already names a live process, otherwise it execs `bin/mx-supervise-daemon.sh` in the foreground.
...
- A message **without** the current operational prefix or a legacy bare marker, and **not** starting with `/afk` -> the maintainer is back.
  Run `bin/mx-afk-return.sh` before acting on the message that brought the maintainer back.
  ...
  If it reports a broker-actionable `blocked:` event, remediate it immediately through the normal lifecycle, or explicitly reclassify it with a durable reason and close its decision key with `resolved [key=...]`, then run `bin/mx-afk-return.sh check`.
  ...
  Do not answer a Catchup request or perform any other ordinary maintainer work until the check exits successfully.
- A message **with** the current operational prefix (`MX_OPERATIONAL_PREFIX`, U+2063 INVISIBLE SEPARATOR followed by `MULTPLX_OP: `), or a legacy bare `MX_INJECT_MARK` daemon escalation -> stay afk and process it.
- Re-invoking `/afk` while already away -> stay afk (refresh the flag); this
  does **not** trigger an exit.

Bias ambiguous cases toward exit: a present maintainer beats token savings, and
a false exit is self-correcting (the maintainer re-runs `/afk`).
...
The daemon constructs every current injection as the `away-supervisor` kind owned by `bin/mx-operational-input.sh`, beginning with `MX_OPERATIONAL_PREFIX`: `MX_INJECT_MARK` (U+2063 INVISIBLE SEPARATOR) followed by the stable `MULTPLX_OP: ` label.
The bare `MX_INJECT_MARK` form remains accepted for legacy daemon escalations during rollout.
U+2063 has no normal keyboard keystroke and survives terminal transport as UTF-8 text.
This is how broker tells a daemon escalation apart from a real message in the same pane.
...
injection, dispatched through `bin/mx-backend.sh` for the supervisor's own
backend (tmux or herdr; see "Auto-discovered supervisor pane" below):

- **`pane_is_busy`** - the harness shows a busy footer (agent mid-turn) on tmux (shared with `mx-send.sh` via `bin/mx-tmux-lib.sh`); on herdr, tries the native `agent.get`-backed busy state first, trusts only `busy` outright, and corroborates every non-`busy` verdict with the same regex-over-capture reader.
- **Composer-state guard** - `inject_msg` reads the full `empty`/`pending`/`unknown` verdict from `mx_backend_composer_state` and injects only when it is affirmatively `empty`.
  ...
  The shared `bin/mx-composer-lib.sh` owns the content decision after each backend captures and structurally identifies its own composer row.
...
In afk mode the composer guard is belt-and-suspenders (no human is typing), but it protects against the race window between the maintainer returning and their message landing, a dead shell, and the daemon's own previous injection sitting unsent.

**Max-defer escape (the daemon must never silently wedge).**
If anything stays buffered past `MX_MAX_DEFER_SECS` (default 300), the daemon
...
`docs/configuration.md` "Away-mode wedge alarm channels" owns the alert channel setup, and `docs/verification/supervision.md` "Wedge-alarm channels" owns active evidence.
...
`mx-send.sh` uses the same primitive and exits non-zero
when a steer's Enter is positively swallowed, so broker learns an instruction
...
busy pane. The shared `mx_tmux_submit_enter_core` falls back to
`mx_pane_is_busy` once the Enter-retry budget is spent: a busy pane means the
...
and the lenient-`pending`-fails-for-`mx-send` policy still holds for steer
...
- `signal` with a terminal maintainer verb (`done:`, `needs-decision:`, `blocked:`, or `failed:`) -> escalate.
  ...
  Other signals with no maintainer-relevant status -> self-handle.
- `signal` or `stale` for a declared `paused:` external wait -> self-handle and track the pause rather than a wedge.
  If it remains declared and idle past `MX_PAUSE_RESURFACE_SECS` (default 3600s), housekeeping sends one awaiting-external recheck and resets the pause window.
- `check` -> always escalate. Check scripts print only when broker should wake.
- `stale` with a terminal status or bare legacy maintainer-relevant line -> escalate.
  Nonterminal progress remains transient even when its prose contains a legacy free-text token or its seen-status marker already matches, so record a marker and self-handle.
  If the pane is still idle past `MX_STALE_ESCALATE_SECS` (default 240s), housekeeping escalates it as a possible wedge.
  This bounds wedge-detection latency to the threshold plus a tick: a delay, never a loss.
  Healthy actors are autonomous and do not wait on broker mid-task.
- `heartbeat` -> self-handle. The daemon runs its own cheap bash system scan
  every `MX_HEARTBEAT_SCAN_SECS` (default 300s) as the catch-all for a
  maintainer-relevant status line the per-wake classifier might miss.
- Unknown reason, or any uncertainty -> escalate fail-safe.

Escalations are buffered up to `MX_ESCALATE_BATCH_SECS` (default 90s; 0 =
...
the operational prefix lets broker distinguish it from a real maintainer message.
...
- **Single-line digest** - embedded newlines are collapsed to a literal
  ...
  harness.
- **Composer guard on the supervisor pane** - before injecting, the daemon checks `pane_is_busy` (harness busy footer means agent mid-turn) and reads `mx_backend_composer_state` directly.
  ...
  Every other result preserves the buffer for retry, so the daemon never merges its digest into the maintainer's half-typed line or types it into a shell.
- The shared composer classifier receives a candidate row only after the active backend performs its own capture and structural row recognition.
  tmux and herdr route their raw styled candidate rows through the shared `mx_composer_strip_ghost` extractor, which removes dim/faint and dark-TRUECOLOR ghost/placeholder text before classification.
  ...
  `MX_COMPOSER_IDLE_RE` still overrides tmux empty-composer matching after shared ghost and border stripping, and `MX_BUSY_REGEX` overrides busy footers.
- **Max-defer escape** - the daemon must never silently wedge. If anything stays
  buffered past `MX_MAX_DEFER_SECS` (default 300s), the daemon attempts one
...
- **Marker strip** - `strip_injection_marker` removes the current operational
  ...
  text broker sees is clean.
- **Portable singleton lock** - the daemon uses the repo's portable lock helper
  (`mx-wake-lib.sh`) instead of `flock`, which is absent on macOS.
- **Dedupe across signal/stale/scan** - `classify_signal` and terminal `classify_stale` paths check the seen-status marker before escalating, so a maintainer-relevant status escalated by one path is not re-escalated by another in the same digest.
...
Always enter through `bin/mx-afk-launch.sh`, which clears prior-session artifacts only for a fresh entry and preserves the current session's buffer on refresh.
Always exit through `bin/mx-afk-launch.sh stop`, which keeps `state/.afk` present through the daemon's shutdown flush and clears it last.
...
- Nothing is lost. The durable queue plus `mx-wake-drain.sh` recover any missed
...
`MX_INJECT_SKIP` (default `heartbeat`) force-self-handles matching kinds,
