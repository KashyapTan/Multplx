---
name: catchup
description: Generate a "pick up where I left off" status report from broker's live system state. Use when the maintainer invokes /catchup or asks for a catchup report, morning brief, status report, catch-up, "where did I leave off", or "what's in the works". Reads bounded local system state cheaply, optionally checks open PRs when requested, composes a scannable dated report to data/status-report-<YYYY-MM-DD>.md, and surfaces a concise version in chat; it is read-mostly and must not tear down, merge, or mutate task state as a side effect of producing the brief.
user-invocable: true
metadata:
  internal: true
---
...
Generate a complete standalone snapshot from the system's current state, so the maintainer can resume in one read after a break, a night, or a context reset.
...
It reads system state and writes exactly one report file.
It never tears down a task, merges a PR, dispatches new work, or mutates any task state as a side effect of producing the brief - those belong to the maintainer's explicit word and the normal task lifecycle.
...
1. **Gather live system state with one deterministic command.**
   ...
   For registered daemons, use the snapshot's structured-home classification and provenance; a parent event or bounded terminal contradiction is fallback evidence, never authority over readable structured home state.
   Structured maintainer-held decisions come from `decision-hold-lifecycle` and appear under `decisions_open`; do not scrape reports or visual-review artifacts to supplement them.
   ...
   The `(main-inventory)` gate is an action-free integrity warning rather than queued work: render it under Charted Next with the related `omitted` disclosure, never invent an Underway row from backlog-only state, and never move it into Maintainer's Call.
...
This skill is the one owner of the `/catchup` chat-response format; the snapshot and classifier own the data that feeds it, and no other file restates this contract.
Every `/catchup` chat response renders EXACTLY these four sections, in THIS order, and nothing else structural (there is no At Anchor section):

1. **Maintainer's Call** - ONLY items that need the maintainer's own action now: a decision to make, a PR to approve or merge, a credential or login to provide, or a blocker only the maintainer can clear.
   Empty-state: "Nothing needs your action right now."
2. **Recently Landed** - the bounded current recent-completions baseline: merged PRs, completed scouts, and finished local-only merges across the main system and every registered daemon home.
   Empty-state: "No recent completions are in the current baseline."
3. **Underway** - live work progressing on its own, one line of current state per routed agent.
   Empty-state: "Nothing is underway."
4. **Charted Next** - queued or gated work waiting on the system or a date, plus action-free system-integrity warnings, never on the maintainer.
...
- The four buckets are mutually exclusive, so every item is forced into exactly one: needs-your-action is Maintainer's Call, done is Recently Landed, self-progressing is Underway, and not-yet-started work or an action-free system-integrity warning is Charted Next.
- The strict boundary keeps action-free items OUT of Maintainer's Call: a working or validating task, a queued item blocked on another task or a date, landed work, a completed scout's report pointer, a declared `paused:` external wait, and a bare recorded PR with no merge-ready signal each belong to one of the other three sections, never Maintainer's Call.
- A daemon's own row appears Underway only for `active_child_work`; `externally_held` belongs in Charted Next, and `unknown` belongs there as an unavailable-state gate unless its reason requires the maintainer's action.
- Do not suppress separately projected decisions, landed records, or gates from a `partial-structured` home merely because that daemon's own row is `unknown`.
- The chat follows `AGENTS-PORTING.md` section 9 during the Rust port and carries one scannable line per item, each PR as the full `https://...` URL; detailed decisions, plans, full gate reasons, and evidence live only in the report file, which the chat links to, so the chat stays materially shorter than that file.

## Tone and content rules

- This report is a private, maintainer-facing internal artifact that lives in gitignored `data/`, so unlike normal maintainer chat it MAY reference task ids, PR URLs, and repo names - the maintainer works with these directly and needs them to resume; keep it organized and scannable, not a raw dump.
...
- Never include PHI or secret values; the report is an operational artifact, but it is still subject to the same security and compliance rules that govern everything else in this system.

## Supervision discipline

This skill is read-mostly and changes no system state.
