...
This skill is the single owner of Multplx's bug-diagnosis reasoning procedure.
The broker applies it when briefing routed investigation and evaluating the resulting evidence, without taking over project-specific investigation itself.
...
Implementation still requires the maintainer's request or another existing lifecycle authority, and the reproduction should become the regression test when a fix is authorized.
