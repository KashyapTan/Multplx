#!/usr/bin/env bash
# mx-doc-audience-check.sh - validate the tracked documentation audience inventory.
#
# Usage:
#   bin/mx-doc-audience-check.sh
#   bin/mx-doc-audience-check.sh --root <repo> [--inventory <path>]
#
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Validate Multplx documentation audiences and local links.")
    ...
    try:
        ...
        print(f"mx-doc-audience-check: {exc}", file=sys.stderr)
        return 1
    print(f"mx-doc-audience-check: ok surfaces={surfaces} local_links={links}")
    return 0
