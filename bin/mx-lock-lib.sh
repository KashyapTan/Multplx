...
#
# ONE owner for the staleness proof that mx-teardown.sh (a worktree index.lock)
# and mx-system-sync.sh (a clone's .git/packed-refs.lock) both rely on: a lock is
...
# Diagnostics print to stderr prefixed by ${MX_LOCK_LOG_PREFIX:-mx-lock} so each
# caller's output stays recognizable.

mx_lock_log() {
  echo "${MX_LOCK_LOG_PREFIX:-mx-lock}: $*" >&2
}
...
# mx_lock_lsof_holder <target>: 0 a process holds it, 1 provably none, 2 lsof
...
mx_lock_lsof_holder() {
  ...
  if [ -n "$output" ]; then
    while IFS= read -r line; do
      mx_lock_log "lsof check failed: $line"
    done <<< "$output"
  else
    mx_lock_log "lsof check failed for $target with exit $status"
  fi
  return 2
}

# mx_lock_has_live_holder <lock> <dir>: 0 if a live process holds $lock or the
...
mx_lock_has_live_holder() {
  local lock=$1 dir=$2 status
  command -v lsof >/dev/null 2>&1 || return 0
  if [ -n "$lock" ]; then
    if mx_lock_lsof_holder "$lock"; then
      return 0
    else
      status=$?
  ...
  if [ -n "$dir" ]; then
    if mx_lock_lsof_holder "$dir"; then
      return 0
    else
      status=$?
      [ "$status" -eq 1 ] || return 0
...
# mx_lock_age <lock>: prints the lock's mtime age in whole seconds, or fails.
mx_lock_age() {
  local lock=$1 m now
  m=$(mx_lock_path_mtime "$lock") || return 1
...
# mx_lock_is_provably_stale <lock> <dir> <min_age_secs>: THE proof. Returns 0 iff
...
mx_lock_is_provably_stale() {
  local lock=$1 dir=$2 min_age=$3 age
  [ -n "$lock" ] && [ -e "$lock" ] || return 1
  mx_lock_has_live_holder "$lock" "$dir" && return 1
  if ! age=$(mx_lock_age "$lock"); then
    mx_lock_log "cannot read mtime for git lock $lock; leaving it in place"
    return 1
  fi
  [ "$age" -ge "$min_age" ]
}
