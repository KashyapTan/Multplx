#!/usr/bin/env bash
# Safe, home-scoped (re-)arm of the broker watcher, with honest verification.
#
# The watcher (bin/mx-watch.sh) blocks until it has an actionable wake to
...
# call and NOTIFIES on exit, so broker must run this script as the harness's
...
# (bin/mx-claude-stop-autoarm.sh), where the harness owns the process group and
...
# On a harness with a PreToolUse-equivalent hook, bin/mx-arm-pretool-check.sh
...
# liveness beacon (state/.last-watcher-beat) is fresh within MX_GUARD_GRACE (the
# single source of truth, shared with mx-watch.sh and mx-guard.sh), and prints
...
#
# --restart: stop ONLY this MX_HOME's watcher (the pid recorded in THIS home's
...
# bin/mx-watch.sh`: that pattern matches every Multplx home's watcher
# (daemon homes run the same script) and would kill siblings.
set -u
...
if [ "${BASH_SOURCE[0]}" = "${0}" ]; then
  ...
  if [ "$mx_supervision_adapter_implementation" = rust ]; then
    ...
    exec "$mx_supervision_adapter_bin" supervision mx-watch-arm.sh "$@"
  fi
fi
...
CONFIRM_TIMEOUT=${MX_ARM_CONFIRM_TIMEOUT:-10}
# Poll interval while attached to an existing healthy watcher.
ATTACH_POLL=${MX_ARM_ATTACH_POLL:-0.5}
CYCLE_LOG="$STATE/.watch-cycle-exits.log"
CYCLE_LOG_LOCK="$STATE/.watch-cycle-exits.lock"
CYCLE_LOG_MAX_BYTES=${MX_WATCH_CYCLE_LOG_MAX_BYTES:-262144}
CYCLE_LOG_KEEP_LINES=${MX_WATCH_CYCLE_LOG_KEEP_LINES:-1000}
ARM_PID=${BASHPID:-$$}
...
cycle_log_append() {
  ...
  [ "$cycle_active" -eq 1 ] || return 0
  ended_at=$(date +%s)
  beacon_age=$(mx_path_age "$BEAT")
  lock_after=$(lock_snapshot)

  i=0
  while ! mx_lock_try_acquire "$CYCLE_LOG_LOCK"; do
    [ "$i" -lt 20 ] || return 0
    sleep 0.02
    i=$((i + 1))
  done
  ...
  mx_lock_release "$CYCLE_LOG_LOCK"
  cycle_active=0
}
...
cycle_mark_predecessor_successor() {
  local successor=$1 predecessor=${MX_WATCH_PREDECESSOR_ARM_PID:-} i tmp
  case "$predecessor" in
    ''|*[!0-9]*) return 0 ;;
  esac
...
clear_stale_recorded_watcher_lock() {
  ...
  lock_home=$(cat "$WATCH_LOCK/mx-home" 2>/dev/null || true)
  lock_path=$(cat "$WATCH_LOCK/watcher-path" 2>/dev/null || true)
  lock_identity=$(cat "$WATCH_LOCK/pid-identity" 2>/dev/null || true)
  [ "$lock_home" = "$MX_HOME" ] || return 0
  [ "$lock_path" = "$WATCH" ] || return 0
  [ -n "$lock_identity" ] || return 0
  mx_lock_remove_path "$WATCH_LOCK" || true
}
...
healthy_watcher() {
  HEALTHY_PID=
  mx_watcher_healthy "$STATE" "$WATCH" "$GRACE" "$MX_HOME" || return 1
  HEALTHY_PID=$MX_WATCHER_HEALTHY_PID
}
...
if [ "$mode" = restart ]; then
  ...
  if mx_pid_alive "$lock_pid"; then
    if mx_watcher_lock_matches_pid "$STATE" "$WATCH" "$lock_pid" "$MX_HOME"; then
      kill -TERM "$lock_pid" 2>/dev/null || true
      # Wait for it to actually exit before relaunching, so the fresh watcher
      # either takes a released lock or reclaims a now-dead-pid stale lock instead
      # of seeing the dying one as a live holder and no-opping.
      i=0
      while [ "$i" -lt 50 ] && mx_pid_alive "$lock_pid"; do
        sleep 0.1
        i=$((i + 1))
      done
    else
...
# wake exit propagates out so the harness re-notifies broker.
child=
child_out=
cleanup_child() {
  if [ -n "$child" ] && mx_pid_alive "$child"; then
...
while :; do
  ...
  if [ "$child_done" -eq 0 ] && ! mx_pid_alive "$child"; then
    wait "$child"
    rc=$?
    child_done=1
    owned_child_finished "$rc"
