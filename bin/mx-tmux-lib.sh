#!/usr/bin/env bash
# mx-tmux-lib.sh — shared tmux pane primitives for broker.
#
...
# daemon (bin/mx-supervise-daemon.sh) and bin/mx-send.sh so the composer/submit
...
# idle pane as holding pending input and the daemon deferred injection / broker
...
# shared, system-wide mx_composer_strip_ghost (bin/mx-composer-lib.sh), which drops
...
# surfaced (mx-peek and every human/LLM-facing path stay plain), and only the
...
# sent to such a busy pane, and `mx-send` exits non-zero on a normal
# maintainer instruction. The submit core now falls back to `mx_pane_is_busy` once
...
#
# Per-harness override: MX_COMPOSER_IDLE_RE matches an empty composer after
# ghost and structural border stripping. MX_BUSY_REGEX overrides the busy
# footer set (mirrors mx-watch.sh / the daemon).
#
# All functions are `set -u` and `set -e` safe (guarded tmux calls, explicit
# returns) so they can be sourced into either context.
#
# Composer-content classification (empty|pending|unknown, and the system-wide
# rule that a BARE shell prompt glyph is a dead shell, not an empty agent
# composer) is NOT owned here: it is the shared bin/mx-composer-lib.sh, sourced
...
mx_tmux_strip_ghost() { mx_composer_strip_ghost; }

# mx_tmux_composer_state: classify the cursor/composer line of <target> as
...
# read from the PLAIN row (mx_composer_strip_ansi keeps ghost text so the box
# border is still visible), while the real-typed CONTENT is extracted with the
# shared mx_composer_strip_ghost so dim/faint AND dark-truecolor ghost text drops
...
# decision to the shared owner mx_composer_classify_content
# (bin/mx-composer-lib.sh). The bordered flag is what lets a bordered `│ > │`
# (claude's own idle composer) read empty while a bare, unbordered `$ ` dead-shell
# prompt reads unknown.
mx_tmux_composer_state() {  # <target> -> empty|pending|unknown
  ...
  if [ -n "$stripped" ] \
     && printf '%s' "$stripped" | grep -qiE "${MX_BUSY_REGEX:-$MX_TMUX_BUSY_REGEX_DEFAULT}"; then
    printf 'empty'; return 0
  fi
  mx_composer_classify_content "$bordered" "$stripped" "${MX_COMPOSER_IDLE_RE:-}" insensitive "$plain"
}

# mx_pane_input_pending: 0 (pending) if the cursor line holds real unsubmitted
...
mx_pane_input_pending() {  # <target>
  [ "$(mx_tmux_composer_state "$1")" = pending ]
}

# mx_pane_is_busy: 0 if the pane's last few non-blank lines show a busy footer
# (an agent mid-turn). Scans a 40-line tail like mx-watch.sh.
mx_pane_is_busy() {  # <target>
  local win=$1 tail40
...
# mx_tmux_submit_core: type <text> into <target> ONCE, then submit with Enter,
...
#   - mx-send fails only on "pending" (lenient: a positively-confirmed swallow),
...
# reads "pending", the submit core falls back to `mx_pane_is_busy`: a busy pane
...
# mx-send's lenient success policies both treat a busy-queued Enter as
# delivered.
mx_tmux_submit_enter_core() {  # <target> <retries> <enter-sleep>
  local target=$1 retries=$2 sleep_s=$3 i=0 state
  while :; do
    tmux send-keys -t "$target" Enter 2>/dev/null || true
    sleep "$sleep_s"
    state=$(mx_tmux_composer_state "$target")
  ...
  if mx_pane_is_busy "$target"; then
    printf 'empty'
  else
    printf 'pending'
  fi
...
mx_tmux_submit_core() {  # <target> <text> <retries> <enter-sleep> <settle>
  local target=$1 text=$2 retries=$3 sleep_s=$4 settle=$5
  tmux send-keys -t "$target" -l "$text" 2>/dev/null || { printf 'send-failed'; return 0; }
  sleep "$settle"
  mx_tmux_submit_enter_core "$target" "$retries" "$sleep_s"
}
