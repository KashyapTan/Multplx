...
#
#   2. The STATUS -> ACTION POLICY TABLE (mx_transition_policy) - the SINGLE
...
# (mx_backend_herdr_wait_transition) for the herdr producer and bin/mx-watch.sh
...
MX_TRANSITION_FIELD_SEP=$'\t'

# mx_transition_record: THE constructor for a normalized transition record.
...
mx_transition_record() {  # <pane_id> <workspace_id> <from_status> <to_status> <agent>
  local pane_id ws from to agent
  pane_id=$(mx_transition_clean_field "${1:-}")
  ws=$(mx_transition_clean_field "${2:-}")
  from=$(mx_transition_clean_field "${3:-}")
  to=$(mx_transition_clean_field "${4:-}")
  agent=$(mx_transition_clean_field "${5:-}")
  printf '%s\t%s\t%s\t%s\t%s' "$pane_id" "$ws" "$from" "$to" "$agent"
}

# mx_transition_clean_field: collapse any TAB/CR/LF in a field value to spaces
# so a stray control char can never desync the fixed five-field record.
mx_transition_clean_field() {  # <value>
  printf '%s' "${1:-}" | LC_ALL=C tr '\t\r\n' '   '
}

# Field accessors (1-based), so consumers never hardcode the column layout.
mx_transition_field() {  # <record> <n>
  printf '%s' "$1" | cut -d"$MX_TRANSITION_FIELD_SEP" -f"$2"
}

mx_transition_pane_id()      { mx_transition_field "$1" 1; }
mx_transition_workspace_id() { mx_transition_field "$1" 2; }
mx_transition_from_status()  { mx_transition_field "$1" 3; }
mx_transition_to_status()    { mx_transition_field "$1" 4; }
mx_transition_agent()        { mx_transition_field "$1" 5; }

# mx_transition_policy: THE single-owner status -> supervision-action table.
...
#                (an actor resumed/started a turn) is the clearing edge.
...
# immediate action.
mx_transition_policy() {  # <to_status> -> actionable|absorb|defer|fallback
  case "$1" in
