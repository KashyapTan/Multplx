...
# worktree, so it cannot discard unlanded work; set MX_SYSTEM_PRUNE=0 to disable it.
...
# fetch_with_packed_refs_lock_guard and the MX_SYSTEM_SYNC_PACKED_REFS_LOCK_* knobs.
# Usage: mx-system-sync.sh [<project-dir-or-name>]
# The single-project form accepts either a path (absolute, or relative to the
# caller's cwd) or a bare "<name>"/"projects/<name>" form, resolved against
# this home's projects dir ($MX_HOME/projects, or $MX_PROJECTS_OVERRIDE).
...
# `mx-system-sync.sh dotfiles-private` syncs just that one clone, same as
...
MX_LOCK_LOG_PREFIX=system-sync
...
# renaming it - e.g. bootstrap's system-sync timeout kill, or teardown's process
...
SYSTEM_SYNC_PACKED_REFS_LOCK_RETRIES=${MX_SYSTEM_SYNC_PACKED_REFS_LOCK_RETRIES:-3}
SYSTEM_SYNC_PACKED_REFS_LOCK_RETRY_WAIT_SECS=${MX_SYSTEM_SYNC_PACKED_REFS_LOCK_RETRY_WAIT_SECS:-1}
SYSTEM_SYNC_PACKED_REFS_LOCK_AGE_SECS=${MX_SYSTEM_SYNC_PACKED_REFS_LOCK_AGE_SECS:-30}
case "$SYSTEM_SYNC_PACKED_REFS_LOCK_RETRIES" in ''|*[!0-9]*) SYSTEM_SYNC_PACKED_REFS_LOCK_RETRIES=3 ;; esac
case "$SYSTEM_SYNC_PACKED_REFS_LOCK_AGE_SECS" in ''|*[!0-9]*) SYSTEM_SYNC_PACKED_REFS_LOCK_AGE_SECS=30 ;; esac
if ! [[ "$SYSTEM_SYNC_PACKED_REFS_LOCK_RETRY_WAIT_SECS" =~ ^([0-9]+([.][0-9]*)?|[.][0-9]+)$ ]]; then
  echo "system-sync: invalid packed-refs lock retry wait '$SYSTEM_SYNC_PACKED_REFS_LOCK_RETRY_WAIT_SECS'; using 1s" >&2
  SYSTEM_SYNC_PACKED_REFS_LOCK_RETRY_WAIT_SECS=1
fi

usage() {
  echo "usage: mx-system-sync.sh [<project-dir-or-name>]" >&2
}
...
# signature ONLY: retry up to SYSTEM_SYNC_PACKED_REFS_LOCK_RETRIES times (a
# transient lock self-clears as the owning process exits), then - only if the lock
# is provably stale per mx-lock-lib.sh (still present, mtime age past the
...
# a session-start refresh (which discards system-sync stderr) still surfaces it.
fetch_with_packed_refs_lock_guard() {
  ...
  while [ "$attempt" -lt "$SYSTEM_SYNC_PACKED_REFS_LOCK_RETRIES" ]; do
    attempt=$(( attempt + 1 ))
    echo "$label: fetch blocked by packed-refs lock ($lock_desc); waiting ${SYSTEM_SYNC_PACKED_REFS_LOCK_RETRY_WAIT_SECS}s and retrying ($attempt/${SYSTEM_SYNC_PACKED_REFS_LOCK_RETRIES}) (owning process may be exiting)" >&2
    sleep "$SYSTEM_SYNC_PACKED_REFS_LOCK_RETRY_WAIT_SECS"
    FETCH_OUTPUT=$(git -C "$PROJ" fetch origin --prune --quiet 2>&1); rc=$?
    if [ "$rc" -eq 0 ]; then
      echo "$label: fetch succeeded on retry; packed-refs lock cleared on its own" >&2
      # One stdout summary so a session-start refresh (which discards system-sync
  ...
  if [ -n "$lock" ] && [ -e "$lock" ]; then
    if mx_lock_is_provably_stale "$lock" "$PROJ" "$SYSTEM_SYNC_PACKED_REFS_LOCK_AGE_SECS"; then
      if ! rm -f "$lock"; then
        echo "$label: failed to remove provably-stale packed-refs lock $lock; leaving it in place" >&2
        return "$rc"
      fi
      echo "$label: removed provably-stale packed-refs lock $lock (age >= ${SYSTEM_SYNC_PACKED_REFS_LOCK_AGE_SECS}s, no live holder) and retrying fetch" >&2
      FETCH_OUTPUT=$(git -C "$PROJ" fetch origin --prune --quiet 2>&1); rc=$?
      if [ "$rc" -eq 0 ]; then
    ...
    echo "$label: fetch blocked by packed-refs lock $lock that persisted across ${SYSTEM_SYNC_PACKED_REFS_LOCK_RETRIES} retries and is not provably stale (may belong to a live process); leaving it in place" >&2
    return "$rc"
  fi
  echo "$label: fetch packed-refs lock signature persisted across ${SYSTEM_SYNC_PACKED_REFS_LOCK_RETRIES} retries even after the lock file disappeared" >&2
  return "$rc"
}

prune_gone_branches() {
  # Delete local branches whose upstream tracking branch is gone - the remote
  # branch was deleted, which in this system means its PR merged - as long as
  ...
  # this system are squash-merged, so a merged branch is never an ancestor and
  # such a check would prune nothing. The no-worktree guard is the real safety
  # net. Set MX_SYSTEM_PRUNE=0 to skip pruning entirely.
  [ "${MX_SYSTEM_PRUNE:-1}" != "0" ] || return 0
...
sync_project() {
  ...
  mode_line=$("$MX_ROOT/bin/mx-project-mode.sh" "$label" 2>/dev/null || echo "deep-review off")
  mode=${mode_line%% *}
