...
#
# Usage:
#   <PreToolUse JSON on stdin> | bin/mx-arm-pretool-check.sh
#   bin/mx-arm-pretool-check.sh --command '<cmd>' [--background true|false]
#
# Stdin mode extracts .tool_input.command for Claude and Codex (with a legacy
# .toolInput.command fallback).
# CLI mode is used by Pi after its adapter extracts the exact command string.
...
# mx-watch byte sequence AFTER the classifier's byte normalization, so a command
# that cannot contain mx-watch even after that normalization can never be a
...
# protected paths such as mx-watc\<newline>h-arm.sh or mx-"watch"-arm.sh still
# delegate. Stripping only these non-alphanumeric bytes can never destroy an
# existing mx-watch run.
#
# The fast path may allow ONLY when BOTH hold: (a) the stripped/normalized text
# lacks the mx-watch watcher substring, AND (b) the raw command carries no
...
# decodes and can therefore reconstruct mx-watch from bytes this cheap byte
# strip cannot. This marker set is COUPLED to the classifier's decoder set in
# bin/mx-arm-command-policy.mjs: adding any new quote/expansion form the
