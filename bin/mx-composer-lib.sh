#!/usr/bin/env bash
# bin/mx-composer-lib.sh - the ONE system-wide owner of composer-content
# classification, shared by every session-provider adapter: the tmux path
# through bin/mx-tmux-lib.sh, and bin/backends/{herdr,cmux}.sh directly.
#
# WHY THIS EXISTS (task mx-composer-shellglyph-safety): the four adapters each
...
# AGENT composer. The away-mode escalation injector (bin/mx-supervise-daemon.sh)
...
# consolidation). mx_composer_strip_ghost is the ONE ANSI-aware extractor of
...
# adapters (tmux via bin/mx-tmux-lib.sh, herdr via bin/backends/herdr.sh) cannot
...
# composer row it hands the RAW styled row to mx_composer_strip_ghost for the
# real-typed-content extraction, strips the box borders, trims, and hands the
# result plus a <bordered> flag to mx_composer_classify_content for the shared
...
# include guard (matching bin/mx-tmux-lib.sh).

# mx_composer_strip_ansi: drop every CSI escape sequence, leaving plain text.
...
# uses mx_composer_strip_ghost instead. Reads the styled text on stdin and prints
# plain text (stdin-only, matching mx_composer_strip_ghost). The character class
# includes ':' so an ITU colon-form SGR (38:2::r:g:b) is stripped whole, not left
# with a dangling tail.
mx_composer_strip_ansi() {
  local esc; esc=$(printf '\033')
  LC_ALL=C sed "s/${esc}\\[[0-9;:?]*[[:alpha:]]//g"
}

# mx_composer_strip_ghost: the ONE system-wide ANSI-aware extractor of "real typed
...
#     MX_COMPOSER_GHOST_LUMA_MAX (default 128): how some harnesses render their
...
#     dark-foreground run. This assumes a DARK terminal theme, the broker
#     system reality, where real typed input is bright and only de-emphasised UI
...
#     no system harness uses it for ghost text, so it is kept (real text wins:
...
mx_composer_strip_ghost() {
  LC_ALL=C awk -v lumamax="${MX_COMPOSER_GHOST_LUMA_MAX:-128}" '
...
# mx_composer_classify_content: the single shared composer-content verdict.
...
mx_composer_classify_content() {  # <bordered> <content> [idle_re] [idle_case] [plain_content]
  ...
  if mx_composer_idle_matches "$content" "$idle_re" "$idle_case"; then
    printf 'empty'; return 0
  fi
