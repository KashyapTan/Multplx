#!/usr/bin/env bash
# Hand already-identified, in-scope backlog items off from the main broker
# backlog to a daemon's own home backlog. Use this when a daemon is
# created (or whenever an existing queued item should become its domain's work)
# so the daemon owns its queue from day one instead of the item staying
...
#   - proving the destination is a genuine seeded daemon home
#     (.mx-daemon-home marker, AGENTS.md + bin/), never a project clone, the
#     active home, or the Multplx repo;
...
daemon_home() {
  local id=$1 line
  [ -f "$REG" ] || { echo "error: no daemon registry at $REG" >&2; return 1; }
  line=$(grep -E "^- $id( |$)" "$REG" | tail -1 || true)
  [ -n "$line" ] || { echo "error: daemon $id is not registered in $REG" >&2; return 1; }
...
validate_daemon_home() {
  ...
  if [ ! -f "$abs_home/.mx-daemon-home" ]; then
    echo "error: Multplx home $home is not a seeded daemon home" >&2
    return 1
  fi
  marker_id=$(cat "$abs_home/.mx-daemon-home" 2>/dev/null || true)
...
RAW_HOME=$(daemon_home "$ID") || exit 1
[ -n "$RAW_HOME" ] || { echo "error: daemon $ID has no home in $REG" >&2; exit 1; }
SUB_HOME=$(validate_daemon_home "$ID" "$RAW_HOME") || exit 1
SUB_BACKLOG="$SUB_HOME/data/backlog.md"
validate_backlog_file "main backlog" "$MAIN_BACKLOG" || exit 1
validate_backlog_file "daemon backlog" "$SUB_BACKLOG" || exit 1
