...
# Rust owns executable behavior. Keep the body below only as the documented
# sourced-function ABI used by tests/mx-supervision-events.test.sh.
...
if [ "${BASH_SOURCE[0]}" = "${0}" ]; then
  ...
  exec "$mx_supervision_adapter_bin" supervision mx-watch.sh "$@"
fi
