...
# acknowledgement.
#
# Record location (parent MX_HOME):
#   state/pending-replies/<corr_id>
# Each record is a key=value file owned by this library. Schema:
#   schema=mx-pending-reply.v1
#   corr_id=                privacy-safe correlation token
#   task_id=                daemon task id in the parent home
#   parent_home=            absolute parent MX_HOME
...
#   wrong_home_hits=        count of corr sightings under the daemon home
...
#
# Tunables (env):
#   MX_PENDING_REPLY_GRACE_SECS   default 120
#   MX_PENDING_REPLY_DIR_OVERRIDE override the pending-replies directory (tests)
#   MX_PENDING_REPLY_SEND_HOOK    optional command template for recovery delivery
#                                 (tests); receives task_id and full message as args
#   MX_PENDING_REPLY_NOW          optional fixed epoch for deterministic tests

# shellcheck source=bin/mx-marker-lib.sh
_MX_PENDING_REPLY_LIB_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd 2>/dev/null)" || _MX_PENDING_REPLY_LIB_DIR="."
# shellcheck source=bin/mx-marker-lib.sh
. "$_MX_PENDING_REPLY_LIB_DIR/mx-marker-lib.sh"
# shellcheck source=bin/mx-backend.sh
. "$_MX_PENDING_REPLY_LIB_DIR/mx-backend.sh"
# shellcheck source=bin/mx-tmux-lib.sh
. "$_MX_PENDING_REPLY_LIB_DIR/mx-tmux-lib.sh"

MX_PENDING_REPLY_SCHEMA='mx-pending-reply.v1'
MX_PENDING_REPLY_CORR_RE='corr=[A-Fa-f0-9]{16}'
MX_PENDING_REPLY_GRACE_DEFAULT=120

mx_pending_reply_now() {
  if [ -n "${MX_PENDING_REPLY_NOW:-}" ]; then
    printf '%s' "$MX_PENDING_REPLY_NOW"
    return 0
  fi
  date +%s
}

mx_pending_reply_grace_secs() {
  local g=${MX_PENDING_REPLY_GRACE_SECS:-$MX_PENDING_REPLY_GRACE_DEFAULT}
  case "$g" in
    ''|*[!0-9]*) g=$MX_PENDING_REPLY_GRACE_DEFAULT ;;
  esac
  printf '%s' "$g"
}

# Directory holding durable pending-reply records for <state-dir>.
mx_pending_reply_dir() {  # <state-dir>
  local state=$1
  if [ -n "${MX_PENDING_REPLY_DIR_OVERRIDE:-}" ]; then
    printf '%s' "$MX_PENDING_REPLY_DIR_OVERRIDE"
    return 0
  fi
  printf '%s/pending-replies' "$state"
}

mx_pending_reply_path() {  # <state-dir> <corr_id>
  printf '%s/%s' "$(mx_pending_reply_dir "$1")" "$2"
}

# Privacy-safe correlation id: 16 lowercase hex chars (64 bits of entropy).
mx_pending_reply_new_id() {
  local raw hex
...
mx_pending_reply_corr_token() {  # <corr_id>
  printf 'corr=%s' "$1"
}

# Extract the first corr=<16hex> token from free text, or empty.
mx_pending_reply_extract_corr() {  # <text>
  local text=$1
  printf '%s' "$text" | grep -oE "$MX_PENDING_REPLY_CORR_RE" 2>/dev/null | head -1 | cut -d= -f2- | tr 'A-F' 'a-f' || true
}

# 0 if <text> carries the exact correlation token for <corr_id>.
mx_pending_reply_text_has_corr() {  # <text> <corr_id>
...
mx_pending_reply_summarize() {  # <text>
...
mx_pending_reply_get() {  # <record-path> <key>
  local rec=$1 key=$2
  [ -f "$rec" ] || return 0
  grep "^${key}=" "$rec" 2>/dev/null | tail -1 | cut -d= -f2- || true
}

mx_pending_reply_corr_reusable() {  # <state-dir> <corr_id> <task_id>
  local state=$1 corr=$2 task_id=$3 rec phase
  printf '%s' "$corr" | grep -Eq '^[A-Fa-f0-9]{16}$' || return 1
  rec=$(mx_pending_reply_path "$state" "$corr")
  [ -f "$rec" ] || return 1
  [ "$(mx_pending_reply_get "$rec" task_id)" = "$task_id" ] || return 1
  phase=$(mx_pending_reply_get "$rec" phase)
  case "$phase" in
...
mx_pending_reply_set() {  # <record-path> <key> <value>
...
mx_pending_reply_embed_corr() {  # <message> <corr_id> <result-var>
...
mx_pending_reply_create() {  # <parent-home> <state-dir> <task_id> <request-text>
  ...
  dir=$(mx_pending_reply_dir "$state")
  mkdir -p "$dir" || return 1
  chmod 700 "$dir" 2>/dev/null || true
  corr=$(mx_pending_reply_new_id)
  [ "${#corr}" -eq 16 ] || return 1
  rec=$(mx_pending_reply_path "$state" "$corr")
  ...
  now=$(mx_pending_reply_now)
  summary=$(mx_pending_reply_summarize "$request_text")
  ...
  tmp="$dir/.${corr}.tmp.$$"
  cat > "$tmp" <<EOF
schema=$MX_PENDING_REPLY_SCHEMA
corr_id=$corr
task_id=$task_id
...
wrong_home_scan_signature=
grace_secs=$(mx_pending_reply_grace_secs)
EOF
...
mx_pending_reply_mark_delivered() {  # <state-dir> <corr_id> [confirmed-epoch]
  ...
  delivered=$(mx_pending_reply_get "$rec" delivered_epoch)
  if [ -z "$delivered" ]; then
    now=${confirmed_epoch:-$(mx_pending_reply_now)}
    mx_pending_reply_set "$rec" delivered_epoch "$now" || return 1
  fi
  if [ "$phase" = delivery_unknown ]; then
    mx_pending_reply_set "$rec" phase awaiting_report || return 1
  fi
  return 0
}

mx_pending_reply_delivery_confirmation_path() {  # <state-dir> <corr_id>
  printf '%s/.delivery-confirmed-%s' "$(mx_pending_reply_dir "$1")" "$2"
}

mx_pending_reply_write_delivery_confirmation() {  # <state-dir> <corr_id> <state> <value>
  local pending_state=$1 corr=$2 delivery_state=$3 value=$4 marker dir tmp
  marker=$(mx_pending_reply_delivery_confirmation_path "$pending_state" "$corr")
  dir=$(dirname "$marker")
  mkdir -p "$dir" || return 1
  tmp="$marker.tmp.$$"
...
mx_pending_reply_prepare_delivery() {  # <state-dir> <corr_id>
  ...
  [ -f "$marker" ] && return 0
  now=$(mx_pending_reply_now)
  mx_pending_reply_write_delivery_confirmation "$state" "$corr" attempted "$now"
}

mx_pending_reply_confirm_delivery() {  # <state-dir> <corr_id>
  local state=$1 corr=$2 now marker
  marker=$(mx_pending_reply_delivery_confirmation_path "$state" "$corr")
  if ! mx_pending_reply_prepare_delivery "$state" "$corr"; then
    return 1
  fi
  now=$(mx_pending_reply_now)
  mx_pending_reply_write_delivery_confirmation "$state" "$corr" confirmed "$now" || return 1
  if mx_pending_reply_mark_delivered "$state" "$corr" "$now"; then
...
mx_pending_reply_reconcile_delivery() {  # <state-dir> <corr_id>
  ...
  case "$delivery_state" in
    confirmed)
      ...
      mx_pending_reply_mark_delivered "$state" "$corr" "$epoch" || return 1
      rm -f "$marker" 2>/dev/null || true
      return 0
      ;;
    attempted)
      epoch=$value
      case "$epoch" in ''|*[!0-9]*) return 1 ;; esac
      grace=$(mx_pending_reply_get "$rec" grace_secs)
      case "$grace" in ''|*[!0-9]*) grace=$(mx_pending_reply_grace_secs) ;; esac
      now=$(mx_pending_reply_now)
      age=$((now - epoch))
      [ "$age" -ge "$grace" ] || return 1
      phase=$(mx_pending_reply_get "$rec" phase)
      [ "$phase" = awaiting_report ] || return 1
      mx_pending_reply_set "$rec" phase delivery_unknown || return 1
      return 0
      ;;
  esac
  return 1
...
mx_pending_reply_discard_undelivered() {  # <state-dir> <corr_id>
...
mx_pending_reply_line_resolves() {  # <line> <corr_id>
  local line=$1 corr=$2
  [ -n "$line" ] && [ -n "$corr" ] || return 1
  case "$line" in
    *pending-reply-missed*) return 1 ;;
  esac
  mx_pending_reply_text_has_corr "$line" "$corr"
}

# Scan a status file for a correlated resolve. Prints the matching line or empty.
mx_pending_reply_find_resolve_line() {  # <status-file> <corr_id>
  local status_file=$1 corr=$2 line
  [ -f "$status_file" ] || return 0
  while IFS= read -r line || [ -n "$line" ]; do
    [ -n "$line" ] || continue
    if mx_pending_reply_line_resolves "$line" "$corr"; then
      printf '%s' "$line"
      return 0
    fi
  done < "$status_file"
...
mx_pending_reply_file_signature() {  # <path>
  local path=$1
...
mx_pending_reply_status_set_signature() {  # <status-dir>
  local status_dir=$1 status_file signature
  {
    for status_file in "$status_dir"/*.status; do
      [ -f "$status_file" ] || continue
      signature=$(mx_pending_reply_file_signature "$status_file")
...
mx_pending_reply_resolve_via_of_line() {  # <line>
  local line=$1
  case "$line" in
...
mx_pending_reply_try_resolve() {  # <state-dir> <corr_id> [status-file-override]
  ...
  status_file=${status_override:-$(mx_pending_reply_get "$rec" parent_status)}
  if [ -z "$status_override" ] && [ "$unconfirmed" = 0 ]; then
    signature=$(mx_pending_reply_file_signature "$status_file")
    previous=$(mx_pending_reply_get "$rec" parent_status_scan_signature)
    [ "$signature" != "$previous" ] || return 1
  fi
  line=$(mx_pending_reply_find_resolve_line "$status_file" "$corr")
  if [ -z "$line" ]; then
    if [ -z "$status_override" ] && [ "$unconfirmed" = 0 ]; then
      mx_pending_reply_set "$rec" parent_status_scan_signature "$signature" || return 1
    fi
    return 1
  fi
  via=$(mx_pending_reply_resolve_via_of_line "$line")
  now=$(mx_pending_reply_now)
  mx_pending_reply_set "$rec" phase resolved || return 1
  if [ -z "$delivered" ]; then
    mx_pending_reply_mark_delivered "$state" "$corr" "$now" || return 1
    rm -f "$marker" 2>/dev/null || true
  fi
  mx_pending_reply_set "$rec" resolved_epoch "$now" || return 1
  mx_pending_reply_set "$rec" resolved_via "$via" || return 1
  return 0
}

# Observe backend busy/idle evidence for the active turn without reading chat.
# busy_state must be one of: busy | idle | unknown.
mx_pending_reply_observe_busy() {  # <state-dir> <corr_id> <busy_state>
  ...
  seen=$(mx_pending_reply_get "$rec" "$field_seen")
  completed=$(mx_pending_reply_get "$rec" "$field_completed")
  case "$busy_state" in
    busy)
      if [ "$seen" != 1 ]; then
        mx_pending_reply_set "$rec" "$field_seen" 1 || return 1
      fi
      ;;
    idle)
      if [ -z "$completed" ]; then
        ...
        if [ "$seen" = 1 ] || [ "$seen" = 0 ]; then
          now=$(mx_pending_reply_now)
          mx_pending_reply_set "$rec" "$field_completed" "$now" || return 1
        fi
      fi
      ;;
    unknown)
...
mx_pending_reply_fallback_idle_eligible() {  # <record-path>
  local rec=$1 phase start seen grace now age
  phase=$(mx_pending_reply_get "$rec" phase)
  case "$phase" in
    awaiting_report)
      start=$(mx_pending_reply_get "$rec" delivered_epoch)
      seen=$(mx_pending_reply_get "$rec" turn_seen_busy)
      ;;
    recovery_sent)
      start=$(mx_pending_reply_get "$rec" recovery_sent_epoch)
      seen=$(mx_pending_reply_get "$rec" recovery_turn_seen_busy)
      ;;
    *) return 1 ;;
  esac
  [ "$seen" = 1 ] && return 0
...
mx_pending_reply_backend_observation() {  # <backend> <target> [expected-label]
  local backend=$1 target=$2 expected_label=${3-} native tail40
  native=$(mx_backend_busy_state "$backend" "$target" 2>/dev/null || printf 'unknown')
  case "$native" in
    busy|idle) printf '%s' "$native"; return 0 ;;
  esac
  tail40=$(mx_backend_capture "$backend" "$target" 40 "$expected_label" 2>/dev/null) \
    || { printf 'unknown'; return 0; }
  if printf '%s' "$tail40" | grep -v '^[[:space:]]*$' | tail -6 \
    | grep -qiE "${MX_BUSY_REGEX:-$MX_TMUX_BUSY_REGEX_DEFAULT}"; then
    printf 'busy'
  else
...
mx_pending_reply_busy_state_from_observation() {  # <record-path> <observation>
  local rec=$1 observation=$2
  case "$observation" in
    busy|idle|unknown) printf '%s' "$observation" ;;
    fallback-idle)
      if mx_pending_reply_fallback_idle_eligible "$rec"; then
        printf 'idle'
      else
        printf 'unknown'
      fi
...
mx_pending_reply_mark_turn_completed() {  # <state-dir> <corr_id> [which: request|recovery]
  ...
  now=$(mx_pending_reply_now)
  mx_pending_reply_set "$rec" "$field" "$now" || return 1
...
mx_pending_reply_recovery_message() {  # <record-path>
...
# turn completed and grace elapsed. Uses MX_PENDING_REPLY_SEND_HOOK when set
# (tests), otherwise invokes mx-send with MX_PENDING_REPLY_EXISTING_CORR so a
# second expectation is not created.
mx_pending_reply_send_recovery() {  # <state-dir> <corr_id>
  ...
  phase=$(mx_pending_reply_get "$rec" phase)
  [ "$phase" = awaiting_report ] || return 1
  attempted=$(mx_pending_reply_get "$rec" recovery_attempted_epoch)
  if [ -n "$attempted" ]; then
    mx_pending_reply_reconcile_recovery "$state" "$corr" || true
    return 1
  fi
  completed=$(mx_pending_reply_get "$rec" request_turn_completed_epoch)
  ...
  age=$((now - delivered))
  [ "$age" -ge "$grace" ] || return 1
  task_id=$(mx_pending_reply_get "$rec" task_id)
  parent_home=$(mx_pending_reply_get "$rec" parent_home)
  msg=$(mx_pending_reply_recovery_message "$rec")
  sender_pid=${BASHPID:-$$}
  sender_identity=$(mx_pending_reply_pid_identity "$sender_pid") || return 1
  mx_pending_reply_set "$rec" recovery_sender_pid "$sender_pid" || return 1
  mx_pending_reply_set "$rec" recovery_sender_identity "$sender_identity" || return 1
  mx_pending_reply_set "$rec" recovery_attempted_epoch "$now" || return 1
  mx_pending_reply_set "$rec" phase recovery_sending || return 1
  if [ -n "${MX_PENDING_REPLY_SEND_HOOK:-}" ]; then
    # Hook receives: task_id message
    # shellcheck disable=SC2086
    if ! eval "$MX_PENDING_REPLY_SEND_HOOK" "$(printf '%q' "$task_id")" "$(printf '%q' "$msg")"; then
      send_status=1
    fi
  else
    if [ -z "$parent_home" ] || [ ! -d "$parent_home" ]; then
      send_status=1
    elif ! env MX_HOME="$parent_home" MX_PENDING_REPLY_EXISTING_CORR="$corr" \
      "$_MX_PENDING_REPLY_LIB_DIR/mx-send.sh" "$task_id" "$msg"; then
      send_status=1
    fi
  fi
  if [ "$send_status" = 0 ]; then
    mx_pending_reply_finish_recovery "$state" "$corr" confirmed
    return $?
  fi
  mx_pending_reply_finish_recovery "$state" "$corr" failed || return 1
  return 1
}

mx_pending_reply_pid_identity() {  # <pid>
...
mx_pending_reply_sender_alive() {  # <record-path>
  local rec=$1 pid expected actual
  pid=$(mx_pending_reply_get "$rec" recovery_sender_pid)
  expected=$(mx_pending_reply_get "$rec" recovery_sender_identity)
  [ -n "$expected" ] || return 1
  actual=$(mx_pending_reply_pid_identity "$pid") || return 1
  [ "$actual" = "$expected" ]
}

mx_pending_reply_finish_recovery() {  # <state-dir> <corr_id> <confirmed|failed>
  local state=$1 corr=$2 outcome=$3 rec phase now sent
  rec=$(mx_pending_reply_path "$state" "$corr")
  [ -f "$rec" ] || return 1
  phase=$(mx_pending_reply_get "$rec" phase)
  [ "$phase" = recovery_sending ] || return 1
  mx_pending_reply_set "$rec" recovery_delivery_outcome "$outcome" || return 1
  if [ "$outcome" = confirmed ]; then
    sent=$(mx_pending_reply_get "$rec" recovery_sent_epoch)
    if [ -z "$sent" ]; then
      now=$(mx_pending_reply_now)
      mx_pending_reply_set "$rec" recovery_sent_epoch "$now" || return 1
    fi
    mx_pending_reply_set "$rec" recovery_turn_seen_busy 0 || return 1
    mx_pending_reply_set "$rec" recovery_turn_completed_epoch "" || return 1
    mx_pending_reply_set "$rec" phase recovery_sent || return 1
  else
    [ "$outcome" = failed ] || return 1
    mx_pending_reply_set "$rec" phase recovery_failed || return 1
  fi
}

mx_pending_reply_reconcile_recovery() {  # <state-dir> <corr_id>
  ...
  outcome=$(mx_pending_reply_get "$rec" recovery_delivery_outcome)
  case "$outcome" in
    confirmed) mx_pending_reply_finish_recovery "$state" "$corr" confirmed; return $? ;;
    failed) mx_pending_reply_finish_recovery "$state" "$corr" failed; return $? ;;
    unknown)
      mx_pending_reply_set "$rec" phase recovery_unknown || return 1
      return 0
      ;;
  esac
  mx_pending_reply_sender_alive "$rec" && return 1
  mx_pending_reply_set "$rec" recovery_delivery_outcome unknown || return 1
  mx_pending_reply_set "$rec" phase recovery_unknown || return 1
}

# Escalate once after a missed recovery report or failed delivery outcome.
# Retains the durable unresolved record. Never loops.
mx_pending_reply_maybe_escalate() {  # <state-dir> <corr_id>
  local state=$1 corr=$2
  ...
  phase=$(mx_pending_reply_get "$rec" phase)
  if [ "$phase" = delivery_unknown ]; then
    mx_pending_reply_reconcile_delivery "$state" "$corr" || true
    phase=$(mx_pending_reply_get "$rec" phase)
    [ "$phase" = delivery_unknown ] || return 0
  fi
  case "$phase" in
    recovery_sent)
      completed=$(mx_pending_reply_get "$rec" recovery_turn_completed_epoch)
      [ -n "$completed" ] || return 1
      ;;
  ...
  # mistaken for a daemon acknowledgement by mx_pending_reply_line_resolves.
  outcome=$(mx_pending_reply_get "$rec" recovery_delivery_outcome)
  case "$phase" in
    delivery_unknown)
  ...
  now=$(mx_pending_reply_now)
  mx_pending_reply_set "$rec" escalated_epoch "$now" || return 1
  mx_pending_reply_set "$rec" phase escalated || return 1
  return 0
}

# Detect a correlated report written under the daemon home (wrong home)
# without treating it as acknowledgement.
mx_pending_reply_detect_wrong_home() {  # <state-dir> <corr_id> <daemon-home>
  ...
  delivered=$(mx_pending_reply_get "$rec" delivered_epoch)
  [ -n "$delivered" ] || return 0
  snapshot=$(mx_pending_reply_status_set_signature "$sm_home/state")
  previous=$(mx_pending_reply_get "$rec" wrong_home_scan_signature)
  [ "$snapshot" != "$previous" ] || return 0
  hits=$(mx_pending_reply_get "$rec" wrong_home_hits)
  case "$hits" in ''|*[!0-9]*) hits=0 ;; esac
  sightings=$(mx_pending_reply_get "$rec" wrong_home_sightings)
  ...
  if [ "$changed" = 1 ]; then
    mx_pending_reply_set "$rec" wrong_home_sightings "$sightings" || return 1
    mx_pending_reply_set "$rec" wrong_home_hits "$hits" || return 1
  fi
  mx_pending_reply_set "$rec" wrong_home_scan_signature "$snapshot" || return 1
  return 0
}

# One reconciliation tick for a single record: resolve, observe, recover, escalate.
# busy_state is busy|idle|unknown for the daemon endpoint.
# daemon_home may be empty when unknown.
mx_pending_reply_tick_one() {  # <state-dir> <corr_id> <busy_state> [daemon-home]
  ...
  if [ -z "$delivered" ]; then
    case "$phase" in
      delivery_unknown) mx_pending_reply_maybe_escalate "$state" "$corr" 2>/dev/null || true ;;
      escalated) mx_pending_reply_try_resolve "$state" "$corr" >/dev/null 2>&1 || true ;;
    esac
    return 0
  fi
  ...
  case "$phase" in
    awaiting_report|recovery_sending)
      if [ -n "$(mx_pending_reply_get "$rec" recovery_attempted_epoch)" ]; then
        mx_pending_reply_reconcile_recovery "$state" "$corr" || true
        phase=$(mx_pending_reply_get "$rec" phase)
      fi
      ;;
  ...
  case "$phase" in
    ...
    escalated)
      ...
      if [ -n "$sm_home" ]; then
        mx_pending_reply_detect_wrong_home "$state" "$corr" "$sm_home" || true
      fi
      return 0
      ;;
    recovery_sending) return 0 ;;
    recovery_failed|recovery_unknown)
      mx_pending_reply_maybe_escalate "$state" "$corr" 2>/dev/null || true
      return 0
      ;;
  esac
  if [ -n "$sm_home" ]; then
  ...
  mx_pending_reply_observe_busy "$state" "$corr" "$busy_state" || true
  ...
  phase=$(mx_pending_reply_get "$rec" phase)
  if [ "$phase" = awaiting_report ]; then
    mx_pending_reply_send_recovery "$state" "$corr" 2>/dev/null || true
  fi
  phase=$(mx_pending_reply_get "$rec" phase)
  case "$phase" in
    recovery_sent|recovery_failed|recovery_unknown)
...
# Never scrapes daemon conversation; uses only parent status, backend busy
# state, and optional daemon-home wrong-home path checks.
mx_pending_reply_tick() {  # <state-dir>
  ...
  for rec in "$dir"/*; do
    ...
    if [ -z "$delivered" ]; then
      case "$phase" in
        delivery_unknown|escalated)
          mx_pending_reply_tick_one "$state" "$corr" unknown "" || true
          ;;
      esac
      continue
    fi
    ...
    if [ -f "$meta" ]; then
      ...
      if [ -n "$target" ]; then
        ...
        if [ "$found" = 0 ]; then
          observation=$(mx_pending_reply_backend_observation "$backend" "$target" "$label")
          observation_tasks+=("$task_id")
          observation_values+=("$observation")
        fi
        busy=$(mx_pending_reply_busy_state_from_observation "$rec" "$observation")
      fi
    fi
    mx_pending_reply_tick_one "$state" "$corr" "$busy" "$sm_home" || true
  done
  return 0
}

# True when any open (non-resolved) pending reply exists for a task.
mx_pending_reply_task_has_open() {  # <state-dir> <task_id>
  local state=$1 task_id=$2 dir rec phase tid
  dir=$(mx_pending_reply_dir "$state")
  [ -d "$dir" ] || return 1
  for rec in "$dir"/*; do
    [ -f "$rec" ] || continue
    tid=$(mx_pending_reply_get "$rec" task_id)
