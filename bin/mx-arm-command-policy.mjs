...
//
// This file is the sole owner of broker's shell command classification.
...
// (bin/mx-cd-command-policy.mjs) reuses the same proven parser instead of
...
const REASONS = {
  ...
  "broad-watcher-kill": "a broad process kill targeting the broker watcher is forbidden",
  "unclassifiable-protected-command": "unsupported or malformed shell syntax contains a protected watcher command",
  "watcher-direct": "bin/mx-watch.sh must not be run directly; arm the watcher with bin/mx-watch-arm.sh or run bin/mx-watch-checkpoint.sh instead",
};
...
function rawMentionsProtected(command) {
  return /(?:^|[/\s'"`(])mx-watch(?:-(?:arm|checkpoint))?\.sh\b/.test(normalizeLineContinuations(command));
}

function rawMentionsBroadKill(command) {
  const normalized = normalizeLineContinuations(command);
  return /mx-watch/.test(normalized) && /\b(?:pkill|kill)\b/.test(normalized);
}
...
const PROTECTED_SCRIPTS = [
  { relative: "bin/mx-watch-arm.sh", kind: "arm" },
  { relative: "bin/mx-watch-checkpoint.sh", kind: "checkpoint" },
  { relative: "bin/mx-watch.sh", kind: "watch" },
];
...
function hasUnclassifiableProtectedExpansion(word, root) {
  ...
  return /(?:^|\/)mx-watch/.test(word.value);
}
...
function contextWithAssignments(context, words) {
  ...
  for (const word of words) {
    ...
    if (/mx-watch/.test(value) || wordReferencesAny(word, watcherPatterns)) watcherPatterns.add(name);
...
function isWatcherPgrep(position, context) {
  ...
  return position.words.slice(position.index + 1).some((word) => /(?:^|\/)mx-watch(?:\.sh)?\b/.test(word.value) || wordReferencesAny(word, context.watcherPatterns));
}
...
function analyzeProgram(command, context, depth = 0) {
  ...
  for (const tokens of program.nodes) {
    ...
    if (commandName === "pkill" && args.some((word) => /mx-watch/.test(word.value) || wordReferencesAny(word, nodeContext.watcherPatterns))) broadKill = true;
...
function blessedProgram(analysis) {
...
function decision(command, root, home) {
  ...
  if (blessedProgram(analysis)) return { decision: "allow" };
...
// Run the CLI only when invoked directly (node mx-arm-command-policy.mjs ...),
// never when imported by a sibling policy such as bin/mx-cd-command-policy.mjs.
