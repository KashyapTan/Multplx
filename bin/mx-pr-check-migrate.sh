#!/usr/bin/env bash
# Non-executing migration for watcher PR checks created by older Multplx
...
# Usage: mx-pr-check-migrate.sh [--checks-safe]
set -u
...
current_checks_authenticated() {
  ...
  for check in "$STATE"/*.check.sh; do
    ...
    mx_custom_check_registered "$STATE" "$id" && continue
...
private_migration_boundaries_valid() {
  ...
  if [ -e "$LOG" ] || [ -L "$LOG" ]; then
    mx_pr_private_file_valid "$LOG" 600 "$state_device" || return 1
  fi
  if [ -e "$QUARANTINE" ] || [ -L "$QUARANTINE" ]; then
    [ -d "$QUARANTINE" ] && [ ! -L "$QUARANTINE" ] || return 1
    [ "$(mx_pr_file_mode "$QUARANTINE")" = 700 ] || return 1
    [ "$(mx_pr_file_device "$QUARANTINE")" = "$state_device" ] || return 1
    for artifact in "$QUARANTINE"/* "$QUARANTINE"/.[!.]* "$QUARANTINE"/..?*; do
      [ -e "$artifact" ] || [ -L "$artifact" ] || continue
      mx_pr_private_file_valid "$artifact" 600 "$state_device" || return 1
    done
  fi
}
...
diagnostic_obligation_message() {
  ...
  if [ "$prefix" = "$NONCANONICAL_PREFIX" ] \
    ...
    mx_pr_task_id_valid "$prefix" || return 1
    case "$kind" in
      ...
      failure-ambiguous)
        ...
        ;;
      failure-replacement)
        MIGRATION_DIAGNOSTIC_MESSAGE="task $prefix: replacement poll lacks canonical provenance or metadata binding; poll remains unarmed; republish it through mx-pr-check.sh"
        ;;
      ambiguous)
...
scan_complete() {
  ...
  mx_pr_private_file_valid "$SCAN_MARKER" 600 "$state_device" || return 1
...
migration_complete() {
  ...
  mx_pr_private_file_valid "$MARKER" 600 "$state_device" || return 1
...
if mx_pid_alive "$pid"; then
  if ! mx_watcher_lock_matches_pid "$STATE" "$WATCH" "$pid" "$MX_HOME"; then
  ...
  stopped_watcher=1
  i=0
  while [ "$i" -lt 100 ] && mx_pid_alive "$pid"; do
    sleep 0.05
    i=$((i + 1))
  done
  if mx_pid_alive "$pid"; then
...
if ! mx_pr_poll_retirement_recover_all "$STATE" "$TEMPLATE"; then
  echo "PR_CHECK_MIGRATION: pending PR poll retirement could not be validated:$MX_PR_POLL_RETIREMENT_REJECTED" >&2
  exit 1
fi
...
if [ -e "$MARKER" ] || [ -L "$MARKER" ]; then
  mx_pr_private_file_valid "$MARKER" 600 "$STATE_DEVICE" || exit 1
  rm -f -- "$MARKER" || exit 1
  [ ! -e "$MARKER" ] && [ ! -L "$MARKER" ] || exit 1
fi
if [ -e "$SCAN_MARKER" ] || [ -L "$SCAN_MARKER" ]; then
  mx_pr_private_file_valid "$SCAN_MARKER" 600 "$STATE_DEVICE" || exit 1
...
migration_needed() {
  ...
  for check in "$STATE"/*.check.sh; do
    ...
    id=$(basename "$check" .check.sh)
    mx_custom_check_registered "$STATE" "$id" && continue
    if ! mx_pr_poll_artifacts_valid "$STATE" "$id" "$TEMPLATE"; then
      return 0
    fi
  done
  return 1
...
revoke_migration_marker() {
  if [ -e "$MARKER" ] || [ -L "$MARKER" ]; then
    if [ -f "$MARKER" ] && [ ! -L "$MARKER" ]; then
      [ "$(mx_pr_file_link_count "$MARKER")" = 1 ] || return 1
    fi
...
publish_migration_marker() {
  mx_pr_regular_destination_on_device_or_absent "$MARKER" "$STATE_DEVICE" || return 1
  MIGRATION_MARKER_TMP=$(mktemp "$STATE/.mx-pr-check-migration.XXXXXX") || return 1
  mx_pr_private_file_valid "$MIGRATION_MARKER_TMP" 600 "$STATE_DEVICE" || return 1
...
revoke_scan_marker() {
  if [ -e "$SCAN_MARKER" ] || [ -L "$SCAN_MARKER" ]; then
    if [ -f "$SCAN_MARKER" ] && [ ! -L "$SCAN_MARKER" ]; then
      [ "$(mx_pr_file_link_count "$SCAN_MARKER")" = 1 ] || return 1
    fi
...
publish_scan_marker() {
  mx_pr_regular_destination_on_device_or_absent "$SCAN_MARKER" "$STATE_DEVICE" || return 1
  MIGRATION_SCAN_MARKER_TMP=$(mktemp "$STATE/.mx-pr-check-scan.XXXXXX") || return 1
  mx_pr_private_file_valid "$MIGRATION_SCAN_MARKER_TMP" 600 "$STATE_DEVICE" || return 1
...
quarantine_dir_valid() {
  [ -d "$QUARANTINE" ] && [ ! -L "$QUARANTINE" ] || return 1
  [ "$(mx_pr_file_mode "$QUARANTINE")" = 700 ] || return 1
  [ "$(mx_pr_file_device "$QUARANTINE")" = "$STATE_DEVICE" ]
}
...
quarantine_tree_repair_and_validate() {
  ...
  for artifact in "$QUARANTINE"/* "$QUARANTINE"/.[!.]* "$QUARANTINE"/..?*; do
    ...
    [ "$(mx_pr_file_device "$artifact")" = "$STATE_DEVICE" ] || return 1
    [ "$(mx_pr_file_link_count "$artifact")" = 1 ] || return 1
    chmod 0600 "$artifact" || return 1
    [ "$(mx_pr_file_mode "$artifact")" = 600 ] || return 1
    [ "$(mx_pr_file_device "$artifact")" = "$STATE_DEVICE" ] || return 1
    [ "$(mx_pr_file_link_count "$artifact")" = 1 ] || return 1
  done
  quarantine_dir_valid
...
metadata_pr_is_canonical() {
  ...
  MIGRATION_PROVIDER=$MX_PR_META_PROVIDER
  MIGRATION_URL=$MX_PR_META_URL
  MIGRATION_HOST=$MX_PR_META_HOST
  MIGRATION_PATH=$MX_PR_META_PATH
  MIGRATION_NUMBER=$MX_PR_META_NUMBER
}
...
quarantine_artifact() {
  ...
  source_device=$(mx_pr_file_device "$source") || return 1
  [ "$source_device" = "$STATE_DEVICE" ] || return 1
  [ "$(mx_pr_file_link_count "$source")" = 1 ] || return 1
  [ -z "$MIGRATION_QUARANTINE_TMP" ] || rm -f -- "$MIGRATION_QUARANTINE_TMP"
  MIGRATION_QUARANTINE_TMP=
  ...
  [ "$(mx_pr_file_device "$MIGRATION_QUARANTINE_TMP")" = "$STATE_DEVICE" ] || return 1
  ...
  [ "$(mx_pr_file_link_count "$destination")" = 1 ] || return 1
  chmod 0600 "$destination" || return 1
  [ -f "$destination" ] && [ ! -L "$destination" ] || return 1
  [ "$(mx_pr_file_mode "$destination")" = 600 ] || return 1
  [ "$(mx_pr_file_device "$destination")" = "$STATE_DEVICE" ] || return 1
  [ "$(mx_pr_file_link_count "$destination")" = 1 ] || return 1
  [ ! -e "$source" ] && [ ! -L "$source" ]
}
...
diagnostic_log_valid() {
  mx_pr_private_file_valid "$LOG" 600 "$STATE_DEVICE"
}
...
revoke_migration_log() {
  if [ -e "$LOG" ] || [ -L "$LOG" ]; then
    if [ -f "$LOG" ] && [ ! -L "$LOG" ]; then
      [ "$(mx_pr_file_link_count "$LOG")" = 1 ] || return 1
    fi
    rm -f -- "$LOG" || return 1
  fi
  [ ! -e "$LOG" ] && [ ! -L "$LOG" ]
...
record_diagnostic() {
  ...
  mx_pr_regular_destination_on_device_or_absent "$LOG" "$STATE_DEVICE" || return 1
  [ ! -e "$LOG" ] || diagnostic_log_valid || return 1
  [ -z "$MIGRATION_LOG_TMP" ] || rm -f -- "$MIGRATION_LOG_TMP"
  MIGRATION_LOG_TMP=
  MIGRATION_LOG_TMP=$(mktemp "$STATE/.mx-pr-check-log.XXXXXX") || return 1
  [ -f "$MIGRATION_LOG_TMP" ] && [ ! -L "$MIGRATION_LOG_TMP" ] || return 1
  [ "$(mx_pr_file_device "$MIGRATION_LOG_TMP")" = "$STATE_DEVICE" ] || return 1
  if [ -f "$LOG" ]; then
...
migrate_legacy_quarantine_entry() {
  ...
  mx_pr_private_file_valid "$source" 600 "$STATE_DEVICE" || return 1
  mx_pr_regular_destination_on_device_or_absent "$destination" "$STATE_DEVICE" || return 1
  if [ -e "$destination" ] || [ -L "$destination" ]; then
    mx_pr_private_file_valid "$destination" 600 "$STATE_DEVICE" || return 1
  ...
  [ ! -e "$source" ] && [ ! -L "$source" ] \
    && mx_pr_private_file_valid "$destination" 600 "$STATE_DEVICE"
}
...
ensure_diagnostic_obligation() {
  ...
  [ "$prefix" = "$NONCANONICAL_PREFIX" ] || mx_pr_task_id_valid "$prefix" || return 1
  ensure_quarantine_dir || return 1
  ...
  [ -z "$MIGRATION_OBLIGATION_TMP" ] || rm -f -- "$MIGRATION_OBLIGATION_TMP"
  MIGRATION_OBLIGATION_TMP=
  MIGRATION_OBLIGATION_TMP=$(mktemp "$QUARANTINE/.mx-pr-check-obligation.XXXXXX") || return 1
  ...
  MIGRATION_OBLIGATION_TMP=
  if ! mx_pr_private_file_valid "$destination" 600 "$STATE_DEVICE" \
...
canonical_terminal_success() {
  local id=$1
  mx_pr_poll_artifacts_valid "$STATE" "$id" "$TEMPLATE" \
...
canonical_repair_from_pending() {
  ...
  mx_pr_poll_prepare "$STATE" "$id" "$provider" "$url" "$host" "$path" "$number" "$TEMPLATE" || return 1
  mx_pr_poll_publish_prepared || return 1
...
if migration_needed; then
  ...
  for check in "$STATE"/*.check.sh; do
    ...
    id=$(basename "$check" .check.sh)
    mx_custom_check_registered "$STATE" "$id" && continue
    mx_pr_poll_artifacts_valid "$STATE" "$id" "$TEMPLATE" && continue

    if mx_pr_task_id_valid "$id"; then
      prefix=$id
      meta="$STATE/$id.meta"
      data="$STATE/$id.pr-poll"
      registration="$STATE/$id.pr-poll-registration"
