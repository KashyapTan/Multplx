...
# Portion 09 Rust-default adapter. Selection occurs before read-only probes or
# either proof-bound repair path can inspect state.
...
if [ "$implementation" = rust ]; then
  ...
  exec "$rust_bin" session mx-doctor.sh "$@"
fi
