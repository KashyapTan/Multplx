#!/usr/bin/env bash
# mx-decision-hold.sh - deterministic mechanics for durable maintainer decisions.
#
...
# All backlog mutations run in the active MX_HOME, which keeps main-home and
# daemon-home ownership aligned with the work that discovered the decision.
#
# Usage:
#   mx-decision-hold.sh id <origin-id> <decision-key>
#   mx-decision-hold.sh hold <origin-id> <decision-key> \
...
#   mx-decision-hold.sh resolve <origin-id> <decision-key> \
#     --decision-file <path> --routed-to <task-id> [--routed-to <task-id>...]
#
# `complete` is the shared investigation and visual-review completion gate.
# `--none` is an explicit semantic attestation that the just-reviewed surface has
# no unresolved maintainer decision. Later review passes may add keys; a live task's
...
# It writes the maintainer decision and routed identities into the hold body, clears
# those dependency edges, and only then marks the hold Done. A failure before the
# final step leaves the maintainer hold open.
set -eu
...
verify_hold_active() {  # <hold-id>
  ...
  show=$(task_show "$id") || fail "maintainer hold $id is absent from $MX_HOME/data/backlog.md"
  state=$(show_field "$show" state)
  held=$(show_field "$show" held)
  kind=$(show_field "$show" kind)
  hold_kind=$(show_field "$show" hold_kind)
  [ "$state" = queued ] || fail "maintainer hold $id is not queued (state=$state)"
  [ "$held" = yes ] || fail "maintainer hold $id is not active"
  [ "$kind" = maintainer ] || fail "backlog item $id is not kind maintainer"
  [ "$hold_kind" = maintainer ] || fail "backlog item $id is not held for the maintainer"
}
...
verify_hold_resolved() {  # <hold-id>
  ...
  [ "$state" = "done" ] || return 1
  [ "$kind" = maintainer ] || return 1
  case "$body" in
    *"Resolution recorded by mx-decision-hold."*"Routed work:"*) return 0 ;;
  esac
  return 1
}
...
verify_hold_durable() {  # <hold-id>
  ...
  show=$(task_show "$id") || fail "maintainer decision $id is absent from $MX_HOME/data/backlog.md"
  state=$(show_field "$show" state)
  held=$(show_field "$show" held)
  kind=$(show_field "$show" kind)
  hold_kind=$(show_field "$show" hold_kind)
  body=$(show_field "$show" body)
  if [ "$state" = queued ] && [ "$held" = yes ] && [ "$kind" = maintainer ] && [ "$hold_kind" = maintainer ]; then
    return 0
  fi
  if [ "$state" = "done" ] && [ "$kind" = maintainer ]; then
    case "$body" in
      *"Resolution recorded by mx-decision-hold."*"Routed work:"*) return 0 ;;
    esac
  fi
  fail "maintainer decision $id is neither actively held nor durably resolved"
}
...
verify_resolution_identity() {
  ...
  resolution_prefix='"Resolution recorded by mx-decision-hold.\nDecision digest: '
  case "$hold_body" in
    "$resolution_prefix"*) resolution_fields=${hold_body#"$resolution_prefix"} ;;
    *) fail "maintainer hold $id has no retry identity record" ;;
  esac
  case "$resolution_fields" in
    *'\nRouted identities: '*'\n\nMaintainer decision:'*) : ;;
    *) fail "maintainer hold $id has an invalid retry identity record" ;;
  esac
  ...
  [ "$recorded_digest" = "$decision_digest" ] \
    || fail "maintainer hold $id records a different maintainer decision"
  [ "$recorded_routes" = "$routed_csv" ] \
    || fail "maintainer hold $id records different routed work"
}
...
command_hold() {
  ...
  if show=$(task_show "$id"); then
    ...
    [ "$state" != "done" ] || fail "maintainer decision $id is already durably resolved; use a new decision key for a new decision"
    [ "$kind" = maintainer ] || fail "existing backlog identity $id is not kind maintainer"
    [ "$existing_title" = "$title" ] || fail "existing maintainer hold $id has a different title"
  else
...
EOF
  ...
  while IFS=$'\t' read -r key _verb _summary; do
    ...
    list_has_key "$keys" "$key" \
      || fail "open structured decision $origin/$key has no maintainer-held inventory entry"
  done <<EOF
$open
EOF
  ...
  if [ "$has_meta" = 1 ]; then
    ...
    # live status fold does not duplicate the same Maintainer's Call item.
    while IFS=$'\t' read -r key _verb _summary; do
      [ -n "$key" ] || continue
      list_has_key "$keys" "$key" || continue
      printf 'maintainer-held [key=%s]: tracked by %s\n' "$key" "$(hold_id "$origin" "$key")" >> "$status_file"
      key_seen=1
    done <<EOF
$raw_open
EOF
...
command_resolve() {
  ...
  for dep in $routed; do
    ...
    case ",$blocked," in
      ...
      *)
        case "$hold_body" in
          *"Resolution recorded by mx-decision-hold."*"- $dep"*) : ;;
  ...
  body=$(printf 'Resolution recorded by mx-decision-hold.\nDecision digest: %s\nRouted identities: %s\n\nMaintainer decision:\n%s\n\nRouted work:\n' "$decision_digest" "$routed_csv" "$decision")
  for dep in $routed; do
    body="${body}- ${dep}"$'\n'
  done
