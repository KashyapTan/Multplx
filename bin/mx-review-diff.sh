#!/usr/bin/env bash
# Review an actor branch against the authoritative base.
#
...
# current after any post-delivery update to the PR. A recorded pr_head= is
...
# Usage: mx-review-diff.sh <task-id> [--stat]
...
if [ "$implementation" = rust ]; then
  ...
  exec "$rust_bin" review mx-review-diff.sh "$@"
fi
...
usage() {
  echo "usage: mx-review-diff.sh <task-id> [--stat]" >&2
}
...
if ! git -C "$WT" rev-parse --verify --quiet "refs/heads/$BRANCH" >/dev/null; then
  ...
  [ -n "$BRANCH" ] || { echo "error: branch mx/$ID does not exist and worktree $WT is detached" >&2; exit 1; }
...
fetch_pull_head() {
  ...
  git -C "$WT" fetch --quiet origin \
    "+refs/pull/$n/head:refs/mx-review/pull/$n/head" >/dev/null 2>&1 || return 1
  resolved=$(git -C "$WT" rev-parse --verify "refs/mx-review/pull/$n/head^{commit}" 2>/dev/null) || return 1
  [ -n "$resolved" ] || return 1
  printf '%s' "$resolved"
}
