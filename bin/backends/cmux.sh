...
# layer, so workspace titles are scoped by Multplx home and installation
...
#      (mx_backend_cmux_current_path below).
...
#      as mx_backend_cmux_target_ready's liveness probe (the design sketch's
...
#      gap and is used instead (mx_backend_cmux_surface_exists), a
...
#      workspace in its window. `mx_backend_cmux_kill` handles the documented
...
#      No live app restart of the maintainer's own content was performed to
#      confirm this; see docs/cmux-backend.md for the reasoning. Recovery
#      therefore uses scoped-title matching from the caller-facing mx-<id>
...
#      shared cmux app cannot cross-match another Multplx home's task.
#
...
#   inside cmux can connect"). Since broker always drives cmux from an
...
# see mx_backend_cmux_bin), jq (JSON parsing). Bootstrap detects these through
# mx_backend_required_tools only when cmux is the resolved backend; this adapter
# also gates them again before spawning.

# MX_HOME fallback: every real caller already sets MX_HOME as a global before
# sourcing mx-backend.sh (which sources this file); this exists only so this
# file's own unit tests, which source it directly, resolve sanely.
MX_BACKEND_CMUX_ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")/../.." && pwd)"
MX_ROOT="${MX_ROOT_OVERRIDE:-${MX_ROOT:-$MX_BACKEND_CMUX_ROOT}}"
MX_HOME="${MX_HOME:-${MX_ROOT_OVERRIDE:-$MX_ROOT}}"

# shellcheck source=bin/mx-backend-hometag-lib.sh
. "$MX_BACKEND_CMUX_ROOT/bin/mx-backend-hometag-lib.sh"
...
. "$MX_BACKEND_CMUX_ROOT/bin/mx-composer-lib.sh"

# Verified minimum: the version the live pass ran against (docs/cmux-backend.md).
MX_BACKEND_CMUX_MIN_MAJOR=0
MX_BACKEND_CMUX_MIN_MINOR=64

# mx_backend_cmux_bin: resolve the cmux CLI binary. cmux does not reliably
# land on PATH after a plain app install - it delivers an OPTIONAL "install CLI"
...
MX_BACKEND_CMUX_BUNDLE_BIN="${MX_BACKEND_CMUX_BUNDLE_BIN:-/Applications/cmux.app/Contents/Resources/bin/cmux}"
mx_backend_cmux_bin() {
  if command -v cmux >/dev/null 2>&1; then
    printf 'cmux'
    return 0
  fi
  if [ -x "$MX_BACKEND_CMUX_BUNDLE_BIN" ]; then
    printf '%s' "$MX_BACKEND_CMUX_BUNDLE_BIN"
    return 0
  fi
  return 1
}

mx_backend_cmux_tool_check() {
  mx_backend_cmux_bin >/dev/null 2>&1 || { echo "error: backend=cmux selected but the 'cmux' CLI was not found on PATH or at $MX_BACKEND_CMUX_BUNDLE_BIN (https://cmux.com)" >&2; return 1; }
...
# mx_backend_cmux_password: the optional socket password from
...
# is absent - mx_backend_cmux_cli only exports this when it resolves non-empty.
mx_backend_cmux_password() {
  local config_dir="${MX_CONFIG_OVERRIDE:-$MX_HOME/config}" f line
...
# mx_backend_cmux_cli: run `cmux <args...>`, quieted (suppresses legacy-alias
...
mx_backend_cmux_cli() {  # <cmux-subcommand-and-args...>
  local bin pw
  bin=$(mx_backend_cmux_bin) || return 1
  pw=$(mx_backend_cmux_password)
  if [ -n "$pw" ]; then
...
# mx_backend_cmux_version_check: refuse loudly on a missing/incompatible cmux
...
# separate from reachability/auth (mx_backend_cmux_ping_state below).
mx_backend_cmux_version_check() {
  mx_backend_cmux_tool_check || return 1
  local raw ver major rest minor
  raw=$(mx_backend_cmux_cli version 2>/dev/null) || { echo "error: 'cmux version' failed; is cmux installed correctly?" >&2; return 1; }
  ...
  if [ "$major" -lt "$MX_BACKEND_CMUX_MIN_MAJOR" ] || { [ "$major" -eq "$MX_BACKEND_CMUX_MIN_MAJOR" ] && [ "$minor" -lt "$MX_BACKEND_CMUX_MIN_MINOR" ]; }; then
    echo "error: cmux $ver is older than the verified minimum $MX_BACKEND_CMUX_MIN_MAJOR.$MX_BACKEND_CMUX_MIN_MINOR; update cmux before using backend=cmux" >&2
    return 1
  fi
  return 0
}

# mx_backend_cmux_ping_state: classify socket reachability/auth from `cmux
...
# relaunching the app.
mx_backend_cmux_ping_state() {
  local out
  out=$(mx_backend_cmux_cli ping 2>&1)
  if [ "$out" = "PONG" ]; then
    printf 'ok'
    return 0
  fi
...
# mx_backend_cmux_refuse_denied / mx_backend_cmux_refuse_unauth: the two
...
# mx_backend_cmux_ensure_running: launch cmux (mirrors the CLI's own
...
# launch on the wire.
mx_backend_cmux_ensure_running() {
  local state i
  state=$(mx_backend_cmux_ping_state)
  case "$state" in
    ok) return 0 ;;
    denied)
      mx_backend_cmux_refuse_denied
...
# mx_backend_cmux_container_ensure: the full spawn-time container-ensure
...
# straight to mx_backend_cmux_create_task.
mx_backend_cmux_container_ensure() {
  mx_backend_cmux_version_check || return 1
  mx_backend_cmux_ensure_running || return 1
  return 0
}

# mx_backend_cmux_home_label: readable home prefix plus a short hash of the
# resolved MX_ROOT path. cmux has one app-global workspace namespace, so the
# path hash distinguishes every Multplx installation, including multiple
...
# itself lives in bin/mx-backend-hometag-lib.sh.
mx_backend_cmux_home_label() {
  mx_backend_hometag
}

mx_backend_cmux_scoped_title() {  # <mx-task-label>
  ...
  printf 'mx-%s-%s' "$home" "$rest"
}

# mx_backend_cmux_workspace_id_for_label: the live workspace id whose title
...
# mx_backend_cmux_create_task: create the task's workspace (one surface),
...
mx_backend_cmux_create_task() {  # <label> <cwd>
  local label=$1 cwd=$2 title dup out wsid sfid
  title=$(mx_backend_cmux_scoped_title "$label")
  dup=$(mx_backend_cmux_workspace_id_for_label "$title")
  if [ -n "$dup" ]; then
    echo "error: cmux workspace '$title' already exists" >&2
    return 1
  fi
  out=$(mx_backend_cmux_cli new-workspace --name "$title" --cwd "$cwd" --focus false --id-format uuids 2>&1) || {
    echo "error: cmux new-workspace failed for '$title': $out" >&2
    return 1
  }
  wsid=$(mx_backend_cmux_workspace_id_for_label "$title")
...
# mx_backend_cmux_parse_target: split "<workspace_uuid>:<surface_uuid>" on the
# FIRST colon (neither UUID contains a colon, so this is unambiguous). Sets
# MX_BACKEND_CMUX_WORKSPACE and MX_BACKEND_CMUX_SURFACE for the caller.
mx_backend_cmux_parse_target() {  # <target>
  local target=$1
  MX_BACKEND_CMUX_WORKSPACE=${target%%:*}
  MX_BACKEND_CMUX_SURFACE=${target#*:}
  [ -n "$MX_BACKEND_CMUX_WORKSPACE" ] && [ -n "$MX_BACKEND_CMUX_SURFACE" ] && [ "$MX_BACKEND_CMUX_SURFACE" != "$target" ]
}

# mx_backend_cmux_surface_exists: does <surface_id> currently appear as one of
...
# would make read-screen unusable as mx_backend_cmux_target_ready's liveness
...
mx_backend_cmux_target_ready() {  # <target> [expected-label]
  ...
  if [ -n "$expected_label" ]; then
    ...
    if [ "$title" = "$expected_title" ]; then
      ...
      wsid=$(mx_backend_cmux_workspace_id_for_label "$expected_title")
      [ -n "$wsid" ] || return 1
    fi
    sfid=$(mx_backend_cmux_surface_id_for_workspace "$wsid")
    [ -n "$sfid" ] || return 1
    MX_BACKEND_CMUX_WORKSPACE=$wsid
    MX_BACKEND_CMUX_SURFACE=$sfid
    return 0
  fi
...
mx_backend_cmux_current_path() {  # <target> [expected-label]
  ...
  mx_backend_cmux_send_text_line "$target" "printf '%s\n' '$marker_begin'; pwd; printf '%s\n' '$marker_end'" "$expected_label" || return 0
  sleep 0.3
  out=$(mx_backend_cmux_capture "$target" 200 "$expected_label") || return 0
...
# mx_backend_cmux_send_literal: send TEXT as literal, UNSUBMITTED input - the
...
mx_backend_cmux_send_literal() {  # <target> <text> [expected-label]
  mx_backend_cmux_target_ready "$1" "${3:-}" || return 1
  mx_backend_cmux_cli send --workspace "$MX_BACKEND_CMUX_WORKSPACE" --surface "$MX_BACKEND_CMUX_SURFACE" -- "$2" >/dev/null 2>&1
}

# mx_backend_cmux_normalize_key: map broker's key vocabulary (Enter,
...
# sigint/sigtstp/sigquit - `TerminalSurface+Input.swift`), but broker's
# shared vocabulary across backends only needs these three today.
mx_backend_cmux_normalize_key() {  # <key>
  case "$1" in
...
# mx_backend_cmux_send_key: one named special key. Escape IS natively
# supported here, so it is wired directly.
mx_backend_cmux_send_key() {  # <target> <key> [expected-label]
  mx_backend_cmux_target_ready "$1" "${3:-}" || return 1
  local key
  key=$(mx_backend_cmux_normalize_key "$2")
  mx_backend_cmux_cli send-key --workspace "$MX_BACKEND_CMUX_WORKSPACE" --surface "$MX_BACKEND_CMUX_SURFACE" "$key" >/dev/null 2>&1
}
...
# mx_backend_cmux_capture: bounded plain-text surface capture. No herdr-style
...
mx_backend_cmux_capture() {  # <target> <lines> [expected-label]
  mx_backend_cmux_target_ready "$1" "${3:-}" || return 1
  local lines=${2:-200} fetch raw out
  case "$lines" in ''|*[!0-9]*) lines=200 ;; esac
  fetch=$lines
  case "$fetch" in ''|*[!0-9]*) fetch=200 ;; *) [ "$fetch" -ge 200 ] || fetch=200 ;; esac
  raw=$(mx_backend_cmux_cli read-screen --workspace "$MX_BACKEND_CMUX_WORKSPACE" --surface "$MX_BACKEND_CMUX_SURFACE" --scrollback --lines "$fetch" --json 2>/dev/null) || return 1
  out=$(printf '%s' "$raw" | jq -r '.text // empty' 2>/dev/null) || return 1
  printf '%s' "$out" | tail -n "$lines"
}

# mx_backend_cmux_composer_state: classify the composer's own row as
# empty|pending|unknown. Adapted from the bordered-row branch of herdr's
# structural classifier (mx_backend_herdr_composer_state) per the build task's
...
MX_BACKEND_CMUX_COMPOSER_LINES=${MX_BACKEND_CMUX_COMPOSER_LINES:-20}
MX_BACKEND_CMUX_IDLE_RE=${MX_BACKEND_CMUX_IDLE_RE:-'^Type a message\.\.\.$'}

mx_backend_cmux_composer_state() {  # <target> [expected-label] -> empty|pending|unknown
  local target=$1 expected_label=${2:-} cap line trimmed stripped="" found=0
  cap=$(mx_backend_cmux_capture "$target" "$MX_BACKEND_CMUX_COMPOSER_LINES" "$expected_label") || { printf 'unknown'; return 0; }
  while IFS= read -r line; do
  ...
  mx_composer_classify_content 1 "$stripped" "$MX_BACKEND_CMUX_IDLE_RE"
}

# mx_backend_cmux_send_text_submit: type <text> into <target> once (raw,
# unsubmitted, via send_literal), then submit with a named Enter key, retried
# (Enter only, never retyped) until the composer's own row reads empty.
# Mirrors mx_backend_herdr_send_text_submit's ORIGINAL (composer-row)
...
mx_backend_cmux_send_text_submit() {  # <target> <text> <retries> <enter-sleep> <settle> [expected-label]
  local target=$1 text=$2 retries=$3 sleep_s=$4 settle=$5 expected_label=${6:-} i=0 state
  mx_backend_cmux_parse_target "$target" || { printf 'unknown'; return 0; }
  mx_backend_cmux_send_literal "$target" "$text" "$expected_label" || { printf 'send-failed'; return 0; }
  sleep "$settle"
  while :; do
    mx_backend_cmux_send_key "$target" Enter "$expected_label" || true
    sleep "$sleep_s"
    state=$(mx_backend_cmux_composer_state "$target" "$expected_label")
...
# mx_backend_cmux_window_of_workspace: echo "<window_id> <workspace_count>" for
...
mx_backend_cmux_window_of_workspace() {  # <workspace_id> -> "<window_id> <count>"
  local wsid=$1 wins wid wss count
  wins=$(mx_backend_cmux_cli list-windows --json --id-format uuids 2>/dev/null) || return 0
  while IFS= read -r wid; do
    [ -n "$wid" ] || continue
    wss=$(mx_backend_cmux_cli workspace list --json --id-format uuids --window "$wid" 2>/dev/null) || continue
...
# mx_backend_cmux_kill: remove the task's whole workspace, best-effort (mirrors
...
# leaving that window a fresh default workspace (never an mx-<home>- title, so
# recovery/list_live ignore it) - cmux's own "closed the last tab" outcome.
mx_backend_cmux_kill() {  # <target> [unused] [expected-label]
  local expected_label=${3:-} wsid wininfo win count
  if [ -n "$expected_label" ]; then
    mx_backend_cmux_target_ready "$1" "$expected_label" || return 0
  else
    mx_backend_cmux_parse_target "$1" || return 0
  fi
  wsid=$MX_BACKEND_CMUX_WORKSPACE
  wininfo=$(mx_backend_cmux_window_of_workspace "$wsid")
  win=${wininfo%% *}
  count=${wininfo##* }
  if [ -n "$win" ] && [ "$count" = 1 ]; then
    mx_backend_cmux_cli new-workspace --window "$win" --focus false --id-format uuids >/dev/null 2>&1 || true
  fi
  mx_backend_cmux_cli close-workspace --workspace "$wsid" >/dev/null 2>&1 || true
}

# mx_backend_cmux_list_live: recovery/orphan discovery. Lists every workspace
# whose title is scoped to this Multplx home, by TITLE - never by trusting a
