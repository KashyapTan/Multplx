#!/usr/bin/env bash
# Provision and route persistent daemon homes.
#
# Usage:
#   mx-home-seed.sh <id> <home|-> {<project>...|--no-projects}
#       Provision <home> as an isolated Multplx home. If <home> is "-", acquire
#       a fresh broker worktree via "treehouse get --lease", which durably
#       leases the worktree under the daemon <id> so the home survives with
#       no live process and is never recycled until the lease is released with
#       "treehouse return". Projects are cloned
#       from the active home into the daemon home's projects/ directory.
...
#       subject is the Multplx repo itself; it is mutually exclusive with a
...
#       is copied to data/charter.md, an ignored .mx-daemon-home identity marker
#       is written, and
#       data/daemons.md is updated.
...
#       Set MX_DAEMON_CHARTER='<charter>' to seed from inline charter text
#       when no filled charter brief exists. Set MX_DAEMON_SCOPE='<scope>'
...
usage() {
  echo "usage: mx-home-seed.sh <id> <home|-> {<project>...|--no-projects}" >&2
  echo "       mx-home-seed.sh validate" >&2
}
...
registry_summary_for_brief() {
  local brief=$1
  if [ -n "${MX_DAEMON_CHARTER:-}" ]; then
    printf '%s\n' "$MX_DAEMON_CHARTER" | normalize_registry_text
  else
...
registry_scope_for_brief() {
  local brief=$1
  if [ -n "${MX_DAEMON_SCOPE:-}" ]; then
    printf '%s\n' "$MX_DAEMON_SCOPE" | normalize_registry_text
  else
...
validate_registry_home_text() {
  ...
  case "$home" in
    *';'*|*')'*|*$'\n'*)
      echo "error: daemon home path contains registry delimiters: $home" >&2
      return 1
      ;;
  esac
}
...
validate_registry() {
  ...
  tmp=$(mktemp "${TMPDIR:-/tmp}/mx-brokers.XXXXXX")
  if [ -f "$REG" ]; then
    while IFS= read -r line; do
      case "$line" in
        "- "*)
  ...
  ' "$tmp" 2>/dev/null) || {
    rm -f "$tmp"
    printf 'error: duplicate daemon home assignment:\n%s\n' "$duplicate_homes" >&2
    return 1
  }
  duplicate_ids=$(awk -F '\t' '
    {
  ...
  ' "$tmp" 2>/dev/null) || {
    rm -f "$tmp"
    printf 'error: duplicate daemon id assignment:\n%s\n' "$duplicate_ids" >&2
    return 1
  }
  overlaps=$(awk -F '\t' '
  ...
  ' "$tmp" 2>/dev/null) || {
    rm -f "$tmp"
    printf 'error: overlapping daemon home assignment:\n%s\n' "$overlaps" >&2
    return 1
  }
  rm -f "$tmp"
  return 0
...
refuse_active_home_path() {
  ...
  abs_active_home=$(resolved_path "$MX_HOME")
  abs_root=$(resolved_path "$MX_ROOT")
  if [ "$abs_home" = "/" ]; then
...
validate_seed_leaf_files() {
  ...
  for label in "data/projects.md" "data/charter.md" "$SUB_HOME_MARKER"; do
    path="$home/$label"
    if [ -L "$path" ]; then
      echo "error: daemon leaf file must not be a symlink: $path" >&2
      return 1
    fi
    [ -e "$path" ] || continue
    ...
    case "$abs_path" in
      "$abs_home"/*) ;;
      *)
        echo "error: daemon leaf file must resolve inside the daemon home: $path" >&2
        return 1
        ;;
    esac
  done
...
validate_project_destination() {
  ...
  if ! path_is_ancestor_of "$abs_home" "$abs_projects"; then
    echo "error: daemon projects directory must resolve inside the daemon home: $projects_dir" >&2
    return 1
  fi
  if ! path_is_ancestor_of "$abs_projects" "$abs_dst"; then
    echo "error: seeded project $project destination must resolve inside the daemon projects directory: $dst" >&2
    return 1
  fi
  if [ "$abs_dst" = "$abs_active_home" ] || path_is_ancestor_of "$abs_active_home" "$abs_dst"; then
    echo "error: seeded project $project destination cannot be inside the active Multplx home: $dst" >&2
    return 1
  fi
  if [ "$abs_dst" = "$abs_root" ] || path_is_ancestor_of "$abs_root" "$abs_dst"; then
    echo "error: seeded project $project destination cannot be inside the Multplx repo: $dst" >&2
    return 1
  fi
  printf '%s\n' "$abs_dst"
}
...
acquire_treehouse_home() {
  local id=$1 home
  # Durably lease a broker worktree from the pool. The lease persists with no
  ...
  home=$(cd "$MX_ROOT" && treehouse get --lease --lease-holder "$id") || {
    echo "error: treehouse get --lease failed to lease a Multplx home" >&2
    return 1
  }
  [ -n "$home" ] || { echo "error: treehouse get --lease did not report a Multplx home" >&2; return 1; }
  printf '%s\n' "$home"
}
...
verify_broker_home() {
  local home=$1
  refuse_active_home_path "$home" || return 1
  [ -f "$home/AGENTS.md" ] || { echo "error: $home is not a Multplx home (missing AGENTS.md)" >&2; return 1; }
  [ -d "$home/bin" ] || { echo "error: $home is not a Multplx home (missing bin/)" >&2; return 1; }
...
validate_home_assignment() {
  ...
  if [ -f "$home/$SUB_HOME_MARKER" ]; then
    ...
    if [ "$marker_id" != "$id" ]; then
      echo "error: daemon home $home is already marked for ${marker_id:-unknown}" >&2
      return 1
    fi
  fi
  id_conflict=$(registry_id_conflict_for_assignment "$id" "$home" || true)
  if [ -n "$id_conflict" ]; then
    echo "error: daemon id $id is already registered to home $id_conflict; retire it before assigning $home" >&2
    return 1
  fi
...
EOF
  if [ "$conflict_type" = exact ]; then
    echo "error: daemon home $home is already registered to $owner" >&2
    return 1
  fi
  echo "error: daemon home $home overlaps registered daemon home $registered_home for $owner" >&2
  return 1
}
...
clone_project() {
  ...
  read -r mode _ <<EOF
$(MX_HOME="$MX_HOME" MX_DATA_OVERRIDE="$DATA" "$MX_ROOT/bin/mx-project-mode.sh" "$project")
EOF
  if [ "$mode" = local-only ]; then
    echo "error: project $project is local-only; daemon routes support only deep-review and direct-PR projects" >&2
    return 1
  fi
  if [ -e "$dst" ]; then
...
seed_rollback_target() {
  ...
  abs_home=$(resolved_path "$MX_HOME")
  abs_root=$(resolved_path "$MX_ROOT")
  if [ "$abs_target" = "$abs_home" ]; then
    echo "REFUSED: unsafe $label rollback target $target is the active Multplx home" >&2
    return 1
  fi
  if [ "$abs_target" = "$abs_root" ]; then
    echo "REFUSED: unsafe $label rollback target $target is the Multplx repo" >&2
    return 1
  fi
  if path_is_ancestor_of "$abs_target" "$abs_home"; then
    echo "REFUSED: unsafe $label rollback target $target is an ancestor of the active Multplx home" >&2
    return 1
  fi
  if path_is_ancestor_of "$abs_target" "$abs_root"; then
    echo "REFUSED: unsafe $label rollback target $target is an ancestor of the Multplx repo" >&2
    return 1
  fi
  if path_is_ancestor_of "$abs_home" "$abs_target"; then
    echo "REFUSED: unsafe $label rollback target $target is inside the active Multplx home" >&2
    return 1
  fi
  if path_is_ancestor_of "$abs_root" "$abs_target"; then
    echo "REFUSED: unsafe $label rollback target $target is inside the Multplx repo" >&2
    return 1
  fi
  printf '%s\n' "$abs_target"
}
...
seed_return_treehouse_home() {
  ...
  ( cd "$MX_ROOT" && treehouse return --force "$abs_home" >/dev/null ) || {
...
seed_project_rollback_target() {
  ...
  if ! path_is_ancestor_of "$abs_home" "$abs_projects"; then
    echo "REFUSED: unsafe created project rollback target $target has projects directory outside the daemon home" >&2
    return 1
  fi
  if ! path_is_ancestor_of "$abs_projects" "$abs_target"; then
    echo "REFUSED: unsafe created project rollback target $target is outside the daemon projects directory" >&2
    return 1
  fi
  printf '%s\n' "$abs_target"
}
...
seed_rollback() {
  ...
  if [ -n "${SEED_BACKUP_DIR:-}" ]; then
    restore_seed_file "$SEED_PARENT_REG_EXISTED" "$SEED_BACKUP_DIR/parent-daemons.md" "$REG"
...
project_mode_in_home() {
  ...
  read -r mode _ <<EOF
$(MX_ROOT_OVERRIDE='' MX_STATE_OVERRIDE='' MX_DATA_OVERRIDE='' MX_PROJECTS_OVERRIDE='' MX_CONFIG_OVERRIDE='' MX_HOME="$home" "$MX_ROOT/bin/mx-project-mode.sh" "$project")
EOF
  printf '%s\n' "$mode"
}
...
refuse_populated_projectless_home() {
  ...
  echo "error: cannot seed project-less daemon home $home because it contains project data" >&2
  if [ "${#clones[@]}" -gt 0 ]; then
...
refuse_projectful_projectless_charter() {
  ...
  printf 'error: cannot seed project-less daemon home because existing charter brief at %s conflicts with --no-projects\n' "$brief" >&2
  printf 'error: re-scaffold it with mx-brief.sh %s --daemon --no-projects or remove the stale brief before seeding\n' "$id" >&2
  return 1
}
...
seed_home() {
  ...
  if [ "$no_projects" -eq 1 ]; then
    ...
    [ $# -gt 0 ] || { echo "error: daemon needs at least one project, or --no-projects for a project-less home" >&2; return 1; }
  fi
  ...
  SEED_HOME_ACQUIRED=0
  SEED_HOME_BACKED_UP=0
  SEED_BACKUP_DIR=$(mktemp -d "${TMPDIR:-/tmp}/mx-home-seed.XXXXXX")
  ...
  if [ -f "$REG" ]; then
    ...
    cp "$REG" "$SEED_BACKUP_DIR/parent-daemons.md"
  fi
  ...
  if [ ! -f "$SEED_PARENT_BRIEF" ]; then
    [ -n "${MX_DAEMON_CHARTER:-}" ] || {
      echo "error: no filled daemon charter brief at $SEED_PARENT_BRIEF; set MX_DAEMON_CHARTER or scaffold one and replace {TASK}" >&2
      return 1
    }
    [ -d "$DATA/$id" ] || SEED_PARENT_BRIEF_DIR_CREATED=1
    if [ "$no_projects" -eq 1 ]; then
      "$MX_ROOT/bin/mx-brief.sh" "$id" --daemon --no-projects
    else
      "$MX_ROOT/bin/mx-brief.sh" "$id" --daemon "$@"
    fi
    SEED_PARENT_BRIEF_CREATED=1
  fi
  if grep -F '{TASK}' "$SEED_PARENT_BRIEF" >/dev/null 2>&1; then
    echo "error: daemon charter brief at $SEED_PARENT_BRIEF still contains {TASK}; fill it before seeding" >&2
    return 1
  fi
  charter_summary=$(registry_summary_for_brief "$SEED_PARENT_BRIEF")
  [ -n "$charter_summary" ] || {
    echo "error: daemon charter brief at $SEED_PARENT_BRIEF has an empty Charter section; fill it before seeding" >&2
    return 1
  }
  charter_scope=$(registry_scope_for_brief "$SEED_PARENT_BRIEF")
  [ -n "$charter_scope" ] || {
    echo "error: daemon charter brief at $SEED_PARENT_BRIEF has an empty Routing scope section; fill it before seeding" >&2
    return 1
  }
