...
#       initialized, an ignored .mx-daemon-home identity marker is written, and
...
EOF
  if [ "$mode" = local-only ]; then
    echo "error: project $project is local-only; daemon routes support only no-mistakes and direct-PR projects" >&2
    return 1
  fi
  if [ -e "$dst" ]; then
