...
# teardown, and daemon backlog handoff.
# Usage: . bin/mx-tasks-axi-lib.sh
...
# by daemon handoffs (introduced in tasks-axi 0.2.2).
# `config/backlog-backend=manual` opts out of tasks-axi for routine broker
# backlog mutations, but validated daemon handoffs always use `tasks-axi mv`.
# Absent or any other value keeps the default tasks-axi backend path, falling
# back to manual mutation when the tool is not compatible.

mx_tasks_axi_version_parts() {
  local output
...
mx_tasks_axi_compatible() {
  local parts major minor patch rest
  parts=$(mx_tasks_axi_version_parts) || return 1
  [ -n "$parts" ] || return 1
  major=${parts%% *}
  rest=${parts#* }
  minor=${rest%% *}
  ...
  if [ "$major" -gt 0 ] ||
    ...
    mx_tasks_axi_update_has_archive_body && mx_tasks_axi_mv_has_multi_id
    return $?
  fi
  return 1
}

mx_tasks_axi_update_has_archive_body() {
  local output
  command -v tasks-axi >/dev/null 2>&1 || return 1
...
mx_tasks_axi_backend_available() {
  local config_dir=$1
  mx_backlog_backend_manual "$config_dir" && return 1
  mx_tasks_axi_compatible
}
