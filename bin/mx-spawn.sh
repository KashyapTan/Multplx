...
#   axes chosen by broker at intake. They are only threaded into harnesses whose
#   installed CLIs were verified to support that axis; unsupported axes are omitted
#   from that harness's launch rather than guessed.
#   --backend <name> is the explicit runtime session-provider backend for this
#   spawn. Without it, the script resolves MX_BACKEND, then config/backend, then
#   runtime auto-detection (the runtime broker itself is executing inside -
...
#   auto-detected tmux stays silent.
#   codex-app is not a known backend yet; docs/codex-app-backend.md owns that
#   blocked backend contract. Default tmux spawns do not write backend= to meta;
#   absent backend= means tmux. cmux does not support --daemon spawns yet.
#   A backend spawn refusal (missing dependency, version gate, unauthenticated
#   socket, or unsupported daemon mode) is terminal for that selected backend;
...
#   immediately after its owning parent (broker or daemon-<id>) contiguous
...
#   With no harness arg, an actor/scout spawn resolves the ACTOR harness only when
#   config/actor-dispatch.json is absent. When that file exists, actor/scout
#   spawns require an explicit harness so broker cannot silently skip dispatch
...
#   new adapters.
#   config/daemon-harness may also carry an optional model and effort as extra
#   whitespace-separated tokens ("<harness> [<model>] [<effort>]"). For a
#   --daemon spawn, those tokens apply only when this spawn also resolves its
#   harness from config/daemon-harness. An explicit per-spawn --harness,
...
#   A --daemon spawn also propagates the primary's declared inherited local
#   material, so the daemon's OWN actors inherit primary config and the
#   daemon receives the primary's read-only shared maintainer-preference file
#   (mx-config-inherit-lib.sh). A successful launch clears pending inherited
#   config reread generations because the new agent reads the converged files.
#   --scout records kind=scout in the task's meta (report deliverable, scratch worktree;
#   see AGENTS.md task lifecycle); --daemon records kind=daemon and launches in a
#   provisioned Multplx home; the default is kind=delivery.
#   Before a daemon launch, the home is locally fast-forwarded to the primary
...
#   If config/actor-dispatch.json exists, shared --harness is required for actor
...
# daemon spawns record mode=daemon, yolo=off, home=, and projects=.
set -eu
...
# Backend selection (data/mx-backend-design-d7): explicit --backend, else
# MX_BACKEND env, else config/backend, else runtime auto-detection, else
# default tmux (mx_backend_name). mx_backend_validate_spawn refuses unknown or
# non-spawn-capable backends. The resolved value is
# recorded in meta only when it is NOT tmux (mx-teardown.sh and mx-watch.sh's
# window_backend/mx_backend_of_meta already treat an absent backend= as tmux),
...
mx_backend_validate_spawn "$BACKEND" || exit 1
mx_backend_source "$BACKEND" || exit 1
if [ "$BACKEND" = cmux ] && [ "$KIND" = daemon ]; then
  echo "error: backend=cmux does not support --daemon spawns yet" >&2
  exit 1
fi
...
spawn_abort_cleanup() {
  ...
  if [ "$HERDR_PRESENTATION_ORDER_LOCK_HELD" = 1 ]; then
    HERDR_PRESENTATION_ORDER_LOCK_HELD=0
    mx_lock_release "$HERDR_PRESENTATION_ORDER_LOCK" || true
  fi
  if [ "$SPAWN_TASK_LOCK_HELD" = 1 ]; then
    SPAWN_TASK_LOCK_HELD=0
    mx_lock_release "$SPAWN_TASK_LOCK" || true
  fi
...
# <session> is required so daemon and primary spawns serialize against the
# same session without writing any other home's state directory.
spawn_herdr_presentation_order_lock_acquire() {
  local session=${1:-} attempt lock_path
  [ -n "$session" ] || session=$(mx_backend_herdr_session)
  lock_path=$(mx_backend_herdr_presentation_session_lock_path "$session") || return 1
  HERDR_PRESENTATION_ORDER_LOCK="$lock_path"
  attempt=0
  while [ "$attempt" -lt 50 ]; do
    if mx_lock_try_acquire "$HERDR_PRESENTATION_ORDER_LOCK"; then
...
# the MX_ROOT path (not $0) so it works whatever cwd or relative path invoked us, and reuse
...
if [ "${#POS[@]}" -gt 0 ] && [ "${POS[0]}" != "$idpart" ] && case "$idpart" in */*) false ;; *) true ;; esac; then
  ...
  for pair in "${POS[@]}"; do
    ...
    if [ "$KIND" = daemon ]; then
      echo "error: batch dispatch does not support --daemon; spawn each daemon explicitly" >&2
      rc=2
      continue
    elif [ "$KIND" = scout ]; then
      if MX_SPAWN_NO_GUARD=1 "$MX_ROOT/bin/mx-spawn.sh" "${pair%%=*}" "${pair#*=}" "${shared_args[@]+"${shared_args[@]}"}" --scout; then :; else echo "batch: FAILED to spawn ${pair%%=*} (${pair#*=})" >&2; rc=1; fi
    else
      if MX_SPAWN_NO_GUARD=1 "$MX_ROOT/bin/mx-spawn.sh" "${pair%%=*}" "${pair#*=}" "${shared_args[@]+"${shared_args[@]}"}"; then :; else echo "batch: FAILED to spawn ${pair%%=*} (${pair#*=})" >&2; rc=1; fi
    fi
  done
  exit "$rc"
fi
...
launch_template() {
  local harness=$1 kind=${2:-delivery}
  # shellcheck disable=SC2016  # single quotes are deliberate: $(cat ...) expands in the actor pane, not here
  case "$harness" in
    # CLAUDE_CODE_ENABLE_PROMPT_SUGGESTION=false disables claude's interactive
    # predicted-next-prompt ghost text, which renders as dim/faint text inside an
    # otherwise-empty composer and would otherwise read like real typed input when
    # broker captures the pane (see the harness-adapters skill). It is a per-launch env
    # prefix scoped to this broker-launched agent; it never touches the maintainer's
...
case "$ARG3" in
  *' '*)  # raw launch command (unverified-adapter escape hatch)
    ...
    ;;
  '')
    # No explicit harness: resolve from config. A daemon AGENT launches on the
    # daemon harness (config/daemon-harness -> config/actor-harness -> own);
    # every other kind uses the actor harness only when no dispatch profile file is
    # active. Resolving here on every spawn is what makes the split DURABLE - a
    # respawn (recovery, /updatemultplx, restart) re-resolves, so
    # config/daemon-harness keeps governing daemon launches across restarts.
    # The launch_template lookup below is the unverified-adapter guard for both
    # kinds: a harness with no template aborts the spawn.
    if [ "$KIND" = daemon ]; then
      HARNESS=$("$MX_ROOT/bin/mx-harness.sh" daemon)
      harness_src='config/daemon-harness (falling back to config/actor-harness)'
    else
      if [ -f "$CONFIG/actor-dispatch.json" ]; then
        echo "error: config/actor-dispatch.json is active - pass an explicit harness resolved from the dispatch rules (the consultation backstop, so the rules are never silently skipped)." >&2
        exit 1
      fi
      HARNESS=$("$MX_ROOT/bin/mx-harness.sh" actor)
      harness_src='config/actor-harness'
    fi
...
# config/daemon-harness may carry optional model/effort tokens alongside the
# harness ("<harness> [<model>] [<effort>]"). They apply only when this is a
# --daemon spawn and no explicit per-spawn harness/raw launch was supplied, so
# the harness itself came from the daemon config fallback chain. Resolving
# here on every spawn makes the pin durable across respawns. Precedence: explicit
# --model/--effort flags still win over the file's tokens.
if [ "$KIND" = daemon ] && [ -z "$ARG3" ]; then
  if [ "$MODEL_SET" -eq 0 ]; then
    SM_MODEL=$("$SCRIPT_DIR/mx-harness.sh" daemon-model)
    [ -z "$SM_MODEL" ] || MODEL=$SM_MODEL
  fi
  if [ "$EFFORT_SET" -eq 0 ]; then
    SM_EFFORT=$("$SCRIPT_DIR/mx-harness.sh" daemon-effort)
    if [ -n "$SM_EFFORT" ]; then
      case "$SM_EFFORT" in
        low|medium|high|xhigh|max) EFFORT=$SM_EFFORT ;;
        *) echo "warning: config/daemon-harness effort token '$SM_EFFORT' is not one of low, medium, high, xhigh, max; ignoring" >&2 ;;
      esac
    fi
  fi
fi

daemon_registry_value() {
...
resolved_existing_dir() {
  local path=$1
  [ -d "$path" ] || { echo "error: Multplx home does not exist or is not a directory: $path" >&2; return 1; }
  cd "$path" && pwd -P
}
...
validate_broker_home_for_spawn() {
  local id=$1 home=$2 abs_home abs_active_home abs_root marker_id
  abs_home=$(resolved_existing_dir "$home") || return 1
  abs_active_home=$(resolved_existing_dir "$MX_HOME")
  abs_root=$(resolved_existing_dir "$MX_ROOT")
  if [ "$abs_home" = "/" ]; then
    echo "error: daemon home cannot be the filesystem root: $home" >&2
    return 1
  fi
  if [ "$abs_home" = "$abs_active_home" ]; then
    echo "error: daemon home cannot be the active Multplx home: $home" >&2
    return 1
  fi
  if [ "$abs_home" = "$abs_root" ]; then
    echo "error: daemon home cannot be the Multplx repo: $home" >&2
    return 1
  fi
  if path_is_ancestor_of "$abs_active_home" "$abs_home"; then
    echo "error: daemon home cannot be inside the active Multplx home: $home" >&2
    return 1
  fi
  if path_is_ancestor_of "$abs_root" "$abs_home"; then
    echo "error: daemon home cannot be inside the Multplx repo: $home" >&2
    return 1
  fi
  if path_is_ancestor_of "$abs_home" "$abs_active_home"; then
    echo "error: daemon home cannot be an ancestor of the active Multplx home: $home" >&2
    return 1
  fi
  if path_is_ancestor_of "$abs_home" "$abs_root"; then
    echo "error: daemon home cannot be an ancestor of the Multplx repo: $home" >&2
    return 1
  fi
  validate_broker_operational_dirs "$abs_home" "$abs_active_home" "$abs_root" || return 1
  if [ ! -f "$abs_home/$SUB_HOME_MARKER" ]; then
    echo "error: Multplx home $home is not a seeded daemon home" >&2
    return 1
  fi
  marker_id=$(cat "$abs_home/$SUB_HOME_MARKER" 2>/dev/null || true)
  if [ "$marker_id" != "$id" ]; then
    echo "error: Multplx home $home is marked for daemon ${marker_id:-unknown}, expected $id" >&2
    return 1
  fi
  if [ ! -f "$abs_home/AGENTS.md" ]; then
    echo "error: $home is not a Multplx home (missing AGENTS.md)" >&2
    return 1
  fi
  if [ ! -d "$abs_home/bin" ]; then
    echo "error: $home is not a Multplx home (missing bin/)" >&2
    return 1
  fi
  printf '%s\n' "$abs_home"
}

validate_broker_operational_dirs() {
  local abs_home=$1 abs_active_home=$2 abs_root=$3 name dir abs_dir
  for name in data state config projects; do
    dir="$abs_home/$name"
    if [ -L "$dir" ] && [ ! -e "$dir" ]; then
      echo "error: daemon $name directory must resolve inside the daemon home: $dir" >&2
      return 1
    fi
    if [ -d "$dir" ]; then
      abs_dir=$(cd "$dir" && pwd -P)
    elif [ -e "$dir" ]; then
      echo "error: daemon $name path is not a directory: $dir" >&2
      return 1
    else
    ...
    if [ "$abs_dir" = "$abs_active_home" ] || path_is_ancestor_of "$abs_active_home" "$abs_dir"; then
      echo "error: daemon $name directory cannot be inside the active Multplx home: $dir" >&2
      return 1
    fi
    if [ "$abs_dir" = "$abs_root" ] || path_is_ancestor_of "$abs_root" "$abs_dir"; then
      echo "error: daemon $name directory cannot be inside the Multplx repo: $dir" >&2
      return 1
    fi
  done
}
...
if [ "$KIND" = daemon ]; then
  if [ -z "$MULTPLX_HOME" ] && [ -f "$STATE/$ID.meta" ]; then
    MULTPLX_HOME=$(grep '^home=' "$STATE/$ID.meta" | cut -d= -f2- || true)
  fi
  if [ -z "$MULTPLX_HOME" ]; then
    MULTPLX_HOME=$(daemon_registry_value "$ID" home || true)
  fi
fi

if [ "$KIND" = daemon ]; then
  [ -n "$MULTPLX_HOME" ] || { echo "error: no Multplx home supplied or registered for $ID" >&2; exit 1; }
  PROJ_ABS=$(validate_broker_home_for_spawn "$ID" "$MULTPLX_HOME")
  WT="$PROJ_ABS"
  # Local-HEAD sync: before launch, fast-forward this daemon's worktree to the
  # PRIMARY checkout's current default-branch commit, so a freshly spawned or
  # recovery-respawned daemon always runs the primary's version (AGENTS.md
  ...
  if sm_primary_head=$(primary_head_commit "$MX_ROOT"); then
    sm_ff_out=$(ff_target "$PROJ_ABS" "daemon $ID" "$sm_primary_head" yes yes 2>&1 || true)
    case "$sm_ff_out" in
      *': skipped:'*)
        sm_ff_line=$(first_line "$sm_ff_out")
        sm_ff_prefix="daemon $ID: skipped: "
        sm_ff_reason=${sm_ff_line#"$sm_ff_prefix"}
        echo "warning: daemon $ID sync skipped before launch: $sm_ff_reason" >&2
        ;;
    esac
  else
    echo "warning: daemon $ID sync skipped before launch: primary default-branch commit cannot be resolved" >&2
  fi
  mkdir -p "$PROJ_ABS/state" || {
    echo "error: could not create daemon state directory for $PROJ_ABS" >&2
    exit 1
  }
  CONFIG_INHERIT_LOCK=$(mx_config_inherit_lock_path "$PROJ_ABS") || {
    echo "error: could not resolve daemon inheritance lock for $PROJ_ABS" >&2
    exit 1
  }
  if ! mx_lock_acquire_wait "$CONFIG_INHERIT_LOCK"; then
    echo "error: could not acquire daemon inheritance lock for $PROJ_ABS" >&2
    exit 1
  fi
  CONFIG_INHERIT_LOCK_HELD=1
  # Inheritance propagation: push the primary-authoritative local inheritance
  # surface into this daemon home (mx-config-inherit-lib.sh).
  propagate_daemon_inheritance "$MX_HOME" "$PROJ_ABS" "$CONFIG" "$DATA" \
    || echo "warning: daemon $ID inheritance failed for $PROJ_ABS" >&2
...
# /private/tmp) when it came from the delivery/scout branch's logical `pwd` above.
# Every backend's own current-path read (tmux's pane_current_path, herdr's
# foreground_cwd, cmux's active pwd probe against the live shell) can
...
# per-backend routing (mx_backend_resolve_selector).
...
herdr_projection_existing_meta_allows_flat() {  # <meta>
  ...
  old_backend=$(mx_backend_of_meta "$meta")
  old_target=$(mx_backend_target_of_meta "$meta")
  [ -n "$old_target" ] || {
  ...
  if [ "$old_backend" = herdr ]; then
    mx_backend_herdr_parse_target "$old_target" || {
      echo "error: existing herdr endpoint for $ID is malformed; refusing duplicate launch" >&2
      return 1
    }
    target_session=$MX_BACKEND_HERDR_SESSION
    target_pane=$MX_BACKEND_HERDR_PANE
    ...
    mx_backend_herdr_server_ensure "$old_session" || {
      echo "error: existing herdr endpoint for $ID could not be inspected; refusing duplicate launch" >&2
      return 1
    }
    old_state=$(mx_backend_herdr_pane_agent_state "$old_session" "$old_pane")
    case "$old_state" in
  ...
  old_state=$(mx_backend_agent_alive "$old_backend" "$old_target")
  case "$old_state" in
    dead) return 0 ;;
    alive|unknown)
...
case "$BACKEND" in
  tmux)
    SES=$(mx_backend_tmux_container_ensure)
    T="$SES:$W"
    # #134 robustness (tmux): mx_backend_tmux_create_task captures a stable window
    # id and pins the window name (automatic-rename/allow-rename off) so a maintainer's
    # non-default tmux config cannot rename the window away from mx-<id> once
    # treehouse cd's into the worktree. WT_TARGET carries that stable id for the
    # rename-critical worktree-detection steps below; the persisted window= handle
    # stays $T (the name form), which is safe now that rename is disabled.
    WID=$(mx_backend_tmux_create_task "$SES" "$W" "$PROJ_ABS") || exit 1
    WT_TARGET="$WID"
    ;;
  herdr)
    # mx_backend_herdr_workspace_label resolves the target workspace from
    # MX_HOME. For every KIND except daemon, this process's own MX_HOME is
    # already the right home (the primary spawning its own actor/scout, or
    # a daemon spawning ITS OWN actor/scout from its own process's
    # MX_HOME - the latter needs no glue at all). A --daemon spawn is the
    # one case that does: it is the PRIMARY's own mx-spawn.sh process
    # launching a DIFFERENT home (PROJ_ABS, already validated above as the
    # daemon's home), so MX_HOME here still names the primary. Shadow it
    # to PROJ_ABS for just these two calls (bash restores it automatically
    # after each prefixed simple-command call) so the daemon's tab lands
    # in the daemon's own workspace, not the primary's "broker" one.
    HERDR_LABEL_HOME=$MX_HOME
    if [ "$KIND" = daemon ]; then
      HERDR_LABEL_HOME=$PROJ_ABS
    fi
    HERDR_PRESENTATION_JOURNAL=$(mx_backend_herdr_projection_journal_path "$STATE" "$ID")
    HERDR_PROJECTED=0
    if [ "$KIND" != daemon ] && [ -f "$CONFIG/herdr-presentation-spaces" ]; then
      HERDR_SES=$(mx_backend_herdr_session)
      HERDR_PARENT_LABEL=$(MX_HOME="$HERDR_LABEL_HOME" mx_backend_herdr_workspace_label)
      if [ -e "$HERDR_PRESENTATION_JOURNAL" ] || [ -L "$HERDR_PRESENTATION_JOURNAL" ]; then
        mx_backend_herdr_server_ensure "$HERDR_SES" || {
        ...
        if [ "${HERDR_RECOVERY_BACKEND:-}" = herdr ]; then
          set +e
          MX_HOME="$HERDR_LABEL_HOME" mx_backend_herdr_projection_reclaim_task \
          ...
          case "$HERDR_RECLAIM_STATUS" in
            0)
              ...
              HERDR_TAB_ID=$MX_BACKEND_HERDR_PROJECTION_TAB_ID
              HERDR_PANE_ID=$MX_BACKEND_HERDR_PROJECTION_PANE_ID
        ...
        if ! mx_backend_herdr_server_ensure "$HERDR_SES"; then
          ...
          HERDR_PARENT_WORKSPACE_ID=$(mx_backend_herdr_projection_parent_workspace_exact \
          ...
          if [ -z "$HERDR_PARENT_WORKSPACE_ID" ]; then
            ...
            HERDR_PROJECTION_ID=$(mx_backend_herdr_projection_journal_create "$STATE" "$ID") || exit 1
            HERDR_PROJECTION_LABEL=$(mx_backend_herdr_projection_workspace_label "$ID" "$HERDR_PROJECTION_ID")
            if ! MX_HOME="$HERDR_LABEL_HOME" mx_backend_herdr_projection_create_task \
              "$PROJ_ABS" "$HERDR_PROJECTION_LABEL" "$W"; then
              if [ "${MX_BACKEND_HERDR_PROJECTION_CLEANUP_SAFE:-0}" = 1 ]; then
                HERDR_PROJECTION_ABORT_CLEANUP=1
                HERDR_PROJECTION_ABORT_SESSION=$MX_BACKEND_HERDR_PROJECTION_SESSION
                HERDR_PROJECTION_ABORT_TASK_PANE=$MX_BACKEND_HERDR_PROJECTION_PANE_ID
                HERDR_PROJECTION_ABORT_SEEDED_PANE=$MX_BACKEND_HERDR_PROJECTION_SEEDED_PANE_ID
              fi
              exit 1
            fi
            HERDR_PROJECTED=1
            HERDR_SES=$MX_BACKEND_HERDR_PROJECTION_SESSION
            HERDR_WORKSPACE_ID=$MX_BACKEND_HERDR_PROJECTION_WORKSPACE_ID
            HERDR_SEEDED_DEFAULT_TAB_ID=$MX_BACKEND_HERDR_PROJECTION_SEEDED_TAB_ID
            HERDR_TAB_ID=$MX_BACKEND_HERDR_PROJECTION_TAB_ID
            HERDR_PANE_ID=$MX_BACKEND_HERDR_PROJECTION_PANE_ID
            HERDR_PROJECTION_ABORT_CLEANUP=1
            HERDR_PROJECTION_ABORT_SESSION=$HERDR_SES
            ...
            HERDR_HOME_ID=$(mx_backend_herdr_projection_home_identity "$HERDR_LABEL_HOME" 2>/dev/null || true)
            if [ -n "$HERDR_HOME_ID" ] \
    ...
    if [ "$HERDR_PROJECTED" -ne 1 ]; then
      HERDR_CONTAINER_RAW=$(MX_HOME="$HERDR_LABEL_HOME" mx_backend_herdr_container_ensure "$PROJ_ABS") || exit 1
      # mx_backend_herdr_container_ensure echoes "<session>:<workspace_id>\t<seeded_default_tab_id>"
      ...
      HERDR_TASK_IDS=$(MX_HOME="$HERDR_LABEL_HOME" mx_backend_herdr_create_task "$CONTAINER" "$W" "$PROJ_ABS" "$HERDR_SEEDED_DEFAULT_TAB_ID") || exit 1
...
EOF
    ...
    ;;
  cmux)
    mx_backend_cmux_container_ensure || exit 1
    CMUX_TASK_IDS=$(mx_backend_cmux_create_task "$W" "$PROJ_ABS") || exit 1
...
spawn_send_text_line() {  # <target> <text>
  case "$BACKEND" in
    tmux) mx_backend_tmux_send_text_line "$1" "$2" ;;
    herdr) mx_backend_herdr_send_text_line "$1" "$2" ;;
    cmux) mx_backend_cmux_send_text_line "$1" "$2" "$W" ;;
  esac
}
spawn_current_path() {  # <target>
  case "$BACKEND" in
    tmux) mx_backend_tmux_current_path "$1" ;;
    herdr) mx_backend_herdr_current_path "$1" ;;
    cmux) mx_backend_cmux_current_path "$1" "$W" ;;
  esac
}
spawn_send_literal() {  # <target> <text>
  case "$BACKEND" in
    tmux) mx_backend_tmux_send_literal "$1" "$2" ;;
    herdr) mx_backend_herdr_send_literal "$1" "$2" ;;
    cmux) mx_backend_cmux_send_literal "$1" "$2" "$W" ;;
  esac
}
spawn_send_key() {  # <target> <key>
  case "$BACKEND" in
    tmux) mx_backend_tmux_send_key "$1" "$2" ;;
    herdr) mx_backend_herdr_send_key "$1" "$2" ;;
...
if [ "$KIND" != daemon ]; then
  ...
  # active client's window, which would misread broker's OWN pane path as the
...
# Per-task temp root: /tmp/mx-<id>/ with Go's build temp nested at gotmp/. Go won't
# create GOTMPDIR, so mkdir before it is used; mx-teardown removes the whole root.
# Nested (not a bare /tmp/mx-<id>/gotmp) so other per-task temp can live alongside
...
// Multplx turn-end signal; written by mx-spawn.
// Use "turn_end" (fires after each turn the agent finishes), not "agent_end"
// (fires once, only when the whole run exits): the watcher needs a signal at
// every turn boundary so an idle actor is surfaced, not just at shutdown.
...
# Per-project delivery mode + yolo flag (bin/mx-project-mode.sh; the project-management skill and AGENTS.md task lifecycle).
# Recorded in meta so mx-teardown's safety check and the validate/merge stages can
# branch on them. Mode governs delivery tasks; a scout's deliverable is a report, not a
# merge, so scout teardown ignores mode.
DAEMON_PROJECTS=
if [ "$KIND" = daemon ]; then
  MODE=daemon
  YOLO=off
  DAEMON_PROJECTS=$(daemon_registry_value "$ID" projects || true)
else
  PROJ_NAME=$(basename "$PROJ_ABS")
  read -r MODE YOLO <<EOF
$("$MX_ROOT/bin/mx-project-mode.sh" "$PROJ_NAME")
EOF
fi
...
if [ "$KIND" = daemon ]; then
  if ! mx_config_reread_discard_pending "$PROJ_ABS" "$ID" "$MX_HOME"; then
    if mx_config_reread_quarantine_pending "$PROJ_ABS" "$ID" "$MX_HOME"; then
      echo "CONFIG_REREAD: daemon $ID: quarantined pre-relaunch generations after cleanup failure (destination=$PROJ_ABS/state/.mx-inherited-config-reread-quarantine source=$MX_HOME/state/.mx-inherited-config-reread-quarantine)" >&2
    else
