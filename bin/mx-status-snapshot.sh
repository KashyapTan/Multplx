#!/usr/bin/env bash
# mx-status-snapshot.sh - compact, bounded, TOON-by-default catchup projection.
#
# A thin wrapper OVER the canonical bin/mx-system-snapshot.sh. It does not parse
# system state itself: it shells out to `mx-system-snapshot.sh --json`, projects that
...
# projected model. The projection is view-specific: it DROPS fields from the catchup
...
# backlog roles, unresolved blockers, and maintainer actionability. It never infers
...
# Catchup never invents Underway rows from backlog-only ids; it discloses those
# gaps in omitted[] and, when invalid, a Charted Next gate line so the four-section
# chat cannot claim an empty system while main current state is broken.
#
# The landed section merges this home's Done with the canonical snapshot's
# daemon_landed roll-up (mx-system-snapshot.sh), so merges a daemon managed -
...
# daemon home whose backlog was unreadable; no GitHub/network call is involved.
...
#   --all-daemons include every aggregated daemon record
...
#   -h,--help        usage
#
# Output contract: `mx-catchup.v1`. Read-only; no locks, no mutation, no reports.
set -u
...
MX_STATUS_LANDED=${MX_STATUS_LANDED:-6}
MX_STATUS_LANDED_PER_HOME=${MX_STATUS_LANDED_PER_HOME:-$MX_STATUS_LANDED}
MX_STATUS_IN_FLIGHT=${MX_STATUS_IN_FLIGHT:-20}
MX_STATUS_DECISIONS=${MX_STATUS_DECISIONS:-20}
MX_STATUS_DAEMONS=${MX_STATUS_DAEMONS:-20}
MX_STATUS_GATES=${MX_STATUS_GATES:-20}
MX_STATUS_REPORTS=${MX_STATUS_REPORTS:-20}
MX_STATUS_RECORDED_PRS=${MX_STATUS_RECORDED_PRS:-20}
MX_STATUS_UNHEALTHY=${MX_STATUS_UNHEALTHY:-20}
MX_STATUS_PR_REPOS=${MX_STATUS_PR_REPOS:-10}
MX_STATUS_PR_LIMIT=${MX_STATUS_PR_LIMIT:-20}
MX_STATUS_PR_TIMEOUT=${MX_STATUS_PR_TIMEOUT:-20}
case "$MX_STATUS_PR_TIMEOUT" in ''|*[!0-9]*|0) MX_STATUS_PR_TIMEOUT=20 ;; esac
validate_bound() {  # <name> <value>
  case "$2" in ''|*[!0-9]*|0) echo "mx-status-snapshot: $1 must be a positive integer" >&2; exit 2 ;; esac
}
validate_bound MX_STATUS_LANDED "$MX_STATUS_LANDED"
validate_bound MX_STATUS_LANDED_PER_HOME "$MX_STATUS_LANDED_PER_HOME"
validate_bound MX_STATUS_IN_FLIGHT "$MX_STATUS_IN_FLIGHT"
validate_bound MX_STATUS_DECISIONS "$MX_STATUS_DECISIONS"
validate_bound MX_STATUS_DAEMONS "$MX_STATUS_DAEMONS"
validate_bound MX_STATUS_GATES "$MX_STATUS_GATES"
validate_bound MX_STATUS_REPORTS "$MX_STATUS_REPORTS"
validate_bound MX_STATUS_RECORDED_PRS "$MX_STATUS_RECORDED_PRS"
validate_bound MX_STATUS_UNHEALTHY "$MX_STATUS_UNHEALTHY"
validate_bound MX_STATUS_PR_REPOS "$MX_STATUS_PR_REPOS"
validate_bound MX_STATUS_PR_LIMIT "$MX_STATUS_PR_LIMIT"

usage() {
  cat <<'EOF'
usage: mx-status-snapshot.sh [--json] [--include-prs] [--fields <list>]
                               [--all-in-flight] [--all-decisions]
                               [--all-daemons] [--all-landed]
                               [--all-reports] [--all-queued]
                               [--all-recorded-prs] [--all-unhealthy]
                               [--all-pr-repos]

Compact catchup projection over mx-system-snapshot.sh. TOON by default.
...
Default fields: schema, home, generated, prs, in_flight{id,kind,state,doing},
  daemons{id,state,doing,provenance,freshness,age_seconds,contradiction,reason},
  decisions_open{id,key,verb,summary,owner}, landed{id,what,artifact,owner},
  gates{id,title,blocked_by,reason,owner}, reports{id,path}, recorded_prs{id,url},
  unhealthy_endpoints{...} (only when non-empty), omitted{surface,reveal}.
landed merges this home's Done with registered daemon homes' Done, bounded by
  a per-home cap (MX_STATUS_LANDED_PER_HOME) and an overall cap (MX_STATUS_LANDED),
...
For every registered daemon, readable structured facts from its own home are
...
Opt-in surfaces: --fields bodies|paths|actions|endpoints, --all-in-flight,
  --all-decisions, --all-daemons, --all-landed, --all-reports, --all-queued, --all-recorded-prs,
  --all-unhealthy, --all-pr-repos, --include-prs (adds candidate_prs).
Raise MX_STATUS_PR_LIMIT to expand per-repository open-PR results.
EOF
}
...
command -v jq >/dev/null 2>&1 || { echo "mx-status-snapshot: jq not found" >&2; exit 1; }

# The deterministic return-catch-up owner must clear before this or any other
# ordinary maintainer request proceeds. Catchup does not reproduce that policy;
# it only consults the shared read-only gate.
"$SCRIPT_DIR/mx-afk-return.sh" guard || exit $?

NOW=${MX_STATUS_NOW:-$(date -u +%Y-%m-%dT%H:%M:%SZ)}
if [ "$ALL_LANDED" = 1 ] || [ "$ALL_DAEMONS" = 1 ]; then
  if [ "$ALL_LANDED" = 1 ]; then
    SNAP=$(MX_SNAPSHOT_NOW="$NOW" MX_SNAPSHOT_DAEMONS=0 MX_SNAPSHOT_DAEMON_LANDED_PER_HOME=0 "$SYSTEM" --json) || exit $?
  else
    SNAP=$(MX_SNAPSHOT_NOW="$NOW" MX_SNAPSHOT_DAEMONS=0 "$SYSTEM" --json) || exit $?
  fi
else
  SNAP=$(MX_SNAPSHOT_NOW="$NOW" "$SYSTEM" --json) || exit $?
fi
HOME_LABEL=$(printf '%s' "$SNAP" | jq -er '.mx_home | strings | split("/") | (.[-2:] | join("/"))') \
  || { echo "mx-status-snapshot: invalid canonical snapshot" >&2; exit 1; }

# --- optional live PR enrichment (the ONLY network path) --------------------
PR_STATUS='not_requested (run: /catchup include PRs)'
CANDIDATE_PRS='[]'
PR_REPOS_TOTAL=0
PR_REPOS_SHOWN=0
PR_ROWS_CAPPED=0
...
gh_bounded() {  # <args...>
  if command -v timeout >/dev/null 2>&1; then
    GH_PROMPT_DISABLED=1 GH_NO_UPDATE_NOTIFIER=1 timeout "$MX_STATUS_PR_TIMEOUT" gh "$@"
  elif command -v gtimeout >/dev/null 2>&1; then
    GH_PROMPT_DISABLED=1 GH_NO_UPDATE_NOTIFIER=1 gtimeout "$MX_STATUS_PR_TIMEOUT" gh "$@"
  elif command -v perl >/dev/null 2>&1; then
    GH_PROMPT_DISABLED=1 GH_NO_UPDATE_NOTIFIER=1 perl -e 'my $t = shift; my $pid = fork; die "fork failed" unless defined $pid; if (!$pid) { setpgrp(0, 0); exec @ARGV } local $SIG{ALRM} = sub { kill "TERM", -$pid; select undef, undef, undef, 0.2; kill "KILL", -$pid; exit 124 }; alarm $t; waitpid $pid, 0; exit($? >> 8)' "$MX_STATUS_PR_TIMEOUT" gh "$@"
  else
    return 124
  fi
}
...
$(printf '%s' "$SNAP" | jq -r '.tasks[] | select(.kind != "daemon") | .paths.worktree.path // empty')
EOF

    for repo in $repos; do PR_REPOS_TOTAL=$((PR_REPOS_TOTAL + 1)); done
    nrepos=0; npr=0; nwarn=0; ncapped=0; rows='[]'
    pr_fetch_limit=$((MX_STATUS_PR_LIMIT + 1))
    for repo in $repos; do
      if [ "$ALL_PR_REPOS" != 1 ] && [ "$nrepos" -ge "$MX_STATUS_PR_REPOS" ]; then break; fi
      nrepos=$((nrepos + 1))
      ...
      [ -n "$out" ] || out='[]'
      repo_result=$(printf '%s' "$out" | jq --arg repo "$repo" --argjson limit "$MX_STATUS_PR_LIMIT" '
        [ .[] | {
          num:(.number|tostring),
          repo:$repo,
          task:(if (.headRefName // "" | startswith("mx/")) then (.headRefName | ltrimstr("mx/")) else "-" end),
          url:(.url // "-"),
      ...
      [ "$returned" -gt "$MX_STATUS_PR_LIMIT" ] && ncapped=$((ncapped + 1))
      npr=$((npr + cnt))
...
# --- projection: canonical snapshot -> mx-catchup.v1 model (JSON) ----------
MODEL=$(printf '%s' "$SNAP" | jq \
  --arg home "$HOME_LABEL" \
  --arg now "$NOW" \
  --arg prs "$PR_STATUS" \
  --arg fields "$FIELDS" \
  --argjson landed_n "$MX_STATUS_LANDED" \
  --argjson landed_per_home_n "$MX_STATUS_LANDED_PER_HOME" \
  --argjson in_flight_n "$MX_STATUS_IN_FLIGHT" \
  --argjson decisions_n "$MX_STATUS_DECISIONS" \
  --argjson daemons_n "$MX_STATUS_DAEMONS" \
  --argjson gates_n "$MX_STATUS_GATES" \
  --argjson reports_n "$MX_STATUS_REPORTS" \
  --argjson recorded_prs_n "$MX_STATUS_RECORDED_PRS" \
  --argjson unhealthy_n "$MX_STATUS_UNHEALTHY" \
  --argjson include_prs "$INCLUDE_PRS" \
  --argjson all_in_flight "$ALL_IN_FLIGHT" \
  --argjson all_decisions "$ALL_DECISIONS" \
  --argjson all_daemons "$ALL_DAEMONS" \
  ...
  | ((.daemon_landed.records) // []) as $mate_done
  ...
  | ([.tasks[] | select(.kind != "daemon") | .id]) as $live_ids
  | ([.tasks[] | select(.kind != "daemon" and .current_state.state == "working") | .id]) as $working_ids
  ...
  | ([ .tasks[]
     ...
     + [ (.daemon_current.records // [])[] as $m | $m.endpoints[]?
         | select(.endpoint.exists == false or .endpoint.agent_alive == "dead")
         | {id:($m.id + "/" + .id),backend:"daemon-home",target:(.endpoint.target // "-"),exists:.endpoint.exists,agent:.endpoint.agent_alive} ]) as $unhealthy_all
  | ([ (.daemon_current.records // [])[]
       | ([.decisions_open[]? | select(.source == "backlog" and .verb == "maintainer-hold")]) as $maintainer_holds
       | ([.holds[]? | select(.source == "backlog")]) as $backlog_holds
       | . + {
           status_maintainer_holds:$maintainer_holds,
           status_holds:(if .current.state == "maintainer_decision" then $backlog_holds else .holds end),
           status_state:(
             if .current.state == "maintainer_decision" then
               if ($maintainer_holds | length) > 0 then "maintainer_decision"
  ...
  | ([ if .daemon_current.registry.available == false then
         {id:"(registry)",state:"unknown",doing:(.daemon_current.registry.reason // "Registered daemon table unavailable"),
          provenance:(.daemon_current.registry.provenance // "registered-table"),
          freshness:(.daemon_current.registry.freshness.status // "unavailable"),
          age_seconds:null,contradiction:false,reason:(.daemon_current.registry.reason // "Registered daemon table unavailable")}
       else empty end ]
     + [ $daemon_views[]
       | {id,state:.status_state,
          doing:((if .status_state == "active_child_work" then
                    ([.active_children[] | .id + ": " + (.doing // .state)] | join("; "))
                  elif .status_state == "maintainer_decision" then
                    ([.status_maintainer_holds[] | .summary] | join("; "))
                  elif .status_state == "externally_held" then
                    ([.status_holds[] | .id + ": " + (.reason // "held")] | join("; "))
                  elif .status_state == "no_active_work" then "No active child work"
          ...
          reason:(.current.reason // "-")} ]) as $daemons_all
  | ([ .tasks[]
       | select(.kind != "daemon")
     ...
     + [ $daemon_views[]
         | select(.status_state == "active_child_work")
         | {id,kind:"daemon",state:.status_state,
            doing:([.active_children[] | .id + ": " + (.doing // .state)] | join("; ") | trunc(90))} ]) as $in_flight_all
  | ([ .backlog.records[]
         | select(.structured and .maintainer_actionable == true)
         | {id,key:.id,verb:"maintainer-hold",
            summary:((.title + ": " + .hold_reason) | trunc(90)),owner:"(main)"} ]
     + [ (.daemon_current.records // [])[] as $m | $m.decisions_open[]?
         | select(.source == "backlog" and .verb == "maintainer-hold")
         | {id:($m.id + "/" + .id),key,verb,
            summary:(((.summary // .id) + ": " + (.reason // "maintainer decision pending")) | trunc(90)),owner:$m.id} ]) as $decisions_all
  ...
  | ([ .tasks[] | select(.kind != "daemon" and .pr.url != null and .pr.source == "meta") | {id, url:.pr.url} ]) as $recorded_prs_all
  | . as $snap
  | {
      ...
      daemons: (if $all_daemons == 1 then $daemons_all else $daemons_all[:$daemons_n] end),
  ...
  | . + {omitted: (
      [ (if $f_bodies then empty else {surface:"backlog item bodies", reveal:"--fields bodies"} end),
        ...
        (if $all_landed == 0 and ($per_home_capped | length) > ($done | length) then {surface:("landed showing \($done | length) of \($per_home_capped | length)" + (($done | map(.home_id) | unique | map(select(. != "(main)")) | length) as $k | if $k > 0 then " (incl. \($k) daemon home(s))" else "" end)), reveal:"--all-landed"} else empty end),
        ...
        (if (($snap.daemon_landed.unreadable // []) | length) > 0 then {surface:("daemon home(s) with unreadable backlog: \(($snap.daemon_landed.unreadable // []) | length)"), reveal:"inspect the listed daemon home backlogs"} else empty end),
        (if $all_landed == 0 and (($snap.daemon_landed.truncated // []) | length) > 0 then {surface:("daemon home Done capped at the snapshot layer for \(($snap.daemon_landed.truncated // []) | length) home(s)"), reveal:"--all-landed"} else empty end),
        ...
        (if $all_daemons == 0 and ($daemons_all | length) > $daemons_n then {surface:("daemons showing \($daemons_n) of \($daemons_all | length)"), reveal:"--all-daemons"} else empty end),
        (if (($snap.daemon_current.truncated // 0) > 0) then {surface:("registered daemons omitted by snapshot bound: \($snap.daemon_current.truncated)"), reveal:"raise MX_SNAPSHOT_DAEMONS"} else empty end),
        (if $snap.daemon_current.registry.input_truncated == true then {surface:"daemon registry input truncated by bounded read", reveal:"raise MX_SNAPSHOT_REGISTRY_LINES or MX_SNAPSHOT_REGISTRY_BYTES"} else empty end),
        (if $snap.daemon_current.registry.records_truncated == true then {surface:"daemon registry records omitted by bounded read", reveal:"raise MX_SNAPSHOT_REGISTRY_RECORDS"} else empty end),
        (if $snap.daemon_current.registry.available == false then {surface:("daemon registry unavailable: " + ($snap.daemon_current.registry.reason // "read failed")), reveal:"inspect data/daemons.md"} else empty end),
        (([($snap.daemon_current.records // [])[] | select(.parent_event.activity_scan.input_truncated == true or .parent_event.activity_scan.retained_truncated == true)] | length) as $n | if $n > 0 then {surface:("daemon parent activity evidence truncated for \($n) record(s)"), reveal:"raise MX_SNAPSHOT_PARENT_ACTIVITY_LINES, MX_SNAPSHOT_PARENT_ACTIVITY_BYTES, or MX_SNAPSHOT_PARENT_ACTIVITIES"} else empty end),
        (([($snap.daemon_current.records // [])[] | select(.parent_event.activity_scan.available == false)] | length) as $n | if $n > 0 then {surface:("daemon parent activity evidence unavailable for \($n) record(s)"), reveal:"inspect the parent status logs"} else empty end),
        ...
        (if $include_prs == 1 and $pr_rows_capped > 0 then {surface:("candidate_prs showing \($candidate_prs | length) of at least \($pr_rows_min_total); capped in \($pr_rows_capped) repo(s)"), reveal:"raise MX_STATUS_PR_LIMIT"} else empty end),
        (if $include_prs == 1 then empty else {surface:"live PR discovery + checks", reveal:"--include-prs"} end) ]) }
') || { echo "mx-status-snapshot: projection failed" >&2; exit 1; }
...
') || { echo "mx-status-snapshot: TOON rendering failed" >&2; exit 1; }
printf '%s\n' "$TOON"
