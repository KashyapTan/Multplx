#!/usr/bin/env bash
# mx-supervise-daemon.sh — presence-gated sub-supervisor (closes #27's P2).
#
# Wraps bin/mx-watch.sh: runs it as a child, classifies each wake reason, and
# either SELF-HANDLES the routine majority in bash (no broker turn) or
# ESCALATES a batched, distilled digest to the supervisor pane on
# maintainer-relevant events plus bounded declared-pause rechecks. This is the
# token-efficient replacement for the prior always-inject daemon: routine
# signal/stale/heartbeat wakes cost zero broker context; only done/
...
# user message clears it and broker resumes full responsiveness.
# When afk is off, normal mx-watch.sh always-on triage is the active mechanism.
...
#
# IN-BAND OPERATIONAL INPUT. bin/mx-operational-input.sh constructs every
# current daemon injection as the typed away-supervisor kind after the stable
# MX_OPERATIONAL_PREFIX. A human cannot type its leading U+2063 from a normal
# keyboard at the start of a message, and Herdr transports it as text.
# Multplx's contract: a message that starts with the current prefix, or a
# legacy bare-marker daemon escalation, is internal (stay afk); an unmarked
# message means the maintainer is back (exit afk, flush catch-up, resume per-wake
...
#     crash/restart/missed injection is recovered on the next mx-wake-drain.sh.
...
#     (configurable), rechecked once. A wedged actor is therefore detected
...
#     Actors are autonomous, so a delayed stale response does not stall a
#     healthy actor's own progress.
#     Buffered escalation delivery also has a max-defer alarm: if a digest stays
#     undelivered past MX_MAX_DEFER_SECS, the daemon retries a normal flush and
...
#     state/*.status for a maintainer-relevant line the per-wake classifier might
#     have missed (e.g. a status verb outside MAINTAINER_RE) and escalates it.
#
...
#          MX_INJECT_SKIP           |-prefixes force-self-handle bypassing
#                                   classification (default "heartbeat"); empty
#                                   disables. Use sparingly: it overrides the
#                                   maintainer-relevant escalation for matching
#                                   kinds.
#          MX_STALE_ESCALATE_SECS   idle seconds before a stale pane escalates
#                                   as a possible wedge (default 240)
#          MX_PAUSE_RESURFACE_SECS  idle seconds before a declared external wait
#                                   re-surfaces as a recheck (default 3600)
#          MX_ESCALATE_BATCH_SECS   buffer window for batched escalation
#                                   digests; 0 = flush immediately (default 90)
#          MX_HEARTBEAT_SCAN_SECS   cadence for the catch-all status scan
#                                   (default 300)
#          MX_HOUSEKEEPING_TICK     seconds between housekeeping passes while
#                                   the watcher is mid-cycle (default 15)
#          MX_BUSY_REGEX            OR-ed busy signatures (mirrors mx-watch.sh)
#          MX_COMPOSER_IDLE_RE      empty-composer regex applied after dim-ghost
#                                   and structural border stripping (default:
#                                   bare prompt glyphs plus busy footers)
#          MX_MAX_DEFER_SECS        max seconds a buffered escalation may sit
#                                   undelivered before one normal flush attempt;
#                                   if that cannot confirm a submit, a wedge
#                                   alarm fires (default 300; 0 disables)
#          MX_WEDGE_ALARM_CHANNEL   override config/wedge-alarm with a single
...
#                                   is the only signal until the maintainer
#                                   configures a channel. See wedge_alarm_notify
#                                   below and docs/configuration.md.
#          MX_WEDGE_ALARM_EXEC      notifier seam: when set, every notifier
...
#          MX_WEDGE_ALARM_TIMEOUT_SECS seconds allowed for each notifier before
#                                   its watchdog terminates it and continues to the
#                                   next channel (default 10; invalid/zero uses the
#                                   default).
#          MX_INJECT_CONFIRM_RETRIES Enter-retry attempts on a swallowed Enter
#                                   (default 3); the digest is typed once, only
#                                   Enter is retried. Composer-empty detection is
#                                   structural and style-aware (bin/mx-tmux-lib.sh):
...
#          MX_INJECT_CONFIRM_SLEEP  seconds between daemon submit checks
#                                   (default 0.5)
#          MX_LOG_MAX_BYTES / MX_LOG_KEEP_LINES / MX_CRASH_*  log + crash guards
#          MX_STATE_OVERRIDE        alternate state dir (testing)
...
#          lock. A crashing mx-watch.sh is logged and restarted, never killing
...
# corrected composer detection. Stale task rechecks use mx-backend.sh below.
# shellcheck source=bin/mx-tmux-lib.sh
. "$MX_DAEMON_DIR/mx-tmux-lib.sh"

# shellcheck source=bin/mx-backend.sh
. "$MX_DAEMON_DIR/mx-backend.sh"

# Canonical construction and parsing for every Multplx operational input.
# shellcheck source=bin/mx-operational-input.sh
. "$MX_DAEMON_DIR/mx-operational-input.sh"

# Shared wake classifier (last_status_line, status_is_maintainer_relevant,
# window_to_task, scan_maintainer_relevant_statuses). The SAME library backs the
# always-on watcher's triage, so the maintainer-relevant verb set and the
# classification predicates have exactly one definition.
# shellcheck source=bin/mx-classify-lib.sh
. "$MX_DAEMON_DIR/mx-classify-lib.sh"

# Supervisor-pane discovery (MX_SUPERVISOR_TARGET_DEFAULT,
# MX_SUPERVISOR_BACKEND_DEFAULT, discover_supervisor_target,
# discover_supervisor_backend). Shared with the script-owned away launcher
# (bin/mx-afk-launch.sh) so the maintainer-pane resolution has exactly one owner.
# shellcheck source=bin/mx-supervisor-target-lib.sh
. "$MX_DAEMON_DIR/mx-supervisor-target-lib.sh"

# --- tunables ---------------------------------------------------------------
# Supervisor backends this daemon knows how to inject into today. cmux is a
# real backend elsewhere in broker (bin/mx-backend.sh) but this
# daemon has no verified composer/busy primitives wired up for it yet - see
...
# pane.
MX_SUPERVISOR_SUPPORTED_BACKENDS="tmux herdr"
...
# The maintainer-relevant verb set and the status classifiers (last_status_line,
# status_is_maintainer_relevant, window_to_task, scan_maintainer_relevant_statuses) now
# live in bin/mx-classify-lib.sh, shared with the always-on watcher.
# Composer-empty detection and the tmux busy-footer fallback live in
# bin/mx-tmux-lib.sh (MX_TMUX_BUSY_REGEX_DEFAULT / mx_tmux_composer_state);
# MX_BUSY_REGEX still overrides the fallback busy set here, as before.
...
# --- presence-gating --------------------------------------------------------
# bin/mx-operational-input.sh owns the U+2063 MULTPLX_OP bytes and typed
...
# Resolve the effective state dir. MX_STATE_OVERRIDE wins (testing); otherwise
# $MX_HOME/state. Kept as a function so the pure
# classifiers can take an explicit state arg without depending on globals.
_state_root() { printf '%s' "${MX_STATE_OVERRIDE:-$MX_HOME/state}"; }

# --- portable stat (same trap as mx-watch.sh: no `stat -f || stat -c`) -------
...
# skill (enter) and by broker on user return (exit). Durable: a plain file,
...
# should_exit_afk: encodes broker's afk-exit contract as a testable function.
...
#   anything else           -> 0 (maintainer is back; exit afk)
# Bias toward exit: only the marker and an explicit /afk invocation keep afk
# alive. A false exit is self-correcting (the maintainer re-runs /afk).
...
# marker (a daemon escalation), 1 otherwise (a real user message). Multplx's
# afk-exit contract uses this: marker present -> stay afk; absent -> maintainer is
# back. Bias ambiguous cases toward exit (a false exit is self-correcting).
message_is_injection() {  # <message-text>
  local msg=$1
  [ -n "$msg" ] || return 1
  case "$msg" in
    "$MX_INJECT_MARK"*) return 0 ;;
  esac
  return 1
}

# strip_injection_marker: remove a current typed away envelope, the landed
# untyped MULTPLX_OP prefix, or the legacy bare sentinel. Current grammar is
# delegated to its owner rather than reimplemented here.
strip_injection_marker() {  # <message-text>
  local msg=$1 body
  if mx_operational_input_body "$msg" body; then
    printf '%s' "$body"
    return
  fi
  case "$msg" in
    "$MX_OPERATIONAL_PREFIX"*) msg=${msg#"$MX_OPERATIONAL_PREFIX"} ;;
    "$MX_INJECT_MARK"*) msg=${msg#"$MX_INJECT_MARK"} ;;
  esac
  printf '%s' "$msg"
}
...
# bin/mx-supervisor-target-lib.sh (sourced above). mx_super_main below calls
# them exactly as before; the away launcher reuses the identical resolution to
# pass the maintainer pane in as MX_SUPERVISOR_TARGET.

# --- classification helpers (PURE: no side effects, testable) ---------------
# last_status_line, status_is_maintainer_relevant, window_to_task, and
# scan_maintainer_relevant_statuses come from bin/mx-classify-lib.sh (sourced above),
# the single classifier shared with bin/mx-watch.sh. The decision-string wrappers
...
# summary broker would otherwise have to re-read.
...
classify_stale() {  # <window> <state>
  ...
  if [ -n "$last" ] && status_is_paused "$last"; then
    # A DECLARED external-wait pause (mx-classify-lib.sh): an idle pane is EXPECTED,
    ...
    # status line already read, no mx-actor-state.sh call, mirroring the daemon's
    # existing status-log classification.
    printf 'pause|paused (awaiting external), rechecked on a long cadence: %s' "$last"
    return
  fi
  if [ -n "$last" ] && status_is_maintainer_relevant "$last"; then
    # Independent of free-text maintainer-relevant matching: a nonterminal progress
    ...
    # prose once looked maintainer-relevant. Real terminal verbs and legacy free-text
    # maintainer lines without those verbs keep the terminal escalate/dedupe path.
    if ! status_is_terminal_verb "$last"; then
      case "$(status_line_verb "$last")" in
        working|resolved|maintainer-held)
...
classify_check() {  # <full reason>  — check scripts print only when broker should wake
  printf 'escalate|%s' "$1"
}
...
migrate_watcher_pause_markers() {  # <state>
  ...
  for meta in "$state"/*.meta; do
    [ -e "$meta" ] || continue
    win=$(mx_backend_target_of_meta "$meta")
    [ -n "$win" ] || continue
...
# Record the seen-status marker for a maintainer-relevant status line so the
...
# Mark every maintainer-relevant status line a per-wake classification escalated as
...
# Busy + composer-empty detection are the shared primitives in mx-tmux-lib.sh
# (one source of truth with mx-send.sh). These thin wrappers keep the daemon's
...
# bin/mx-backend.sh's generic per-backend primitives (mx_backend_busy_state,
# mx_backend_capture, mx_backend_composer_state) rather than hand-rolling a
...
# pre-existing mx_pane_is_busy, since mx_backend_capture's tmux arm runs the
# exact same `tmux capture-pane -p -t <target> -S -40`).
pane_is_busy() {  # <target> [backend]
  local target=$1 backend=${2:-tmux} bs tail40
  bs=$(mx_backend_busy_state "$backend" "$target" 2>/dev/null)
  case "$bs" in
    busy) return 0 ;;
  esac
  tail40=$(mx_backend_capture "$backend" "$target" 40 2>/dev/null) || return 1
...
# dispatching through mx_backend_composer_state (byte-identical to a direct
# mx_tmux_composer_state call for the default/omitted-backend case). inject_msg
...
pane_input_pending() {  # <target> [backend]
  ...
  [ "$(mx_backend_composer_state "$backend" "$target" 2>/dev/null)" = pending ]
}
...
task_window_backend() {  # <window> <state>
  ...
  mx_backend_of_meta "$meta"
}

stale_window_is_busy() {  # <window> <state>
  local win=$1 state=$2 backend label tail40 bs
  backend=$(task_window_backend "$win" "$state")
  label="mx-$(window_to_task "$win" "$state")"
  tail40=$(mx_backend_capture "$backend" "$win" 40 "$label" 2>/dev/null) || return 2
  bs=$(mx_backend_busy_state "$backend" "$win" 2>/dev/null)
  case "$bs" in
    busy) return 0 ;;
  esac
  printf '%s' "$tail40" | grep -v '^[[:space:]]*$' | tail -6 \
...
# surfaces until the next system action (that night, 20 escalations sat buffered
...
# maintainer-supplied command (push to a phone, etc.).
...
# non-empty, non-comment line. MX_WEDGE_ALARM_CHANNEL overrides the file with a
...
# maintainer wires a herdr or command: directive.

# Print the configured channel directives, one per line. MX_WEDGE_ALARM_CHANNEL
...
wedge_alarm_configured_channels() {
  ...
  if [ -n "${MX_WEDGE_ALARM_CHANNEL:-}" ]; then
    printf '%s\n' "$MX_WEDGE_ALARM_CHANNEL"
    return 0
  fi
  cfg="${MX_CONFIG_OVERRIDE:-$MX_HOME/config}/wedge-alarm"
  if [ -f "$cfg" ]; then
...
# built-in OS channel (the maintainer wires a herdr or command: directive), so this
...
wedge_alarm_run_bounded() {
  ...
  shift
  timeout=${MX_WEDGE_ALARM_TIMEOUT_SECS:-$WEDGE_ALARM_TIMEOUT_SECS_DEFAULT}
  case "$timeout" in
...
# MX_WEDGE_ALARM_EXEC, when set, REPLACES the real notifier: the resolved channel
# name and summary are handed to that command instead of ever invoking herdr or
# a maintainer-supplied command. This is the one injection point the test harness forces to a recorder
...
wedge_alarm_os_notifier_override() {  # <channel> <summary>
  local channel=$1 summary=$2 rc exec_override=${MX_WEDGE_ALARM_EXEC:-}
...
wedge_alarm_via_herdr() {  # <summary>
  ...
  wedge_alarm_run_bounded herdr herdr notification show "broker: away-mode escalations WEDGED" \
    --body "$summary" --sound request >/dev/null 2>&1 && return 0
  log "wedge alarm: herdr notification failed"
  return 1
}

# Run a maintainer-supplied command with the summary on $1 and on stdin, so an
# alert can reach a phone/pager (ntfy, Slack, SMS) even when the maintainer is away
...
wedge_alarm_via_command() {  # <cmd> <summary>
  ...
  wedge_alarm_run_bounded command sh -c "$cmd" mx-wedge-alarm "$summary" \
...
wedge_alarm_emit() {  # <channel> <summary>
  local channel=$1 summary=$2 cmd=${3:-} rc exec_override=${MX_WEDGE_ALARM_EXEC:-} WEDGE_ALARM_EMIT_ACTIVE=1
  case "$exec_override" in
    '') ;;
    discard) return 0 ;;
    *)
...
# an ERROR, drops a durable marker broker/recovery can surface, flashes
...
inject_wedge_alarm() {  # <state> <age-seconds>
  ...
  max_defer="${MX_MAX_DEFER_SECS:-$MAX_DEFER_SECS_DEFAULT}"
  ...
  target="${MX_SUPERVISOR_TARGET:-$MX_SUPERVISOR_TARGET_DEFAULT}"
  backend="${MX_SUPERVISOR_BACKEND:-$MX_SUPERVISOR_BACKEND_DEFAULT}"
  ...
  # every non-tmux backend), this can reach the maintainer even when every pane and
...
# Four cheap jobs, each guarded so an empty/quiet system costs near zero:
...
#     maintainer-relevant line the per-wake classifier missed and escalate it.
housekeeping() {  # <state>
  ...
  # (1) batch flush
  if [ "${MX_ESCALATE_BATCH_SECS:-$ESCALATE_BATCH_SECS_DEFAULT}" -le 0 ]; then
    escalate_flush "$state" || true
  else
    due=$(_oldest_line_age "$state/.subsuper-escalations")
    if [ "$due" -ge "${MX_ESCALATE_BATCH_SECS:-$ESCALATE_BATCH_SECS_DEFAULT}" ]; then
      escalate_flush "$state" || true
    fi
  fi
  ...
  for marker in "$state"/.subsuper-stale-*; do
    ...
    [ "$age" -ge "${MX_STALE_ESCALATE_SECS:-$STALE_ESCALATE_SECS_DEFAULT}" ] || continue
  ...
  # the window repeats.
  pause_secs=${MX_PAUSE_RESURFACE_SECS:-$MX_PAUSE_RESURFACE_SECS_DEFAULT}
  ...
  # (3) heartbeat scan (catch-all for a maintainer-relevant status the per-wake
  #     classifier may have missed). Cheap: status files only, no tmux. The
  #     maintainer-relevant filtering is the shared classifier's
  #     scan_maintainer_relevant_statuses; the daemon layers its digest dedup on top.
  if [ "$(_file_age "$state/.subsuper-last-scan")" -ge "${MX_HEARTBEAT_SCAN_SECS:-$HEARTBEAT_SCAN_SECS_DEFAULT}" ]; then
...
window_for_task() {  # <task-key> [state]
  ...
  for w in $(tmux list-windows -a -F '#{session_name}:#{window_name}' 2>/dev/null | grep ':mx-' || true); do
...
inject_msg() {  # <message> [state]
  ...
  # daemon self-handles and stays quiet; broker drives the normal always-on
  ...
  mx_operational_input_encode away-supervisor "$msg" encoded || return 1
  msg=$encoded
  target="${MX_SUPERVISOR_TARGET:-$MX_SUPERVISOR_TARGET_DEFAULT}"
  # BACKEND-AWARE (previously a raw `tmux display-message` pane-exists probe):
  # dispatches through bin/mx-backend.sh so a herdr supervisor pane is checked
  # via the herdr adapter instead of always assuming tmux. Falls back to tmux
  # when unset (sourced/test contexts that never ran mx_super_main's startup
  # discovery), matching this function's pre-existing default assumption.
  backend="${MX_SUPERVISOR_BACKEND:-tmux}"
  mx_backend_target_exists "$backend" "$target" || return 1
  ...
  #      composer. The shared classifier (mx_backend_composer_state ->
  #      mx_composer_classify_content, bin/mx-composer-lib.sh) reports 'pending'
  ...
  composer=$(mx_backend_composer_state "$backend" "$target" 2>/dev/null)
  ...
  # Dispatches through mx_backend_send_text_submit (bin/mx-backend.sh): for
  # backend=tmux this calls mx_backend_tmux_send_text_submit, a verbatim
  # re-export of mx_tmux_submit_core - byte-identical to calling it directly.
  retries=${MX_INJECT_CONFIRM_RETRIES:-$INJECT_CONFIRM_RETRIES_DEFAULT}
  sleep_s=${MX_INJECT_CONFIRM_SLEEP:-$INJECT_CONFIRM_SLEEP_DEFAULT}
  verdict=$(mx_backend_send_text_submit "$backend" "$target" "$msg" "$retries" "$sleep_s" "$sleep_s")
...
should_force_self() {  # <reason>
  local reason=$1 skip="${MX_INJECT_SKIP:-$INJECT_SKIP_DEFAULT}" prefix
  [ -n "$skip" ] || return 1
  local -a prefixes
...
handle_wake() {  # <reason> <state>
  ...
  if should_force_self "$reason"; then
    log "wake force-self (MX_INJECT_SKIP): $reason"
    return
  fi
  case "$reason" in
  ...
  case "$action" in
    escalate)
      ...
      mark_escalated_seen "$kind" "$arg" "$state"
      [ "${MX_ESCALATE_BATCH_SECS:-$ESCALATE_BATCH_SECS_DEFAULT}" -le 0 ] && { escalate_flush "$state" || true; }
      ;;
    pause)
    ...
    *)
      ...
      # housekeeping can age it, and drop any pause marker (an actor that left its
      # pause reverts to normal wedge aging). The persistence recheck, not this
      # wake, escalates a wedge.
      if [ "$kind" = "stale" ]; then
        task=$(window_to_task "$arg" "$state")
        last=$(last_status_line "$state/$task.status")
        # Clear wedge aging only for terminal (or legacy free-text) maintainer lines.
        # Nonterminal progress verbs keep possible-wedge markers even if free text
        # once looked maintainer-relevant or was written into a seen marker.
        _clear_wedge=0
        if [ -n "$last" ] && status_is_maintainer_relevant "$last"; then
          if status_is_terminal_verb "$last"; then
            _clear_wedge=1
            ...
            case "$(status_line_verb "$last")" in
              working|resolved|maintainer-held) _clear_wedge=0 ;;
              *) _clear_wedge=1 ;;
            esac
          fi
        fi
...
# --- log --------------------------------------------------------------------
# Uses LOG set by mx_super_main; harmless no-op-ish if unset (tests source fns
...
trim_log() {
  ...
  [ "$sz" -ge "${MX_LOG_MAX_BYTES:-$LOG_MAX_BYTES_DEFAULT}" ] || return 0
  tmp=$(mktemp "${TMPDIR:-/tmp}/mx-daemon-log.XXXXXX") || return 0
  tail -n "${MX_LOG_KEEP_LINES:-$LOG_KEEP_LINES_DEFAULT}" "$LOG" >"$tmp" 2>/dev/null && mv -f "$tmp" "$LOG"
}

# ============================================================================
# Everything below runs only when the script is EXECUTED, not sourced. The pure
# classifiers above are sourceable for unit tests (tests/mx-daemon.test.sh).
# ============================================================================
...
mx_super_main() {
  ...
  MX_STATE_OVERRIDE="$STATE" . "$MX_DAEMON_DIR/mx-wake-lib.sh"

  local WATCH="$MX_DAEMON_DIR/mx-watch.sh"
  ...
  local INJECT_FAIL_SLEEP=${MX_INJECT_FAIL_SLEEP:-$INJECT_FAIL_SLEEP_DEFAULT}
  local CRASH_THRESHOLD=${MX_CRASH_THRESHOLD:-$CRASH_THRESHOLD_DEFAULT}
  local CRASH_WINDOW=${MX_CRASH_WINDOW:-$CRASH_WINDOW_DEFAULT}
  local CRASH_BACKOFF=${MX_CRASH_BACKOFF:-$CRASH_BACKOFF_DEFAULT}
  local CRASH_NORMAL_SLEEP=${MX_CRASH_NORMAL_SLEEP:-$CRASH_NORMAL_SLEEP_DEFAULT}
  ...
  if ! mx_lock_try_acquire "$LOCK"; then
    if [ -n "${MX_LOCK_HELD_PID:-}" ]; then
      echo "error: another mx-supervise-daemon is already running (pid $MX_LOCK_HELD_PID, lock $LOCK held)" >&2
    else
      echo "error: another mx-supervise-daemon is already running (lock $LOCK held)" >&2
    fi
    exit 1
  fi
  echo "$$" > "$PIDFILE"
  mx_pid_identity "${BASHPID:-$$}" > "$LOCK/pid-identity" 2>/dev/null || true

  # --- auto-discover the supervisor BACKEND (tmux vs herdr) first -----------
  # Priority: MX_SUPERVISOR_BACKEND override > $TMUX_PANE (tmux) > $HERDR_ENV=1
  ...
  # into MX_SUPERVISOR_BACKEND makes inject_msg/pane_is_busy/pane_input_pending
  ...
  backend_source="MX_SUPERVISOR_BACKEND"
  if [ -z "${MX_SUPERVISOR_BACKEND:-}" ]; then
    if [ -n "${TMUX_PANE:-}" ]; then
      backend_source="TMUX_PANE"
    elif [ "${HERDR_ENV:-}" = "1" ] && [ -n "${HERDR_PANE_ID:-}" ]; then
      backend_source="HERDR_ENV"
    else
      backend_source="FALLBACK($MX_SUPERVISOR_BACKEND_DEFAULT)"
    fi
  fi
  discovered_backend=$(discover_supervisor_backend) || true
  MX_SUPERVISOR_BACKEND="$discovered_backend"
  local BACKEND="$MX_SUPERVISOR_BACKEND"

  # --- refuse an unsupported supervisor backend loudly, before ever trying a
  # tmux/herdr-specific call against it (cmux has no verified
  ...
  if ! mx_backend_list_contains "$MX_SUPERVISOR_SUPPORTED_BACKENDS" "$BACKEND"; then
    echo "error: away-mode daemon does not support supervisor backend '$BACKEND' yet (supported: $MX_SUPERVISOR_SUPPORTED_BACKENDS); set MX_SUPERVISOR_BACKEND=tmux|herdr and MX_SUPERVISOR_TARGET to run broker's own pane under a supported backend" >&2
  ...
  # --- auto-discover the supervisor target (the pane running broker) -----
  # Priority: MX_SUPERVISOR_TARGET override > $TMUX_PANE (tmux; inherited from
  # the pane that launched the daemon, normally broker's own) >
  # $HERDR_PANE_ID (herdr, composed into "<session>:<pane-id>") > broker:0
  # fallback. Exporting the result into MX_SUPERVISOR_TARGET makes inject_msg
  # (which reads that env var) use the discovered pane without an extra global.
  local discovered target_source
  target_source="MX_SUPERVISOR_TARGET"
  if [ -z "${MX_SUPERVISOR_TARGET:-}" ]; then
    if [ -n "${TMUX_PANE:-}" ]; then
      target_source="TMUX_PANE"
    elif [ "${HERDR_ENV:-}" = "1" ] && [ -n "${HERDR_PANE_ID:-}" ]; then
      target_source="HERDR_ENV(HERDR_PANE_ID)"
    else
      target_source="FALLBACK(broker:0)"
    fi
  fi
  if discovered=$(discover_supervisor_target); then
    : # resolved cleanly
  else
    echo "warn: could not auto-discover supervisor pane (no MX_SUPERVISOR_TARGET, TMUX_PANE, or HERDR_ENV/HERDR_PANE_ID); falling back to '$discovered' — verify this is broker's pane" >&2
  fi
  MX_SUPERVISOR_TARGET="$discovered"
  local TARGET="$MX_SUPERVISOR_TARGET"

  # --- validate supervisor target at startup (a missing target is a typo) ---
  # Dispatches through bin/mx-backend.sh instead of a raw `tmux display-message`
  ...
  if ! mx_backend_target_exists "$BACKEND" "$TARGET"; then
    echo "error: supervisor target '$TARGET' does not resolve to a $BACKEND pane; set MX_SUPERVISOR_TARGET" >&2
  ...
  log "daemon starting (pid $$); target=$TARGET; target_source=$target_source; backend=$BACKEND; backend_source=$backend_source; afk=$afk_status; inject_skip='${MX_INJECT_SKIP:-$INJECT_SKIP_DEFAULT}'; stale_escalate=${MX_STALE_ESCALATE_SECS:-$STALE_ESCALATE_SECS_DEFAULT}s; batch=${MX_ESCALATE_BATCH_SECS:-$ESCALATE_BATCH_SECS_DEFAULT}s"
  migrate_watcher_pause_markers "$STATE"
  ...
  start_watcher() {
    CUR_TMP=$(mktemp "${TMPDIR:-/tmp}/mx-watch.XXXXXX") || { log "error: mktemp failed; retrying in 5s"; sleep 5; return 1; }
  ...
  while true; do
    ...
    # has nowhere to go, and broker itself is the consumer of escalations.
    ...
    # The watcher child runs on its own MX_POLL cadence internally; we only need
    ...
    # cadence. Gating keeps a large system cheap between ticks.
    sleep 1
    if [ "$(_file_age "$STATE/.subsuper-last-housekeep")" -ge "${MX_HOUSEKEEPING_TICK:-$HOUSEKEEPING_TICK_DEFAULT}" ]; then
...
if [ "${BASH_SOURCE[0]}" = "${0}" ]; then
  ...
  # execs the daemon, see bin/mx-afk-start.sh). Make it structurally impossible
  # for a sourced context to fire a real desktop notification from the wedge
  # alarm: default the MX_WEDGE_ALARM_EXEC notifier seam to "discard" unless the
  ...
  : "${MX_WEDGE_ALARM_EXEC:=discard}"
  export MX_WEDGE_ALARM_EXEC
fi
