#!/usr/bin/env bash
# mx-install-herdr.sh - install CI's pinned, verified Herdr build.
#
...
#
# Usage:
#   mx-install-herdr.sh <destination-directory>
#
...
MX_HERDR_CI_VERSION=0.7.4
MX_HERDR_CI_TAG="v${MX_HERDR_CI_VERSION}"
MX_HERDR_CI_MIN_PROTOCOL=16
# Bounded download ceiling (bytes). The largest official 0.7.4 asset is under 20 MiB.
MX_HERDR_CI_MAX_BYTES=25000000
MX_HERDR_CI_REPO=ogulcancelik/herdr

die() {
  printf 'mx-install-herdr.sh: %s\n' "$*" >&2
  exit 1
}

DESTINATION=${1:?usage: mx-install-herdr.sh <destination-directory>}
...
URL="https://github.com/${MX_HERDR_CI_REPO}/releases/download/${MX_HERDR_CI_TAG}/${ASSET}"
TMP=$(mktemp -d "${RUNNER_TEMP:-${TMPDIR:-/tmp}}/mx-herdr.XXXXXX")
trap 'rm -rf "$TMP"' EXIT

printf 'mx-install-herdr.sh: downloading %s from %s\n' "$ASSET" "$URL" >&2
# --fail: HTTP errors; --location: follow redirects; --max-filesize: bound.
curl -fsSL --max-filesize "$MX_HERDR_CI_MAX_BYTES" "$URL" -o "$TMP/$ASSET" \
  || die "download failed for $URL (bounded at $MX_HERDR_CI_MAX_BYTES bytes)"
...
[ "$installed_version" = "$MX_HERDR_CI_VERSION" ] \
  || die "installed herdr version is '${installed_version:-<empty>}', expected exact pin $MX_HERDR_CI_VERSION"
...
[ "$protocol" -ge "$MX_HERDR_CI_MIN_PROTOCOL" ] \
  || die "herdr protocol $protocol is below the required floor $MX_HERDR_CI_MIN_PROTOCOL"

printf 'mx-install-herdr.sh: installed herdr %s (protocol %s) to %s\n' \
