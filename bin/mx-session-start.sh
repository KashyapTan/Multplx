#!/usr/bin/env bash
# mx-session-start.sh - one command for the whole session start.
#
...
# mx-bootstrap.sh, then separately read data/projects.md, data/daemons.md,
# data/maintainer.md, data/maintainer-shared.md, data/learnings.md, then run
# mx-lock.sh, mx-wake-drain.sh, then read data/backlog.md, every state/*.meta,
...
#
# COMPOSITION, NOT DUPLICATION: this script calls mx-lock.sh, mx-bootstrap.sh,
# and mx-wake-drain.sh as real subprocesses and prints their real output. It
...
# standalone with unchanged default behavior - other flows (mx-bootstrap.sh
# install <tools> after consent, /updatemultplx, the afk daemon, existing
# tests) still call them directly. The one seam this script needed -
# bootstrap running its detect-only diagnostics without its four mutating
# sweeps - is an opt-in MX_BOOTSTRAP_DETECT_ONLY=1 flag on mx-bootstrap.sh
...
#                       diagnostics always run. Bootstrap's four MUTATING sweeps
#                       (legacy PR-check migration, daemon fast-forward,
#                       daemon liveness, system sync) also run only when
#                       locked.
#   3. wake-drain     - mutates the durable wake queue, so it also only runs
#                       when locked.
#   4. context digest - data/projects.md, data/daemons.md, data/maintainer.md,
#                       data/maintainer-shared.md, data/learnings.md: read-only,
#                       always safe, always runs.
#   5. system digest   - a compact data/backlog.md identity/metadata listing,
...
# daemon homes, fetching/fast-forwarding every
...
# The context and system-state digests
...
# Portion 09 Rust-default adapter. The Rust boundary is entered before lock
# acquisition so one engine owns the complete ordered startup composition.
...
PRIMARY_HARNESS=$("$SCRIPT_DIR/mx-harness.sh" 2>/dev/null || printf unknown)
...
BACKLOG_LIMIT=${MX_SESSION_START_BACKLOG_LIMIT:-80}
...
# meaningful for every one of these files (maintainer.md absent = broker
  ...
  awk -v max="$BACKLOG_LIMIT" '
    ...
    END {
      if (total == 0) {
        ...
        if (total > shown) {
          printf "(truncated %d item(s); increase MX_SESSION_START_BACKLOG_LIMIT for a larger startup listing)\n", total - shown
        }
      }
    }
  ' "$path"
...
section "SESSION START - $MX_HOME"

# --- 1. lock -----------------------------------------------------------
subsection "LOCK"
LOCK_OUT=$("$SCRIPT_DIR/mx-lock.sh" 2>&1)
LOCK_RC=$?
printf '%s\n' "$LOCK_OUT"
READ_ONLY=0
if [ "$LOCK_RC" -ne 0 ]; then
  ...
  {
    printf '%s\n' "$BAR"
    printf '●  READ-ONLY SESSION - SYSTEM LOCK OWNERSHIP WAS NOT VERIFIED\n'
    printf '●  %s\n' "$LOCK_OUT"
    printf '●  Skipping every mutating step: PR-check migration, stale Herdr child cleanup,\n'
    printf '●  daemon sync, system sync, and wake-queue drain. Detect-only bootstrap\n'
    printf '●  diagnostics and the rest of this read-only-safe digest still ran below.\n'
    printf '●  Operate read-only until this resolves - do not spawn, steer, merge, or\n'
    printf '●  otherwise mutate system state from this session.\n'
    printf '%s\n' "$BAR"
  }
fi
...
if [ "$READ_ONLY" -eq 1 ]; then
  BOOT_OUT=$(MX_BOOTSTRAP_DETECT_ONLY=1 "$SCRIPT_DIR/mx-bootstrap.sh" 2>&1)
else
  BOOT_OUT=$(
    "$SCRIPT_DIR/mx-herdr-session-cleanup.sh" 2>&1 || true
    "$SCRIPT_DIR/mx-bootstrap.sh" 2>&1
  )
fi
if [ -n "$BOOT_OUT" ]; then
  printf '%s\n' "$BOOT_OUT"
...
# drain also runs mx-guard.sh internally on the locked path, so the
...
# mx-guard.sh directly with non-mutating advisory text, so the same alarms
# surface without repair commands.
subsection "WAKE QUEUE"
if [ "$READ_ONLY" -eq 1 ]; then
  QLEN=0
  [ -s "$STATE/.wake-queue" ] && QLEN=$(grep -c . "$STATE/.wake-queue" 2>/dev/null || printf '0')
  printf 'skipped (read-only session) - %s record(s) remain queued because this session lacks verified system-lock ownership.\n' "$QLEN"
  GUARD_OUT=$(MX_GUARD_READ_ONLY=1 "$SCRIPT_DIR/mx-guard.sh" 2>&1)
  [ -n "$GUARD_OUT" ] && printf '%s\n' "$GUARD_OUT"
else
  DRAIN_OUT=$("$SCRIPT_DIR/mx-wake-drain.sh" 2>&1)
  if [ -n "$DRAIN_OUT" ]; then
    printf '%s\n' "$DRAIN_OUT"
  else
...
"$SCRIPT_DIR/mx-supervision-instructions.sh" \
...
print_file_or_absent "$DATA/daemons.md" "data/daemons.md"
print_file_or_absent "$DATA/maintainer.md" "data/maintainer.md"
print_file_or_absent "$DATA/maintainer-shared.md" "data/maintainer-shared.md (shared, main-authoritative, read-only in daemon homes)"
print_file_or_absent "$DATA/learnings.md" "data/learnings.md"

# --- 5. system-state digest ---------------------------------------------
...
for meta in "$STATE"/*.meta; do
  ...
  if [ -n "$window" ]; then
    ...
    if mx_backend_target_exists "$backend" "${target:-$window}" "mx-$id"; then
...
if [ "$READ_ONLY" -eq 1 ]; then
  cat <<'EOF'
This session did not acquire the system lock. Stay read-only: do not arm,
