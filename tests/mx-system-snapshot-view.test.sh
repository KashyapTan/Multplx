#!/usr/bin/env bash
# Behavior tests for the read-only system snapshot and its human renderer.
set -u
...
SNAPSHOT="$ROOT/bin/mx-system-snapshot.sh"
VIEW="$ROOT/bin/mx-system-view.sh"
TMP_ROOT=$(mx_test_tmproot mx-system-snapshot)
...
case "${1:-}" in
  list-windows)
    sed -n 's/^window=[^:]*://p' "${MX_HOME:?}"/state/*.meta
    ;;
  display-message)
    case "$*" in
      *pane_current_command*)
        case "$target" in
          *dead-daemon*) printf 'zsh\n' ;;
          *) printf 'codex\n' ;;
        esac
        ;;
      *) printf '%%1\n' ;;
  ...
  capture-pane)
    case "$target" in
      *delivery-task*|*active-daemon*) printf 'work in progress\nesc to interrupt\n' ;;
      *) printf 'all quiet\n> \n' ;;
    esac
    ;;
esac
...
write_fixture() {  # <home>
  local home=$1
  mkdir -p "$home/projects/alpha-worktree" "$home/projects/scout-worktree" "$home/daemon-home"
  cat > "$home/data/backlog.md" <<EOF
## In flight
- [ ] scout-task - Scout Task data/scout-task/report.md (repo: alpha) (kind: scout) (since 2026-07-07)
- [ ] delivery-task - DELIVERY TASK https://github.com/KashyapTan/Multplx/pull/9 (repo: alpha) (kind: delivery) (priority: 2) (since 2026-07-07)
  Preserve this detail for catchup.

## Queued
- [ ] queued-task - Queued Task blocked-by: delivery-task (repo: alpha) (kind: delivery) (since 2026-07-08)
handoff note without canonical syntax

## Done
- [x] done-task - Done Task https://github.com/KashyapTan/Multplx/pull/7 (repo: alpha) (kind: delivery) (merged 2026-07-06)
EOF
  ...
  mx_write_meta "$home/state/scout-task.meta" \
    "window=broker:mx-scout-task" \
  ...
  mx_write_meta "$home/state/daemon-task.meta" \
    "window=broker:mx-daemon-task" \
    "worktree=$home/daemon-home" \
    "project=$home/daemon-home" \
    "harness=codex" \
    "kind=daemon" \
    "mode=daemon" \
    "home=$home/daemon-home" \
    "projects=alpha, beta, gamma, "
  printf 'working: watching delegated scope\n' > "$home/state/daemon-task.status"
  mx_write_meta "$home/state/cmux-task.meta" \
    "backend=cmux" \
...
test_empty_system_json() {
  ...
  out=$(MX_HOME="$home" "$SNAPSHOT" --json)
  printf '%s' "$out" | jq -e '
  ...
  view=$(MX_HOME="$home" "$VIEW")
  assert_contains "$view" "No live task metadata found." "empty system view should say no live metadata"
  pass "empty system snapshot and view use explicit absence markers"
}
...
test_fixture_snapshot_json() {
  ...
  out=$(PATH="$fakebin:$PATH" MX_HOME="$home" "$SNAPSHOT" --json)
  printf '%s' "$out" | jq -e . >/dev/null || fail "snapshot must be valid JSON"
  ids=$(printf '%s' "$out" | jq -r '.tasks | map(.id) | join(",")')
  [ "$ids" = "cmux-task,daemon-task,delivery-task,scout-task" ] \
  ...
  printf '%s' "$out" | jq -e '
    .tasks[] | select(.id == "daemon-task")
    | .daemon_projects == ["alpha","beta","gamma"]
      and .endpoint.agent_alive == "alive"
      and (.actions.watch | contains("do not routinely mx-peek"))
  ' >/dev/null || fail "daemon return-channel guidance missing"
  printf '%s' "$out" | jq -e '
  ...
  printf '%s' "$out" | jq -e '
    ...
    | .state == "done" and .pr_url == "https://github.com/KashyapTan/Multplx/pull/7"
...
- [ ] orphan-delivery - Structured without meta (repo: alpha) (kind: delivery) (since 2026-07-11)
- [ ] visible-delivery - Structured with meta (repo: alpha) (kind: delivery) (since 2026-07-11)

## Queued
another free-form queued note
- [ ] queued-delivery - Structured queued (repo: alpha) (kind: delivery)
...
EOF
  ...
  printf '%s' "$out" | jq -e '
    .main_inventory.valid == false
      ...
      and (.main_inventory.orphan_in_flight == ["orphan-delivery"])
      and ([.tasks[].id] == ["visible-delivery"])
...
EOF
  ...
  printf '%s' "$out" | jq -e '
    .main_inventory.valid == true
      ...
      and .main_inventory.unstructured_current_count == 0
      and (.main_inventory.orphan_in_flight | length) == 0
      and (([.tasks[].id] | sort) == ["orphan-delivery", "visible-delivery"])
...
- [ ] worker - Real worker (repo: alpha) (kind: delivery)
- [ ] orphan - Ordinary missing worker (repo: alpha) (kind: delivery)

## Queued
- [ ] review - Security review (repo: alpha) (kind: delivery)
- [ ] maintainer-run - Run canary blocked-by: worker blocked-by: review (repo: alpha) (kind: maintainer) (hold: maintainer runs canary) (hold-kind: maintainer)

## Done
EOF
  mx_write_meta "$home/state/worker.meta" \
    "window=broker:mx-worker" "worktree=$home/projects/worker" "project=alpha" \
    "harness=codex" "kind=delivery" "mode=delivery"
  ...
  printf '%s' "$out" | jq -e '
    .main_inventory.orphan_in_flight == ["orphan"]
      ...
      and (.backlog.records[] | select(.id == "maintainer-run")
        | .blocked_by == "review"
...
## Done
- [x] worker - Real worker (repo: alpha) (kind: delivery) (done 2026-07-22)
EOF
  rm "$home/state/worker.meta" "$home/state/worker.status"
  out=$(PATH="$fakebin:$PATH" MX_HOME="$home" "$SNAPSHOT" --json)
  printf '%s' "$out" | jq -e '
    .backlog.records[] | select(.id == "maintainer-run")
    | .blocked_by == "review"
      and .blocked_by_ids == ["worker", "review"]
...
## Done
- [x] worker - Real worker (repo: alpha) (kind: delivery) (done 2026-07-22)
- [x] review - Security review (repo: alpha) (kind: delivery) (done 2026-07-22)
EOF
  out=$(PATH="$fakebin:$PATH" MX_HOME="$home" "$SNAPSHOT" --json)
  printf '%s' "$out" | jq -e '
    .backlog.records[] | select(.id == "maintainer-run")
    | .blocked_by == "review"
      ...
      and .maintainer_actionable == true
  ' >/dev/null || fail "completed blockers did not make the maintainer hold actionable: $out"
...
test_event_hints_follow_reconciled_current_state() {
  ...
  mx_write_meta "$home/state/active-decision.meta" \
    "window=broker:mx-active-decision" \
  ...
  mx_write_meta "$home/state/active-blocked.meta" \
    "window=broker:mx-active-blocked" \
  ...
  mx_write_meta "$home/state/stale-decision.meta" \
    "window=broker:mx-stale-decision-delivery-task" \
  ...
  mx_write_meta "$home/state/stale-blocked.meta" \
    "window=broker:mx-stale-blocked-delivery-task" \
...
## Queued
- [ ] queued-comma - Queued Comma Task (repo: beta, since 2026-07-08) (kind: delivery)
- [ ] parenthetical-title - Refresh sidebar (mobile) (repo: beta) (kind: delivery)
- [ ] blocked-reason - Blocked Reason (repo: beta) (kind: delivery) blocked-by: queued-comma - waits on queued-comma
- [ ] sample-decision-route - Choose sample route (repo: sample) (kind: maintainer) (since 2026-07-14) (hold: maintainer route choice pending) (hold-kind: maintainer)

## Done
- [x] done-comma - Done Comma Task https://github.com/KashyapTan/Multplx/pull/42 (repo: gamma, merged 2026-07-09) (kind: delivery)
- [x] done-bracket-pr - Done Bracket PR - <https://github.com/KashyapTan/Multplx/pull/43> (repo: gamma, merged 2026-07-12) (kind: delivery)
- [x] reported-comma - Reported Scout data/reported-comma/report.md (repo: gamma, reported 2026-07-10) (kind: scout)
- [x] done-note - Done Note local main (repo: delta, done 2026-07-11) (kind: delivery)
EOF
  printf '# Bold Scout\n' > "$data/bold-task/report.md"
  mx_write_meta "$home/state/bold-task.meta" \
    "window=broker:mx-bold-task" \
  ...
  fakebin=$(make_fakebin "$home")
  out=$(PATH="$fakebin:$PATH" MX_HOME="$home" MX_DATA_OVERRIDE="$data" MX_PROJECTS_OVERRIDE="$projects" "$SNAPSHOT" --json)
  ...
  printf '%s' "$out" | jq -e '
    ...
    | .repo == "gamma"
      ...
      and .pr_url == "https://github.com/KashyapTan/Multplx/pull/43"
      and .links == ["https://github.com/KashyapTan/Multplx/pull/43"]
  ...
  view=$(PATH="$fakebin:$PATH" MX_HOME="$home" MX_DATA_OVERRIDE="$data" MX_PROJECTS_OVERRIDE="$projects" "$VIEW")
  ...
  assert_contains "$view" "| blocked-reason | Blocked Reason | beta | delivery | queued-comma - waits on queued-comma | - |" \
    "view should render blocked reason without title metadata"
  assert_contains "$view" "| done-bracket-pr | Done Bracket PR | gamma | delivery | - | https://github.com/KashyapTan/Multplx/pull/43 |" \
...
test_view_renders_snapshot() {
  ...
  assert_contains "$view" "| done-task | Done Task | alpha | delivery | - | https://github.com/KashyapTan/Multplx/pull/7 |" \
    "view should render done backlog row"
  assert_contains "$view" "bin/mx-send.sh mx-daemon-task" \
    "view should show daemon send guidance"
  assert_contains "$view" "| daemon-task | working / status-log | daemon | $home/daemon-home | tmux | present / alive |" \
    "view should show daemon endpoint agent liveness"
  assert_not_contains "$view" "mx-peek.sh mx-daemon-task" \
    "view must not tell broker to routinely peek daemons"
  pass "system view renders the snapshot without daemon peek guidance"
}

test_view_renders_dead_daemon_agent_status() {
  local home fakebin view
  home=$(make_home dead-daemon)
  mx_write_meta "$home/state/dead-daemon.meta" \
    "window=broker:mx-dead-daemon" \
    "project=$home/daemon-home" \
    "harness=codex" \
    "kind=daemon" \
    "mode=daemon" \
    "home=$home/daemon-home" \
    "projects=alpha, beta"
  printf 'working: watching delegated scope\n' > "$home/state/dead-daemon.status"
  fakebin=$(make_fakebin "$home")
  view=$(PATH="$fakebin:$PATH" MX_HOME="$home" "$VIEW")
  assert_contains "$view" "| dead-daemon | unknown / none | daemon | $home/daemon-home | tmux | present / dead |" \
    "view should distinguish a present daemon endpoint from a dead agent"
  assert_contains "$view" "| dead-daemon | unknown / none | daemon | $home/daemon-home | tmux | present / dead | - | $home/daemon-home (absent) |" \
    "view should show a recorded missing daemon home path"
  pass "system view renders daemon agent liveness"
}
...
# durable keyed fold (mx-classify-lib.sh) keeps it open until an explicit resolution.
test_open_decision_survives_later_unrelated_event() {
  local home fakebin out
  home=$(make_home masking)
  mkdir -p "$home/daemon-home"
  mx_write_meta "$home/state/masked-decision.meta" \
    "window=broker:mx-masked-decision" \
    "worktree=$home/daemon-home" \
    "project=$home/daemon-home" \
    "harness=codex" \
    "kind=daemon" \
...
test_daemon_open_decision_survives_live_endpoint() {
  local home fakebin out
  home=$(make_home active-daemon)
  mkdir -p "$home/daemon-home"
  mx_write_meta "$home/state/active-daemon.meta" \
    "window=broker:mx-active-daemon" \
    "worktree=$home/daemon-home" \
    "project=$home/daemon-home" \
    "harness=codex" \
    "kind=daemon" \
    ...
    "home=$home/daemon-home" \
    "projects=alpha"
  printf 'needs-decision [key=race]: choose ordering\n' > "$home/state/active-daemon.status"
  fakebin=$(make_fakebin "$home")
  out=$(PATH="$fakebin:$PATH" MX_HOME="$home" "$SNAPSHOT" --json)
  printf '%s' "$out" | jq -e '
    .tasks[] | select(.id == "active-daemon")
    | .endpoint.agent_alive == "alive"
      and .hints.pending_decision == true
      and (.hints.open_decisions | length) == 1
  ' >/dev/null || fail "a live daemon endpoint must not clear an unrelated keyed decision: $out"
  pass "a live daemon endpoint preserves unrelated open decisions"
}

# An open decision clears ONLY on an explicit resolution referencing its key, never
# on an unrelated terminal line.
test_open_decision_transfers_to_maintainer_hold() {
  local home fakebin out
  home=$(make_home maintainer-held-transfer)
  mkdir -p "$home/daemon-home"
  mx_write_meta "$home/state/transferred-decision.meta" \
    "window=broker:mx-transferred-decision" \
    "worktree=$home/daemon-home" \
    "project=$home/daemon-home" \
    "harness=codex" \
    "kind=daemon" \
  ...
  printf 'maintainer-held [key=route]: tracked by transferred-decision-route\n' >> "$home/state/transferred-decision.status"
  fakebin=$(make_fakebin "$home")
  out=$(PATH="$fakebin:$PATH" MX_HOME="$home" "$SNAPSHOT" --json)
  printf '%s' "$out" | jq -e '
  ...
  ' >/dev/null || fail "maintainer-held transfer must close only the duplicate status copy: $out"
  pass "durable maintainer-held transfer closes the duplicate live status decision"
}
...
test_open_decision_clears_on_keyed_resolution() {
  ...
  mkdir -p "$home/daemon-home"
  mx_write_meta "$home/state/resolved-decision.meta" \
    "window=broker:mx-resolved-decision" \
    "worktree=$home/daemon-home" \
    "project=$home/daemon-home" \
    "harness=codex" \
    "kind=daemon" \
  ...
  printf 'done: an unrelated subtask finished\n' >> "$home/state/resolved-decision.status"
  printf 'resolved [key=race]: maintainer chose subscribe-then-reconcile\n' >> "$home/state/resolved-decision.status"
  fakebin=$(make_fakebin "$home")
  out=$(PATH="$fakebin:$PATH" MX_HOME="$home" "$SNAPSHOT" --json)
  printf '%s' "$out" | jq -e '
...
# decision either answered or captured in the report for the maintainer - must surface
...
# against the actors lifecycle; report prose never opens or reopens a decision.
...
test_parked_scout_decision_stays_pending() {
  ...
  mx_write_meta "$home/state/parked-scout.meta" \
    "window=broker:mx-parked-scout" \
