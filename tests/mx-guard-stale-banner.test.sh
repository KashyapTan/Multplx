#!/usr/bin/env bash
# Regression tests for mx-guard's stale-watcher banner deduplication.
#
# The first stale command in one MX_HOME must print the full actionable watcher
# banner.
...
TMP_ROOT=$(mx_test_tmproot mx-guard-stale-banner)
...
make_guard_case() {
  ...
  mx_write_meta "$home/state/task.meta" "window=broker:mx-task" "kind=delivery"
  printf '%s\n' "$dir"
}
...
run_guard_case() {
  local dir=$1
  MX_ROOT_OVERRIDE="$(case_root "$dir")" \
    MX_HOME="$(case_home "$dir")" \
    MX_GUARD_GRACE=999 \
    "$ROOT/bin/mx-guard.sh" 2>&1
}
...
test_first_stale_call_prints_full_banner() {
  ...
  pass "mx-guard stale banner: first stale call prints the full actionable banner"
}
...
test_healthy_recovery_rearms_next_stale_episode() {
  ...
  pass "mx-guard stale banner: healthy recovery rearms the next stale episode"
}
...
test_read_only_before_writable_does_not_consume_full_banner() {
  ...
  pass "mx-guard stale banner: read-only before writable does not consume full banner"
}
...
test_healthy_read_only_does_not_clear_marker() {
  ...
  pass "mx-guard stale banner: healthy read-only does not clear marker"
}
...
test_read_only_never_mutates_stale_banner_state_files() {
  ...
  pass "mx-guard stale banner: read-only never mutates stale-banner state files"
}
