#!/usr/bin/env bash
# Parent-owned daemon pending-reply guards (bin/mx-pending-reply-lib.sh).
#
...
#   9. Direct unmarked maintainer input creates no expectation
#  10. mx-send daemon path embeds corr and creates durable pending records
#  11. Backend busy/idle observation works through the shared busy abstraction
#      used by Pi/Claude daemon backends (no conversation scrape)
set -u
...
REPORT="$ROOT/bin/mx-daemon-report.sh"
TMP_ROOT=$(mx_test_tmproot mx-pending-reply)

export MX_PENDING_REPLY_GRACE_SECS=0
export MX_SEND_SETTLE=0
...
run_send() {
  ...
  env PATH="$fb:$PATH" \
    ...
    MX_PENDING_REPLY_GRACE_SECS=0 \
    "$SEND" "$@" 2>/dev/null
}

phase_of() {  # <state> <corr>
  mx_pending_reply_get "$(mx_pending_reply_path "$1" "$2")" phase
}
...
test_normal_correlated_reply_resolves_once() {
  ...
  state="$home/state"
  export MX_PENDING_REPLY_NOW=1000
  corr=$(mx_pending_reply_create "$home" "$state" "hibit" "audit the ledger")
  mx_pending_reply_mark_delivered "$state" "$corr"
  status="$state/hibit.status"
  if mx_pending_reply_try_resolve "$state" "$corr"; then
    fail "missing status must not resolve"
  fi
  printf 'done [corr=%s]: ledger clean\n' "$corr" > "$status"
  mx_pending_reply_try_resolve "$state" "$corr" || fail "correlated status should resolve"
  [ "$(phase_of "$state" "$corr")" = resolved ] || fail "phase should be resolved"
  # Idempotent second resolve.
  mx_pending_reply_try_resolve "$state" "$corr" || fail "second resolve must stay successful"
  [ "$(phase_of "$state" "$corr")" = resolved ] || fail "phase must remain resolved"
  rec=$(mx_pending_reply_path "$state" "$corr")
  [ "$(mx_pending_reply_get "$rec" resolved_via)" = status ] \
...
test_completed_turn_no_report_triggers_one_recovery() {
  ...
  : > "$hook_log"
  export MX_PENDING_REPLY_NOW=2000
  export MX_PENDING_REPLY_SEND_HOOK='printf "%s\t%s\n" >>"'"$hook_log"'"'
  # The hook above is wrong for eval form - use a function.
  # Invoked indirectly through MX_PENDING_REPLY_SEND_HOOK.
  # shellcheck disable=SC2329
  recovery_hook() {
    printf '%s\t%s\n' "$1" "$2" >> "$hook_log"
  }
  export -f recovery_hook
  export MX_PENDING_REPLY_SEND_HOOK='recovery_hook'

  corr=$(mx_pending_reply_create "$home" "$state" "hibit" "status of phase 7")
  mx_pending_reply_mark_delivered "$state" "$corr"
  # Turn completes with no parent report (the Hi Bit missed-report shape).
  mx_pending_reply_observe_busy "$state" "$corr" busy
  mx_pending_reply_observe_busy "$state" "$corr" idle
  mx_pending_reply_send_recovery "$state" "$corr" \
  ...
  if mx_pending_reply_send_recovery "$state" "$corr" 2>/dev/null; then
...
test_recovery_attempt_is_never_reinjected() {
  ...
  : > "$hook_log"
  export MX_PENDING_REPLY_NOW=2500
  recovery_fail_hook() {
    printf 'attempted\n' >> "$hook_log"
    return 1
  }
  export -f recovery_fail_hook
  export MX_PENDING_REPLY_SEND_HOOK=recovery_fail_hook
  corr=$(mx_pending_reply_create "$home" "$state" hibit "at most once")
  mx_pending_reply_mark_delivered "$state" "$corr"
  mx_pending_reply_mark_turn_completed "$state" "$corr" request
  if mx_pending_reply_send_recovery "$state" "$corr"; then
  ...
  [ -z "$(mx_pending_reply_get "$(mx_pending_reply_path "$state" "$corr")" recovery_sent_epoch)" ] \
  ...
  mx_pending_reply_maybe_escalate "$state" "$corr" \
  ...
  live_corr=$(mx_pending_reply_create "$home" "$state" hibit "live recovery")
  mx_pending_reply_mark_delivered "$state" "$live_corr"
  mx_pending_reply_mark_turn_completed "$state" "$live_corr" request
  live_rec=$(mx_pending_reply_path "$state" "$live_corr")
  live_pid=${BASHPID:-$$}
  live_identity=$(mx_pending_reply_pid_identity "$live_pid") \
    || fail "live sender identity should be observable"
  mx_pending_reply_set "$live_rec" recovery_attempted_epoch 2500 || fail "live attempt precommit failed"
  mx_pending_reply_set "$live_rec" recovery_sender_pid "$live_pid" || fail "live sender pid commit failed"
  mx_pending_reply_set "$live_rec" recovery_sender_identity "$live_identity" \
    || fail "live sender identity commit failed"
  mx_pending_reply_set "$live_rec" phase recovery_sending || fail "live sending phase failed"
  mx_pending_reply_tick_one "$state" "$live_corr" unknown || fail "live recovery tick failed"
  [ "$(phase_of "$state" "$live_corr")" = recovery_sending ] \
    || fail "live recovery must remain in progress without elapsed-time inference"
  corr=$(mx_pending_reply_create "$home" "$state" hibit "crashed recovery")
  mx_pending_reply_mark_delivered "$state" "$corr"
  mx_pending_reply_mark_turn_completed "$state" "$corr" request
  rec=$(mx_pending_reply_path "$state" "$corr")
  mx_pending_reply_set "$rec" recovery_attempted_epoch 2500 || fail "attempt precommit failed"
  mx_pending_reply_set "$rec" phase recovery_sending || fail "sending phase precommit failed"
  mx_pending_reply_tick_one "$state" "$corr" unknown || fail "recovery reconciliation failed"
  [ "$(phase_of "$state" "$corr")" = escalated ] \
    || fail "interrupted recovery attempt should escalate unknown delivery"
  [ "$(mx_pending_reply_get "$rec" recovery_delivery_outcome)" = unknown ] \
    || fail "interrupted recovery should preserve unknown delivery outcome"
  [ -z "$(mx_pending_reply_get "$rec" recovery_sent_epoch)" ] \
  ...
  unset MX_PENDING_REPLY_SEND_HOOK
...
test_recovery_reply_resolves_original() {
  ...
  export -f recovery_hook
  export MX_PENDING_REPLY_SEND_HOOK='recovery_hook'
  export MX_PENDING_REPLY_NOW=3000

  corr=$(mx_pending_reply_create "$home" "$state" "hibit" "phase 7 status")
  mx_pending_reply_mark_delivered "$state" "$corr"
  mx_pending_reply_mark_turn_completed "$state" "$corr" request
  mx_pending_reply_send_recovery "$state" "$corr" || fail "recovery send failed"
  printf 'done [corr=%s]: phase 7 is Done (reposted)\n' "$corr" > "$state/hibit.status"
  mx_pending_reply_try_resolve "$state" "$corr" || fail "recovery reply should resolve original"
...
test_second_missed_turn_escalates_once_and_stays_durable() {
  ...
  export -f recovery_hook
  export MX_PENDING_REPLY_SEND_HOOK='recovery_hook'
  export MX_PENDING_REPLY_NOW=4000
  # Do not export STATE into the test process: mx-send resolves
  # MX_STATE_OVERRIDE/STATE from the environment and a leak breaks later cases.

  corr=$(mx_pending_reply_create "$home" "$state" "hibit" "why is phase 7 stuck")
  mx_pending_reply_mark_delivered "$state" "$corr"
  mx_pending_reply_mark_turn_completed "$state" "$corr" request
  mx_pending_reply_send_recovery "$state" "$corr" || fail "recovery send failed"
  # Recovery turn also completes with no correlated report.
  mx_pending_reply_mark_turn_completed "$state" "$corr" recovery
  mx_pending_reply_maybe_escalate "$state" "$corr" || fail "escalation should fire"
  ...
  if mx_pending_reply_maybe_escalate "$state" "$corr" 2>/dev/null; then
  ...
  [ "$(mx_pending_reply_get "$rec" parent_status)" = "$state/hibit.status" ] \
...
test_escalation_publication_failure_retries() {
  ...
  state="$home/state"
  export MX_PENDING_REPLY_NOW=4500
  corr=$(mx_pending_reply_create "$home" "$state" "hibit" "retry escalation")
  mx_pending_reply_mark_delivered "$state" "$corr"
  mx_pending_reply_mark_turn_completed "$state" "$corr" request
  export MX_PENDING_REPLY_SEND_HOOK='true'
  mx_pending_reply_send_recovery "$state" "$corr" || fail "recovery send failed"
  mx_pending_reply_mark_turn_completed "$state" "$corr" recovery
  rec=$(mx_pending_reply_path "$state" "$corr")
  target="$state/escalation-target"
  mkdir -p "$target"
  mx_pending_reply_set "$rec" parent_status "$target" || fail "failed to set escalation target"
  if mx_pending_reply_maybe_escalate "$state" "$corr" 2>/dev/null; then
  ...
  rmdir "$target"
  mx_pending_reply_maybe_escalate "$state" "$corr" || fail "escalation retry should succeed"
...
test_transport_success_is_not_reply_success() {
  ...
  state="$home/state"
  export MX_PENDING_REPLY_NOW=5000
  corr=$(mx_pending_reply_create "$home" "$state" "hibit" "ping")
  mx_pending_reply_mark_delivered "$state" "$corr" || fail "mark delivered failed"
...
test_undelivered_records_are_scan_immutable() {
  (
    ...
    export MX_PENDING_REPLY_NOW=5500
    corr=$(mx_pending_reply_create "$home" "$state" hibit "not delivered yet")
    rec=$(mx_pending_reply_path "$state" "$corr")
    printf 'done [corr=%s]: arrived too early\n' "$corr" > "$state/hibit.status"
    printf 'done [corr=%s]: wrong home too early\n' "$corr" > "$sm_home/state/hibit.status"
    mx_write_daemon_meta "$state/hibit.meta" "$sm_home" "sess:mx-hibit"
    before=$(cat "$rec")
    if mx_pending_reply_try_resolve "$state" "$corr"; then
      fail "undelivered expectation must not resolve"
    fi
    mx_pending_reply_detect_wrong_home "$state" "$corr" "$sm_home" \
      || fail "undelivered wrong-home check should be inert"
    mx_pending_reply_tick_one "$state" "$corr" busy "$sm_home" \
      || fail "undelivered direct tick should be inert"
    mx_backend_busy_state() { fail "undelivered watcher tick must not probe the backend"; }
    mx_backend_capture() { fail "undelivered watcher tick must not capture the backend"; }
    mx_pending_reply_tick "$state" || fail "undelivered watcher tick should succeed"
    after=$(cat "$rec")
    [ "$after" = "$before" ] || fail "scan paths must not mutate an undelivered record"
    mx_pending_reply_mark_delivered "$state" "$corr" || fail "delivery marker should succeed"
    mx_pending_reply_tick_one "$state" "$corr" unknown "$sm_home" \
...
test_delivery_confirmation_fallback_reconciles() {
  (
    ...
    # This fixture clock is intentionally scoped to the isolated subshell.
    # shellcheck disable=SC2030,SC2031
    export MX_PENDING_REPLY_NOW=5750
    corr=$(mx_pending_reply_create "$home" "$state" hibit "confirmed delivery")
    rec=$(mx_pending_reply_path "$state" "$corr")
    mx_pending_reply_mark_delivered() { return 1; }
    if mx_pending_reply_confirm_delivery "$state" "$corr"; then
      fail "primary delivery commit failure should be reported"
    else
      rc=$?
    fi
    [ "$rc" = 2 ] || fail "durable fallback should return status 2, got $rc"
    marker=$(mx_pending_reply_delivery_confirmation_path "$state" "$corr")
    [ -f "$marker" ] || fail "delivery confirmation fallback marker should persist"
    [ -z "$(mx_pending_reply_get "$rec" delivered_epoch)" ] \
      || fail "failed primary commit should leave delivered_epoch empty"
    . "$ROOT/bin/mx-pending-reply-lib.sh"
    mx_pending_reply_tick_one "$state" "$corr" unknown \
      || fail "watcher should reconcile the delivery marker"
    [ "$(mx_pending_reply_get "$rec" delivered_epoch)" = 5750 ] \
      || fail "watcher should restore the confirmed delivery epoch"
    [ ! -e "$marker" ] || fail "reconciled delivery marker should be removed"
    prepared_corr=$(mx_pending_reply_create "$home" "$state" hibit "prepared delivery")
    prepared_rec=$(mx_pending_reply_path "$state" "$prepared_corr")
    mx_pending_reply_prepare_delivery "$state" "$prepared_corr" \
      || fail "delivery preparation should persist before transport"
    mx_pending_reply_set "$prepared_rec" grace_secs 10 \
      || fail "delivery-unknown grace fixture should persist"
    prepared_marker=$(mx_pending_reply_delivery_confirmation_path "$state" "$prepared_corr")
    [ -f "$prepared_marker" ] || fail "prepared delivery marker should persist"
    mx_pending_reply_tick_one "$state" "$prepared_corr" unknown \
    ...
    [ -z "$(mx_pending_reply_get "$prepared_rec" delivered_epoch)" ] \
    ...
    export MX_PENDING_REPLY_NOW=5760
    mx_pending_reply_write_delivery_confirmation \
    ...
    mx_pending_reply_tick "$state" || fail "watcher should accept a late delivery report"
    [ "$(phase_of "$state" "$prepared_corr")" = resolved ] \
      || fail "late report should resolve escalated delivery-unknown"
    [ "$(mx_pending_reply_get "$prepared_rec" delivered_epoch)" = 5760 ] \
      || fail "late report should provide delivery evidence"
    escalations=$(grep -Fc "pending-reply-id=$prepared_corr" "$state/hibit.status")
    [ "$escalations" = 1 ] || fail "late report must not re-escalate delivery-unknown"
    mx_pending_reply_tick "$state" || fail "resolved late report should remain idempotent"
    [ "$(phase_of "$state" "$prepared_corr")" = resolved ] \
      || fail "late report resolution should remain durable"
    export MX_PENDING_REPLY_NOW=5800
    reported_corr=$(mx_pending_reply_create "$home" "$state" hibit "reported delivery")
    reported_rec=$(mx_pending_reply_path "$state" "$reported_corr")
    mx_pending_reply_prepare_delivery "$state" "$reported_corr" \
      || fail "reported delivery attempt should persist"
    reported_marker=$(mx_pending_reply_delivery_confirmation_path "$state" "$reported_corr")
    printf 'done [corr=%s]: report proves delivery\n' "$reported_corr" >> "$state/hibit.status"
    mx_pending_reply_try_resolve "$state" "$reported_corr" \
...
test_unrelated_and_stale_corr_cannot_resolve() {
  ...
  export MX_PENDING_REPLY_NOW=6000
  corr=$(mx_pending_reply_create "$home" "$state" "hibit" "need answer")
  mx_pending_reply_mark_delivered "$state" "$corr"
  other=$(mx_pending_reply_new_id)
...
test_restart_preserves_expectation_and_parent_destination() {
  ...
  state="$home/state"
  export MX_PENDING_REPLY_NOW=7000
  corr=$(mx_pending_reply_create "$home" "$state" "hibit" "survive restart")
  mx_pending_reply_mark_delivered "$state" "$corr"
  rec=$(mx_pending_reply_path "$state" "$corr")
  parent_status=$(mx_pending_reply_get "$rec" parent_status)
  parent_home=$(mx_pending_reply_get "$rec" parent_home)
  # Simulate process restart: re-source library and re-read the same record.
  # shellcheck source=bin/mx-pending-reply-lib.sh
  . "$ROOT/bin/mx-pending-reply-lib.sh"
  [ -f "$rec" ] || fail "record must survive restart"
  [ "$(mx_pending_reply_get "$rec" parent_status)" = "$parent_status" ] \
    || fail "parent_status must be stable across restart"
  [ "$(mx_pending_reply_get "$rec" parent_home)" = "$parent_home" ] \
...
test_wrong_home_detected_not_acknowledged() {
  ...
  mkdir -p "$sm_home/state"
  export MX_PENDING_REPLY_NOW=8000
  corr=$(mx_pending_reply_create "$home" "$state" "hibit" "report to parent")
  mx_pending_reply_mark_delivered "$state" "$corr"
  # Historical incident shape: report written under the daemon home.
  printf 'done [corr=%s]: stranded in self-home\n' "$corr" > "$sm_home/state/hibit.status"
  mx_pending_reply_detect_wrong_home "$state" "$corr" "$sm_home" \
    || fail "wrong-home detect should succeed"
  rec=$(mx_pending_reply_path "$state" "$corr")
  hits=$(mx_pending_reply_get "$rec" wrong_home_hits)
...
test_unmarked_maintainer_input_creates_no_expectation() {
  ...
  [ "$(cat "$log")" = "maintainer says hello" ] \
    || fail "actor send should stay unmarked"$'\n'"$(cat "$log" | od -An -c)"
  ...
  pass "direct unmarked maintainer input creates no expectation"
}

test_mx_send_marked_daemon_creates_pending_and_embeds_corr() {
  local dir fb log home rc got corr rec
  dir="$TMP_ROOT/send-pending"; mkdir -p "$dir"
  fb=$(make_stubs "$dir"); log="$dir/send.log"
  home=$(setup_parent send-pending)
  mx_write_daemon_meta "$home/state/hibit.meta" "$home/sm" "sess:mx-hibit"
  ...
  rec=$(mx_pending_reply_path "$home/state" "$corr")
  [ -f "$rec" ] || fail "pending-reply record must exist after marked send"
  [ "$(mx_pending_reply_get "$rec" phase)" = awaiting_report ] \
    || fail "phase should be awaiting_report after delivery"
  [ -n "$(mx_pending_reply_get "$rec" delivered_epoch)" ] \
    || fail "delivered_epoch must be set after successful send"
  [ "$(mx_pending_reply_get "$rec" task_id)" = hibit ] \
    || fail "task_id must match daemon id"
  pass "mx-send marked daemon path creates pending and embeds corr"
}
...
test_document_pointer_resolves() {
  ...
  state="$home/state"
  export MX_PENDING_REPLY_NOW=9000
  corr=$(mx_pending_reply_create "$home" "$state" "hibit" "deep audit")
  mx_pending_reply_mark_delivered "$state" "$corr"
  printf 'done [corr=%s]: see data/hibit/report.md\n' "$corr" > "$state/hibit.status"
  mx_pending_reply_try_resolve "$state" "$corr" || fail "document pointer status should resolve"
  [ "$(mx_pending_reply_get "$(mx_pending_reply_path "$state" "$corr")" resolved_via)" = document ] \
...
test_helper_report_resolves() {
  ...
  state="$home/state"
  export MX_PENDING_REPLY_NOW=9100
  corr=$(mx_pending_reply_create "$home" "$state" "hibit" "quick answer")
  mx_pending_reply_mark_delivered "$state" "$corr"
  "$REPORT" "$state/hibit.status" "done" "$corr" "all good" \
    || fail "helper report failed"
  mx_pending_reply_try_resolve "$state" "$corr" || fail "helper report should resolve"
  [ "$(mx_pending_reply_get "$(mx_pending_reply_path "$state" "$corr")" resolved_via)" = helper ] \
...
test_busy_idle_observation_via_backend_abstraction() {
  ...
  state="$home/state"
  export MX_PENDING_REPLY_NOW=9200
  corr=$(mx_pending_reply_create "$home" "$state" "hibit" "backend turn")
  mx_pending_reply_mark_delivered "$state" "$corr"
  # Simulates Pi/Claude daemon busy_state from mx_backend_busy_state without
  # reading conversation text (herdr native idle/busy or tmux unknown fallback).
  mx_pending_reply_observe_busy "$state" "$corr" unknown
  [ -z "$(mx_pending_reply_get "$(mx_pending_reply_path "$state" "$corr")" request_turn_completed_epoch)" ] \
    || fail "unknown busy_state must not prove turn completion"
  mx_pending_reply_observe_busy "$state" "$corr" busy
  mx_pending_reply_observe_busy "$state" "$corr" idle
  [ -n "$(mx_pending_reply_get "$(mx_pending_reply_path "$state" "$corr")" request_turn_completed_epoch)" ] \
...
test_unknown_backend_state_uses_capture_fallback() {
  ...
  for backend in tmux cmux; do
    (
      ...
      sm_home="$home/sm"
      mkdir -p "$sm_home/state"
      export MX_PENDING_REPLY_GRACE_SECS=10
      # These fixture overrides are intentionally scoped to the isolated subshell.
      # shellcheck disable=SC2030,SC2031
      export MX_PENDING_REPLY_NOW=10000
      corr=$(mx_pending_reply_create "$home" "$state" "hibit" "$backend fallback")
      mx_pending_reply_mark_delivered "$state" "$corr"
      mx_write_daemon_meta "$state/hibit.meta" "$sm_home" "session:mx-hibit"
      [ "$backend" = tmux ] || printf 'backend=%s\n' "$backend" >> "$state/hibit.meta"
      mx_backend_busy_state() { printf 'unknown'; }
      mx_backend_capture() { printf '%s' "$MX_PENDING_TEST_CAPTURE"; }
      # Invoked indirectly through MX_PENDING_REPLY_SEND_HOOK.
      # shellcheck disable=SC2329
      recovery_hook() { :; }
      ...
      # shellcheck disable=SC2030,SC2031
      export MX_PENDING_REPLY_SEND_HOOK=recovery_hook
      export MX_PENDING_TEST_CAPTURE='idle footer'
      mx_pending_reply_tick "$state"
      rec=$(mx_pending_reply_path "$state" "$corr")
      [ -z "$(mx_pending_reply_get "$rec" request_turn_completed_epoch)" ] \
        || fail "$backend fallback must not accept stale idle before grace"
      # Continue advancing the subshell-local fixture clock.
      # shellcheck disable=SC2030,SC2031
      export MX_PENDING_REPLY_NOW=10010
      mx_pending_reply_tick "$state"
      [ "$(phase_of "$state" "$corr")" = recovery_sent ] \
        || fail "$backend fallback idle should trigger recovery after grace"
      export MX_PENDING_REPLY_NOW=10011
      export MX_PENDING_TEST_CAPTURE='Working...'
      mx_pending_reply_tick "$state"
      export MX_PENDING_REPLY_NOW=10012
      export MX_PENDING_TEST_CAPTURE='idle footer'
      mx_pending_reply_tick "$state"
      [ "$(phase_of "$state" "$corr")" = escalated ] \
  ...
  pass "tmux and cmux unknown states use bounded capture fallback"
}
...
test_tick_skips_terminal_and_reuses_target_observation() {
  (
    ...
    # This fixture clock is intentionally scoped to the isolated subshell.
    # shellcheck disable=SC2030,SC2031
    export MX_PENDING_REPLY_NOW=10100
    open1=$(mx_pending_reply_create "$home" "$state" hibit "first open request")
    open2=$(mx_pending_reply_create "$home" "$state" hibit "second open request")
    mx_pending_reply_mark_delivered "$state" "$open1"
    mx_pending_reply_mark_delivered "$state" "$open2"
    resolved=$(mx_pending_reply_create "$home" "$state" resolved "resolved request")
    mx_pending_reply_mark_delivered "$state" "$resolved"
    printf 'done [corr=%s]: complete\n' "$resolved" > "$state/resolved.status"
    mx_pending_reply_try_resolve "$state" "$resolved" || fail "resolved fixture should resolve"
    escalated=$(mx_pending_reply_create "$home" "$state" escalated "escalated request")
    mx_pending_reply_mark_delivered "$state" "$escalated"
    rec=$(mx_pending_reply_path "$state" "$escalated")
    mx_pending_reply_set "$rec" phase escalated || fail "escalated fixture should transition"
    mkdir -p "$home/escalated/state"
    printf 'done [corr=%s]: wrong home\n' "$escalated" > "$home/escalated/state/escalated.status"
    mx_write_daemon_meta "$state/hibit.meta" "$home/hibit" "sess:mx-hibit"
    mx_write_daemon_meta "$state/resolved.meta" "$home/resolved" "sess:mx-resolved"
    mx_write_daemon_meta "$state/escalated.meta" "$home/escalated" "sess:mx-escalated"
    mx_backend_busy_state() {
      printf '%s\t%s\n' "$1" "$2" >> "$probe_log"
      printf 'busy'
    }
    mx_backend_capture() { fail "native busy observations should not capture"; }
    mx_pending_reply_find_resolve_line() {
      local status_file=$1 corr=$2 line
      printf '%s\t%s\n' "$status_file" "$corr" >> "$scan_log"
      [ -f "$status_file" ] || return 0
      while IFS= read -r line || [ -n "$line" ]; do
        mx_pending_reply_line_resolves "$line" "$corr" || continue
        printf '%s' "$line"
        return 0
      done < "$status_file"
      return 0
    ...
    rec=$(mx_pending_reply_path "$state" "$open1")
    [ "$(mx_pending_reply_get "$rec" turn_seen_busy)" = 1 ] \
      || fail "cached observation should update the first open record"
    rec=$(mx_pending_reply_path "$state" "$open2")
    [ "$(mx_pending_reply_get "$rec" turn_seen_busy)" = 1 ] \
      || fail "cached observation should update the second open record"
    rec=$(mx_pending_reply_path "$state" "$escalated")
    snapshot=$(mx_pending_reply_get "$rec" wrong_home_scan_signature)
    ...
    [ "$(mx_pending_reply_get "$rec" wrong_home_scan_signature)" = "$snapshot" ] \
...
test_correlations_reuse_only_for_matching_open_task() {
  ...
  state="$home/state"
  mx_write_daemon_meta "$state/domain.meta" "$home/domain" "sess:mx-domain"
  mx_write_daemon_meta "$state/other.meta" "$home/other" "sess:mx-other"
  run_send "$fb" "$home" "$log" domain "first request" || fail "first marked send failed"
  got=$(cat "$log")
  corr1=$(mx_pending_reply_extract_corr "$got")
  export MX_PENDING_REPLY_EXISTING_CORR=$corr1
  run_send "$fb" "$home" "$log" other "forwarded request" || fail "cross-task send failed"
  unset MX_PENDING_REPLY_EXISTING_CORR
  corr2=$(mx_pending_reply_extract_corr "$(cat "$log")")
  [ -n "$corr2" ] && [ "$corr2" != "$corr1" ] \
    || fail "cross-task send must receive a new correlation"
  rec=$(mx_pending_reply_path "$state" "$corr2")
  [ "$(mx_pending_reply_get "$rec" task_id)" = other ] \
  ...
  rec=$(mx_pending_reply_path "$state" "$corr3")
  [ "$(mx_pending_reply_get "$rec" task_id)" = domain ] \
...
test_tick_end_to_end_missed_then_escalate() {
  ...
  export MX_PENDING_REPLY_SEND_HOOK='recovery_hook'
  # shellcheck disable=SC2031
  export MX_PENDING_REPLY_NOW=9300

  corr=$(mx_pending_reply_create "$home" "$state" "hibit" "e2e miss")
  mx_pending_reply_mark_delivered "$state" "$corr"
  mx_write_daemon_meta "$state/hibit.meta" "$sm_home" "sess:mx-hibit"
  # Override backend busy via direct tick_one (backend may be unknown in hermetic home).
  mx_pending_reply_tick_one "$state" "$corr" busy "$sm_home"
  mx_pending_reply_tick_one "$state" "$corr" idle "$sm_home"
  [ "$(phase_of "$state" "$corr")" = recovery_sent ] \
  ...
  export MX_PENDING_REPLY_NOW=999999
  mx_pending_reply_tick_one "$state" "$corr" idle "$sm_home"
  [ -f "$(mx_pending_reply_path "$state" "$corr")" ] \
...
test_failed_send_discards_undelivered_expectation() {
  ...
  state="$home/state"
  export MX_PENDING_REPLY_NOW=9400
  corr=$(mx_pending_reply_create "$home" "$state" "hibit" "never lands")
  # Not delivered: discard is allowed.
  mx_pending_reply_discard_undelivered "$state" "$corr" || fail "discard undelivered failed"
  [ ! -f "$(mx_pending_reply_path "$state" "$corr")" ] \
    || fail "undelivered record should be removed"
  # Delivered records must not be discarded by this path.
  corr=$(mx_pending_reply_create "$home" "$state" "hibit" "landed")
  mx_pending_reply_mark_delivered "$state" "$corr"
  if mx_pending_reply_discard_undelivered "$state" "$corr" 2>/dev/null; then
    fail "delivered record must not be discarded"
  fi
  [ -f "$(mx_pending_reply_path "$state" "$corr")" ] || fail "delivered record must remain"
