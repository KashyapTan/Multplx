...
# lab teardown verifies that the default system session is byte-identical.
set -u
...
TMP_ROOT=$(mktemp -d "$(cd "${TMPDIR:-/tmp}" && pwd -P)/mx-herdr-session-cleanup-e2e.XXXXXX")
...
HERDR_LAB_SESSION=$("$HERDR_LAB_HELPER" name mx-herdr-session-start-stale-projection-cleanup-r1)
...
production_process_proof() {
  MX_HOME="$HOME_DIR" MX_BACKEND=herdr HERDR_SESSION="$HERDR_LAB_SESSION" \
    MX_HERDR_SESSION_CLEANUP_SOURCE_ONLY=1 PATH="$FAKEBIN:$HERDR_ORIGINAL_PATH" \
    bash -c '. "$1"; mx_herdr_cleanup_process_is_idle_shell "$2" "$3"' \
      _ "$ROOT/bin/mx-herdr-session-cleanup.sh" "$HERDR_LAB_SESSION" "$PANE"
}
focus_snapshot() {
...
ANCHOR=$(lab workspace create --cwd "$ROOT" --label maintainer-anchor --focus) || fail 'could not create focus anchor'
...
MX_HOME="$HOME_DIR" MX_BACKEND=herdr HERDR_SESSION="$HERDR_LAB_SESSION" \
  PATH="$FAKEBIN:$HERDR_ORIGINAL_PATH" "$ROOT/bin/mx-herdr-session-cleanup.sh" \
...
pass 'real named lab cleanup is idempotent and leaves the default system session to the teardown tripwire'
