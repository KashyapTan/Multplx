#!/usr/bin/env bash
# tests/mx-wake-queue.test.sh - wake-queue losslessness (the queue safety matrix):
...
# lives in mx-watcher-lock.test.sh; daemon classification/injection in
# mx-daemon.test.sh.
set -u
...
TMP_ROOT=$(mx_test_tmproot mx-wake-tests)
...
test_signal_catchup_without_running_watcher() {
  ...
  # watcher can absorb no-verb working: notes when the actor is provably working).
  # Use a maintainer-relevant verb so the wake is surfaced and the catch-up path is
  # tested.
...
test_stale_enqueue_before_suppressor() {
  ...
  window="test:mx-stale"
  printf 'idle prompt' > "$capture_file"
  printf 'window=%s\nkind=delivery\n' "$window" > "$state/stale.meta"
  # A stale pane sitting on a maintainer-relevant status is actionable when the actors
...
# whose actor is NOT provably working is surfaced immediately. That new path must keep
...
test_not_working_stale_enqueue_before_suppressor() {
  ...
  # fake mx-actor-state.sh the watcher reads via MX_ACTOR_STATE_BIN.)
  ...
  PATH="$fakebin:$PATH" MX_FAKE_TMUX_WINDOW="$window" MX_FAKE_TMUX_CAPTURE="$capture_file" \
    MX_STATE_OVERRIDE="$state" MX_ACTOR_STATE_BIN="$fakebin/mx-actor-state.sh" \
...
# watcher liveness via mx-guard.sh: a lapsed re-arm chain then surfaces even on a
...
if [ "${1:-}" = -MFcntl=:DEFAULT ]; then
  for arg in "$@"; do
    if [ "$arg" = "${MX_WAKE_ENRICH_SWAP_PATH:-}" ]; then
      rm -f "$arg"
      ln -s "$MX_WAKE_ENRICH_SWAP_TARGET" "$arg"
      break
    fi
  done
fi
exec "$MX_WAKE_ENRICH_REAL_PERL" "$@"
SH
  ...
  append_wake "$state" stale test:mx-task "stale: test:mx-task" || fail "stale wake append failed"
