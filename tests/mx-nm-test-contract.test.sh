...
#
# Multplx must not configure commands.test as a complete tests/*.test.sh walk
...
# composed around bin/mx-test-run.sh.
set -u
...
test_ci_still_runs_broad_behavior_suite() {
  ...
  grep -Fq 'bin/mx-test-run.sh --lane portable-parallel-1' "$CI" \
    || fail "CI must invoke portable parallel shard 1 through mx-test-run.sh"
  grep -Fq 'bin/mx-test-run.sh --lane portable-parallel-2' "$CI" \
    || fail "CI must invoke portable parallel shard 2 through mx-test-run.sh"
  grep -Fq 'bin/mx-test-run.sh --lane portable-serial' "$CI" \
    || fail "CI must invoke the portable serial remainder through mx-test-run.sh"
  grep -Fq 'bin/mx-test-run.sh --check-coverage' "$CI" \
    || fail "CI must prove complete lane coverage through mx-test-run.sh"
  ...
  if grep -Eq 'for test_script in tests/\*\.test\.sh' "$CI"; then
    fail "CI Behavior must not re-spell an inline tests/*.test.sh loop; use mx-test-run.sh"
  fi
