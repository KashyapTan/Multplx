#!/usr/bin/env bash
# Real Pi/Herdr regression for exact-id daemon marker delivery.
#
...
# mx-spawn.sh --daemon launch, captures Pi's before_agent_start prompt bytes,
# and proves both sides of the routing boundary:
#   - exact task id through explicit MX_HOME receives exactly one marker;
#   - direct terminal input remains unmarked.
#
# Every Herdr call, including calls made inside the production backend adapter,
# is routed through bin/mx-herdr-lab.sh. The PATH shim strips only the adapter's
...
if [ "${MX_SEND_MARKER_HERDR_E2E:-0}" != 1 ]; then
  echo "skip: set MX_SEND_MARKER_HERDR_E2E=1 to run the real Pi/Herdr daemon-marker regression"
  exit 0
fi
...
SESSION=$("$LAB_HELPER" name mx-send-daemon-marker-v7)
TMP_ROOT=$(mktemp -d "${TMPDIR:-/tmp}/mx-send-marker-herdr-e2e.XXXXXX")
SENDER_HOME="$TMP_ROOT/sender-home"
SECOND_HOME="$TMP_ROOT/daemon-home"
CAPTURE="$TMP_ROOT/pi-before-agent.jsonl"
FAKEBIN="$TMP_ROOT/fakebin"
ORIGINAL_PATH=$PATH
ID='marker-pi-sm'
REQUEST='MX_MARKER_HERDR_E2E exact-id request'
DIRECT='MX_MARKER_HERDR_DIRECT maintainer input'
...
printf '%s\n' "$ID" > "$SECOND_HOME/.mx-daemon-home"
cat > "$SECOND_HOME/data/charter.md" <<'EOF'
# Isolated marker capture daemon

You are a task-local daemon used only for the marker transport regression.
Stay idle and do not initiate work.
EOF

# The extension is already an explicit Pi -e resource in the real daemon
...
python3 - "$SECOND_HOME/.pi/extensions/mx-primary-turnend-guard.ts" "$CAPTURE_JSON" <<'PY'
...
PATH="$FAKEBIN:$ORIGINAL_PATH" MX_GATE_REFUSE_BYPASS=1 MX_HOME="$SENDER_HOME" HERDR_SESSION="$SESSION" \
  "$ROOT/bin/mx-spawn.sh" "$ID" "$SECOND_HOME" --daemon --harness pi --backend herdr >/dev/null

META="$SENDER_HOME/state/$ID.meta"
[ -f "$META" ] || fail "real daemon spawn did not write exact-id metadata"
[ "$(mx_meta_get "$META" kind)" = daemon ] || fail "real daemon metadata did not record kind=daemon"
TARGET=$(mx_backend_target_of_meta "$META")
PANE=${TARGET#*:}
case "$TARGET" in
  "$SESSION":w*:p*) : ;;
  *) fail "real daemon metadata recorded an unexpected Herdr target: $TARGET" ;;
esac
...
wait_for_prompt 'Isolated marker capture daemon' \
...
PATH="$FAKEBIN:$ORIGINAL_PATH" MX_GATE_REFUSE_BYPASS=1 MX_HOME="$SENDER_HOME" \
...
# therefore remains conversational maintainer input.
