#!/usr/bin/env bash
# Behavior tests for bin/mx-actor-state.sh - the deterministic actors-current-state
# helper.
#
# The status file (state/<id>.status) is a best-effort append-only EVENT LOG, so
# `tail -1` of it reports the last event, not the current state. mx-actor-state
...
#   (k) actor_is_provably_working end-to-end over the REAL helper (not a canned
#       fake mx-actor-state.sh verdict): cross-branch attribution via the runs
...
ACTOR_STATE="$ROOT/bin/mx-actor-state.sh"
...
# (git symbolic-ref) resolves like it would for a live actors worktree.
make_repo_on_branch() {  # <dir> <branch>
  ...
  # Real worktree HEAD for run head-binding (fixtures read MX_FAKE_RUN_HEAD).
  MX_FAKE_RUN_HEAD=$(git -C "$dir" rev-parse HEAD)
  export MX_FAKE_RUN_HEAD
}
...
case "${1:-}" in
  axi)
    ...
    case "${1:-}" in
      status)
        shift
        if [ "${1:-}" = --run ]; then printf '%s\n' "${MX_FAKE_AXI_STATUS_RUN:-}"
        else printf '%s\n' "${MX_FAKE_AXI_STATUS:-}"; fi ;;
      logs)
        printf '%s\n' "${MX_FAKE_CI_LOGS:-}" ;;
    esac
    ;;
...
case "${1:-}" in
  display-message)
    [ "${MX_FAKE_TMUX_MISSING:-0}" = 1 ] && exit 1
    printf '%%1\n' ;;
  capture-pane)
    [ "${MX_FAKE_TMUX_MISSING:-0}" = 1 ] && exit 1
    if [ "${MX_FAKE_BUSY:-0}" = 1 ]; then printf 'work in progress\nesc to interrupt\n'
...
case "${1:-}" in
  ...
  pane)
    case "${2:-}" in
      read)
        [ "${MX_FAKE_HERDR_MISSING:-0}" = 1 ] && exit 1
        if [ "${MX_FAKE_HERDR_BUSY:-0}" = 1 ]; then printf 'work in progress\nesc to interrupt\n'
        else printf 'all quiet\n> \n'; fi
        exit 0 ;;
    esac ;;
  agent)
    case "${2:-}" in
      get)
        [ -n "${MX_FAKE_HERDR_AGENT_STATUS:-}" ] || exit 1
        printf '{"result":{"agent":{"agent_status":"%s"}}}\n' "$MX_FAKE_HERDR_AGENT_STATUS"
        exit 0 ;;
    esac ;;
esac
exit 0
...
# Run the helper for one case dir. MX_FAKE_* env (run output, busy flag) are read
# from the caller's environment by the fakes above.
run_actor_state() {  # <case-dir> <id>
  PATH="$1/fakebin:$PATH" MX_STATE_OVERRIDE="$1/state" "$ACTOR_STATE" "$2"
}
...
reset_fakes() {
  ...
  MX_FAKE_RUNS_LIST=""
  MX_FAKE_BUSY=0
  MX_FAKE_TMUX_MISSING=0
  MX_FAKE_HERDR_BUSY=0
  MX_FAKE_HERDR_MISSING=0
  MX_FAKE_HERDR_AGENT_STATUS=""
  MX_FAKE_CI_LOGS=""
  export MX_FAKE_AXI_STATUS MX_FAKE_AXI_STATUS_RUN MX_FAKE_RUNS_LIST MX_FAKE_BUSY MX_FAKE_TMUX_MISSING
  export MX_FAKE_HERDR_BUSY MX_FAKE_HERDR_MISSING MX_FAKE_HERDR_AGENT_STATUS MX_FAKE_CI_LOGS
}
...
test_active_run_is_authoritative() {
  ...
  make_repo_on_branch "$d/wt" mx/feat-a
  make_fakebin "$d" >/dev/null
  mx_write_meta "$d/state/feat-a.meta" "window=fm:mx-feat-a" "worktree=$d/wt" "kind=delivery"
  MX_FAKE_AXI_STATUS="$(run_running mx/feat-a)"
  local out; out=$(run_actor_state "$d" feat-a)
...
test_stale_needs_decision_superseded() {
  ...
  make_repo_on_branch "$d/wt" mx/feat-b
  make_fakebin "$d" >/dev/null
  mx_write_meta "$d/state/feat-b.meta" "window=fm:mx-feat-b" "worktree=$d/wt" "kind=delivery"
  printf 'working: started\nneeds-decision: pick A or B\n' > "$d/state/feat-b.status"
  MX_FAKE_AXI_STATUS="$(run_fixing mx/feat-b)"
  local out; out=$(run_actor_state "$d" feat-b)
...
test_stale_blocked_superseded() {
  ...
  make_repo_on_branch "$d/wt" mx/feat-bb
  make_fakebin "$d" >/dev/null
  mx_write_meta "$d/state/feat-bb.meta" "window=fm:mx-feat-bb" "worktree=$d/wt" "kind=delivery"
  printf 'blocked: waiting on review answer\n' > "$d/state/feat-bb.status"
  MX_FAKE_AXI_STATUS="$(run_running mx/feat-bb)"
  local out; out=$(run_actor_state "$d" feat-bb)
...
test_genuine_parked_not_superseded() {
  ...
  make_repo_on_branch "$d/wt" mx/feat-c
  make_fakebin "$d" >/dev/null
  mx_write_meta "$d/state/feat-c.meta" "window=fm:mx-feat-c" "worktree=$d/wt" "kind=delivery"
  printf 'needs-decision: review gate\n' > "$d/state/feat-c.status"
  MX_FAKE_AXI_STATUS="$(run_parked mx/feat-c)"
  local out; out=$(run_actor_state "$d" feat-c)
...
test_scalar_gate_parked_not_superseded() {
  ...
  make_repo_on_branch "$d/wt" mx/feat-cs
  make_fakebin "$d" >/dev/null
  mx_write_meta "$d/state/feat-cs.meta" "window=fm:mx-feat-cs" "worktree=$d/wt" "kind=delivery"
  printf 'needs-decision: review gate\n' > "$d/state/feat-cs.status"
  MX_FAKE_AXI_STATUS="$(run_parked_scalar_gate_running mx/feat-cs)"
  local out; out=$(run_actor_state "$d" feat-cs)
...
test_gate_block_parked_not_superseded() {
  ...
  make_repo_on_branch "$d/wt" mx/feat-cb
  make_fakebin "$d" >/dev/null
  mx_write_meta "$d/state/feat-cb.meta" "window=fm:mx-feat-cb" "worktree=$d/wt" "kind=delivery"
  printf 'needs-decision: review gate\n' > "$d/state/feat-cb.status"
  MX_FAKE_AXI_STATUS="$(run_parked_in_gate_block mx/feat-cb)"
  local out; out=$(run_actor_state "$d" feat-cb)
...
test_ci_ready_done_log_beats_monitoring_run() {
  ...
  make_repo_on_branch "$d/wt" mx/feat-ci
  make_fakebin "$d" >/dev/null
  mx_write_meta "$d/state/feat-ci.meta" "window=fm:mx-feat-ci" "worktree=$d/wt" "kind=delivery"
  printf 'done: PR https://github.com/o/r/pull/2 checks green\n' > "$d/state/feat-ci.status"
  MX_FAKE_AXI_STATUS="$(run_ci_monitoring mx/feat-ci)"
  local out; out=$(run_actor_state "$d" feat-ci)
...
# Regression for the PR #252 incident: the actor's own status log never got a
...
# merge/close. mx-actor-state must surface this as done, not "validating
# (running)", so a green PR is never silently absorbed as still-in-progress.
test_ci_monitoring_checks_green_surfaces_done() {
  reset_fakes
  local d; d=$(new_case ci-green)
  make_repo_on_branch "$d/wt" mx/feat-cigreen
  make_fakebin "$d" >/dev/null
  mx_write_meta "$d/state/feat-cigreen.meta" "window=fm:mx-feat-cigreen" "worktree=$d/wt" "kind=delivery"
  # No status-log line at all: the actors never reported its own checks-green line.
  MX_FAKE_AXI_STATUS="$(run_ci_monitoring mx/feat-cigreen)"
  MX_FAKE_CI_LOGS=$(cat <<'EOF'
CI checks running, waiting for results...
all CI checks passed - still monitoring until merged or closed
EOF
)
  local out; out=$(run_actor_state "$d" feat-cigreen)
...
test_top_level_ci_checks_green_surfaces_done() {
  ...
  make_repo_on_branch "$d/wt" mx/feat-topcigreen
  make_fakebin "$d" >/dev/null
  mx_write_meta "$d/state/feat-topcigreen.meta" "window=fm:mx-feat-topcigreen" "worktree=$d/wt" "kind=delivery"
  MX_FAKE_AXI_STATUS="$(run_top_level_ci mx/feat-topcigreen)"
  MX_FAKE_CI_LOGS="all CI checks passed - still monitoring until merged or closed"
  local out; out=$(run_actor_state "$d" feat-topcigreen)
...
test_ci_monitoring_no_checks_terminal_surfaces_done() {
  ...
  make_repo_on_branch "$d/wt" mx/feat-cinochecks
  make_fakebin "$d" >/dev/null
  mx_write_meta "$d/state/feat-cinochecks.meta" "window=fm:mx-feat-cinochecks" "worktree=$d/wt" "kind=delivery"
  MX_FAKE_AXI_STATUS="$(run_ci_monitoring mx/feat-cinochecks)"
  MX_FAKE_CI_LOGS="no CI checks reported - still monitoring until merged or closed"
  local out; out=$(run_actor_state "$d" feat-cinochecks)
...
test_ci_monitoring_green_then_rearm_stays_working() {
  ...
  make_repo_on_branch "$d/wt" mx/feat-cirearm
  make_fakebin "$d" >/dev/null
  mx_write_meta "$d/state/feat-cirearm.meta" "window=fm:mx-feat-cirearm" "worktree=$d/wt" "kind=delivery"
  MX_FAKE_AXI_STATUS="$(run_ci_monitoring mx/feat-cirearm)"
  MX_FAKE_CI_LOGS=$(cat <<'EOF'
all CI checks passed - still monitoring until merged or closed
base branch advanced (aaaaaaa..bbbbbbb), re-arming CI monitor timeout
EOF
)
  local out; out=$(run_actor_state "$d" feat-cirearm)
...
test_ci_monitoring_no_checks_yet_stays_working() {
  ...
  make_repo_on_branch "$d/wt" mx/feat-cinochecksyet
  make_fakebin "$d" >/dev/null
  mx_write_meta "$d/state/feat-cinochecksyet.meta" "window=fm:mx-feat-cinochecksyet" "worktree=$d/wt" "kind=delivery"
  MX_FAKE_AXI_STATUS="$(run_ci_monitoring mx/feat-cinochecksyet)"
  MX_FAKE_CI_LOGS=$(cat <<'EOF'
...
EOF
  ...
  local out; out=$(run_actor_state "$d" feat-cinochecksyet)
...
test_ci_monitoring_still_waiting_stays_working() {
  ...
  make_repo_on_branch "$d/wt" mx/feat-ciwait
  make_fakebin "$d" >/dev/null
  mx_write_meta "$d/state/feat-ciwait.meta" "window=fm:mx-feat-ciwait" "worktree=$d/wt" "kind=delivery"
  MX_FAKE_AXI_STATUS="$(run_ci_monitoring mx/feat-ciwait)"
  MX_FAKE_CI_LOGS="CI checks running, waiting for results..."
  local out; out=$(run_actor_state "$d" feat-ciwait)
...
test_ci_monitoring_green_then_new_issue_stays_working() {
  ...
  make_repo_on_branch "$d/wt" mx/feat-cirelapse
  make_fakebin "$d" >/dev/null
  mx_write_meta "$d/state/feat-cirelapse.meta" "window=fm:mx-feat-cirelapse" "worktree=$d/wt" "kind=delivery"
  MX_FAKE_AXI_STATUS="$(run_ci_monitoring mx/feat-cirelapse)"
  MX_FAKE_CI_LOGS=$(cat <<'EOF'
all CI checks passed - still monitoring until merged or closed
base branch advanced (aaaaaaa..bbbbbbb), re-arming CI monitor timeout
...
EOF
  ...
  local out; out=$(run_actor_state "$d" feat-cirelapse)
...
test_ci_ready_done_log_relapse_stays_working() {
  ...
  make_repo_on_branch "$d/wt" mx/feat-cireadyrelapse
  make_fakebin "$d" >/dev/null
  mx_write_meta "$d/state/feat-cireadyrelapse.meta" "window=fm:mx-feat-cireadyrelapse" "worktree=$d/wt" "kind=delivery"
  printf 'done: PR https://github.com/o/r/pull/2 checks green\n' > "$d/state/feat-cireadyrelapse.status"
  MX_FAKE_AXI_STATUS="$(run_ci_monitoring mx/feat-cireadyrelapse)"
  MX_FAKE_CI_LOGS=$(cat <<'EOF'
all CI checks passed - still monitoring until merged or closed
base branch advanced (aaaaaaa..bbbbbbb), re-arming CI monitor timeout
CI checks running, waiting for results...
EOF
  ...
  local out; out=$(run_actor_state "$d" feat-cireadyrelapse)
...
test_ci_fixing_after_green_stays_working() {
  ...
  make_repo_on_branch "$d/wt" mx/feat-cifixing
  make_fakebin "$d" >/dev/null
  mx_write_meta "$d/state/feat-cifixing.meta" "window=fm:mx-feat-cifixing" "worktree=$d/wt" "kind=delivery"
  printf 'done: PR https://github.com/o/r/pull/2 checks green\n' > "$d/state/feat-cifixing.status"
  MX_FAKE_AXI_STATUS="$(run_ci_fixing mx/feat-cifixing)"
  MX_FAKE_CI_LOGS="all CI checks passed - still monitoring until merged or closed"
  local out; out=$(run_actor_state "$d" feat-cifixing)
...
test_top_level_fixing_ci_running_after_green_stays_working() {
  ...
  make_repo_on_branch "$d/wt" mx/feat-topfixingci
  make_fakebin "$d" >/dev/null
  mx_write_meta "$d/state/feat-topfixingci.meta" "window=fm:mx-feat-topfixingci" "worktree=$d/wt" "kind=delivery"
  MX_FAKE_AXI_STATUS="$(run_fixing_ci_running mx/feat-topfixingci)"
  MX_FAKE_CI_LOGS="all CI checks passed - still monitoring until merged or closed"
  local out; out=$(run_actor_state "$d" feat-topfixingci)
...
test_top_level_fixing_done_log_stays_working() {
  ...
  make_repo_on_branch "$d/wt" mx/feat-topfixing
  make_fakebin "$d" >/dev/null
  mx_write_meta "$d/state/feat-topfixing.meta" "window=fm:mx-feat-topfixing" "worktree=$d/wt" "kind=delivery"
  printf 'done: PR https://github.com/o/r/pull/2 checks green\n' > "$d/state/feat-topfixing.status"
  MX_FAKE_AXI_STATUS="$(run_fixing mx/feat-topfixing)"
  MX_FAKE_CI_LOGS="all CI checks passed - still monitoring until merged or closed"
  local out; out=$(run_actor_state "$d" feat-topfixing)
...
test_terminal_passed() {
  ...
  make_repo_on_branch "$d/wt" mx/feat-d
  make_fakebin "$d" >/dev/null
  mx_write_meta "$d/state/feat-d.meta" "window=fm:mx-feat-d" "worktree=$d/wt" "kind=delivery"
  MX_FAKE_AXI_STATUS="$(run_passed mx/feat-d)"
  local out; out=$(run_actor_state "$d" feat-d)
...
test_terminal_failed() {
  ...
  make_repo_on_branch "$d/wt" mx/feat-e
  make_fakebin "$d" >/dev/null
  mx_write_meta "$d/state/feat-e.meta" "window=fm:mx-feat-e" "worktree=$d/wt" "kind=delivery"
  MX_FAKE_AXI_STATUS="$(run_failed mx/feat-e)"
  local out; out=$(run_actor_state "$d" feat-e)
...
# routine case once more than one actor validates the same underlying repo
...
# the repo-wide answer was not this actor's own branch.
test_cross_branch_attribution_via_runs_list() {
  reset_fakes
  local d short; d=$(new_case crossbranch)
  make_repo_on_branch "$d/wt" mx/feat-f
  short=$(git -C "$d/wt" rev-parse --short=7 HEAD)
  make_fakebin "$d" >/dev/null
  mx_write_meta "$d/state/feat-f.meta" "window=fm:mx-feat-f" "worktree=$d/wt" "kind=delivery"
  # The repo-wide active/most-recent run belongs to a different actor's branch.
  MX_FAKE_AXI_STATUS="$(run_running mx/other-actors)"
  # Real `no-mistakes runs` shape: plain text, newest-first, no run id, no
  # quoting - "<status> <branch> <short-sha> <date> [<pr-url>]".
  MX_FAKE_RUNS_LIST="$(cat <<EOF
  running    mx/other-actors aaaaaaa  2026-07-02 22:10
  running    mx/feat-f ${short}  2026-07-02 22:05
EOF
)"
  local out; out=$(run_actor_state "$d" feat-f)
...
test_cross_branch_attribution_picks_most_recent_row() {
  ...
  make_repo_on_branch "$d/wt" mx/feat-fq
  short=$(git -C "$d/wt" rev-parse --short=7 HEAD)
  make_fakebin "$d" >/dev/null
  mx_write_meta "$d/state/feat-fq.meta" "window=fm:mx-feat-fq" "worktree=$d/wt" "kind=delivery"
  MX_FAKE_AXI_STATUS="$(run_running mx/other-actors)"
  MX_FAKE_RUNS_LIST="$(cat <<EOF
  running    mx/other-actors aaaaaaa  2026-07-02 22:10
  running    mx/feat-fq ${short}  2026-07-02 21:50
  completed  mx/feat-fq bbbbbbb  2026-07-02 20:00  https://github.com/o/r/pull/1
EOF
)"
  local out; out=$(run_actor_state "$d" feat-fq)
...
test_coarse_run_does_not_probe_other_branch_ci_log_for_ready_status() {
  ...
  make_repo_on_branch "$d/wt" mx/feat-coarseready
  short=$(git -C "$d/wt" rev-parse --short=7 HEAD)
  make_fakebin "$d" >/dev/null
  mx_write_meta "$d/state/feat-coarseready.meta" "window=fm:mx-feat-coarseready" "worktree=$d/wt" "kind=delivery"
  printf 'done: PR https://github.com/o/r/pull/4 checks green\n' > "$d/state/feat-coarseready.status"
  MX_FAKE_AXI_STATUS="$(run_ci_monitoring mx/other-actors)"
  MX_FAKE_RUNS_LIST="$(cat <<EOF
  running    mx/other-actors aaaaaaa  2026-07-02 22:10
  running    mx/feat-coarseready ${short}  2026-07-02 22:05
EOF
)"
  MX_FAKE_CI_LOGS="CI checks running, waiting for results..."
  local out; out=$(run_actor_state "$d" feat-coarseready)
...
test_other_branch_run_ignored() {
  ...
  make_repo_on_branch "$d/wt" mx/feat-g
  make_fakebin "$d" >/dev/null
  mx_write_meta "$d/state/feat-g.meta" "window=fm:mx-feat-g" "worktree=$d/wt" "kind=delivery"
  printf 'done: implemented, ready to validate\n' > "$d/state/feat-g.status"
  MX_FAKE_AXI_STATUS="$(run_running mx/some-other)"
  MX_FAKE_RUNS_LIST="$(cat <<'EOF'
  running    mx/some-other aaaaaaa  2026-07-02 22:10
EOF
)"
  MX_FAKE_BUSY=0
  local out; out=$(run_actor_state "$d" feat-g)
...
# (f) no run for this actors + a busy pane -> working via pane
test_no_run_busy_pane() {
  reset_fakes
  local d; d=$(new_case busy)
  make_repo_on_branch "$d/wt" mx/feat-h
  make_fakebin "$d" >/dev/null
  mx_write_meta "$d/state/feat-h.meta" "window=fm:mx-feat-h" "worktree=$d/wt" "kind=delivery"
  # No matching run anywhere.
  MX_FAKE_AXI_STATUS=""
  MX_FAKE_RUNS_LIST=""
  MX_FAKE_BUSY=1
  local out; out=$(run_actor_state "$d" feat-h)
...
test_no_run_herdr_unknown_uses_backend_capture() {
  ...
  make_repo_on_branch "$d/wt" mx/feat-herdr
  make_fakebin "$d" >/dev/null
  mx_write_meta "$d/state/feat-herdr.meta" "window=default:w1:p2" "worktree=$d/wt" "kind=delivery" "backend=herdr"
  MX_FAKE_AXI_STATUS=""
  MX_FAKE_RUNS_LIST=""
  MX_FAKE_TMUX_MISSING=1
  MX_FAKE_HERDR_BUSY=1
  MX_FAKE_HERDR_AGENT_STATUS=""
  local out; out=$(run_actor_state "$d" feat-herdr)
...
# not "this actor's tool call is still in progress". an actor blocked on its own
...
# regression pins: actor_pane_is_busy previously returned "not busy" on a bare
...
test_no_run_herdr_idle_agent_status_corroborated_by_busy_pane() {
  ...
  make_repo_on_branch "$d/wt" mx/feat-herdr-idle
  make_fakebin "$d" >/dev/null
  mx_write_meta "$d/state/feat-herdr-idle.meta" "window=default:w1:p3" "worktree=$d/wt" "kind=delivery" "backend=herdr"
  ...
  MX_FAKE_RUNS_LIST=""
  MX_FAKE_TMUX_MISSING=1
  MX_FAKE_HERDR_AGENT_STATUS=idle
  MX_FAKE_HERDR_BUSY=1
  local out; out=$(run_actor_state "$d" feat-herdr-idle)
...
test_no_run_herdr_idle_agent_status_and_idle_pane_stays_idle() {
  ...
  make_repo_on_branch "$d/wt" mx/feat-herdr-stopped
  make_fakebin "$d" >/dev/null
  mx_write_meta "$d/state/feat-herdr-stopped.meta" "window=default:w1:p4" "worktree=$d/wt" "kind=delivery" "backend=herdr"
  ...
  MX_FAKE_HERDR_AGENT_STATUS=idle
  MX_FAKE_HERDR_BUSY=0
  local out; out=$(run_actor_state "$d" feat-herdr-stopped)
...
test_no_run_idle_pane_uses_log() {
  ...
  make_repo_on_branch "$d/wt" mx/feat-i
  make_fakebin "$d" >/dev/null
  mx_write_meta "$d/state/feat-i.meta" "window=fm:mx-feat-i" "worktree=$d/wt" "kind=delivery"
  printf 'needs-decision: which database?\n' > "$d/state/feat-i.status"
  MX_FAKE_AXI_STATUS=""
  MX_FAKE_BUSY=0
  local out; out=$(run_actor_state "$d" feat-i)
...
test_no_run_idle_pane_uses_keyed_log() {
  ...
  make_repo_on_branch "$d/wt" mx/feat-keyed
  make_fakebin "$d" >/dev/null
  mx_write_meta "$d/state/feat-keyed.meta" "window=fm:mx-feat-keyed" "worktree=$d/wt" "kind=delivery"
  printf 'needs-decision [key=q1]: which database?\n' > "$d/state/feat-keyed.status"
  MX_FAKE_AXI_STATUS=""
  MX_FAKE_BUSY=0
  local out; out=$(run_actor_state "$d" feat-keyed)
...
# coordinator reading the actor sees a distinct pause (and its reason) rather than a
# wedge-suspect idle. This is the reader half the watcher/daemon build on.
test_no_run_idle_pane_paused() {
  reset_fakes
  local d; d=$(new_case paused)
  make_repo_on_branch "$d/wt" mx/feat-pause
  make_fakebin "$d" >/dev/null
  mx_write_meta "$d/state/feat-pause.meta" "window=fm:mx-feat-pause" "worktree=$d/wt" "kind=delivery"
  printf 'paused: holding for the upstream tool release\n' > "$d/state/feat-pause.status"
  MX_FAKE_AXI_STATUS=""
  MX_FAKE_BUSY=0
  local out; out=$(run_actor_state "$d" feat-pause)
...
test_no_run_idle_pane_custom_paused_verb() {
  ...
  make_repo_on_branch "$d/wt" mx/feat-custom-pause
  make_fakebin "$d" >/dev/null
  mx_write_meta "$d/state/feat-custom-pause.meta" "window=fm:mx-feat-custom-pause" "worktree=$d/wt" "kind=delivery"
  printf 'awaiting: vendor maintenance window\n' > "$d/state/feat-custom-pause.status"
  MX_FAKE_AXI_STATUS=""
  MX_FAKE_BUSY=0
  local out; out=$(MX_CLASSIFY_PAUSED_VERB=awaiting run_actor_state "$d" feat-custom-pause)
  ...
  out=$(MX_CLASSIFY_PAUSED_VERB=awaiting run_actor_state "$d" feat-custom-pause)
...
# detail: a healthy idle daemon that just closed a keyed decision falls through
# to the idle default (unknown/none), not `unknown` with the resolution note as its
# `doing`. Regression for the catchup render bug where such a daemon showed
# state=unknown with resolution prose. The one-owner keyed fold in mx-classify-lib.sh
# is untouched; this only stops the deriver from reading a non-state event as state.
test_no_run_idle_daemon_resolved_event_not_state() {
  reset_fakes
  local d; d=$(new_case resolved-idle)
  mkdir -p "$d/wt"
  make_fakebin "$d" >/dev/null
  mx_write_meta "$d/state/mate.meta" "window=fm:mx-mate" "worktree=$d/wt" "kind=daemon" "home=$d/wt"
  printf 'needs-decision [key=race]: pick subscribe order\n' > "$d/state/mate.status"
  printf 'resolved [key=race]: went with subscribe-before-write\n' >> "$d/state/mate.status"
  MX_FAKE_AXI_STATUS=""
  MX_FAKE_BUSY=0
  local out; out=$(run_actor_state "$d" mate)
  assert_contains "$out" "state: unknown" "resolved-then-idle daemon is not a spurious run-state"
...
test_dead_window_ignores_stale_status_log() {
  ...
  make_repo_on_branch "$d/wt" mx/feat-dead
  make_fakebin "$d" >/dev/null
  mx_write_meta "$d/state/feat-dead.meta" "window=fm:mx-feat-dead" "worktree=$d/wt" "kind=delivery"
  printf 'done: old completion event\n' > "$d/state/feat-dead.status"
  MX_FAKE_AXI_STATUS=""
  MX_FAKE_RUNS_LIST=""
  MX_FAKE_TMUX_MISSING=1
  local out; out=$(run_actor_state "$d" feat-dead)
...
# run-step, not the shell. The common case is a finished actor whose agent has
...
test_dead_window_still_reports_terminal_run_step() {
  ...
  make_repo_on_branch "$d/wt" mx/feat-dead-done
  make_fakebin "$d" >/dev/null
  mx_write_meta "$d/state/feat-dead-done.meta" "window=fm:mx-feat-dead-done" "worktree=$d/wt" "kind=delivery"
  printf 'done: PR https://github.com/o/r/pull/3 checks green\n' > "$d/state/feat-dead-done.status"
  MX_FAKE_AXI_STATUS="$(run_passed mx/feat-dead-done)"
  MX_FAKE_TMUX_MISSING=1   # the actor's window has closed
  local out; out=$(run_actor_state "$d" feat-dead-done)
...
test_dead_window_still_reports_active_run_step() {
  ...
  make_repo_on_branch "$d/wt" mx/feat-dead-act
  make_fakebin "$d" >/dev/null
  mx_write_meta "$d/state/feat-dead-act.meta" "window=fm:mx-feat-dead-act" "worktree=$d/wt" "kind=delivery"
  MX_FAKE_AXI_STATUS="$(run_running mx/feat-dead-act)"
  MX_FAKE_TMUX_MISSING=1
  local out; out=$(run_actor_state "$d" feat-dead-act)
...
test_no_timeout_uses_perl_bound() {
  ...
  make_repo_on_branch "$d/wt" mx/feat-timeout
  make_fakebin "$d" >/dev/null
  calls_file="$d/no-mistakes.calls"
  : > "$calls_file"
  cat > "$d/fakebin/no-mistakes" <<'SH'
#!/usr/bin/env bash
printf '%s\n' "$*" >> "${MX_FAKE_NM_CALLS:-/dev/null}"
while :; do :; done
SH
  chmod +x "$d/fakebin/no-mistakes"
  toolbin=$(make_no_timeout_toolbin "$d")
  mx_write_meta "$d/state/feat-timeout.meta" "window=fm:mx-feat-timeout" "worktree=$d/wt" "kind=delivery"
  MX_FAKE_BUSY=1
  start=$SECONDS
  out=$(MX_FAKE_NM_CALLS="$calls_file" PATH="$d/fakebin:$toolbin" MX_STATE_OVERRIDE="$d/state" MX_ACTOR_STATE_NM_TIMEOUT=1 "$ACTOR_STATE" feat-timeout)
...
test_scout_skips_run_lookup() {
  ...
  make_repo_on_branch "$d/wt" mx/scout-j
  make_fakebin "$d" >/dev/null
  mx_write_meta "$d/state/scout-j.meta" "window=fm:mx-scout-j" "worktree=$d/wt" "kind=scout"
  # Even if a run existed on this branch, a scout must not read it.
  MX_FAKE_AXI_STATUS="$(run_running mx/scout-j)"
  MX_FAKE_BUSY=1
  local out; out=$(run_actor_state "$d" scout-j)
...
test_torn_down_worktree() {
  ...
  make_fakebin "$d" >/dev/null
  mx_write_meta "$d/state/gone-k.meta" "window=fm:mx-gone-k" "worktree=$d/no-such-worktree" "kind=delivery"
  local out rc
  out=$(run_actor_state "$d" gone-k); rc=$?
...
test_missing_meta() {
  ...
  make_fakebin "$d" >/dev/null
  local out rc
  out=$(run_actor_state "$d" ghost-z); rc=$?
...
# (k) actor_is_provably_working end-to-end over the REAL mx-actor-state.sh (not a
# canned fake verdict, unlike tests/mx-watch-triage.test.sh's classifier
# coverage). This is the direct regression pair for the 2026-07-02 herdr
# incident: a validating actors whose bare `axi status` answer belongs to
# another branch must still be absorbed by the watcher via the runs-list
# fallback (working), while an actor with genuinely no run anywhere and an idle
...
test_provably_working_via_runs_list_fallback() {
  ...
  make_repo_on_branch "$d/wt" mx/feat-provable
  short=$(git -C "$d/wt" rev-parse --short=7 HEAD)
  make_fakebin "$d" >/dev/null
  mx_write_meta "$d/state/feat-provable.meta" "window=fm:mx-feat-provable" "worktree=$d/wt" "kind=delivery"
  MX_FAKE_AXI_STATUS="$(run_running mx/other-actors)"
  MX_FAKE_RUNS_LIST="$(cat <<EOF
  running    mx/other-actors aaaaaaa  2026-07-02 22:10
  running    mx/feat-provable ${short}  2026-07-02 22:05
EOF
)"
  PATH="$d/fakebin:$PATH" MX_STATE_OVERRIDE="$d/state" actor_is_provably_working feat-provable \
    || fail "cross-branch attribution via the runs list was not treated as provably working"
  pass "actor_is_provably_working absorbs a validating actors found only via the runs-list fallback"
}

test_not_provably_working_when_stopped() {
  reset_fakes
  local d; d=$(new_case provably-working-stopped)
  make_repo_on_branch "$d/wt" mx/feat-stopped
  make_fakebin "$d" >/dev/null
  mx_write_meta "$d/state/feat-stopped.meta" "window=fm:mx-feat-stopped" "worktree=$d/wt" "kind=delivery"
...
EOF
  ...
  MX_FAKE_BUSY=0
  PATH="$d/fakebin:$PATH" MX_STATE_OVERRIDE="$d/state" actor_is_provably_working feat-stopped \
    && fail "a stopped actor with no run anywhere and an idle pane was treated as provably working"
  pass "actor_is_provably_working still surfaces a genuinely stopped actor (safety property preserved)"
}
...
test_historical_same_branch_rewritten_head_not_current() {
  ...
  make_repo_on_branch "$d/wt" mx/todo-flag
  ...
  git -C "$d/wt" branch -q -M mx/todo-flag
  new_head=$(git -C "$d/wt" rev-parse HEAD)
  [ "$old_head" != "$new_head" ] || fail "rewrite did not produce a new head"
  make_fakebin "$d" >/dev/null
  mx_write_meta "$d/state/wishlist.meta" "window=fm:mx-wishlist" "worktree=$d/wt" "kind=delivery"
  ...
  MX_FAKE_RUN_HEAD="$old_head"
  MX_FAKE_AXI_STATUS="$(run_parked mx/todo-flag)"
  MX_FAKE_BUSY=0
  out=$(run_actor_state "$d" wishlist)
...
test_active_run_descendant_fix_head_remains_current() {
  ...
  make_repo_on_branch "$d/wt" mx/feat-pipeline
  ...
  make_fakebin "$d" >/dev/null
  mx_write_meta "$d/state/pipe.meta" "window=fm:mx-pipe" "worktree=$d/wt" "kind=delivery"
  MX_FAKE_RUN_HEAD="$fix_head"
  MX_FAKE_AXI_STATUS="$(run_fixing mx/feat-pipeline)"
  out=$(run_actor_state "$d" pipe)
...
test_local_advanced_past_run_head_invalidates() {
  ...
  make_repo_on_branch "$d/wt" mx/feat-adv
  run_head=$(git -C "$d/wt" rev-parse HEAD)
  git -C "$d/wt" commit -q --allow-empty -m 'local stage-2 work after prior run'
  make_fakebin "$d" >/dev/null
  mx_write_meta "$d/state/adv.meta" "window=fm:mx-adv" "worktree=$d/wt" "kind=delivery"
  printf 'working: stage 2 implementation in progress\n' > "$d/state/adv.status"
  MX_FAKE_RUN_HEAD="$run_head"
  MX_FAKE_AXI_STATUS="$(run_parked mx/feat-adv)"
  MX_FAKE_BUSY=0
  out=$(run_actor_state "$d" adv)
...
test_missing_run_head_falls_back_to_current_state() {
  ...
  make_repo_on_branch "$d/wt" mx/feat-no-head
  make_fakebin "$d" >/dev/null
  mx_write_meta "$d/state/no-head.meta" "window=fm:mx-no-head" "worktree=$d/wt" "kind=delivery"
  printf 'working: current stage still in progress\n' > "$d/state/no-head.status"
  MX_FAKE_AXI_STATUS=$(run_parked mx/feat-no-head | grep -v '^  head:')
  MX_FAKE_RUNS_LIST=""
  MX_FAKE_BUSY=0
  out=$(run_actor_state "$d" no-head)
...
echo "all mx-actor-state tests passed"
