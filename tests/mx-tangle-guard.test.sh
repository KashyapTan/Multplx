...
#            mx-spawn refuses to launch unless the resolved worktree is isolated.
#   GUARD 2 (detection)  - mx-guard and mx-bootstrap alarm when the primary is on
#            a feature branch, and stay silent on the default branch or detached.
# These cases pin: the shared lib's branch classification, the mx-guard banner,
# the mx-bootstrap problem line, the brief assertion ordering, and the mx-spawn
...
. "$ROOT/bin/mx-tangle-lib.sh"

TMP_ROOT=$(mx_test_tmproot mx-tangle-guard)
...
# mx_primary_tangle_branch is the whole scoping decision: a NAMED non-default
...
test_lib_classification() {
  ...
  while IFS='|' read -r label state branch expect; do
    ...
    out=$(mx_primary_tangle_branch "$repo" || true)
    [ "$out" = "$expect" ] || fail "$label: expected tangle='$expect', got '$out'"
  done <<'ROWS'
on the default branch is healthy|default||
on a feature branch is the tangle|feature|mx/readme-restructure-d3|mx/readme-restructure-d3
detached HEAD on default is healthy (worktrees, daemon homes)|detached||
ROWS
  # A non-git directory is not a tangle and must not error.
  out=$(mx_primary_tangle_branch "$TMP_ROOT" || true)
  [ -z "$out" ] || fail "non-git dir wrongly reported a tangle: '$out'"
  pass "mx_primary_tangle_branch: feature branch alarms; default/detached/non-git stay silent"
}

# --- GUARD 2a: mx-guard banner ----------------------------------------------

run_guard() {
  # Scope the guard to a temp repo as the primary checkout; state lives under it.
  MX_ROOT_OVERRIDE="$1" MX_HOME="$1" "$ROOT/bin/mx-guard.sh" 2>&1
}
...
test_guard_banner() {
  ...
  pass "mx-guard: bordered tangle banner fires only for a feature branch and suppresses repair commands in read-only mode"
}

# --- GUARD 2b: mx-bootstrap problem line ------------------------------------

run_bootstrap() {
  # No projects/ under the home keeps system sync inert; grep isolates the line.
  MX_ROOT_OVERRIDE="$1" MX_HOME="$1" "$ROOT/bin/mx-bootstrap.sh" 2>/dev/null
}
...
test_bootstrap_line() {
  ...
  git -C "$repo" checkout -q -B mx/tangle-bb2
  out=$(run_bootstrap "$repo" | grep '^TANGLE:' || true)
  assert_contains "$out" "mx/tangle-bb2" "bootstrap did not report the tangled branch"
  assert_contains "$out" "checkout main" "bootstrap TANGLE line lacked the restore remediation"
  out=$(MX_ROOT_OVERRIDE="$repo" MX_HOME="$repo" MX_BOOTSTRAP_DETECT_ONLY=1 "$ROOT/bin/mx-bootstrap.sh" 2>/dev/null | grep '^TANGLE:' || true)
  assert_contains "$out" "mx/tangle-bb2" "detect-only bootstrap did not report the tangled branch"
  ...
  pass "mx-bootstrap: TANGLE problem line fires only for a feature branch and suppresses repair commands in detect-only mode"
}
...
test_brief_assertion_precedes_branch() {
  ...
  pass "mx-brief: delivery brief asserts worktree isolation before the branch step"
}
...
run_spawn() {
  ...
  MX_ROOT_OVERRIDE='' MX_HOME="$home" \
    ...
    PATH="$fakebin:$PATH" \
    "$ROOT/bin/mx-spawn.sh" "$id" "$proj" codex 2>&1
}
...
# The prevention guard also depends on mx-spawn building robust tmux commands
...
test_spawn_tmux_window_construction() {
  ...
  assert_grep "new-window -dP -F #{window_id} -t broker: -n mx-rec-win-gg7" "$rec" \
    "new-window must append at the session (trailing colon) and capture the window id"
  assert_no_grep "new-window -dP -F #{window_id} -t broker -n" "$rec" \
