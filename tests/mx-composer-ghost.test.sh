...
# the one shared ANSI-aware owner (mx_composer_strip_ghost, bin/mx-composer-lib.sh,
# reached here through the mx_tmux_strip_ghost thin adapter):
...
# These tests pin:
#   1. mx_tmux_strip_ghost drops dim/faint AND dark-truecolor runs, keeping
#      normal-intensity, brightly-coloured text.
#   2. mx_pane_input_pending reads a ghost-only composer (either style) as NOT
#      pending, while still treating real (normal/bright) text as pending.
#   3. The human/LLM-facing capture path (mx-peek.sh) stays PLAIN - no escape codes
#      ever reach broker's context.
set -u

# shellcheck source=tests/lib.sh
. "$(dirname "${BASH_SOURCE[0]}")/lib.sh"

LIB="$ROOT/bin/mx-tmux-lib.sh"
PEEK="$ROOT/bin/mx-peek.sh"

# shellcheck source=/dev/null
. "$LIB"

TMP_ROOT=$(mx_test_tmproot mx-ghost-tests)
...
test_strip_ghost_drops_dim_keeps_normal() {
  ...
  pass "mx_tmux_strip_ghost drops dim/faint runs, keeps normal and bold text"
}
...
test_strip_ghost_handles_combined_and_boundary_codes() {
  ...
  pass "mx_tmux_strip_ghost handles combined SGR, ESC[22m, and reset-then-dim"
}
...
test_strip_ghost_keeps_colored_text_with_2_payloads() {
  ...
  pass "mx_tmux_strip_ghost keeps bright colored text with 2 payloads"
}
...
test_strip_ghost_drops_dark_truecolor_ghost() {
  ...
  pass "mx_tmux_strip_ghost drops a dark/muted truecolor foreground placeholder"
}

# --- mx_pane_input_pending: dim ghost is not pending ------------------------
...
test_dim_ghost_only_composer_is_not_pending() {
  ...
  if PATH="$fb:$PATH" MX_FAKE_STYLED="$capture" MX_FAKE_CY=0 \
     mx_pane_input_pending "fakepane"; then
    fail "dim ghost-only composer falsely read as pending"
  fi
  pass "mx_pane_input_pending: a dim ghost-only composer is NOT pending"
}
...
test_dim_ghost_inside_bordered_composer_is_not_pending() {
  ...
  pass "mx_pane_input_pending: dim ghost inside a bordered composer is NOT pending"
}
...
test_normal_text_still_pending() {
  ...
  PATH="$fb:$PATH" MX_FAKE_STYLED="$capture" MX_FAKE_CY=0 \
    mx_pane_input_pending "fakepane" \
    || fail "real typed text was not detected as pending"
  pass "mx_pane_input_pending: normal-intensity typed text is still pending"
}
...
test_colored_text_with_2_payload_still_pending() {
  ...
  pass "mx_pane_input_pending: bright colored text with 2 payloads is still pending"
}
...
test_dark_truecolor_ghost_only_composer_is_not_pending() {
  ...
  pass "mx_pane_input_pending: a dark truecolor ghost-only composer placeholder is NOT pending"
}
...
test_dark_truecolor_bare_shell_prompt_is_unknown() {
  ...
  for prompt in '$' 'user@host $'; do
    ...
    out=$(PATH="$fb:$PATH" MX_FAKE_STYLED="$capture" MX_FAKE_CY=0 \
      mx_tmux_composer_state "fakepane")
    [ "$out" = unknown ] \
      || fail "dark truecolor bare shell prompt '$prompt' must read unknown, got '$out'"
  done
  pass "mx_tmux_composer_state: dark truecolor shell prompts read unknown"
}
...
test_real_text_with_trailing_ghost_is_pending() {
  ...
  pass "mx_pane_input_pending: real text plus a trailing ghost run is still pending"
}

# --- mx-peek.sh stays escape-free (LLM-facing path) -------------------------
...
test_peek_output_is_escape_free() {
  ...
  # surface NONE of these escape codes into broker's context.
  printf 'normal output line\n\xe2\x9d\xaf \033[2mpredicted next prompt\033[0m\n' > "$capture"
  # Empty MX_HOME so mx-guard.sh finds no in-flight task and stays silent.
  home="$dir/home"; mkdir -p "$home/state"
  # Pass an explicit session:window so resolution needs no metadata.
  out=$(PATH="$fb:$PATH" MX_HOME="$home" MX_FAKE_STYLED="$capture" \
        "$PEEK" "sess:win" 2>/dev/null)
  case "$out" in
    *"$ESC"*) fail "mx-peek surfaced ANSI escape codes into LLM-facing output" ;;
  esac
  # And it should still carry the real content.
  case "$out" in
    *"predicted next prompt"*) : ;;
    *) fail "mx-peek dropped pane content (expected the ghost text body as plain text)" ;;
  esac
  pass "mx-peek output is escape-free (no raw -e bytes reach broker context)"
}
