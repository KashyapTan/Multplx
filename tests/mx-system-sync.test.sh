#!/usr/bin/env bash
# Behavior tests for mx-system-sync.sh drift handling.
#
# mx-system-sync fast-forwards a clone that is cleanly on its default branch. This
...
# must be unchanged, and bootstrap must relay the new outcomes as SYSTEM_SYNC lines.
#
# It also pins the orphaned .git/packed-refs.lock recovery in the fetch step
# (fetch_with_packed_refs_lock_guard, backed by bin/mx-lock-lib.sh's shared
...
TMP_ROOT=$(mx_test_tmproot mx-system-sync-tests)
HOME_N=0

# --- fixtures ---------------------------------------------------------------

# new_home: fresh isolated MX_HOME with an empty projects/ dir. Each test gets its
# own so the whole-system form never sees another test's clones.
new_home() {
  HOME_N=$((HOME_N + 1))
...
# run_sync <home> [args...]: run system-sync against an isolated home, stdout only.
run_sync() {
  local home=$1
  shift
  MX_HOME="$home" MX_ROOT_OVERRIDE="$ROOT" "$ROOT/bin/mx-system-sync.sh" "$@" 2>/dev/null
}
...
# lsof shims mirror tests/mx-teardown.test.sh: no-holder (provably free), a live
...
# lsof shim: a holder ONLY for $SYSTEM_TEST_LIVE_DIR (a live `git -C <clone>` keeping
...
[ -n "${SYSTEM_TEST_LIVE_DIR:-}" ] && [ "$target" = "$SYSTEM_TEST_LIVE_DIR" ] && exit 0
exit 1
SH
  chmod +x "$1/lsof"
}
...
# run_sync_guarded <home> <fakebin> <outfile> <errfile> [args...]: run system-sync
# with the fakebin on PATH and stdout/stderr captured separately. Per-test knobs
# (MX_SYSTEM_SYNC_PACKED_REFS_LOCK_*, GIT_FETCH_COUNTER) are read from the caller's
...
run_sync_guarded() {
  ...
  MX_HOME="$home" MX_ROOT_OVERRIDE="$ROOT" \
    "$ROOT/bin/mx-system-sync.sh" "$@" >"$outf" 2>"$errf"
}
...
test_whole_system_form() {
  local home behind current out
  home=$(new_home)
  behind=$(build_pair "$home" system-behind)
  advance_origin "$home" system-behind C1
  current=$(build_pair "$home" system-current)

  # Whole-system form: no project-dir argument.
  out=$(run_sync "$home")

  assert_contains "$out" "system-behind: synced" "whole-system form syncs a behind clone"
  assert_contains "$out" "system-current: already current" "whole-system form reports a current clone"
  : "$behind $current"
  pass "whole-system form processes every clone under projects/"
}
...
test_bootstrap_relays_recovered_and_stuck() {
  ...
  # Full bootstrap: no state/ dir -> daemon sync no-ops.
  # We only assert the system-sync relay lines; other detect lines are irrelevant.
  out=$(MX_HOME="$home" MX_ROOT_OVERRIDE="$ROOT" "$ROOT/bin/mx-bootstrap.sh" 2>/dev/null)

  assert_contains "$out" "SYSTEM_SYNC: stuck-clone: STUCK:" "bootstrap relays the STUCK outcome"
  assert_contains "$out" "SYSTEM_SYNC: rec-clone: recovered:" "bootstrap relays the recovered outcome"
  pass "bootstrap relays recovered: and STUCK: system-sync outcomes"
}
...
test_orphaned_stale_packed_refs_lock_recovers() {
  ...
  set +e
  MX_SYSTEM_SYNC_PACKED_REFS_LOCK_RETRIES=2 \
  MX_SYSTEM_SYNC_PACKED_REFS_LOCK_RETRY_WAIT_SECS=0 \
  MX_SYSTEM_SYNC_PACKED_REFS_LOCK_AGE_SECS=0 \
...
test_live_packed_refs_lock_is_never_removed() {
  ...
  assert_contains "$(cat "$out")" "locklive: skipped: fetch failed" "live lock: system-sync did not skip"
...
test_transient_packed_refs_lock_self_clears() {
  ...
  MX_SYSTEM_SYNC_PACKED_REFS_LOCK_RETRIES=3 \
  MX_SYSTEM_SYNC_PACKED_REFS_LOCK_RETRY_WAIT_SECS=0 \
...
test_non_signature_fetch_failure_is_not_retried() {
  ...
  assert_contains "$(cat "$out")" "locknonsig: skipped: fetch failed" "non-signature: system-sync did not report the fetch failure"
