#!/usr/bin/env bash
# Focused safety tests for bin/mx-herdr-session-cleanup.sh.
...
TMP_ROOT=$(mx_test_tmproot mx-herdr-session-cleanup)
MX_TEST_CLEANUP_DIRS+=("$TMP_ROOT")
trap mx_test_cleanup EXIT

export MX_HOME="$TMP_ROOT/home"
export MX_STATE_OVERRIDE="$MX_HOME/state"
export MX_CONFIG_OVERRIDE="$MX_HOME/config"
mkdir -p "$MX_STATE_OVERRIDE" "$MX_CONFIG_OVERRIDE"
touch "$MX_CONFIG_OVERRIDE/herdr-presentation-spaces"
printf '%s\n' herdr > "$MX_CONFIG_OVERRIDE/backend"
...
[ "$(mx_herdr_cleanup_process_argv0 "$LINUX_PROCESS_INFO")" = /bin/sh ] \
  || fail "Linux Herdr process argv array was not accepted"
if mx_herdr_cleanup_process_argv0 \
...
mx_lock_release() { rm -rf -- "$1"; }
mx_herdr_cleanup_process_is_idle_shell() { [ ! -e "$FIXTURE_DIR/process-unsafe" ]; }
...
fixture_workspaces() {
  ...
  printf '[{"workspace_id":"w1","label":"broker","focused":%s,"active_tab_id":"w1:t1","tab_count":1,"pane_count":1},' \
...
fixture_tabs() {
  ...
  while [ "$i" -le "$count" ]; do
    [ "$i" -eq 1 ] || printf ','
    printf '{"tab_id":"%s:t%s","workspace_id":"%s","focused":false,"label":"mx-task"}' "$WS" "$i" "$WS"
    i=$((i + 1))
  done
  printf ']'
}
...
mx_backend_herdr_cli() {
  ...
  if [ -e "$FIXTURE_DIR/closed" ]; then
    case "$first $second" in
      ...
      "workspace list") printf '%s\n' '{"result":{"workspaces":[{"workspace_id":"w1","label":"broker","focused":true,"active_tab_id":"w1:t1","tab_count":1,"pane_count":1}]}}'; return 0 ;;
    esac
  fi
  if [ -e "$FIXTURE_DIR/race" ] && [ -e "$FIXTURE_DIR/snapshotted" ] && [ "$first $second" = "workspace list" ]; then
    printf '%s\n' '{"result":{"workspaces":[{"workspace_id":"w1","label":"broker","focused":true,"active_tab_id":"w1:t1","tab_count":1,"pane_count":1},{"workspace_id":"w2","label":"renamed","focused":false,"active_tab_id":"w2:t1","tab_count":1,"pane_count":1}]}}'
    return 0
  fi
  title=$(cat "$FIXTURE_DIR/title")
  tabs=$(cat "$FIXTURE_DIR/tabs")
...
write_v1() { # <id> [token]
  ...
  } > "$MX_STATE_OVERRIDE/$id.herdr-presentation"
}
...
reset_fixture() {
  rm -rf "$FIXTURE_DIR" "$TMP_ROOT"/*.lock "${MX_STATE_OVERRIDE:?}/"*
  mkdir -p "$FIXTURE_DIR"
  : > "$LOCK_LOG"; : > "$CLOSE_LOG"
...
assert_preserved() { # <case>
  ...
  [ -f "$MX_STATE_OVERRIDE/$ID.herdr-presentation" ] && had_journal=1
  mx_herdr_session_cleanup >/dev/null 2>&1
  if [ "$had_journal" -eq 1 ]; then
    [ -f "$MX_STATE_OVERRIDE/$ID.herdr-presentation" ] || fail "$name retired the journal"
  fi
...
reset_fixture
mx_herdr_session_cleanup >/dev/null 2>&1
[ ! -e "$MX_STATE_OVERRIDE/$ID.herdr-presentation" ] || fail "positive cleanup kept the journal"
[ "$(wc -l < "$CLOSE_LOG" | tr -d ' ')" = 1 ] || fail "positive cleanup did not close exactly once"
[ "$(sed -n '1p' "$LOCK_LOG")" = "$MX_STATE_OVERRIDE/.spawn-$ID.lock" ] || fail "task lock was not acquired first"
...
reset_fixture; printf 'version=1\ntask_id=%s\nprojection_id=short\n' "$ID" > "$MX_STATE_OVERRIDE/$ID.herdr-presentation"; assert_preserved "malformed journal"
...
reset_fixture; rm -f "$MX_STATE_OVERRIDE/$ID.herdr-presentation"; assert_preserved "zero journal match"
reset_fixture; write_v1 mx-task; assert_preserved "multiple journal matches"
reset_fixture; rm -f "$MX_STATE_OVERRIDE/$ID.herdr-presentation"; write_cross_home_v2; assert_preserved "cross-home journal"
reset_fixture; write_v2 "$MX_HOME" w9 "$TAB" "$PANE"; assert_preserved "v2 workspace binding mismatch"
reset_fixture; write_v2 "$MX_HOME" "$WS" w9:t1 "$PANE"; assert_preserved "v2 tab binding mismatch"
reset_fixture; write_v2 "$MX_HOME" "$WS" "$TAB" w9:p1; assert_preserved "v2 pane binding mismatch"
reset_fixture; write_v2 "$MX_HOME" "$WS" "$TAB" "$PANE"
mx_herdr_session_cleanup >/dev/null 2>&1
[ ! -e "$MX_STATE_OVERRIDE/$ID.herdr-presentation" ] || fail "matching v2 cleanup kept the journal"
...
reset_fixture; : > "$MX_STATE_OVERRIDE/$ID.meta"; assert_preserved "current task metadata"
...
cat > "$INTEGRATION_ROOT/bin/mx-herdr-session-cleanup.sh" <<'SH'
#!/usr/bin/env bash
printf '%s\n' "${MX_HOME:?}" >> "${MX_HERDR_CLEANUP_TRACE:?}"
SH
chmod +x "$INTEGRATION_ROOT/bin/mx-herdr-session-cleanup.sh"
printf '%s\n' manual > "$INTEGRATION_ROOT/home/config/backlog-backend"
MX_HOME="$INTEGRATION_ROOT/home" MX_HERDR_CLEANUP_TRACE="$TRACE" MX_BOOTSTRAP_DETECT_ONLY=1 \
  "$INTEGRATION_ROOT/bin/mx-bootstrap.sh" >/dev/null 2>&1
[ ! -e "$TRACE" ] || fail "detect-only bootstrap ran stale projection cleanup"
MX_HOME="$INTEGRATION_ROOT/home" MX_HERDR_CLEANUP_TRACE="$TRACE" \
  "$INTEGRATION_ROOT/bin/mx-bootstrap.sh" >/dev/null 2>&1
...
cat > "$INTEGRATION_ROOT/bin/mx-lock.sh" <<'SH'
#!/usr/bin/env bash
printf '%s\n' 'lock acquired'
SH
chmod +x "$INTEGRATION_ROOT/bin/mx-lock.sh"
MX_HOME="$INTEGRATION_ROOT/home" MX_ROOT_OVERRIDE="$INTEGRATION_ROOT" \
  MX_HERDR_CLEANUP_TRACE="$TRACE" \
  "$INTEGRATION_ROOT/bin/mx-session-start.sh" >/dev/null 2>&1 \
...
cat > "$INTEGRATION_ROOT/bin/mx-lock.sh" <<'SH'
#!/usr/bin/env bash
printf '%s\n' 'error: another live broker session holds the lock' >&2
exit 1
SH
chmod +x "$INTEGRATION_ROOT/bin/mx-lock.sh"
MX_HOME="$INTEGRATION_ROOT/home" MX_ROOT_OVERRIDE="$INTEGRATION_ROOT" \
...
printf 'all mx-herdr-session-cleanup tests passed\n'
