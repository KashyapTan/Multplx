#!/usr/bin/env bash
# tests/mx-watch-triage.test.sh - the always-on wake triage built into
# bin/mx-watch.sh and the shared classifier (bin/mx-classify-lib.sh). The watcher
# now absorbs the benign majority of wakes in bash and exits ONLY on an actionable
# wake, so broker's LLM re-arms once per actionable event instead of once per
# wake. These tests cover the classifier predicates as pure functions, then drive
# a real mx-watch.sh subprocess to assert the behavioral contract:
# provably-working no-verb wakes absorbed (no exit, no queue entry, suppressor
# advanced, beacon fresh), stopped-actor no-verb wakes surfaced (queue + exit),
...
#
# Daemon-side classification/injection lives in mx-daemon.test.sh; watcher/lock
# liveness in mx-watcher-lock.test.sh; the durable-queue safety matrix in
...
DRAIN="$ROOT/bin/mx-wake-drain.sh"

TMP_ROOT=$(mx_test_tmproot mx-watch-triage-tests)

# Common watcher knobs: tight poll/grace, no check or heartbeat cadence unless a
# test overrides them, so a test only exercises the path it targets. MX_ACTOR_STATE_BIN
# points at the case's hermetic fake mx-actor-state.sh (installed by make_case) so the
# absorb-only-when-provably-working triage reads a canned verdict; a test fixes that
# verdict via MX_FAKE_ACTOR_STATE in its environment before calling watch_bg.
watch_bg() {  # <state> <fakebin> <out> [extra env assignments...]
  local state=$1 fakebin=$2 out=$3
  shift 3
  PATH="$fakebin:$PATH" MX_STATE_OVERRIDE="$state" MX_ACTOR_STATE_BIN="$fakebin/mx-actor-state.sh" \
    MX_POLL=1 MX_SIGNAL_GRACE=1 MX_CHECK_INTERVAL=999999 MX_HEARTBEAT=999999 "$@" "$WATCH" > "$out" &
}
...
# --- pure classifier predicates (mx-classify-lib.sh) ------------------------
...
test_signal_reason_is_actionable_classifier() {
  ...
  signal_reason_is_actionable "$state/b.status" || fail "maintainer-relevant signal classified benign"
  : > "$state/c.turn-ended"
  signal_reason_is_actionable "$state/c.turn-ended" && fail "a bare turn-ended marker classified actionable"
  # Coalesced batch: one benign + one maintainer-relevant -> actionable.
  signal_reason_is_actionable "$state/a.status" "$state/b.status" || fail "coalesced benign+actionable not actionable"
  pass "signal_reason_is_actionable: benign absorbed, maintainer verbs and coalesced batches surfaced"
}

test_stale_is_terminal_classifier() {
  local dir state
  dir=$(make_case classify-stale); state="$dir/state"
  printf 'done: ready in branch mx/x\n' > "$state/term.status"
  stale_is_terminal "sess:mx-term" "$state" || fail "terminal stale status not classified terminal"
  mx_write_meta "$state/herdr-term.meta" "window=default:w1:p2" "backend=herdr"
  ...
  stale_is_terminal "sess:mx-nonterm" "$state" && fail "non-terminal stale classified terminal"
  stale_is_terminal "sess:mx-missing" "$state" && fail "stale with no status classified terminal"
  pass "stale_is_terminal: terminal status surfaces, non-terminal and no-status are benign"
}

test_scan_maintainer_relevant_statuses_classifier() {
  local dir state out
  dir=$(make_case classify-scan); state="$dir/state"
  printf 'working: a\n' > "$state/one.status"
  printf 'blocked: no perms\n' > "$state/two.status"
  printf 'done: PR https://x/y/pull/1\n' > "$state/three.status"
  out=$(scan_maintainer_relevant_statuses "$state")
  ...
  pass "scan_maintainer_relevant_statuses lists only maintainer-relevant statuses"
}
...
test_classifier_primitives() {
  ...
  status_is_maintainer_relevant "done: b" || fail "done: not recognized as maintainer-relevant"
  status_is_maintainer_relevant "needs-decision [key=q1]: b" || fail "keyed needs-decision not recognized as maintainer-relevant"
  status_is_maintainer_relevant "working: b" && fail "working: wrongly recognized as maintainer-relevant"
  # Incident regression: free-text "merged" inside a nonterminal working: line must
  # not become maintainer-relevant (AFK false-terminal path).
  status_is_maintainer_relevant \
    ...
    && fail "working: ... merged #N wrongly recognized as maintainer-relevant"
  status_is_maintainer_relevant "working: rebased onto predecessor #76" \
    && fail "working: predecessor prose wrongly recognized as maintainer-relevant"
  status_is_maintainer_relevant "working: PR ready checks green merged ready in branch" \
    && fail "working: free-text tokens wrongly recognized as maintainer-relevant"
  status_is_maintainer_relevant "done: PR https://x/pull/76 checks green" \
    || fail "genuine done: checks green not maintainer-relevant"
  ...
  status_is_maintainer_relevant "merged" || fail "legacy bare merged free-text not maintainer-relevant"
  status_is_maintainer_relevant "PR ready https://x/pull/2" \
    || fail "legacy bare PR ready free-text not maintainer-relevant"
  [ "$(window_to_task "sess:mx-fix-login-k3")" = "fix-login-k3" ] || fail "window_to_task did not strip session+mx- prefix"
  ...
  MX_MAINTAINER_RE='custom-verb:' status_is_maintainer_relevant "custom-verb: x" || fail "MX_MAINTAINER_RE override not honored"
  MX_MAINTAINER_RE='custom-verb:' status_is_maintainer_relevant "done: x" && fail "MX_MAINTAINER_RE override did not replace the default verb set"
  MX_MAINTAINER_RE='merged|custom-verb:' status_is_maintainer_relevant "working: rebased onto merged #76" \
    && fail "MX_MAINTAINER_RE override bypassed working: suppression"
  MX_MAINTAINER_RE='checks green|custom-verb:' status_is_maintainer_relevant "paused: checks green pending approval" \
    && fail "MX_MAINTAINER_RE override bypassed paused: suppression"
  MX_MAINTAINER_RE='custom-verb:' status_is_maintainer_relevant "custom-verb: x" \
...
EOF
  ...
  pass "classifier primitives: keyed decisions and activity phases, maintainer relevance, window-to-task, and overrides"
}

# actor_is_provably_working: the absorb-only-when-provably-working predicate. It is
# benign (absorb) ONLY when mx-actor-state.sh reports the actors as working from an
...
# an unknown/torn-down actors, or an empty id - is NOT provable, so it surfaces. The
# fake mx-actor-state.sh (MX_ACTOR_STATE_BIN) returns a canned verdict per case.
test_actor_is_provably_working_classifier() {
  local dir fakebin
  ...
  export MX_ACTOR_STATE_BIN="$fakebin/mx-actor-state.sh"
  ...
  ! actor_is_provably_working a || fail "stale status-log working: treated as provably working"
  MX_FAKE_ACTOR_STATE='state: done · source: run-step · checks green'
  ! actor_is_provably_working a || fail "finished run treated as provably working"
  MX_FAKE_ACTOR_STATE='state: parked · source: run-step · parked at review'
  ! actor_is_provably_working a || fail "parked run treated as provably working"
  MX_FAKE_ACTOR_STATE='state: failed · source: run-step · run failed'
  ! actor_is_provably_working a || fail "failed run treated as provably working"
  MX_FAKE_ACTOR_STATE='state: unknown · source: none · worktree gone'
  ! actor_is_provably_working a || fail "unknown actors treated as provably working"
...
test_status_is_paused_classifier() {
  ...
  # A pause is deliberately NOT maintainer-relevant: it is a stop-nagging signal, not
  # work to keep surfacing.
  status_is_maintainer_relevant 'paused: holding for the upstream release' && fail "paused is maintainer-relevant (should not be)"
  status_is_paused_or_maintainer_held 'paused: holding for the upstream release' \
    || fail "declared pause not recognized by the bounded-idle classifier"
  status_is_paused_or_maintainer_held 'maintainer-held [key=route]: tracked by task-decision-route' \
    || fail "maintainer-held transfer not recognized by the bounded-idle classifier"
  status_is_paused_or_maintainer_held 'resolved [key=route]: maintainer answered' \
    && fail "resolved decision remained classed as maintainer-held"
  pass "status_is_paused: only the leading paused verb matches, and paused is not maintainer-relevant"
}

# actor_absorb_class: the single mx-actor-state.sh read that returns BOTH absorb
# reasons - working (active run/busy pane), paused (declared external wait), or none
# (surface it) - so the watcher's stale path gets both for one bounded call.
# actor_is_paused delegates to it exactly as actor_is_provably_working does.
test_actor_absorb_class_classifier() {
  local dir fakebin
  dir=$(make_case absorb-class); fakebin="$dir/fakebin"
  export MX_ACTOR_STATE_BIN="$fakebin/mx-actor-state.sh"
  export MX_FAKE_ACTOR_STATE
  MX_FAKE_ACTOR_STATE='state: working · source: run-step · validating (running)'
  [ "$(actor_absorb_class a)" = working ] || fail "active run-step not classed working"
  MX_FAKE_ACTOR_STATE='state: working · source: pane · harness busy'
  [ "$(actor_absorb_class a)" = working ] || fail "busy pane not classed working"
  MX_FAKE_ACTOR_STATE='state: paused · source: status-log · awaiting upstream'
  [ "$(actor_absorb_class a)" = paused ] || fail "declared pause not classed paused"
  actor_is_paused a || fail "actor_is_paused did not recognize a paused verdict"
  ! actor_is_provably_working a || fail "a paused actors was treated as provably working"
  MX_FAKE_ACTOR_STATE='state: working · source: status-log · working: compiling'
  [ "$(actor_absorb_class a)" = none ] || fail "stale working: status-log classed absorbable"
  MX_FAKE_ACTOR_STATE='state: unknown · source: none · worktree gone'
  [ "$(actor_absorb_class a)" = none ] || fail "unknown actors classed absorbable"
  ! actor_is_paused a || fail "unknown actors classed paused"
  [ "$(actor_absorb_class "")" = none ] || fail "empty id not classed none"
  unset MX_FAKE_ACTOR_STATE
  pass "actor_absorb_class: working/paused/none from one read; actor_is_paused and actor_is_provably_working agree"
}

# signal_actor_provably_working: a no-verb "signal:" wake is benign ONLY when EVERY
# task it references is provably working; if any actors has stopped, or no task can be
# resolved, it surfaces. Files map to ids by stripping .status / .turn-ended.
test_signal_actor_provably_working_classifier() {
  local dir fakebin state
  dir=$(make_case signal-provably-working); fakebin="$dir/fakebin"; state="$dir/state"
  export MX_ACTOR_STATE_BIN="$fakebin/mx-actor-state.sh"
  export MX_FAKE_ACTOR_STATE_a='state: working · source: run-step · running'
  export MX_FAKE_ACTOR_STATE_b='state: done · source: run-step · run passed'
  signal_actor_provably_working "$state/a.status" "$state/a.turn-ended" \
    || fail "a single provably-working actors (status+turn-end) was not benign"
  ! signal_actor_provably_working "$state/a.status" "$state/b.turn-ended" \
    || fail "a coalesced batch including a stopped actor was treated as benign"
  ! signal_actor_provably_working "$state/b.turn-ended" \
    || fail "a stopped actor's bare turn-end was treated as benign"
  ! signal_actor_provably_working "$state/a.meta" \
    || fail "a non-signal file resolved to a benign verdict"
  ! signal_actor_provably_working \
    || fail "an empty signal file list was treated as benign"
  unset MX_FAKE_ACTOR_STATE_a MX_FAKE_ACTOR_STATE_b
  pass "signal_actor_provably_working: benign only when every referenced actors is provably working"
}

# --- benign wakes are absorbed ONLY when the actor is provably working ---------
...
test_provably_working_signal_absorbed() {
  ...
  # The actor's pipeline is in an actively-running step: positive evidence it is
  ...
  if ! wait_live "$pid" 30; then
    reap "$pid"; fail "watcher exited for a working: signal whose actor is provably working (should absorb): $(cat "$out")"
  fi
...
test_turn_ended_provably_working_absorbed() {
  ...
  export MX_FAKE_ACTOR_STATE='state: working · source: pane · harness busy'
  watch_bg "$state" "$fakebin" "$out"
  pid=$!
  if ! wait_live "$pid" 30; then
    reap "$pid"; fail "watcher exited for a turn-end whose actor is provably working (should absorb): $(cat "$out")"
  fi
  ...
  reap "$pid"
  pass "a bare turn-end whose actor is provably working (busy pane) is absorbed"
}

# --- a no-verb signal whose actor is NOT provably working SURFACES -------------
# This is the swallowed-finish fix: an actor that finished (or stopped and waits)
# reports its final turn-end with no maintainer-relevant status and no running
...
test_turn_ended_not_working_surfaced() {
  ...
  : > "$state/task.turn-ended"
  # No running pipeline, no busy pane: the actor has stopped (e.g. it finished via
  ...
  owned_wake "$state" "$DRAIN" > "$drain_out" 2>/dev/null || fail "drain after the surfaced turn-end failed"
  grep "$(printf '\tsignal\t')" "$drain_out" | grep -F "$state/task.turn-ended" >/dev/null || fail "surfaced turn-end was not queued"
  pass "a bare turn-end whose actor is not provably working is surfaced (the swallowed-finish fix)"
}
...
test_working_note_not_working_surfaced() {
  ...
  status_file="$state/task.status"
  printf 'working: compiling step 2\n' > "$status_file"
  # A non-deep-review actors (no run) whose pane went idle: mx-actor-state falls back
  ...
  owned_wake "$state" "$DRAIN" > "$drain_out" 2>/dev/null || fail "drain after the surfaced working: note failed"
  ...
  pass "a no-verb working: note whose actor is idle with no running pipeline is surfaced"
}
...
test_actionable_signal_surfaced() {
  ...
  owned_wake "$state" "$DRAIN" > "$drain_out" 2>/dev/null || fail "drain after the actionable signal failed"
  ...
  pass "maintainer-relevant signal is surfaced (queue + exit) and marked surfaced"
}
...
test_terminal_stale_surfaced() {
  ...
  owned_wake "$state" "$DRAIN" > "$drain_out" 2>/dev/null || fail "drain after the terminal stale failed"
...
# Regression for the 2026-07 herdr false-surface incidents: an actor's own status
# log gets no new entry once broker hands it to a deep-review validation
...
# immediately surfacing an actor that is actively validating. actor_is_provably_working
# must get a chance to override a maintainer-relevant-but-stale status line, exactly
...
test_stale_terminal_status_overridden_by_active_run() {
  ...
  out="$dir/watch.out"; drain_out="$dir/drain.out"; capture_file="$dir/pane.txt"
  window="test:mx-validating"
  printf 'deep-review local gate: validating...' > "$capture_file"
  printf 'window=%s\nkind=delivery\n' "$window" > "$state/validating.meta"
  # The actors reported done BEFORE broker triggered deep-review validation;
  ...
  key=$(printf '%s' "$window" | tr ':/.' '___')
  pane_hash=$(hash_text "deep-review local gate: validating...")
  printf '%s' "$pane_hash" > "$state/.hash-$key"
  printf '1\n' > "$state/.count-$key"
  export MX_FAKE_ACTOR_STATE='state: working · source: run-step · validating (running)'

  # Phase A: a high escalation threshold means the first sighting is absorbed,
  # not surfaced, despite the maintainer-relevant "done:" status-log line.
  PATH="$fakebin:$PATH" MX_FAKE_TMUX_WINDOW="$window" MX_FAKE_TMUX_CAPTURE="$capture_file" \
...
# --- non-terminal stale, actors provably working: absorbed, then wedge-escalated ---
# A provably-working actors (an actively-running pipeline) legitimately sits on a
...
test_nonterminal_stale_provably_working_absorbed_then_escalated() {
  ...
  printf '%s' "$pane_hash" > "$state/.hash-$key"
  printf '1\n' > "$state/.count-$key"
  # The actor's pipeline is actively running: a static pane is normal (waiting on CI).
  export MX_FAKE_ACTOR_STATE='state: working · source: run-step · ci running'
  ...
  # (The subsequent-sight timer path does not re-read the actor state.)
  echo $(( $(date +%s) - 500 )) > "$state/.stale-since-$key"
  : > "$out"
  PATH="$fakebin:$PATH" MX_FAKE_TMUX_WINDOW="$window" MX_FAKE_TMUX_CAPTURE="$capture_file" \
    MX_STATE_OVERRIDE="$state" MX_ACTOR_STATE_BIN="$fakebin/mx-actor-state.sh" MX_STALE_ESCALATE_SECS=240 MX_POLL=1 MX_SIGNAL_GRACE=1 \
  ...
  owned_wake "$state" "$DRAIN" > "$drain_out" 2>/dev/null || fail "drain after the wedge escalation failed"
...
# --- non-terminal stale, actors NOT provably working: surfaced immediately ------
# The key requirement: an actor with no running pipeline that has gone quiet (and is
...
# non-deep-review actors, or any actors with no running pipeline) are never left hanging.

test_nonterminal_stale_not_working_surfaced() {
  local dir state fakebin out drain_out capture_file window key pane_hash sig pid
  dir=$(make_case nonterminal-stale-stopped); state="$dir/state"; fakebin="$dir/fakebin"
  out="$dir/watch.out"; drain_out="$dir/drain.out"; capture_file="$dir/pane.txt"
  window="test:mx-stopped"
  printf 'idle prompt, finished' > "$capture_file"
  printf 'window=%s\nkind=delivery\n' "$window" > "$state/stopped.meta"
  # Non-terminal status (the actors never wrote a maintainer-relevant verb), .seen-*
  ...
  owned_wake "$state" "$DRAIN" > "$drain_out" 2>/dev/null || fail "drain after the immediate stale failed"
  grep "$(printf '\tstale\t')" "$drain_out" | grep -F "$window" >/dev/null || fail "immediate stale wake was not queued"
  pass "a not-provably-working non-terminal stale is surfaced immediately (never left to wait out the timer)"
}

# --- non-terminal stale, actors DECLARED a pause: absorbed, re-surfaced on a long
#     cadence, never wedge-escalated ------------------------------------------
# The live 2026-07-09/10 case: an actor intentionally held awaiting an upstream tool
# release (paused: ...) whose idle pane tripped repeated possible-wedge escalations
# all day. With the paused verb, its stale is absorbed like a working actors but never
...
test_nonterminal_stale_paused_absorbed_then_resurfaced() {
  ...
  # A DECLARED pause (not maintainer-relevant), .seen-* primed so the signal scan does
  ...
  printf '%s' "$pane_hash" > "$state/.hash-$key"
  printf '1\n' > "$state/.count-$key"
  # actor_absorb_class reads the declared pause from mx-actor-state.sh.
  export MX_FAKE_ACTOR_STATE='state: paused · source: status-log · holding for the upstream tool release'

  # Phase A: a fresh pause (status file just written) under a high re-surface
  # threshold is absorbed - no wake, no wedge timer.
  PATH="$fakebin:$PATH" MX_FAKE_TMUX_WINDOW="$window" MX_FAKE_TMUX_CAPTURE="$capture_file" \
    MX_FAKE_TMUX_CURRENT_COMMAND=zsh \
  ...
  owned_wake "$state" "$DRAIN" > "$drain_out" 2>/dev/null || fail "drain after the paused re-surface failed"
  grep "$(printf '\tstale\t')" "$drain_out" | grep -F "$window" >/dev/null || fail "paused re-surface was not queued"
  pass "a declared pause is absorbed on first sight, then re-surfaced as a recheck past the threshold, never wedge-escalated"
}

# A maintainer-held actors can leave a stable backend endpoint after its agent exits.
# mx-actor-state then authoritatively reports stopped rather than paused, but the
# confirmed-dead agent plus the declared wait or maintainer-held transfer must retain
...
test_exited_declared_pause_is_bounded_but_live_gate_surfaces() {
  ...
  printf 'window=%s\nkind=delivery\nharness=claude\nbackend=tmux\n' "$window" > "$state/held.meta"
  printf 'paused: held per maintainer while an external decision is pending\n' > "$statusf"
  back=$(( $(date +%s) - 500 ))
  if [ "$(uname)" = Darwin ]; then touch -mt "$(date -r "$back" '+%Y%m%d%H%M.%S')" "$statusf"
  else touch -m -d "@$back" "$statusf"; fi
  sig=$(seen_sig "$statusf"); printf '%s' "$sig" > "$state/.seen-held_status"
  ...
  while [ "$round" -le 6 ]; do
    PATH="$fakebin:$PATH" MX_FAKE_TMUX_WINDOW="$window" MX_FAKE_TMUX_CAPTURE="$capture_file" \
      MX_FAKE_TMUX_CURRENT_COMMAND=zsh MX_FAKE_ACTOR_STATE='state: stopped · source: pane · bare shell' \
      MX_STATE_OVERRIDE="$state" MX_ACTOR_STATE_BIN="$fakebin/mx-actor-state.sh" MX_PAUSE_RESURFACE_SECS=240 MX_POLL=1 MX_SIGNAL_GRACE=1 \
      MX_CHECK_INTERVAL=999999 MX_HEARTBEAT=999999 "$WATCH" >> "$out" &
    pid=$!
  ...
  [ "$bare" -eq 0 ] || fail "dead-agent declared pause surfaced as $bare bare stopped-actor wakes"
  grep -F "awaiting external" "$state/.wake-queue" >/dev/null \
    || fail "dead-agent declared pause did not use the bounded paused recheck"

  dir=$(make_case exited-maintainer-held); state="$dir/state"; fakebin="$dir/fakebin"
  out="$dir/watch.out"; capture_file="$dir/pane.txt"; statusf="$state/held.status"
  window="test:mx-held"
  printf 'idle bare shell after maintainer-held transfer\n' > "$capture_file"
  printf 'window=%s\nkind=delivery\nharness=claude\nbackend=tmux\n' "$window" > "$state/held.meta"
  printf 'maintainer-held [key=route]: tracked by held-decision-route\n' > "$statusf"
  back=$(( $(date +%s) - 500 ))
  if [ "$(uname)" = Darwin ]; then touch -mt "$(date -r "$back" '+%Y%m%d%H%M.%S')" "$statusf"
  else touch -m -d "@$back" "$statusf"; fi
  sig=$(seen_sig "$statusf"); printf '%s' "$sig" > "$state/.seen-held_status"
  key=$(printf '%s' "$window" | tr ':/.' '___')
  pane_hash=$(hash_text "idle bare shell after maintainer-held transfer")
  printf '%s' "$pane_hash" > "$state/.hash-$key"
  printf '1\n' > "$state/.count-$key"
  PATH="$fakebin:$PATH" MX_FAKE_TMUX_WINDOW="$window" MX_FAKE_TMUX_CAPTURE="$capture_file" \
    MX_FAKE_TMUX_CURRENT_COMMAND=zsh MX_FAKE_ACTOR_STATE='state: stopped · source: pane · bare shell' \
  ...
  PATH="$fakebin:$PATH" MX_FAKE_TMUX_WINDOW="$window" MX_FAKE_TMUX_CAPTURE="$capture_file" \
    MX_FAKE_TMUX_CURRENT_COMMAND=claude MX_FAKE_ACTOR_STATE='state: paused · source: status-log · waiting at an active external-decision gate' \
    MX_STATE_OVERRIDE="$state" MX_ACTOR_STATE_BIN="$fakebin/mx-actor-state.sh" MX_PAUSE_RESURFACE_SECS=999 MX_POLL=1 MX_SIGNAL_GRACE=1 \
    MX_CHECK_INTERVAL=999999 MX_HEARTBEAT=999999 "$WATCH" >> "$out" &
  pid=$!
  ...
  PATH="$fakebin:$PATH" MX_FAKE_TMUX_WINDOW="$window" MX_FAKE_TMUX_CAPTURE="$capture_file" \
    MX_FAKE_TMUX_CURRENT_COMMAND=claude MX_FAKE_ACTOR_STATE='state: paused · source: status-log · waiting at an active external-decision gate' \
    MX_STATE_OVERRIDE="$state" MX_ACTOR_STATE_BIN="$fakebin/mx-actor-state.sh" MX_STALE_ESCALATE_SECS=240 MX_PAUSE_RESURFACE_SECS=999 MX_POLL=1 MX_SIGNAL_GRACE=1 \
    MX_CHECK_INTERVAL=999999 MX_HEARTBEAT=999999 "$WATCH" >> "$out" &
  pid=$!
  if ! wait_live "$pid" 30; then
    reap "$pid"
  ...
  pass "exited declared-pause and maintainer-held panes use bounded pause cadence while a live decision gate still surfaces once"
}

test_daemon_paused_resurfaces_in_normal_mode() {
  local dir state fakebin out capture_file statusf window key pane_hash sig pid back
  dir=$(make_case daemon-paused-resurface); state="$dir/state"; fakebin="$dir/fakebin"
  out="$dir/watch.out"; capture_file="$dir/pane.txt"; statusf="$state/daemon-held.status"
  window="test:mx-daemon-held"
  printf 'idle awaiting external\n' > "$capture_file"
  printf 'window=%s\nkind=daemon\n' "$window" > "$state/daemon-held.meta"
  printf 'paused: awaiting the upstream release\n' > "$statusf"
  back=$(( $(date +%s) - 500 ))
  if [ "$(uname)" = Darwin ]; then touch -mt "$(date -r "$back" '+%Y%m%d%H%M.%S')" "$statusf"
  else touch -m -d "@$back" "$statusf"; fi
  sig=$(seen_sig "$statusf"); printf '%s' "$sig" > "$state/.seen-daemon-held_status"
  key=$(printf '%s' "$window" | tr '.:/' '___')
  pane_hash=$(hash_text "idle awaiting external")
  printf '%s' "$pane_hash" > "$state/.hash-$key"
  printf '1\n' > "$state/.count-$key"
  export MX_FAKE_ACTOR_STATE='state: paused · source: status-log · awaiting the upstream release'
  PATH="$fakebin:$PATH" MX_FAKE_TMUX_WINDOW="$window" MX_FAKE_TMUX_CAPTURE="$capture_file" \
    MX_STATE_OVERRIDE="$state" MX_ACTOR_STATE_BIN="$fakebin/mx-actor-state.sh" MX_PAUSE_RESURFACE_SECS=240 MX_POLL=1 MX_SIGNAL_GRACE=1 \
    MX_CHECK_INTERVAL=999999 MX_HEARTBEAT=999999 "$WATCH" > "$out" &
  pid=$!
  ...
  unset MX_FAKE_ACTOR_STATE
  pass "a declared paused daemon re-surfaces on the bounded normal-mode cadence"
}

test_daemon_nonpaused_stale_remains_suppressed() {
  local dir state fakebin out capture_file statusf window key pane_hash sig pid
  dir=$(make_case daemon-stale-suppressed); state="$dir/state"; fakebin="$dir/fakebin"
  out="$dir/watch.out"; capture_file="$dir/pane.txt"; statusf="$state/daemon-working.status"
  window="test:mx-daemon-working"
  printf 'idle while the parent supervises\n' > "$capture_file"
  printf 'window=%s\nkind=daemon\n' "$window" > "$state/daemon-working.meta"
  printf 'working: the parent supervises this daemon\n' > "$statusf"
  sig=$(seen_sig "$statusf"); printf '%s' "$sig" > "$state/.seen-daemon-working_status"
  key=$(printf '%s' "$window" | tr '.:/' '___')
  ...
  pid=$!
  if ! wait_live "$pid" 30; then
    reap "$pid"; fail "watcher surfaced an ordinary daemon stale pane: $(cat "$out")"
  fi
  [ ! -s "$out" ] || { reap "$pid"; fail "ordinary daemon stale pane printed a wake reason: $(cat "$out")"; }
  reap "$pid"
  pass "a non-paused daemon retains normal stale suppression"
}

test_daemon_unpause_clears_pause_tracking() {
  local dir state fakebin out statusf window key pid
  dir=$(make_case daemon-unpause-clears); state="$dir/state"; fakebin="$dir/fakebin"
  out="$dir/watch.out"; statusf="$state/daemon-resumed.status"; window="test:mx-daemon-resumed"
  printf 'window=%s\nkind=daemon\n' "$window" > "$state/daemon-resumed.meta"
  printf 'working: upstream landed\n' > "$statusf"
  printf '%s' "$(seen_sig "$statusf")" > "$state/.seen-daemon-resumed_status"
  key=${window//:/_}
  key=${key//\//_}
  key=${key//./_}
  : > "$state/.paused-$key"
  ...
  watch_bg "$state" "$fakebin" "$out"
  pid=$!
  wait_live "$pid" 20 || fail "watcher exited while reconciling a resumed daemon: $(cat "$out")"
  [ ! -e "$state/.paused-$key" ] || { reap "$pid"; fail "resumed daemon retained the pause marker"; }
  [ ! -e "$state/.stale-$key" ] || { reap "$pid"; fail "resumed daemon retained stale tracking"; }
  [ ! -e "$state/.wedge-escalations-$key" ] || { reap "$pid"; fail "resumed daemon retained wedge tracking"; }
  reap "$pid"
  pass "a resumed daemon clears pause and stale tracking before stale exemption"
}
...
test_nonterminal_stale_pause_transitions_reclassify_unchanged_hash() {
  ...
  out="$dir/watch.out"; capture_file="$dir/pane.txt"; window="test:mx-transition"
  printf 'idle awaiting external\n' > "$capture_file"
  ...
  reap "$pid"
  unset MX_FAKE_ACTOR_STATE
  pass "unchanged stale hashes reclassify when an actor enters or leaves pause"
}

test_nonterminal_paused_rechecks_authoritative_state() {
  local dir state fakebin out capture_file window key pane_hash sig pid
  dir=$(make_case nonterminal-paused-recheck); state="$dir/state"; fakebin="$dir/fakebin"
  out="$dir/watch.out"; capture_file="$dir/pane.txt"; window="test:mx-pause-recheck"
  printf 'idle awaiting external\n' > "$capture_file"
...
test_paused_authoritative_working_preserves_wedge_timer() {
  ...
  out="$dir/watch.out"; capture_file="$dir/pane.txt"; window="test:mx-paused-working"
  printf 'idle awaiting external\n' > "$capture_file"
...
# MX_WEDGE_DEMAND_INSPECT_COUNT consecutive escalations on the SAME pane, the
...
test_wedge_escalation_marks_demand_deep_inspection_after_threshold() {
  ...
  while [ "$n" -le 3 ]; do
    ...
    # path does not re-read the actor state).
    echo $(( $(date +%s) - 500 )) > "$state/.stale-since-$key"
    : > "$out"
    PATH="$fakebin:$PATH" MX_FAKE_TMUX_WINDOW="$window" MX_FAKE_TMUX_CAPTURE="$capture_file" \
      MX_STATE_OVERRIDE="$state" MX_ACTOR_STATE_BIN="$fakebin/mx-actor-state.sh" MX_STALE_ESCALATE_SECS=240 MX_POLL=1 MX_SIGNAL_GRACE=1 \
...
test_wedge_escalation_resets_when_pane_becomes_active() {
  ...
  # The pane content changes (the actor is active again): the hash no longer
  # matches, so the watcher resets escalation bookkeeping instead of escalating.
  printf 'new output, actors active again' > "$capture_file"
  PATH="$fakebin:$PATH" MX_FAKE_TMUX_WINDOW="$window" MX_FAKE_TMUX_CAPTURE="$capture_file" \
    MX_STATE_OVERRIDE="$state" MX_ACTOR_STATE_BIN="$fakebin/mx-actor-state.sh" MX_STALE_ESCALATE_SECS=240 MX_POLL=1 MX_SIGNAL_GRACE=1 \
    MX_CHECK_INTERVAL=999999 MX_HEARTBEAT=999999 "$WATCH" > "$out" &
  pid=$!
...
test_nonterminal_stale_repairs_missing_or_corrupt_timer() {
  ...
  PATH="$fakebin:$PATH" MX_FAKE_TMUX_WINDOW="$window" MX_FAKE_TMUX_CAPTURE="$capture_file" \
    MX_STATE_OVERRIDE="$state" MX_STALE_ESCALATE_SECS=999 MX_POLL=1 MX_SIGNAL_GRACE=1 \
    MX_CHECK_INTERVAL=999999 MX_HEARTBEAT=999999 "$WATCH" > "$out" &
  pid=$!
...
SH
  ...
  export MX_FAKE_ACTOR_STATE='state: working · source: run-step · validating (running)'
  PATH="$fakebin:$PATH" MX_STATE_OVERRIDE="$state" MX_ACTOR_STATE_BIN="$fakebin/mx-actor-state.sh" MX_POLL=1 MX_SIGNAL_GRACE=1 \
    MX_CHECK_INTERVAL=999999 MX_HEARTBEAT=999999 MX_WATCH_TRIAGE_LOG_MAX_BYTES=1 "$WATCH" > "$out" &
  pid=$!
  if ! wait_live "$pid" 30; then
...
test_heartbeat_no_change_absorbed() {
  ...
  # A truly quiet system (no windows, no statuses) with a fast heartbeat cadence.
  ...
  reap "$pid"
  pass "a heartbeat with no maintainer-relevant change is absorbed and backs off the cadence"
}
...
test_heartbeat_backstop_surfaces_unsurfaced_status() {
  ...
  out="$dir/watch.out"; drain_out="$dir/drain.out"
  # A maintainer-relevant status whose .seen-* signature ALREADY matches (so the
  ...
  # system-scan backstop must catch it and wake broker.
  ...
  owned_wake "$state" "$DRAIN" > "$drain_out" 2>/dev/null || fail "drain after the backstop heartbeat failed"
  grep "$(printf '\theartbeat\t')" "$drain_out" >/dev/null || fail "backstop heartbeat was not queued"
  pass "heartbeat backstop fail-safe surfaces a maintainer-relevant status the per-wake path missed"
}
...
test_beacon_stays_fresh_while_absorbing() {
  ...
  reap "$pid"
  pass "the liveness beacon stays fresh while the watcher absorbs benign wakes (mx-guard never false-alarms)"
}
...
test_afk_present_reverts_watcher_to_one_shot() {
  ...
  owned_wake "$state" "$DRAIN" > "$drain_out" 2>/dev/null || fail "drain after the afk-mode signal failed"
...
test_afk_paused_changed_pane_hands_off_plain_stale() {
  ...
  PATH="$fakebin:$PATH" MX_FAKE_TMUX_WINDOW="$window" MX_FAKE_TMUX_CAPTURE="$capture_file" \
    MX_FAKE_ACTOR_STATE='state: paused · source: status-log · awaiting the upstream tool release' \
  ...
  owned_wake "$state" "$DRAIN" > "$drain_out" 2>/dev/null || fail "drain after AFK paused stale failed"
