...
# herdr_safe_stop_and_delete on a private mx-lab-* session, never a bare/ambient
# `herdr server stop`. Every lifecycle op goes through bin/mx-herdr-lab.sh, which
# refuses the default session and verifies the system-state tripwire.
set -u
...
SESSION="mx-lab-eventwait-smoke-$$"
...
mx_herdr_lab_prepare "$SESSION" || fail "could not prepare the isolated Herdr lab session"
...
if ! mx_backend_herdr_events_capable "$SESSION"; then
...
IDS=$(mx_backend_herdr_create_task "$CONTAINER" "mx-evwait1" /tmp "$SEEDED_TAB_ID") || fail "create_task failed"
...
# scratch broker state so window_to_task and the wake queue resolve
SCRATCH=$(mktemp -d "${TMPDIR:-/tmp}/mx-evwait.XXXXXX")
...
mx_herdr_lab_cli "$SESSION" pane report-agent "$PANE_ID" --source mx-evwait-test --agent claude --state idle >/dev/null 2>&1 \
...
( mx_backend_herdr_wait_transition "$SESSION" 8 "$STATE" "$TARGET" > "$OUT"; echo $? > "$RCF" ) &
WPID=$!
sleep 0.5   # let it connect, subscribe, and reconcile the idle baseline

START=$(python3 -c 'import time; print(time.time())')
mx_herdr_lab_cli "$SESSION" pane report-agent "$PANE_ID" --source mx-evwait-test --agent claude --state blocked >/dev/null 2>&1 \
...
REC_PANE=$(mx_transition_pane_id "$REC")
REC_TO=$(mx_transition_to_status "$REC")
...
# shellcheck source=/dev/null
. "$ROOT/bin/mx-push-transition-lib.sh"
wake() { return 0; }
