#!/usr/bin/env bash
# tests/mx-daemon-lifecycle-e2e.test.sh - the happy-path daemon operator
...
TMP_ROOT=$(mx_test_tmproot mx-daemon-lifecycle)
...
setup_world() {
  ...
  mx_git_add_origin "$HOME_DIR/projects/alpha" "$TMP_ROOT/remotes/alpha.git"
  mx_git_add_origin "$HOME_DIR/projects/beta" "$TMP_ROOT/remotes/beta.git"
  mx_git_add_origin "$HOME_DIR/projects/gamma" "$TMP_ROOT/remotes/gamma.git"
...
EOF
  ...
  MX_DAEMON_SCOPE='customer onboarding from brief' \
    scaffold_daemon_charter "$HOME_DIR" design 'customer onboarding charter' alpha beta gamma \
    || fail "filled daemon charter scaffold failed"
}

phase_seed() {
  local out
  out=$(PATH="$FAKEBIN:$PATH" MX_HOME="$HOME_DIR" \
    "$ROOT/bin/mx-home-seed.sh" design "$SUB" alpha beta gamma) \
    || fail "seed failed"
  ...
  [ "$(MX_HOME="$SUB" "$ROOT/bin/mx-project-mode.sh" beta)" = "direct-PR off" ] \
...
phase_spawn() {
  : > "$LOG"
  PATH="$FAKEBIN:$PATH" MX_HOME="$HOME_DIR" MX_CONFIG_OVERRIDE="$HOME_DIR/parent-config" \
    MX_FAKE_TMUX_LOG="$LOG" MX_FAKE_TMUX_CAPTURE="$PANE" \
    "$ROOT/bin/mx-spawn.sh" design "$SUB" codex --daemon >/dev/null \
    || fail "daemon spawn failed"
...
EOF
  local out before
  out=$(MX_HOME="$HOME_DIR" "$ROOT/bin/mx-backlog-handoff.sh" design feat-x feat-y) \
  ...
  MX_HOME="$HOME_DIR" "$ROOT/bin/mx-backlog-handoff.sh" design feat-x feat-y >/dev/null 2>&1 \
...
phase_recovery() {
  ...
  PATH="$FAKEBIN:$PATH" MX_HOME="$HOME_DIR" MX_FAKE_TMUX_LOG="$LOG" MX_FAKE_TMUX_CAPTURE="$PANE" \
    "$ROOT/bin/mx-spawn.sh" design "echo relaunch" --daemon >/dev/null 2>&1 \
...
phase_teardown() {
  local teardown_out
  : > "$LOG"
  teardown_out=$(PATH="$FAKEBIN:$PATH" MX_HOME="$HOME_DIR" MX_FAKE_TMUX_LOG="$LOG" MX_FAKE_TMUX_CAPTURE="$PANE" \
    "$ROOT/bin/mx-teardown.sh" design 2>&1) \
