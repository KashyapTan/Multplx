...
TMP_ROOT=$(mx_test_tmproot mx-calm-pi-extension)
EXT="$ROOT/.pi/extensions/mx-calm.ts"
ASSISTANT_LAYOUT="$ROOT/.pi/extensions/lib/mx-calm-assistant-layout.ts"
OPERATIONAL_USER_LAYOUT="$ROOT/.pi/extensions/lib/mx-calm-operational-user-layout.ts"
VISIBILITY="$ROOT/.pi/extensions/lib/mx-calm-visibility.ts"
WATCH_EXT="$ROOT/.pi/extensions/mx-primary-pi-watch.ts"
OPERATIONAL_INPUT="$ROOT/bin/mx-operational-input.sh"
PI_OPERATIONAL_INPUT="$ROOT/.pi/extensions/lib/mx-operational-input.ts"
PI_PACKAGE_DIR=${MX_PI_PACKAGE_DIR:-"$(npm root -g 2>/dev/null)/@earendil-works/pi-coding-agent"}
TMUX_SOCKET="mx-calm-$$"
TMUX_SESSION="mx-calm-e2e"
...
find_chrome() {
  local candidate
  if [ -n "${MX_CHROME_BIN:-}" ] && [ -x "$MX_CHROME_BIN" ]; then
    printf '%s\n' "$MX_CHROME_BIN"
    return 0
  fi
  for candidate in \
...
test_static_contract() {
  ...
  assert_contains "$text" 'persistCalmPreference(active)' "Pi calm extension does not persist the maintainer's toggle choice"
  ...
  assert_not_contains "$visibility" 'deliverMultplxSyntheticInput' "Pi calm visibility policy can still replace operational input semantics"
  assert_not_contains "$visibility" 'classifyMultplxSyntheticInput' "Pi calm visibility policy still classifies operational input for interception"
  ...
  assert_contains "$operational_user_layout" 'classifyMultplxCurrentOperationalText(text)' "Pi Calm operational-user layout bypasses canonical current classification"
  assert_contains "$operational_user_layout" 'text.includes("\u2063")' "Pi Calm operational-user layout spawns its classifier for ordinary maintainer rows"
  ...
  assert_not_contains "$operational_user_layout" 'MULTPLX_OP: ' "Pi Calm operational-user layout duplicates the canonical marker grammar"
  ...
  assert_not_contains "$text" 'MULTPLX_PI_LAUNCH_BRIEF_ENV' "Pi calm presentation still depends on launch-input provenance"
  ...
  assert_contains "$operational" 'mx-operational-input.sh' "Pi adapter does not delegate to the canonical cross-language owner"
  assert_not_contains "$visibility" 'MULTPLX WATCHER WAKE:' "current Calm classification still matches watcher payload prose"
  ...
  assert_not_contains "$visibility" 'Run `bin/mx-session-start.sh`' "current Calm classification still matches session-start payload prose"
  assert_not_contains "$visibility" 'MULTPLX_OP: ' "current Calm classification duplicates the canonical marker grammar"
  assert_contains "$watch" 'calmHides("assistant-tool-call")' "Multplx watcher tool does not participate in Calm presentation"
  assert_contains "$watch" 'renderShell: "self"' "Multplx watcher tool cannot remove its complete shell"
  ...
  pass "Pi calm extension is presentation-only with one persisted visibility choice, no Calm status row, native working visibility, supported redraw controls, and the Multplx watcher-tool integration"
}
...
test_home_resolution() {
  ...
  mkdir -p \
    ...
    "$fixture/override" \
    "$fixture/launch-cwd"
  cp "$EXT" "$fixture/project/.pi/extensions/mx-calm.ts"
  cp "$ASSISTANT_LAYOUT" "$fixture/project/.pi/extensions/lib/mx-calm-assistant-layout.ts"
  cp "$OPERATIONAL_USER_LAYOUT" "$fixture/project/.pi/extensions/lib/mx-calm-operational-user-layout.ts"
  cp "$VISIBILITY" "$fixture/project/.pi/extensions/lib/mx-calm-visibility.ts"
  cp "$PI_OPERATIONAL_INPUT" "$fixture/project/.pi/extensions/lib/mx-operational-input.ts"
  ...
  out=$(cd "$fixture/launch-cwd" && \
    EXT="$fixture/project/.pi/extensions/mx-calm.ts" \
...
delete process.env.MX_HOME;
delete process.env.MX_CONFIG_OVERRIDE;
process.env.MX_ROOT_OVERRIDE = process.env.OVERRIDE_HOME;
...
if (readFileSync(`${process.env.OVERRIDE_HOME}/config/calm`, "utf8") !== "on\n") {
  throw new Error("Calm ignored MX_ROOT_OVERRIDE when MX_HOME was unset");
}

delete process.env.MX_ROOT_OVERRIDE;
calm = registerCalm();
calm.sessionStart({ reason: "startup" }, context);
await calm.calmCommand.handler("", context);
if (readFileSync(`${process.env.EXTENSION_HOME}/config/calm`, "utf8") !== "on\n") {
  throw new Error("Calm did not derive the Multplx home from its extension path");
}
...
test_rendering_and_session_lifecycle() {
  ...
  cp "$EXT" "$fixture/mx-calm.ts"
  cp "$ASSISTANT_LAYOUT" "$fixture/lib/mx-calm-assistant-layout.ts"
  cp "$OPERATIONAL_USER_LAYOUT" "$fixture/lib/mx-calm-operational-user-layout.ts"
  cp "$VISIBILITY" "$fixture/lib/mx-calm-visibility.ts"
  cp "$ROOT/.pi/extensions/lib/mx-operational-input.ts" "$fixture/lib/mx-operational-input.ts"
  cp "$WATCH_EXT" "$fixture/mx-primary-pi-watch.ts"
...
#!/usr/bin/env bash
printf '%s\n' "${1-}" >>"$MX_OPERATIONAL_INPUT_CALLS"
exec "$MX_OPERATIONAL_INPUT_OWNER" "$@"
SH
  chmod +x "$fixture/operational-input-probe.sh"

  out=$(cd "$fixture" && EXT="$fixture/mx-calm.ts" WATCH_EXT="$fixture/mx-primary-pi-watch.ts" MX_HOME="$fixture/home" MX_OPERATIONAL_INPUT_SCRIPT="$fixture/operational-input-probe.sh" MX_OPERATIONAL_INPUT_OWNER="$OPERATIONAL_INPUT" MX_OPERATIONAL_INPUT_CALLS="$fixture/operational-input-calls" PI_PACKAGE_DIR="$PI_PACKAGE_DIR" node --input-type=module 2>&1 <<'JS'
...
extension.default(pi);
const visibility = await import(`${pathToFileURL(`${process.cwd()}/lib/mx-calm-visibility.ts`).href}?policy=${Date.now()}`);
const operationalInput = await import(`${pathToFileURL(`${process.cwd()}/lib/mx-operational-input.ts`).href}?input=${Date.now()}`);
...
if (
  ...
  "Toggle Multplx's supported conversation-only transcript presentation."
) {
...
const watcherBody =
  "MULTPLX WATCHER WAKE: signal: /tmp/probe.status\n\n" +
  "Run bin/mx-wake-drain.sh first and handle the queued wake. Watcher continuity is extension-owned.";
const watcherMessage = operationalInput.encodeMultplxOperationalInput("watcher", watcherBody);
...
const callsBeforePlainReplay = readFileSync(process.env.MX_OPERATIONAL_INPUT_CALLS, "utf8");
...
if (readFileSync(process.env.MX_OPERATIONAL_INPUT_CALLS, "utf8") !== callsBeforePlainReplay) {
...
const watchTool = tools.find((tool) => tool.name === "mx_watch_arm_pi");
if (!watchTool) throw new Error("Multplx watcher extension did not register mx_watch_arm_pi");
...
if (JSON.stringify(watchActual.render(100)) !== JSON.stringify(watchBaseline.render(100))) {
  throw new Error("Multplx watcher tool changed stock rendering while Calm was off");
}
...
const presentationEntry = {
  customType: "broker-synthetic-input-presentation",
...
if (readFileSync(`${process.env.MX_HOME}/config/calm`, "utf8") !== "on\n") {
  throw new Error("Calm did not persist the active choice in the effective Multplx home");
}
presentationComponent.setExpanded(!expanded);
if (presentationComponent.hasContent() || presentationComponent.render(100).length !== 0) {
  throw new Error("Calm left a synthetic Multplx presentation row or spacer visible");
}
...
const operationalNearMisses = [
  {
    content: `Maintainer quote: ${watcherMessage}`,
    visible: "Maintainer quote:",
  },
  {
    content: "MULTPLX_OP: v1 watcher: ASCII_ONLY_MAINTAINER_MESSAGE",
    visible: "ASCII_ONLY_MAINTAINER_MESSAGE",
  },
  {
    content: `Ordinary maintainer text before ${watcherMessage}`,
    visible: "Ordinary maintainer text before",
  },
  {
    content: "\u2063ordinary maintainer text after an unrelated separator",
    visible: "ordinary maintainer text after an unrelated separator",
  },
  {
    content: "\u2063MULTPLX_OP: legacy untyped maintainer message",
    visible: "legacy untyped maintainer message",
  },
  {
    content: "Run `bin/mx-session-start.sh` now, exactly once, before executing any other instructions.",
    visible: "before executing any other instructions",
  },
  {
    content:
      "MULTPLX WATCHER WAKE: maintainer-authored legacy-shaped message\n\n" +
      "Run bin/mx-wake-drain.sh first and handle the queued wake. Watcher continuity is extension-owned.",
    visible: "maintainer-authored legacy-shaped message",
  },
  {
    content:
      ...
      "maintainer-authored legacy-shaped message",
    visible: "maintainer-authored legacy-shaped message",
  },
  {
    content: [
...
async function assertStockHtmlRendering(command, submitData) {
  ...
  const exportCases = [
    ...
    ["mx_watch_arm_pi", watchArgs, watchResult],
  ];
...
if (watchActual.render(100).length !== 0) {
  throw new Error("Calm left the mx_watch_arm_pi call/result shell visible");
}
...
if (readFileSync(`${process.env.MX_HOME}/config/calm`, "utf8") !== "off\n") {
  throw new Error("Calm did not persist the inactive choice in the effective Multplx home");
}
presentationComponent.setExpanded(expanded);
if (
  !presentationComponent.hasContent() ||
...
test_operational_followup_turn_e2e() {
  local project home config sessions version label case_name calm_state expected_notifications session_file pane i maintainer_line handled_line geometry_gap exact_session
  ...
  cp "$EXT" "$project/.pi/extensions/mx-calm.ts"
  cp "$ASSISTANT_LAYOUT" "$project/.pi/extensions/lib/mx-calm-assistant-layout.ts"
  cp "$OPERATIONAL_USER_LAYOUT" "$project/.pi/extensions/lib/mx-calm-operational-user-layout.ts"
  cp "$VISIBILITY" "$project/.pi/extensions/lib/mx-calm-visibility.ts"
  cp "$PI_OPERATIONAL_INPUT" "$project/.pi/extensions/lib/mx-operational-input.ts"
...
import { encodeMultplxOperationalInput } from "./.pi/extensions/lib/mx-operational-input.ts";

let phase: "idle" | "maintainer" | "monitor" = "idle";
let label = "";
let adjacent = false;
let latestInputRole: "user" | "custom" | undefined;

const EXACT_WATCHER_INPUT =
  "\u2063MULTPLX_OP: v1 watcher: MULTPLX WATCHER WAKE: signal: /Users/maintainer/github/KashyapTan/Multplx/state/oss-triage-t4.status\n\n" +
  "Run bin/mx-wake-drain.sh first and handle the queued wake. Watcher continuity is extension-owned.";
...
function monitorInput(suffix: "ONE" | "TWO"): string {
  ...
  return encodeMultplxOperationalInput("watcher", `MONITOR_${label}_${suffix}`);
}
...
export default function (pi: ExtensionAPI): void {
  pi.on("message_start", (event) => {
    ...
    if (event.message.role !== "assistant" || phase !== "maintainer") return;
    phase = "monitor";
  ...
  pi.registerCommand("followup-e2e", {
    description: "Run one maintainer prompt followed by typed monitoring input.",
    handler: async (args, ctx) => {
      ...
      phase = "maintainer";
      pi.sendUserMessage(`MAINTAINER_PROMPT_${label}`);
    },
  });
}
TS
  ...
  run_followup_case() {
    ...
    if [ "$calm_state" = absent ]; then
      ...
      rm -f "$home/config/calm"
      extensions='-e ./.pi/extensions/mx-calm.ts -e ./followup-e2e.ts'
    else
    ...
    tmux -L "$TMUX_SOCKET" new-session -d -s "$TMUX_SESSION" -x 160 -y 36 \
      "cd '$project' && env MX_HOME='$home' PI_CODING_AGENT_DIR='$config' MX_OPERATIONAL_INPUT_SCRIPT='$OPERATIONAL_INPUT' PI_OFFLINE=1 pi --approve --no-context-files --no-skills --no-prompt-templates --no-extensions $extensions $session_arg; rc=\$?; printf '\nPI_EXIT=%s\n' \"\$rc\"; sleep 20"
    i=0
    while [ "$i" -lt 120 ]; do
    ...
    i=0
    while [ "$i" -lt 240 ]; do
      session_file=$(find "$sessions" -type f -name '*.jsonl' -exec grep -l "MAINTAINER_PROMPT_$label" {} + 2>/dev/null | head -1 || true)
    ...
    pane=$(tmux -L "$TMUX_SOCKET" capture-pane -p -t "$TMUX_SESSION" -S - 2>/dev/null || true)
    [ "$(printf '%s\n' "$pane" | grep -Fc "MAINTAINER_ANSWER_$label" || true)" -eq 1 ] \
      || fail "Pi follow-up $label case rendered a duplicate maintainer answer"
    assert_contains "$pane" "MAINTAINER_PROMPT_$label" "Pi follow-up $label case hid the genuine maintainer prompt"
    ...
    if [ "$calm_state" = on ]; then
      ...
      if [ "$label" = exact_watcher ]; then
        assert_not_contains "$pane" "MULTPLX WATCHER WAKE: signal: /Users/maintainer/github/KashyapTan/Multplx/state/oss-triage-t4.status" \
          "Pi exact watcher case rendered the Calm-hidden authoritative payload"
        assert_not_contains "$pane" "Run bin/mx-wake-drain.sh first and handle the queued wake." \
      ...
      maintainer_line=$(printf '%s\n' "$pane" | grep -Fn "MAINTAINER_ANSWER_$label" | tail -1 | cut -d: -f1)
      handled_line=$(printf '%s\n' "$pane" | grep -Fn "MONITOR_HANDLED_${label}_ONE" | tail -1 | cut -d: -f1)
      geometry_gap=$((handled_line - maintainer_line))
      [ "$geometry_gap" -eq 2 ] \
...
const maintainerPrompt = `MAINTAINER_PROMPT_${label}`;
const maintainerAnswer = `MAINTAINER_ANSWER_${label}`;
...
const expectedOperationalTexts = Array.from({ length: expected }, (_, index) => {
  ...
  return label === "exact_watcher" && suffix === "ONE"
    ...
    : label === "legacy_away" && suffix === "ONE"
      ...
      : `\u2063MULTPLX_OP: v1 watcher: MONITOR_${label}_${suffix}`;
});
...
if (text(userEntries[0].message.content) !== maintainerPrompt) throw new Error("genuine maintainer prompt changed");
...
if (assistantText.filter((value) => value === maintainerAnswer).length !== 1) {
  throw new Error(`maintainer answer count changed: ${JSON.stringify(assistantText)}`);
}
...
if (assistantEntries.length !== 2) throw new Error(`expected one maintainer and one processing turn, found ${assistantEntries.length}`);
...
JS
  ...
  replay_exact_case() {
    ...
    printf '%s\n' on >"$home/config/calm"
    tmux -L "$TMUX_SOCKET" new-session -d -s "$TMUX_SESSION" -x 160 -y 36 \
      "cd '$project' && env MX_HOME='$home' PI_CODING_AGENT_DIR='$config' MX_OPERATIONAL_INPUT_SCRIPT='$OPERATIONAL_INPUT' PI_OFFLINE=1 pi --approve --no-context-files --no-skills --no-prompt-templates --no-extensions -e ./.pi/extensions/mx-calm.ts -e ./followup-e2e.ts --session '$exact_session'; rc=\$?; printf '\nPI_EXIT=%s\n' \"\$rc\"; sleep 20"
    i=0
    while [ "$i" -lt 120 ]; do
      pane=$(tmux -L "$TMUX_SOCKET" capture-pane -p -t "$TMUX_SESSION" -S - 2>/dev/null || true)
    ...
    assert_contains "$pane" "MAINTAINER_PROMPT_exact_watcher" "Pi restart lost the genuine maintainer prompt"
    ...
    maintainer_line=$(printf '%s\n' "$pane" | grep -Fn 'MAINTAINER_ANSWER_exact_watcher' | tail -1 | cut -d: -f1)
...
const text = (content) => typeof content === "string"
  ? content
  : (content ?? []).filter((item) => item.type === "text").map((item) => item.text).join("");
const exact = "\u2063MULTPLX_OP: v1 watcher: MULTPLX WATCHER WAKE: signal: /Users/maintainer/github/KashyapTan/Multplx/state/oss-triage-t4.status\n\nRun bin/mx-wake-drain.sh first and handle the queued wake. Watcher continuity is extension-owned.";
...
test_hidden_block_geometry_e2e() {
  ...
  cat >"$project/.agents/skills/recap/SKILL.md" <<'MD'
---
name: recap
...
TS
  ...
  start_geometry_pi() {
    ...
    tmux -L "$TMUX_SOCKET" new-session -d -s "$TMUX_SESSION" -x 100 -y 44 \
      "cd '$project' && env MX_HOME='$home' PI_CODING_AGENT_DIR='$config' PI_OFFLINE=1 pi --approve --no-context-files --no-prompt-templates --no-extensions -e ./.pi/extensions/mx-calm.ts -e ./geometry-provider.ts $session_arg; rc=\$?; printf '\nPI_EXIT=%s\n' \"\$rc\"; sleep 20"
  }
  ...
  assert_geometry_gap() {
    ...
    skill_line=$(grep -n -m1 '\[skill\] recap' "$file" | cut -d: -f1)
  ...
  tmux -L "$TMUX_SOCKET" send-keys -t "$TMUX_SESSION" Enter
  sleep 0.1
  tmux -L "$TMUX_SOCKET" send-keys -t "$TMUX_SESSION" -l '/skill:recap'
  tmux -L "$TMUX_SOCKET" send-keys -t "$TMUX_SESSION" Enter
  wait_for_geometry_text "$snapshot" "visible row two" \
    || fail "Pi Calm hidden-block geometry E2E did not complete the /skill:recap turn"
  i=0
  while [ "$i" -lt 120 ]; do
  ...
  assert_contains "$(cat "$snapshot")" "[skill] recap" "Calm hid the collapsed skill header"
  ...
  assert_geometry_gap "$snapshot" "completed native Calm /skill:recap turn"
  ...
  sleep 0.2
  tmux -L "$TMUX_SOCKET" kill-session -t "$TMUX_SESSION" 2>/dev/null || true
  pass "Pi Calm native /skill:recap geometry keeps every collapsed thinking and tool block at zero height while preserving expansion, history, restart, and Calm-off rendering"
}
...
test_interactive_terminal_e2e() {
  ...
  cp "$WATCH_EXT" "$project/.pi/extensions/mx-primary-pi-watch.ts"
  ...
  cat >"$project/.pi/extensions/mx-calm-e2e-inject.ts" <<'TS'
import {
  type AssistantMessage,
  createAssistantMessageEventStream,
} from "@earendil-works/pi-ai";
import type { ExtensionAPI } from "@earendil-works/pi-coding-agent";
import { encodeMultplxOperationalInput } from "./lib/mx-operational-input.ts";
...
export default function (pi: ExtensionAPI): void {
  ...
  pi.registerCommand("calm-inject-e2e", {
    ...
    handler: async (args, ctx) => {
      ...
      await pi.sendUserMessage(encodeMultplxOperationalInput(kind, body), {
        deliverAs: "followUp",
      });
    },
  });
...
{"type":"message","id":"a0000007","parentId":"a0000006","timestamp":"$now","message":{"role":"assistant","content":[{"type":"thinking","thinking":"third internal reasoning block"},{"type":"toolCall","id":"call_watch_e2e","name":"mx_watch_arm_pi","arguments":{}}],"api":"anthropic-messages","provider":"anthropic","model":"claude-sonnet-4-5","usage":{"input":2,"output":1,"cacheRead":0,"cacheWrite":0,"totalTokens":3,"cost":{"input":0,"output":0,"cacheRead":0,"cacheWrite":0,"total":0}},"stopReason":"toolUse","timestamp":7}}
{"type":"message","id":"a0000008","parentId":"a0000007","timestamp":"$now","message":{"role":"toolResult","toolCallId":"call_watch_e2e","toolName":"mx_watch_arm_pi","content":[{"type":"text","text":"watcher: started Pi extension arm child 1"}],"details":{"ok":true,"message":"watcher: started Pi extension arm child 1"},"isError":false,"timestamp":8}}
{"type":"custom","id":"a0000009","parentId":"a0000008","timestamp":"$now","customType":"broker-synthetic-input-presentation","data":{"content":"MULTPLX WATCHER WAKE: signal: /tmp/probe.status\\n\\nRun bin/mx-wake-drain.sh first and handle the queued wake. Watcher continuity is extension-owned.","kind":"watcher"}}
{"type":"custom_message","id":"a0000010","parentId":"a0000009","timestamp":"$now","customType":"broker-synthetic-input","content":"MULTPLX WATCHER WAKE: signal: /tmp/probe.status\\n\\nRun bin/mx-wake-drain.sh first and handle the queued wake. Watcher continuity is extension-owned.","display":false,"details":{"kind":"watcher"}}
{"type":"message","id":"a0000011","parentId":"a0000010","timestamp":"$now","message":{"role":"user","content":[{"type":"text","text":"MULTPLX WATCHER WAKE: can you explain this phrase?"}],"timestamp":11}}
{"type":"message","id":"a0000012","parentId":"a0000011","timestamp":"$now","message":{"role":"user","content":[{"type":"text","text":"Maintainer quote: \u2063MULTPLX_OP: v1 watcher: QUOTED_CURRENT_NEAR_MISS"}],"timestamp":12}}
{"type":"message","id":"a0000013","parentId":"a0000012","timestamp":"$now","message":{"role":"user","content":[{"type":"text","text":"MULTPLX_OP: v1 watcher: ASCII_ONLY_NEAR_MISS"}],"timestamp":13}}
{"type":"message","id":"a0000014","parentId":"a0000013","timestamp":"$now","message":{"role":"user","content":[{"type":"text","text":"Ordinary maintainer text before \u2063MULTPLX_OP: v1 watcher: EMBEDDED_CURRENT_NEAR_MISS"}],"timestamp":14}}
{"type":"message","id":"a0000015","parentId":"a0000014","timestamp":"$now","message":{"role":"user","content":[{"type":"text","text":"\u2063ordinary maintainer text after unrelated separator"}],"timestamp":15}}
...
JSON
  ...
  tmux -L "$TMUX_SOCKET" new-session -d -s "$TMUX_SESSION" -x 180 -y 44 \
    "cd '$project' && env MX_HOME='$home' PI_CODING_AGENT_DIR='$config' MX_OPERATIONAL_INPUT_SCRIPT='$OPERATIONAL_INPUT' PI_OFFLINE=1 pi --approve --no-skills --no-prompt-templates --no-context-files --session '$session_file'; rc=\$?; printf '\nPI_EXIT=%s\n' \"\$rc\"; sleep 30"
  ...
  assert_contains "$(cat "$default_snapshot")" "mx_watch_arm_pi" "Calm-off transcript did not show the Multplx watcher tool"
  assert_contains "$(cat "$default_snapshot")" "MULTPLX WATCHER WAKE: signal: /tmp/probe.status" "Calm-off transcript did not show the synthetic Multplx presentation row"
  assert_contains "$(cat "$default_snapshot")" "Thinking..." "reasoning fixture did not render Pi's collapsed thinking label"
  assert_contains "$(cat "$default_snapshot")" "mx-calm.ts" "project-local Pi calm extension did not auto-load"
  # shellcheck disable=SC2016 # Backticks are literal prompt markup.
  assert_not_contains "$(cat "$default_snapshot")" 'Run `bin/mx-session-start.sh` now' \
  ...
  assert_not_contains "$(cat "$hidden_snapshot")" "mx_watch_arm_pi" "/calm left the Multplx watcher tool call shell in the transcript"
  assert_not_contains "$(cat "$hidden_snapshot")" "watcher: started Pi extension arm child" "/calm left the Multplx watcher tool result in the transcript"
  assert_not_contains "$(cat "$hidden_snapshot")" "MULTPLX WATCHER WAKE: signal: /tmp/probe.status" "/calm left a synthetic Multplx user-role presentation in the transcript"
  ...
  assert_contains "$(cat "$hidden_snapshot")" "MULTPLX WATCHER WAKE: can you explain this phrase?" "/calm hid a genuine near-miss user prompt"
  for near_miss in \
    QUOTED_CURRENT_NEAR_MISS \
    ASCII_ONLY_NEAR_MISS \
    EMBEDDED_CURRENT_NEAR_MISS \
    "ordinary maintainer text after unrelated separator"
  do
...
if (
  ...
  !nativeSessionStart.content?.startsWith("\u2063MULTPLX_OP: v1 session-start: ")
) {
...
JS
  ...
  # shellcheck disable=SC2016 # Backticks are literal prompt markup.
  assert_not_contains "$(cat "$active_hidden_snapshot")" 'Run `bin/mx-session-start.sh` now' \
    "Calm showed the native session-start operational input"
  for hidden in \
    CURRENT_WATCHER_E2E \
    CURRENT_TURN_END_E2E \
    CURRENT_AWAY_E2E \
    CURRENT_FROM_MULTPLX_E2E \
...
if (!serialized.includes("broker-synthetic-input") || !serialized.includes("/tmp/probe.status")) process.exit(1);
const synthetic = entries.find((entry) => entry.type === "custom_message" && entry.customType === "broker-synthetic-input");
...
if (messages.includes("[broker-synthetic-input]")) process.exit(1);
for (const current of ["CURRENT_WATCHER_E2E", "CURRENT_TURN_END_E2E", "CURRENT_AWAY_E2E", "CURRENT_FROM_MULTPLX_E2E", "CURRENT_LAUNCH_BRIEF_E2E"]) {
  if (!messages.includes(current)) process.exit(1);
}
if (!tree.includes("broker-synthetic-input") || !tree.includes("/tmp/probe.status")) process.exit(1);
JS
  ...
  assert_contains "$(cat "$restored_snapshot")" "mx_watch_arm_pi" "second /calm did not restore the Multplx watcher tool shell"
  assert_contains "$(cat "$restored_snapshot")" "MULTPLX WATCHER WAKE: signal: /tmp/probe.status" "second /calm did not restore the synthetic Multplx user row"
  for restored in \
    CURRENT_WATCHER_E2E \
    CURRENT_TURN_END_E2E \
    CURRENT_AWAY_E2E \
  ...
  assert_not_contains "$(cat "$working_snapshot")" "MULTPLX WATCHER WAKE: signal: /tmp/probe.status" "the real provider wait restored a hidden operational row"
  ...
  assert_not_contains "$(cat "$restarted_snapshot")" "mx_watch_arm_pi" "restart/resume reset Calm and restored the Multplx watcher tool"
  assert_not_contains "$(cat "$restarted_snapshot")" "MULTPLX WATCHER WAKE: signal: /tmp/probe.status" "restart/resume reset Calm and restored a legacy presentation row"
  for hidden in \
    CURRENT_WATCHER_E2E \
    CURRENT_TURN_END_E2E \
    CURRENT_AWAY_E2E \
  ...
  tmux -L "$TMUX_SOCKET" send-keys -t "$TMUX_SESSION" -l "/quit"
  tmux -L "$TMUX_SOCKET" send-keys -t "$TMUX_SESSION" M-s
  pass "Pi calm native E2E keeps Working and maintainer turns visible, hides exact operational user rows without changing persistence, restores them Calm-off, survives restart, and preserves export plus Ctrl+O behavior"
}
