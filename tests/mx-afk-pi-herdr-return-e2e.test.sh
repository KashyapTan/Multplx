...
# Herdr call is routed through mx-herdr-lab.sh. The scenario proves:
...
#   - an unmarked return request opens the catch-up gate before Catchup;
...
. "$ROOT/bin/mx-supervise-daemon.sh"
# shellcheck source=/dev/null
. "$ROOT/bin/mx-backend.sh"

if [ "${MX_AFK_PI_HERDR_E2E:-0}" != 1 ]; then
  echo "skip: set MX_AFK_PI_HERDR_E2E=1 to run the real Pi/Herdr away-return regression"
  exit 0
fi
...
LAB_HELPER=${HERDR_LAB_HELPER:-$ROOT/bin/mx-herdr-lab.sh}
SESSION=$("$LAB_HELPER" name mx-afk-pi-return-e2e)
TMP_ROOT=$(mx_test_tmproot mx-afk-pi-return-e2e)
...
cleanup() {
  ...
  if [ "$DAEMON_STARTED" -eq 1 ]; then
    PATH="$FAKEBIN:$ORIGINAL_PATH" HERDR_SESSION="$SESSION" MX_HOME="$HOME_DIR" MX_STATE_OVERRIDE="$STATE" \
      MX_SUPERVISOR_BACKEND=herdr MX_SUPERVISOR_TARGET="$PRIMARY_TARGET" \
      "$ROOT/bin/mx-afk-launch.sh" stop >/dev/null 2>&1 || true
  fi
...
printf '# Synthetic isolated Multplx primary\n' > "$PROJECT/AGENTS.md"
...
# in this synthetic primary, so nothing except the test can mutate system state.
...
const capturePath = process.env.MX_PI_CAPTURE_PATH!;
...
export MX_ESCALATE_BATCH_SECS=0
export MX_HOUSEKEEPING_TICK=1
export MX_POLL=1
export MX_SIGNAL_GRACE=1
export MX_HEARTBEAT=999999
export MX_CHECK_INTERVAL=999999
export MX_MAX_DEFER_SECS=3
export MX_STALE_ESCALATE_SECS=999999
export MX_WEDGE_ALARM_EXEC='$TMP_ROOT/wedge-recorder'
exec '$ROOT/bin/mx-afk-start.sh'
EOF
...
EXT="$CAPTURE_EXT"
PI_CMD=$(printf 'exec env PI_CODING_AGENT_DIR=%q MX_HOME=%q MX_PI_CAPTURE_PATH=%q pi -e %q --no-context-files --no-session' "$PI_DIR" "$HOME_DIR" "$CAPTURE" "$EXT")
...
CHILD_OUT=$("$LAB_HELPER" run "$SESSION" tab create --workspace "$WORKSPACE" --cwd "$PROJECT" --label mx-repair-task --no-focus)
...
PATH="$FAKEBIN:$ORIGINAL_PATH" HERDR_SESSION="$SESSION" MX_HOME="$HOME_DIR" MX_STATE_OVERRIDE="$STATE" \
  MX_SUPERVISOR_BACKEND=herdr MX_SUPERVISOR_TARGET="$PRIMARY_TARGET" MX_AFK_LAUNCH_ENTRY="$TMP_ROOT/daemon-entry" \
  "$ROOT/bin/mx-afk-launch.sh" start >/dev/null
DAEMON_STARTED=1
...
sleep 0.5
composer=$(PATH="$FAKEBIN:$ORIGINAL_PATH" HERDR_SESSION="$SESSION" mx_backend_composer_state herdr "$PRIMARY_TARGET")
[ "$composer" = pending ] || fail "real Pi draft did not classify pending (got $composer)"
CHILD_CMD=$(printf "printf 'blocked [key=synthetic-dependency]: broker can refresh the synthetic token\\n' >> %q; exec sleep 120" "$STATE/repair-task.status")
...
# The maintainer returns with an ordinary unmarked Catchup request. The request is
...
"$LAB_HELPER" run "$SESSION" pane send-text "$PRIMARY_PANE" 'Synthetic Catchup request' >/dev/null
"$LAB_HELPER" run "$SESSION" pane send-keys "$PRIMARY_PANE" enter >/dev/null
wait_for_prompt 'any(.[]; .prompt == "Synthetic Catchup request")' || fail "real Pi did not receive the unmarked return request"
RETURN_PROMPT=$(jq -r 'select(.prompt == "Synthetic Catchup request") | .prompt' "$CAPTURE" | tail -1)
...
set +e
RETURN_OUT=$(PATH="$FAKEBIN:$ORIGINAL_PATH" HERDR_SESSION="$SESSION" MX_ROOT_OVERRIDE="$PROJECT" MX_HOME="$HOME_DIR" MX_STATE_OVERRIDE="$STATE" \
  MX_SUPERVISOR_BACKEND=herdr MX_SUPERVISOR_TARGET="$PRIMARY_TARGET" "$ROOT/bin/mx-afk-return.sh" begin 2>&1)
RETURN_RC=$?
set -e
DAEMON_STARTED=0
...
STATUS_RC=$?
set -e
[ "$STATUS_RC" -eq 3 ] || fail "Catchup bypassed the return gate (rc=$STATUS_RC): $STATUS_OUT"
pass "real unmarked Pi return opens catch-up and blocks Catchup before the unresolved blocker can be deferred"

printf 'resolved [key=synthetic-dependency]: refreshed the synthetic token and resumed the task\n' >> "$STATE/repair-task.status"
PATH="$FAKEBIN:$ORIGINAL_PATH" HERDR_SESSION="$SESSION" MX_ROOT_OVERRIDE="$PROJECT" MX_HOME="$HOME_DIR" MX_STATE_OVERRIDE="$STATE" \
  "$ROOT/bin/mx-afk-return.sh" check >/dev/null || fail "remediated blocker did not clear return catch-up"
PATH="$FAKEBIN:$ORIGINAL_PATH" HERDR_SESSION="$SESSION" MX_ROOT_OVERRIDE="$PROJECT" MX_HOME="$HOME_DIR" MX_STATE_OVERRIDE="$STATE" \
  "$ROOT/bin/mx-status-snapshot.sh" --json >/dev/null || fail "Catchup remained gated after blocker remediation"
...
DAEMON_STARTED=1
PATH="$FAKEBIN:$ORIGINAL_PATH" HERDR_SESSION="$SESSION" MX_ROOT_OVERRIDE="$PROJECT" MX_HOME="$HOME_DIR" MX_STATE_OVERRIDE="$STATE" \
  MX_SUPERVISOR_BACKEND=herdr MX_SUPERVISOR_TARGET="$PRIMARY_TARGET" "$ROOT/bin/mx-afk-return.sh" begin >/dev/null \
...
pass "resolved return catch-up allows Catchup and a clean idempotent away re-entry"
