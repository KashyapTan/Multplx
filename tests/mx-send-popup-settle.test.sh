#!/usr/bin/env bash
# mx-send pre-submit popup-settle selection (the codex `$<skill>` fix).
#
...
# for a leading `$<skill>` invocation (e.g. `$deep-review`). Submitting before the
# popup settles lets it swallow the Enter, so the line never submits. mx-send
# absorbs this by pausing `settle` seconds AFTER typing and BEFORE the (retried)
# Enter - the first sleep mx_tmux_submit_core makes. These tests pin the
...
#
# The popup-settle is the FIRST sleep recorded: mx_tmux_submit_core types the text,
# then `sleep "$settle"`, then the Enter-retry loop (sleep 0.4 each) and finally
# mx-send's own post-submit MX_SEND_SETTLE pause. So tail-vs-head matters: this
# suite asserts on the HEAD sleep, distinct from mx-send-settle.test.sh which pins
# the TAIL (post-submit) pause. The retried Enter in mx_tmux_submit_core remains the
...
TMP_ROOT=$(mx_test_tmproot mx-send-popup-settle)

# Same stub shape as mx-send-settle.test.sh: a fake tmux that drives the submit
# path to a clean "empty" verdict on the first Enter, and a fake sleep that records
# every requested duration (one per line) into MX_SLEEP_LOG instead of sleeping.
...
# (the popup-settle) equals <expected>. MX_SEND_SETTLE=0 strips the trailing
# post-submit pause so the log holds only the popup-settle plus the 0.4 Enter wait,
# keeping the head assertion crisp. MX_ROOT_OVERRIDE points at a non-repo dir so
# mx-guard's tangle check stays silent; its watcher-liveness note goes to stderr
# (discarded).
first_settle() {  # <expected> <label> <harness|--explicit> <message> [selector-form]
  ...
  pass "mx-send popup-settle: $label -> ${expected}s"
}

# Codex `$<skill>` gets the long settle so its `$` popup clears (the fix).
first_settle 1.2 'codex $skill -> long settle' codex '$deep-review'

# The same Codex `$<skill>` path must work when the target is addressed by exact
# task id, not only by the legacy `mx-<id>` window label.
first_settle 1.2 'codex $skill exact task id -> long settle' codex '$deep-review' exact

# Same `$` message to claude keeps the fast path: `$` is ordinary text there.
first_settle 0.3 'claude $-message -> fast path' claude '$deep-review'
...
first_settle 0.3 'explicit target $message -> fast path (unknown harness)' --explicit '$deep-review'

# The `/` slash case stays universal and unchanged: long settle regardless of
# harness (here a non-codex claude target).
first_settle 1.2 'claude /command -> long settle (slash unchanged)' claude '/deep-review'
