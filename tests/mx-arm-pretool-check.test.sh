...
matrix_case A01 allow 'bin/mx-watch-arm.sh'
matrix_case A02 allow './bin/mx-watch-arm.sh --restart'
matrix_case A03 allow 'exec bin/mx-watch-arm.sh'
matrix_case A04 allow 'bin/mx-watch-checkpoint.sh --seconds 180'
matrix_case A05 allow 'exec bin/mx-watch-checkpoint.sh --seconds 180'
matrix_case A06 allow "$ROOT/bin/mx-watch-checkpoint.sh --seconds 180"
matrix_case A07 allow "cd '$ROOT'; exec bin/mx-watch-arm.sh"
matrix_case A08 allow "cd '../broker'; bin/mx-watch-checkpoint.sh --seconds 180"
matrix_case A09 allow "export MX_HOME='$ROOT'; bin/mx-watch-checkpoint.sh --seconds 180"
matrix_case A15 allow "cd $ROOT && exec bin/mx-watch-arm.sh"
matrix_case A16 allow "export MX_HOME=$ROOT && bin/mx-watch-checkpoint.sh --seconds 180"

matrix_case R01 allow "pgrep -fl '/bin/mx-watch.sh' || true"
matrix_case R02 allow "ps aux | rg '/bin/mx-watch.sh'"
matrix_case R03 allow "rg -n 'mx-watch-arm.sh &' docs tests"
matrix_case R04 allow "rg -n 'bin/mx-watch-arm.sh; echo bad' docs"
matrix_case R05 allow "git grep 'mx-watch-checkpoint.sh && echo bad'"
matrix_case R06 allow "sed -n '/mx-watch-checkpoint.sh/p' docs/arm-pretool-check.md"
matrix_case R07 allow 'assert_contains "$content" '\''mx-watch-arm.sh &'\'''
matrix_case R08 allow "printf '%s\\n' 'bin/mx-watch-checkpoint.sh --seconds 180 >/tmp/out'"
matrix_case R09 allow "tmux send-keys -t isolated-pi-lab 'bin/mx-watch-arm.sh &' Enter"
...
matrix_case R11 allow "python3 -c 'print(\"bin/mx-watch-arm.sh; echo data\")'"
matrix_case R12 allow "bash -lc \"rg -n 'mx-watch-arm.sh &' docs\""
matrix_case R13 allow "echo 'pkill -f mx-watch'"
matrix_case R14 allow "rg -n 'pkill -f mx-watch' docs tests"
matrix_case R15 allow "echo ok # bin/mx-watch-arm.sh &"
matrix_case R16 allow $'# bin/mx-watch-arm.sh &\necho ok'
matrix_case R17 allow "printf '%s\\n' 'mx-watch.sh; a && b || c > out' | sed -n '1p'"
matrix_case R18 allow "sh -c 'tmux send-keys -t lab \"bin/mx-watch-arm.sh &\" Enter'"
matrix_case R19 allow "eval 'printf \"%s\\n\" \"bin/mx-watch-arm.sh &\"'"

matrix_case D01 deny 'bin/mx-watch-arm.sh &'
matrix_case D02 deny 'nohup bin/mx-watch-arm.sh'
matrix_case D03 deny 'bin/mx-watch-arm.sh & disown'
matrix_case D04 deny '(bin/mx-watch-arm.sh) &'
matrix_case D05 deny "bash -lc 'bin/mx-watch-arm.sh &'"
matrix_case D06 deny '$(bin/mx-watch-arm.sh)'
matrix_case D07 deny 'echo "$(bin/mx-watch-checkpoint.sh --seconds 180)"'
matrix_case D08 deny 'cat <(bin/mx-watch-arm.sh)'
matrix_case D09 deny 'bin/mx-watch-arm.sh >/tmp/out'
matrix_case D10 deny 'bin/mx-watch-checkpoint.sh --seconds 180 </dev/null'
matrix_case D11 deny 'bin/mx-watch-arm.sh 2>&1 | head -2'
matrix_case D12 deny 'bin/mx-watch-arm.sh | cat'
matrix_case D13 deny 'bin/mx-watch-checkpoint.sh --seconds 180 | timeout 1 cat'
matrix_case D14 deny 'echo before; bin/mx-watch-arm.sh'
matrix_case D15 deny 'bin/mx-watch-checkpoint.sh --seconds 180; echo after'
matrix_case D16 deny 'true && bin/mx-watch-arm.sh'
matrix_case D17 deny 'bin/mx-watch-checkpoint.sh --seconds 180 || true'
matrix_case D18 deny $'bin/mx-watch-arm.sh\nbin/mx-watch-checkpoint.sh --seconds 180'
matrix_case D19 deny "pkill -f '/bin/mx-watch.sh'"
matrix_case D20 deny "command pkill -f '/bin/mx-watch.sh'"
matrix_case D21 deny "/usr/bin/pkill -f '/bin/mx-watch.sh'"
matrix_case D22 deny "sudo pkill -f '/bin/mx-watch.sh'"
matrix_case D23 deny 'kill "$(pgrep -f '\''/bin/mx-watch.sh'\'')"'
matrix_case D24 deny $'bin/mx-watc\\\nh-arm.sh &'
matrix_case D25 deny 'sudo -u root bin/mx-watch-arm.sh &'
matrix_case D26 deny 'env -u PATH bin/mx-watch-arm.sh &'
matrix_case D27 deny "bash -c \$'bin/mx-watch-arm.sh &'"
matrix_case D28 deny $'bash <<\'EOF\'\nbin/mx-watch-arm.sh &\nEOF'
matrix_case D29 deny "WATCHER='bin/mx-watch-arm.sh &' bash -c 'eval \"\$WATCHER\"'"
matrix_case D30 deny "p=\$(pgrep -f '/bin/mx-watch.sh'); kill \"\$p\""
matrix_case D31 deny "env -S 'bin/mx-watch-arm.sh &'"
matrix_case D32 deny "env --split-string='$ROOT/bin/mx-watch-arm.sh &'"
matrix_case D33 deny 'bin/mx-"watch-arm.sh" &'
matrix_case D34 deny "WATCHER='bin/mx-watch-arm.sh'; \"\$WATCHER\" &"
matrix_case D35 deny "bash -c -- 'bin/mx-watch-arm.sh &'"
matrix_case D36 deny 'bash bin/mx-watch-arm.sh &'
matrix_case D37 deny '. bin/mx-watch-arm.sh &'
matrix_case D38 deny "bash <<< 'bin/mx-watch-arm.sh &'"
matrix_case D39 deny "eval 'true;' 'bin/mx-watch-arm.sh &'"
matrix_case D40 deny 'timeout 30 bin/mx-watch-arm.sh &'
matrix_case D41 deny 'gtimeout 30 bin/mx-watch-arm.sh &'
matrix_case D42 deny 'bin/mx-watch-{arm,checkpoint}.sh &'
matrix_case D43 deny 'bin/mx-watch-arm.sh* &'
matrix_case D44 deny "pattern='mx-watch'; pkill -f \"\$pattern\""
matrix_case D45 deny "p=\$(pgrep -f '/bin/mx-watch.sh'); q=\$p; kill \$q"
matrix_case D46 deny '$MX_HOME/bin/mx-watch-arm.sh &'
matrix_case D47 deny '$HOME/broker/bin/mx-watch-arm.sh | cat'
matrix_case D48 deny '~/broker/bin/mx-watch-arm.sh &'
matrix_case D49 deny 'bin/mx-watch.sh'
matrix_case D50 deny '$MX_HOME/bin/mx-watch.sh'
matrix_case D51 deny '~/broker/bin/mx-watch.sh --restart'
matrix_case D52 deny "bin/mx-\$'\x77'atch-arm.sh &"
matrix_case D53 deny 'bin/mx-$"watch"-arm.sh &'
matrix_case D54 deny 'bin/mx-watch-$"arm".sh &'
matrix_case D55 deny 'while true; do pkill -f mx-watch; done'
matrix_case D56 deny 'for x in 1; do pkill -f mx-watch; done'
matrix_case D57 deny 'case x in x) pkill -f mx-watch ;; esac'
matrix_case D58 deny 'until false; do kill $(pgrep -f mx-watch); done'

matrix_case E01 allow "bin/mx-watch-checkpoint.sh --seconds '180;still-one-arg'"
matrix_case E02 allow "bin/mx-watch-checkpoint.sh --label 'mx-watch-arm.sh; literal argument'"
matrix_case E03 allow 'bin/mx-watch-arm.sh # output > file &'
matrix_case E05 deny "MX_HOME=$ROOT bin/mx-watch-checkpoint.sh --seconds 180"
matrix_case E06 deny "env MX_HOME=$ROOT bin/mx-watch-arm.sh"
matrix_case E08 deny "bash -lc 'bin/mx-watch-checkpoint.sh --seconds 180'"
matrix_case E09 deny '(bin/mx-watch-checkpoint.sh --seconds 180)'
matrix_case E10 deny "eval 'bin/mx-watch-arm.sh &'"
matrix_case E11 deny "exec bash -lc 'bin/mx-watch-arm.sh &'"
matrix_case E12 allow 'bash -lc "$WATCHER_COMMAND" # mx-watch-arm.sh'
matrix_case E13 allow "printf '%s\\n' 'argument has ; and mx-watch-arm.sh and &&'"
matrix_case E14 allow '$MX_HOME/bin/mx-teardown.sh &'
matrix_case E15 allow '$MX_HOME/bin/mx-watch-arm.sh'
matrix_case E16 allow '~/broker/bin/mx-watch-checkpoint.sh --seconds 180'
matrix_case E17 allow 'for f in 1; do echo mx-watch; done'

MATRIX_TMP=$(mktemp -d "${TMPDIR:-/tmp}/mx-arm-policy-matrix.XXXXXX")
...
test_direct_policy_contract() {
  ...
  assert_policy direct-data-pkill allow "echo 'pkill -f mx-watch'"
  assert_policy direct-broad-pkill $'deny\tbroad-watcher-kill' "pkill -f '/bin/mx-watch.sh'"
  assert_policy direct-loop-broad-pkill $'deny\tbroad-watcher-kill' 'while true; do pkill -f mx-watch; done'
  assert_policy direct-loop-broad-kill-pgrep $'deny\tbroad-watcher-kill' 'until false; do kill $(pgrep -f mx-watch); done'
  assert_policy direct-loop-no-kill-allowed allow 'for f in 1; do echo mx-watch; done'
  assert_policy direct-pipeline $'deny\twatcher-pipeline' 'bin/mx-watch-arm.sh | cat'
  assert_policy direct-leading-redirection $'deny\twatcher-redirection' '>/tmp/out bin/mx-watch-arm.sh'
  assert_policy direct-unclassifiable $'deny\tunclassifiable-protected-command' "bin/mx-watch-arm.sh 'unterminated"
  assert_policy direct-unsupported $'deny\tunclassifiable-protected-command' 'if true; then bin/mx-watch-arm.sh; fi'
  assert_policy direct-constructed-payload $'deny\twatcher-nested' "WATCHER='bin/mx-watch-arm.sh &'; bash -lc \"\$WATCHER\""
  assert_policy direct-parameter-export allow 'export MX_HOME=${HOME}; bin/mx-watch-checkpoint.sh --seconds 180'
  assert_policy direct-expanded-arm-blessed allow '$MX_HOME/bin/mx-watch-arm.sh'
  assert_policy direct-expanded-arm-background $'deny\twatcher-background' '$MX_HOME/bin/mx-watch-arm.sh &'
  assert_policy direct-expanded-arm-pipeline $'deny\twatcher-pipeline' '$HOME/broker/bin/mx-watch-arm.sh | cat'
  assert_policy direct-watch-not-blessed $'deny\twatcher-direct' 'bin/mx-watch.sh'
  assert_policy direct-watch-expanded $'deny\twatcher-direct' '$MX_HOME/bin/mx-watch.sh'
  assert_policy direct-watch-safe-shape $'deny\twatcher-direct' 'cd /tmp; bin/mx-watch.sh'
  heredoc_data=$'cat <<\'EOF\'\nbin/mx-watch-arm.sh &\nEOF'
  heredoc_watcher=$'bin/mx-watch-arm.sh <<\'EOF\'\ndata only\nEOF'
...
test_command_equals_form() {
  "$CHECK" --command='bin/mx-watch-arm.sh &' >/dev/null 2>&1
...
test_background_flag_accepted_and_non_gating() {
  local rc_bg rc_nobg
  "$CHECK" --command 'exec bin/mx-watch-arm.sh' --background true >/dev/null 2>&1
  rc_bg=$?
  "$CHECK" --command 'exec bin/mx-watch-arm.sh' >/dev/null 2>&1
  rc_nobg=$?
...
test_stdin_claude_codex_schema_allow() {
  local rc
  printf '%s' '{"tool_input":{"command":"exec bin/mx-watch-arm.sh"},"tool_name":"Bash"}' | "$CHECK" >/dev/null 2>&1
  rc=$?
...
test_stdin_claude_codex_schema_deny() {
  local rc
  printf '%s' '{"tool_input":{"command":"bin/mx-watch-arm.sh &"},"tool_name":"Bash"}' | "$CHECK" >/dev/null 2>&1
  rc=$?
...
test_prefilter_is_strict_superset() {
  local rc
  # A command with no mx-watch substring is fast-allowed by the transport
  # prefilter without ever invoking the classifier.
  "$CHECK" --command 'ls -la /bin && echo done' >/dev/null 2>&1
  rc=$?
  [ "$rc" -eq 0 ] || fail "a command with no mx-watch substring must be fast-allowed, got exit $rc"
  # A deniable protected execution carries the mx-watch bytes, so the prefilter
  # must delegate to the classifier and the deny must survive.
  "$CHECK" --command 'bin/mx-watch-arm.sh &' >/dev/null 2>&1
  rc=$?
  [ "$rc" -eq 2 ] || fail "prefilter must delegate a deniable mx-watch command, not fast-allow it, got exit $rc"
  # A broad watcher kill also contains the mx-watch bytes and must still deny.
  "$CHECK" --command "pkill -f '/bin/mx-watch.sh'" >/dev/null 2>&1
  rc=$?
  [ "$rc" -eq 2 ] || fail "prefilter must delegate a broad watcher kill, not fast-allow it, got exit $rc"
  # Obfuscated protected paths lose the literal mx-watch bytes (a line
  ...
  "$CHECK" --command "$(printf 'bin/mx-watc\\\nh-arm.sh &')" >/dev/null 2>&1
  rc=$?
  ...
  # A quoting-decoder marker ($' ANSI-C or $" locale) hides the mx-watch bytes
  ...
  "$CHECK" --command "bin/mx-\$'\x77'atch-arm.sh &" >/dev/null 2>&1
  rc=$?
  ...
  "$CHECK" --command '$MX_HOME/bin/mx-teardown.sh &' >/dev/null 2>&1
  rc=$?
  ...
  # A benign command that only mentions mx-watch as data still reaches the
  # classifier and is allowed there, proving the prefilter owns no verdict.
  "$CHECK" --command "echo 'pkill -f mx-watch'" >/dev/null 2>&1
  rc=$?
  [ "$rc" -eq 0 ] || fail "a benign mx-watch-substring command must be classified and allowed, got exit $rc"
  pass "transport prefilter is a strict superset: non-mx-watch fast-allows, every mx-watch and quoting-decoder-marker command reaches the classifier"
}
...
test_failopen_missing_jq() {
  ...
  dir=$(mx_test_tmproot mx-arm-pretool-check)
  fakebin="$dir/fakebin"
  mkdir -p "$fakebin"
  local tool
  ...
  PATH="$fakebin" bash -c "printf '%s' '{\"tool_input\":{\"command\":\"bin/mx-watch-arm.sh &\"}}' | '$CHECK'" >/dev/null 2>&1
  rc=$?
...
test_claude_mode_stdout_empty_on_deny() {
  ...
  stderr_file=$(mktemp "${TMPDIR:-/tmp}/mx-arm-pretool-check-claude-stderr.XXXXXX")
  out=$("$CHECK" --claude --command 'bin/mx-watch-arm.sh &' 2>"$stderr_file")
  rc=$?
...
test_default_mode_stdout_has_decision_json_on_deny() {
  local out rc
  out=$("$CHECK" --command 'bin/mx-watch-arm.sh &' 2>/dev/null)
  rc=$?
  [ "$rc" -eq 2 ] || fail "default deny must exit 2, got $rc"
  printf '%s' "$out" | jq -e '.decision == "deny"' >/dev/null 2>&1 \
    || fail "default (non-claude) deny must put a decision JSON on stdout: $out"
  pass "default mode: stdout carries a decision JSON on deny"
}

test_allow_is_silent_both_modes() {
  local out1 out2
  out1=$("$CHECK" --command 'exec bin/mx-watch-arm.sh' 2>&1)
  out2=$("$CHECK" --claude --command 'exec bin/mx-watch-arm.sh' 2>&1)
...
test_claude_settings_pretool_hook_wired() {
  ...
  assert_contains "$command" 'mx-arm-pretool-check.sh' "claude pretool hook must invoke the shared checker"
  assert_contains "$command" '--claude' "claude pretool hook must pass --claude so stdout stays empty on deny"
  [ "$command" = '"$CLAUDE_PROJECT_DIR"/bin/mx-arm-pretool-check.sh --claude' ] \
...
test_codex_hooks_pretool_wired() {
  ...
  assert_contains "$command" 'mx-arm-pretool-check.sh' "codex pretool hook must invoke the shared checker"
  assert_contains "$command" 'pwd -P' "codex pretool hook must anchor to the hook process root like the Stop hook does"
  assert_contains "$command" 'printf "%s" "$payload" | "$root/bin/mx-arm-pretool-check.sh"' "codex pretool hook must forward the exact captured payload to the checker"
  local matcher
  matcher=$(jq -r '.hooks.PreToolUse[0].matcher // empty' "$settings")
...
test_pi_extension_carries_pretool_check() {
  ...
  assert_contains "$content" 'mx-arm-pretool-check.sh' "pi extension must invoke the shared checker"
