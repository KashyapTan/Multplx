#!/usr/bin/env bash
# Tests for the local-HEAD daemon sync: every daemon home tracks the
# PRIMARY Multplx checkout's current default-branch commit by a purely LOCAL
# fast-forward (no origin fetch). Two hook points drive it - bin/mx-spawn.sh
# (before launching a daemon) and bin/mx-bootstrap.sh (a startup sweep of
# every live daemon home) - and both share the ff machinery in
# bin/mx-ff-lib.sh.
#
...
TMP_ROOT=$(mx_test_tmproot mx-daemon-sync)
export MX_BACKEND=tmux

# --- world builders --------------------------------------------------------

# new_world <name>: a PRIMARY Multplx repo on `main` with one commit (the
...
new_world() {
  ...
  # Fresh watcher beacon keeps mx-guard quiet for the spawn path.
  touch "$w/home/state/.last-watcher-beat"

  git init -q -b main "$w/main"
  # Mirror the real repo: the gitignored operational dirs never dirty a worktree,
  # so a daemon home's data/state/projects can never block its fast-forward.
...
# add_sm_worktree <w> <id> <commit>: a daemon home as a DETACHED worktree of
# the primary at <commit>, plus its seed marker and a LIVE kind=daemon meta
# (a window= makes it a running daemon).
add_sm_worktree() {
...
# the tracked .gitignore and commit it on main. The marker (.mx-daemon-home)
# is broker-generic, written by bin/mx-home-seed.sh into every seeded home; once
...
ignore_marker_commit() {
  local w=$1
  printf '.mx-daemon-home\n' >> "$w/main/.gitignore"
  git -C "$w/main" add -A
  git -C "$w/main" commit -qm "gitignore seed marker"
}

# seed_marked_home <w> <id> <commit>: a daemon home matching what
# bin/mx-home-seed.sh actually lays down - a detached worktree at <commit>, the
# seed marker, a live kind=daemon meta, and the gitignored operational dirs
...
# status line. Uses allow_detached=yes, ignore_seed_marker=yes (the daemon
# home contract).
FF_OUT=""
run_ff() {
...
# A daemon home carrying its own in-flight work sits on a named feature
...
test_sweep_nudge_requires_instruction_change() {
  ...
  MX_ROOT="$w/main" MX_HOME="$w/home"
  FF_NUDGE_WINDOWS=""
  FF_SEEN_HOMES=""
  sweep_live_daemon_metas "$w/home/state" "$base" yes >/dev/null
...
test_bootstrap_sweep_nudges_only_instruction_change() {
  ...
  # A home with NO live meta must never be swept (live = a running daemon).
  git -C "$w/main" worktree add -q --detach "$w/sm-nonlive" "$c1"
  printf 'sm-nonlive\n' > "$w/sm-nonlive/.mx-daemon-home"
...
test_bootstrap_nudge_send_uses_state_override() {
  ...
  out=$(PATH="$fakebin:$BASE_PATH" MX_HOME="$w/home" MX_ROOT_OVERRIDE="$w/main" \
    MX_STATE_OVERRIDE="$override_state" MX_SEND_SETTLE=0 MX_FAKE_TMUX_LOG="$log" \
    "$ROOT/bin/mx-bootstrap.sh" 2>/dev/null)
  ...
  pass "T8a bootstrap nudge send respects MX_STATE_OVERRIDE"
}
...
test_bootstrap_nudge_retry_rejects_malformed_marker_id() {
  ...
  printf '../escape\n' > "$evil/.mx-daemon-home"
  mkdir -p "$w/home/state/.daemon-nudge-pending"
  marker="$w/home/state/.daemon-nudge-pending/bad.pending"
  {
    printf 'id=../escape\n'
    printf 'selector=mx-../escape\n'
    printf 'home=%s\n' "$evil"
    printf 'commit=%s\n' "$c1"
    printf 'instructions=AGENTS.md\n'
    printf 'message=broker was updated to the latest - please re-read your AGENTS.md to pick up the new instructions.\n'
  } > "$marker"
  {
    printf 'window=broker:mx-evil\n'
    printf 'kind=daemon\n'
    printf 'home=%s\n' "$evil"
  } > "$w/home/escape.meta"
  fakebin=$(make_fake_toolchain "$w")
  ...
  assert_absent "$log" "malformed retry marker should not invoke mx-send"
...
test_bootstrap_nudge_retry_is_idempotent() {
  ...
  out2=$(PATH="$fakebin:$BASE_PATH" MX_HOME="$w/home" MX_ROOT_OVERRIDE="$w/main" \
    MX_SEND_SETTLE=0 "$ROOT/bin/mx-bootstrap.sh" 2>/dev/null)
...
# --- T8b: stale herdr nudge failures retry through current mx-<id> metadata ---
# Reproduces the 2026-07-07 session-start bug: daemon_sync used to print raw
# backend targets (default:w9:pY) that liveness respawn immediately replaced
# (default:wA:p2), so mx-send with the printed target fell back to tmux and failed
# while mx-<id> resolved through current meta.
...
SH
  chmod +x "$spawn_stub"
  cp "$spawn_stub" "$w/main/bin/mx-spawn.sh"
  ...
  out=$(PATH="$herdrfb:$toolchain:$BASE_PATH" HERDR_ENV=1 MX_BACKEND=herdr \
    MX_SEND_SETTLE=0 \
  ...
  resolved=$(bash -c '. "$0/bin/mx-backend.sh"; mx_backend_resolve_selector mx-sm-instr "$1"' "$ROOT" "$w/home/state")
  [ "$resolved" = "$fresh" ] || fail "mx-<id> should resolve through post-respawn meta, got '$resolved'"

  # shellcheck disable=SC2016  # $0/$1 belong to the inner bash -c process.
  stale_send=$(PATH="$herdrfb:$toolchain:$BASE_PATH" bash -c \
    '. "$0/bin/mx-backend.sh"; mx_backend_source herdr; mx_backend_herdr_send_literal "$1" "nudge"' "$ROOT" "$stale" 2>/dev/null; printf '%s' "$?")
  [ "$stale_send" != 0 ] || fail "explicit stale herdr endpoint send should fail"

  # shellcheck disable=SC2016  # $0/$1 belong to the inner bash -c process.
  fresh_send=$(PATH="$herdrfb:$toolchain:$BASE_PATH" bash -c \
    '. "$0/bin/mx-backend.sh"; mx_backend_source herdr; mx_backend_herdr_send_literal "$1" "nudge"' "$ROOT" "$fresh" 2>/dev/null; printf '%s' "$?")
  [ "$fresh_send" = 0 ] || fail "send through mx-<id>-resolved fresh endpoint should succeed"

  pass "T8b stale herdr nudge failures leave a retry marker after respawn rotates mx-<id> metadata"
}

# --- T9: bootstrap surfaces a skipped dirty live daemon home --------------
test_bootstrap_sweep_surfaces_skipped_home() {
  ...
  pass "T9 bootstrap surfaces a skipped dirty live daemon home"
}

# --- T10: spawning a daemon fast-forwards its worktree before launch ------
...
SH
  ...
  PATH="$fakebin:$BASE_PATH" TMUX='' \
    ...
    MX_SPAWN_NO_GUARD=1 \
    "$ROOT/bin/mx-spawn.sh" sm "$w/sm" codex --daemon >/dev/null 2>&1 || true

  [ "$(head_of "$w/sm")" = "$c2" ] \
    || fail "spawn did not fast-forward the daemon worktree to the primary's HEAD"
  pass "T10 spawn fast-forwards a daemon worktree to the primary's local HEAD before launch"
}
...
# The seed marker used to leave every home permanently dirty: bin/mx-system-sync.sh
...
test_seed_marker_clean_when_gitignored() {
  ...
  # The exact dirtiness signal bin/mx-system-sync.sh reads (its line: dirty=yes when
  ...
  # And the daemon ff sweep sees no dirt: an at-HEAD home is a clean no-op.
  run_ff "$w/sm" "$base"
  [ "$FF_STATUS" = current ] || fail "fresh home not read as clean/current, got '$FF_STATUS': $FF_OUT"
  pass "T12 gitignored marker: a freshly seeded home reads clean to system-sync and the ff sweep"
}
...
# Standalone-clone homes converge through /updatemultplx's origin fetch instead.
...
# --- T15: the delivered Multplx repo gitignores the seed marker -----------------
...
test_repo_gitignores_seed_marker() {
  grep -qxF '.mx-daemon-home' "$ROOT/.gitignore" \
    || fail "the Multplx repo .gitignore must ignore the seed marker (.mx-daemon-home)"
  pass "T15 the Multplx repo gitignores the daemon seed marker"
}
...
echo "# all mx-daemon-sync tests passed"
