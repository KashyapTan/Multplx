...
#     A maintainer-relevant status must deliver exactly ONE sentinel-prefixed,
...
# private socket. The daemon points at a throwaway state dir (MX_STATE_OVERRIDE)
# and the test pane (MX_SUPERVISOR_TARGET). Nothing touches the live system.
# MX_SUPERVISOR_BACKEND=tmux is passed explicitly (not left to auto-detection):
...
STATE_DIR=$(mktemp -d "${TMPDIR:-/tmp}/mx-afk-e2e.XXXXXX")
mkdir -p "$STATE_DIR"
LOG_FILE="$STATE_DIR/submitted.log"
: > "$LOG_FILE"

# Source the daemon to get MX_INJECT_MARK, afk_enter, afk_exit.
...
TMUX_SHIM_DIR=$(mktemp -d "${TMPDIR:-/tmp}/mx-shim.XXXXXX")
...
# Create a fake actor window (the watcher lists mx-* windows for stale
# detection). The pane is an inert shell - it just needs to exist.
"$REAL_TMUX" -L "$SOCKET" new-window -d -n mx-fake-c1 -t supervisor

start_daemon() {
  PATH="$TMUX_SHIM_DIR:$PATH" \
  MX_STATE_OVERRIDE="$STATE_DIR" \
  MX_SUPERVISOR_TARGET="$SUPERVISOR_PANE" \
  MX_SUPERVISOR_BACKEND=tmux \
...
test_scenario_a() {
  ...
  # maintainer returning and starting to type before afk has been cleared.
  ...
  # Write a maintainer-relevant status to trigger a real escalation through the
  # real watcher child.
...
test_scenario_b() {
  ...
  # Write a maintainer-relevant status to trigger a real escalation.
...
# No human input, no swallowed Enter: a maintainer-relevant status must produce
...
test_scenario_c() {
  ...
  stop_daemon
  pass "Scenario C: a normal maintainer status injects exactly one clean single-line sentinel digest"
}
