...
test_spawn_daemon_harness_model_token() {
  ...
  assert_contains "$launch" "claude --dangerously-skip-permissions" \
    "model-token: launch did not invoke claude"
...
test_spawn_daemon_harness_model_and_effort_tokens() {
  ...
  launch=$(cat "$launchlog")
  assert_contains "$launch" "claude --dangerously-skip-permissions" \
    "model-effort-tokens: launch did not invoke claude"
  assert_contains "$launch" "--model 'opus'" \
    "model-effort-tokens: launch did not carry --model opus"
  assert_contains "$launch" "--effort 'high'" \
    "model-effort-tokens: launch did not carry --effort high"
...
test_spawn_explicit_harness_does_not_inherit_daemon_harness_tokens() {
  ...
  assert_contains "$launch" "--dangerously-bypass-approvals-and-sandbox" \
    "explicit-harness-no-tokens: launch omitted codex's sandbox flag"
...
SH
  ...
  # Successful post-respawn delivery may consume the generation immediately,
  # while an incomplete fake delivery leaves it for the retry tests that own
  # that contract. This case owns only the ordering proved by the log above.
