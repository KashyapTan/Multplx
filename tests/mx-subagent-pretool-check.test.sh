...
TMP_ROOT=$(mx_test_tmproot mx-subagent-pretool-tests)
...
BRIEF_ONLY_ROUTE='first classify the work under the AGENTS.md intake contract, then use bin/mx-brief.sh followed by bin/mx-spawn.sh for dispatched work'
SCOUT_ROUTE='first classify the work under the AGENTS.md intake contract: work already classified as a scout goes to bin/mx-scout.sh "<question>" [project], while authorized delivery work and its bounded research go to bin/mx-brief.sh then bin/mx-spawn.sh'
...
# This inventory is shape-classification coverage for the delivered guard and the
# recommended local Claude deny-list hardening list, but tracked settings must
# not land that Claude-only permissions layer.
...
run_tool() {
  ...
  : > "$OUT"
  : > "$ERR"
  env MX_ROOT_OVERRIDE="$PRIMARY" MX_HOME="$PRIMARY" MX_STATE_OVERRIDE="$STATE" "$@" \
...
test_tracked_settings_do_not_ship_permissions_deny() {
  ...
  pass "tracked Claude settings do not land permissions.deny"
}
...
test_guard_never_classifies_mcp_tools() {
  ...
  # has nothing to do with system dispatch.
  local tool
...
test_deny_message_defers_to_intake_classification() {
  local actual
  printf '#!/usr/bin/env bash\n' > "$PRIMARY/bin/mx-scout.sh"
  run_tool Agent && fail "scout-present case must still deny"
  actual=$(jq -r '.systemMessage' "$ERR")
  case "$actual" in
    *"$SCOUT_ROUTE"*) ;;
    *) fail "deny must reserve bin/mx-scout.sh for classified scout work: $actual" ;;
  esac
  case "$actual" in
    *'investigation or diagnosis goes to bin/mx-scout.sh'*) fail "deny must not classify all investigation or diagnosis as scout work: $actual" ;;
  esac
  rm -f "$PRIMARY/bin/mx-scout.sh"
  run_tool Agent && fail "scout-absent case must still deny"
  actual=$(jq -r '.systemMessage' "$ERR")
  case "$actual" in
    *"$BRIEF_ONLY_ROUTE"*) ;;
    *) fail "deny must degrade to brief-then-spawn when mx-scout.sh is absent: $actual" ;;
  esac
  pass "deny defers to intake classification and degrades gracefully without mx-scout.sh"
}

test_escape_hatch_allows_deliberate_use() {
  local rc value
  expect_allow "escape hatch set" Agent MX_ALLOW_SUBAGENT=1
  expect_deny "escape hatch unset" Agent
  for value in '' 0 yes true 11; do
    rc=0
    run_tool Agent "MX_ALLOW_SUBAGENT=$value" || rc=$?
    [ "$rc" -eq 2 ] || fail "MX_ALLOW_SUBAGENT='$value' must not release the guard, got exit $rc"
  done
  pass "the single documented escape hatch releases the guard only on the exact opt-in value"
}

test_task_worktree_and_non_broker_repo_are_inert() {
  ...
  : > "$OUT"
  : > "$ERR"
  MX_ROOT_OVERRIDE="$child" MX_HOME="$child" MX_STATE_OVERRIDE="$child/state" \
    "$CHECK" --claude --tool Agent > "$OUT" 2> "$ERR" || rc=$?
  [ "$rc" -eq 0 ] || fail "an actor task worktree must be out of scope, got exit $rc: $(cat "$ERR")"
  ...
  git -C "$plain" init -q
  rc=0
  MX_ROOT_OVERRIDE="$plain" MX_HOME="$plain" MX_STATE_OVERRIDE="$plain/state" \
    "$CHECK" --claude --tool Agent > "$OUT" 2> "$ERR" || rc=$?
  [ "$rc" -eq 0 ] || fail "a non-Multplx repo must be out of scope, got exit $rc"
  pass "the guard is inert in an actor task worktree and in a non-Multplx repo"
}

test_daemon_home_is_in_scope() {
  local second="$TMP_ROOT/second" rc=0
  git -C "$PRIMARY" worktree add -q -b fixture-second "$second"
  mkdir -p "$second/bin" "$second/state"
  printf '# fixture\n' > "$second/AGENTS.md"
  printf 'sm-fixture\n' > "$second/.mx-daemon-home"
  MX_ROOT_OVERRIDE="$second" MX_HOME="$second" MX_STATE_OVERRIDE="$second/state" \
    "$CHECK" --claude --tool Agent > "$OUT" 2> "$ERR" || rc=$?
  [ "$rc" -eq 2 ] || fail "a marked daemon home operates a system and must be guarded, got exit $rc"
  pass "a marked daemon home is guarded even though it is a linked worktree"
}
...
test_stdin_transports_and_output_shapes() {
  ...
  printf '%s' '{"tool_name":"Agent","tool_input":{"prompt":"go"}}' \
    | MX_ROOT_OVERRIDE="$PRIMARY" MX_HOME="$PRIMARY" MX_STATE_OVERRIDE="$STATE" \
...
test_missing_jq_stdin_transport_fails_open() {
  ...
  printf '%s' '{"tool_name":"Agent"}' \
    | env PATH="$fakebin" MX_ROOT_OVERRIDE="$PRIMARY" MX_HOME="$PRIMARY" MX_STATE_OVERRIDE="$STATE" \
      "$CHECK" --claude > "$OUT" 2> "$ERR" || rc=$?
...
test_claude_hook_registration_preserves_bash_seatbelts() {
  jq -e '
    [.hooks.PreToolUse[] | .hooks[].command]
      | any(contains("mx-subagent-pretool-check.sh --claude"))
  ...
  jq -e '
    [.hooks.PreToolUse[] | select(.hooks[].command | contains("mx-subagent-pretool-check.sh")) | .matcher] | .[0]
      | . == ".*"
