...
OWNER="$ROOT/bin/mx-operational-input.sh"
...
test_current_generic_matrix() {
  ...
  prefix_hex=$(printf '%s' "$MX_OPERATIONAL_PREFIX" | od -An -tx1 | tr -d ' \n')
  [ "$prefix_hex" = e281a34d554c54504c585f4f503a20 ] \
    || fail "current operational prefix lost the landed U+2063 MULTPLX_OP bytes: $prefix_hex"

  for kind in session-start watcher turn-end-guard away-supervisor launch-brief; do
    body="CURRENT_BODY_FOR_${kind}"
    mx_operational_input_encode "$kind" "$body" encoded \
    ...
    mx_operational_input_body "$encoded" stripped \
...
test_landed_untyped_prefix_is_explicitly_legacy() {
  ...
  untyped="${MX_OPERATIONAL_PREFIX}body whose historical subtype is unknowable"
  mx_legacy_operational_input_kind "$untyped" parsed \
    || fail "landed untyped MULTPLX_OP input was not retained"
  [ "$parsed" = legacy-operational ] \
    || fail "landed untyped MULTPLX_OP input falsely became $parsed"
  ! mx_operational_input_kind "$untyped" parsed \
    || fail "untyped MULTPLX_OP input passed the current typed parser"
  [ "$(classify_cli "$untyped")" = legacy-operational ] \
    || fail "CLI did not expose the untyped prefix as legacy-operational"
  pass "operational input: untyped landed MULTPLX_OP transcripts are explicit legacy-operational input"
}

test_isolated_legacy_matrix() {
  local watcher turnend away parsed
  watcher="${MX_LEGACY_WATCHER_PREFIX}signal: legacy${MX_LEGACY_WATCHER_SUFFIX}"
  turnend="${MX_LEGACY_TURNEND_PREFIX}watcher: FAILED - legacy"
  away="${MX_LEGACY_AWAY_PREFIX}1 event(s)): done: legacy"

  for fixture in \
    "session-start|$MX_LEGACY_SESSIONSTART" \
    "watcher|$watcher" \
  ...
  do
    expected=${fixture%%|*}
    message=${fixture#*|}
    ! mx_operational_input_kind "$message" parsed \
      || fail "legacy $expected fixture leaked into the current parser"
    mx_legacy_operational_input_kind "$message" parsed \
...
test_genuine_near_misses_remain_unclassified() {
  ...
  marker=$MX_OPERATIONAL_MARK
  while IFS= read -r fixture || [ -n "$fixture" ]; do
    [ -n "$fixture" ] || continue
    ! mx_operational_input_classify "$fixture" parsed \
...
Maintainer quote: ${MX_OPERATIONAL_PREFIX}v1 watcher
MULTPLX_OP: v1 watcher
$marker arbitrary maintainer text
Maintainer quote: $MX_LEGACY_SESSIONSTART
${MX_LEGACY_SESSIONSTART} Please explain this sentence.
