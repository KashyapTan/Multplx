#!/usr/bin/env bash
# Behavior tests for mx-spawn.sh batch dispatch (`id=repo` pairs).
#
...
# the tests create no windows or worktrees. MX_SPAWN_NO_GUARD=1 keeps them off the
...
TMP_ROOT=$(mx_test_tmproot mx-spawn-batch)
export MX_BACKEND=tmux

# Clear ambient broker overrides so the behavior test owns its environment.
run_spawn() {
  MX_ROOT_OVERRIDE='' \
    MX_HOME='' \
    MX_STATE_OVERRIDE='' \
...
# A projects/ path is resolved through the Multplx home, never the caller cwd,
...
test_projects_path_scoping() {
  ...
  while IFS='|' read -r label use_override id; do
    ...
    if [ "$use_override" = yes ]; then
      out=$(MX_ROOT_OVERRIDE='' MX_STATE_OVERRIDE='' MX_DATA_OVERRIDE='' MX_CONFIG_OVERRIDE='' \
        MX_HOME="$home" MX_PROJECTS_OVERRIDE="$projects" MX_SPAWN_NO_GUARD=1 \
        "$SPAWN" "$id" projects/alpha codex 2>&1)
    else
      mkdir -p "$home/projects/alpha"
      out=$(MX_ROOT_OVERRIDE='' MX_STATE_OVERRIDE='' MX_DATA_OVERRIDE='' MX_PROJECTS_OVERRIDE='' MX_CONFIG_OVERRIDE='' \
        MX_HOME="$home" MX_SPAWN_NO_GUARD=1 \
        "$SPAWN" "$id" projects/alpha codex 2>&1)
    fi
    status=$?
...
MX_HOME scopes projects/|no|nope-home-z7
MX_PROJECTS_OVERRIDE scopes projects/|yes|nope-override-z8
ROWS
  pass "projects/ paths are scoped through the Multplx home for single-task spawn"
}
