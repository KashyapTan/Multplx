#!/usr/bin/env bash
# Behavior tests for the no-mistakes GATE-agent system-lifecycle refusal.
#
# A confused no-mistakes gate agent runs inside a Multplx checkout, adopts the
# maintainer identity from AGENTS.md, and reaches for mx-spawn/mx-send/mx-teardown.
# bin/mx-gate-refuse-lib.sh is the broker capability-removal half: sourced at
# the top of those three entrypoints and called before any system mutation, it
...
# A normal broker session (real primary, real actors worktree) has NEITHER
...
#
# Finally, assert broker's TRACKED .no-mistakes.yaml parses and sets
...
TMP=$(mx_test_tmproot mx-gate-refuse)
...
# normal primary/actors checkout: its git-common-dir is <dir>/.git, never a gate.
make_normal_repo() {
  local dir=$1
...
run_guard_lib() {
  ...
  (
    cd "$cwd" || exit 111
    unset NO_MISTAKES_GATE MX_GATE_REFUSE_BYPASS
    case "$marker" in
...
test_helper_env_marker_refuses() {
  ...
  pass "mx-gate-refuse-lib: refuses when NO_MISTAKES_GATE is set"
}
...
test_helper_empty_env_marker_refuses() {
  ...
  pass "mx-gate-refuse-lib: refuses when NO_MISTAKES_GATE is set empty"
}
...
test_helper_path_backstop_refuses() {
  ...
  pass "mx-gate-refuse-lib: refuses from a gate worktree via git-common-dir (marker unset)"
}
...
test_helper_normal_is_noop() {
  ...
  pass "mx-gate-refuse-lib: no-op for a normal session (neither signal, set -eu clean)"
}

# --- mx-spawn ---------------------------------------------------------------

# A fake tmux/treehouse so mx-spawn resolves the actors worktree from a controlled
# pane path and completes without a live terminal (mirrors tests/mx-tangle-guard).
make_spawn_fakebin() {
  local dir=$1 fakebin
...
run_spawn() {
  ...
  ( cd "$cwd" && env -u NO_MISTAKES_GATE -u MX_GATE_REFUSE_BYPASS \
      ...
      "MX_SPAWN_NO_GUARD=1" "MX_FAKE_PANE_PATH=$pane" "TMUX=fake,1,0" \
      "PATH=$fakebin:$PATH" "$@" \
...
test_spawn_refuses_and_admits() {
  ...
  pass "mx-spawn: refuses on marker and gate-worktree backstop; a normal actor spawn is unaffected"
}

# --- mx-send ----------------------------------------------------------------

# A fake tmux that logs send-keys to MX_TMUX_LOG and reports live endpoints
# (mirrors tests/mx-send-strict), so a successful send is observable and a
...
run_send() {
  ...
  ( cd "$cwd" && env -u NO_MISTAKES_GATE -u MX_GATE_REFUSE_BYPASS \
      "PATH=$fakebin:$PATH" "MX_HOME=$home" "MX_ROOT_OVERRIDE=$home" \
      "MX_TMUX_LOG=$log" "MX_SEND_SETTLE=0" "$@" \
...
test_send_refuses_and_admits() {
  ...
  # env-marker refuse.
  : > "$log"
  out=$(run_send "$NORMAL_CWD" "$home" "$fakebin" "$log" mx-lane-ok "hello maintainer" NO_MISTAKES_GATE=1); rc=$?
  ...
  : > "$log"
  out=$(run_send "$GATE_WT" "$home" "$fakebin" "$log" mx-lane-ok "hello maintainer"); rc=$?
  ...
  # no-regression.
  : > "$log"
  out=$(run_send "$NORMAL_CWD" "$home" "$fakebin" "$log" mx-lane-ok "hello maintainer"); rc=$?
  ...
  assert_contains "$(cat "$log")" "target=sess:mx-lane-ok literal=1 arg=hello maintainer" "send: normal send should type the text"
  pass "mx-send: refuses on marker and gate-worktree backstop; a normal steer is unaffected"
}

# --- mx-teardown ------------------------------------------------------------

# make_teardown_case <name> -> echoes a case dir holding a LANDED no-mistakes delivery
# task (HEAD reachable from origin), so a normal teardown genuinely succeeds and a
# refused one leaves the task untouched (mirrors tests/mx-teardown make_case).
make_teardown_case() {
...
SH
  ...
  git -C "$case_dir/project" worktree add -q -b mx/task-x1 "$case_dir/wt" main
  ...
  mx_write_meta "$case_dir/state/task-x1.meta" \
    "window=mx-task-x1" "worktree=$case_dir/wt" "project=$case_dir/project" \
    "kind=delivery" "mode=no-mistakes"
...
run_teardown() {
  ...
  ( cd "$cwd" && env -u NO_MISTAKES_GATE -u MX_GATE_REFUSE_BYPASS \
      ...
      "MX_CONFIG_OVERRIDE=$case_dir/config" "PATH=$case_dir/fakebin:$PATH" "$@" \
...
test_teardown_refuses_and_admits() {
  ...
  pass "mx-teardown: refuses on marker and gate-worktree backstop; a normal teardown is unaffected"
}
