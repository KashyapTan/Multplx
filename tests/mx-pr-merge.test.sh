...
# One official-gh mock records merge calls and answers the pr_head lookup.
add_gh_mocks() {
...
test_records_pr_and_head_before_merging() {
  ...
  pass "mx-pr-merge records pr= and pr_head= before invoking gh pr merge"
}
...
test_merge_failure_propagates_after_recording() {
  ...
  expect_code 1 "$rc" "merge-fails: mx-pr-merge should propagate the gh merge failure"
...
test_extra_merge_args_forwarded() {
  ...
  pass "mx-pr-merge forwards extra flags to gh pr merge after the -- separator"
}
...
test_malformed_url_refuses_before_merge() {
  ...
  pass "mx-pr-merge refuses malformed PR URLs before calling gh"
}
...
test_method_equals_merge_method_not_overridden() {
  ...
  grep -qxF 'pr merge 23 --repo example/repo --merge' "$case_dir/gh.log" \
    || fail "method-equals-merge-method: legacy --method=merge was not normalized to official gh --merge"
  pass "mx-pr-merge normalizes legacy --method=<value> onto the official gh flag"
}
