...
# Mirrors tests/mx-backend-herdr-smoke.test.sh and tests/herdr-test-safety.sh's
...
# at MX_SUPERVISOR_BACKEND=herdr, MX_SUPERVISOR_TARGET="<session>:<pane-id>",
...
# bordered branch of mx_backend_herdr_composer_state, and logs every submitted
# line (hex + text + injection/user classification) - the same technique
# tests/mx-afk-inject-e2e.test.sh uses for its tmux supervisor pane, so this
...
# mx_backend_herdr_send_text_submit's confirmation is native agent-state
...
cleanup_all() {
  if [ -n "${DAEMON_PID:-}" ]; then
    ...
    mx_test_stop_process "$DAEMON_PID"
  fi
...
# --- source the daemon (for afk_enter/afk_exit/MX_INJECT_MARK) + the backend -
...
STATE_DIR=$(mktemp -d "${TMPDIR:-/tmp}/mx-afk-herdr-e2e.XXXXXX")
mkdir -p "$STATE_DIR"
...
TASK_IDS=$(mx_backend_herdr_create_task "$CONTAINER" "mx-afk-e2e-supervisor" /tmp "$SEEDED_TAB_ID") \
...
for _ in $(seq 1 100); do
  PROCESS_INFO=$(mx_backend_herdr_cli "$SESSION" pane process-info --pane "$PANE_ID" 2>/dev/null || true)
...
# e2e's fake mx-fake-c1 actor window - not required by scan_signals (which
...
FAKE_ACTOR_IDS=$(mx_backend_herdr_create_task "$CONTAINER" "mx-fake-c1" /tmp) \
  || fail "could not create the fake actor scratch tab"
read -r _FAKE_TAB_ID FAKE_ACTOR_PANE_ID <<EOF
$FAKE_ACTOR_IDS
EOF

# --- deterministic bordered-composer loop, drawn in the scratch pane ---------
# Mirrors tests/mx-afk-inject-e2e.test.sh's supervisor-loop.sh, but draws a
# "│ > <buf> │" border so the bordered branch of
# mx_backend_herdr_composer_state recognizes it, exactly like a bordered-TUI
...
# submission: mx_backend_herdr_send_text_submit's confirmation is now native
...
MARK=$'\xE2\x81\xA3'
LOG="$1"
AGENT_SOURCE=mx-test-supervisor
AGENT_LABEL=mx-test-supervisor
...
submit_line() {
  ...
  # mx_backend_herdr_send_text_submit now confirms against. The 0.6s "working"
  # window comfortably covers the daemon's MX_INJECT_CONFIRM_SLEEP=0.5
...
mx_backend_herdr_send_text_line "$SUPERVISOR_TARGET" "bash '$LOOP_SCRIPT' '$LOG_FILE'" \
...
HERDR_SHIM_DIR=$(mktemp -d "${TMPDIR:-/tmp}/mx-herdr-shim.XXXXXX")
...
start_daemon() {
  ...
  MX_SUPERVISOR_BACKEND=herdr \
  MX_SUPERVISOR_TARGET="$SUPERVISOR_TARGET" \
  MX_ESCALATE_BATCH_SECS=0 \
  MX_HOUSEKEEPING_TICK=1 \
  MX_POLL=1 \
  MX_SIGNAL_GRACE=1 \
  MX_HEARTBEAT=999999 \
  MX_CHECK_INTERVAL=999999 \
  MX_INJECT_CONFIRM_SLEEP=0.5 \
  MX_INJECT_CONFIRM_RETRIES=6 \
  MX_STALE_ESCALATE_SECS=999999 \
...
# Verify pane_input_pending (dispatched through mx_backend_composer_state for
...
selfcheck_pane_input_pending() {
  ...
  mx_backend_herdr_capture "$SUPERVISOR_TARGET" 10 | sed 's/^/    /' >&2
  mx_backend_herdr_send_key "$SUPERVISOR_TARGET" Enter
...
test_scenario_c() {
  ...
  stop_daemon
  pass "real herdr Scenario C: a normal maintainer status injects exactly one clean single-line sentinel digest"
}
...
# must never crash or hot-loop. Exercises mx_backend_composer_state(herdr, ...)
...
test_scenario_d_max_defer() {
  ...
  mx_backend_herdr_send_literal "$SUPERVISOR_TARGET" "stuck-in-the-box"
  sleep 0.5
  ...
  MX_CHECK_INTERVAL=999999 \
  MX_MAX_DEFER_SECS=3 \
  MX_INJECT_CONFIRM_SLEEP=0.3 \
  MX_INJECT_CONFIRM_RETRIES=2 \
  MX_STALE_ESCALATE_SECS=999999 \
  nohup "$DAEMON" >"$STATE_DIR/daemon.out" 2>"$STATE_DIR/daemon.err" &
  DAEMON_PID=$!
  ...
  mx_backend_herdr_send_key "$SUPERVISOR_TARGET" C-c >/dev/null 2>&1 || true
...
mx_backend_herdr_kill "$SUPERVISOR_TARGET" 2>/dev/null || true
mx_backend_herdr_kill "$SESSION:$FAKE_ACTOR_PANE_ID" 2>/dev/null || true
cleanup_all
trap - EXIT
