...
# mx-send swallow reporting, composer-pending ANSI parsing). The operator-visible
# inject flow lives in mx-afk-inject-e2e and mx-wake-daemon-lifecycle-e2e.
set -u

# shellcheck source=tests/wake-helpers.sh
. "$(dirname "${BASH_SOURCE[0]}")/wake-helpers.sh"

DAEMON="$ROOT/bin/mx-supervise-daemon.sh"
AFK_START="$ROOT/bin/mx-afk-start.sh"
...
if [ -z "${MX_TEST_DAEMON_SOURCED:-}" ]; then
  export MX_TEST_DAEMON_SOURCED=1
  # shellcheck source=bin/mx-supervise-daemon.sh
  . "$DAEMON"
fi

TMP_ROOT=$(mx_test_tmproot mx-daemon-tests)
...
test_afk_start_refuses_when_flag_cannot_be_written() {
  ...
  out=$(MX_STATE_OVERRIDE="$state" MX_SUPERVISOR_BACKEND=unsupported "$AFK_START" 2>&1)
  status=$?

  [ "$status" -ne 0 ] || fail "mx-afk-start.sh should fail when state/.afk cannot be written"
  assert_not_contains "$out" "starting supervise daemon" "mx-afk-start.sh continued into daemon startup after .afk write failure"
  assert_absent "$state/.supervise-daemon.log" "mx-afk-start.sh started the daemon after .afk write failure"
  pass "mx-afk-start.sh fails before daemon startup when the afk flag cannot be written"
}
...
test_afk_start_ignores_stale_pidfile_without_lock() {
  ...
  [ "$status" -ne 0 ] || fail "mx-afk-start.sh should attempt daemon startup instead of trusting a pidfile-only live pid"
  assert_contains "$out" "starting supervise daemon" "mx-afk-start.sh did not attempt daemon startup"
  assert_contains "$out" "does not support supervisor backend 'unsupported'" "daemon startup did not reach backend validation"
  assert_not_contains "$out" "daemon already running" "mx-afk-start.sh trusted a stale pidfile-only live pid"
  pass "mx-afk-start.sh ignores stale pidfile-only live pids"
}
...
test_afk_start_reclaims_stale_daemon_lock_reused_pid() {
  ...
  [ "$status" -ne 0 ] || fail "mx-afk-start.sh should attempt daemon startup after rejecting a reused-pid lock"
  assert_contains "$out" "starting supervise daemon" "mx-afk-start.sh did not attempt daemon startup after rejecting the stale lock"
  ...
  assert_not_contains "$out" "daemon already running" "mx-afk-start.sh trusted a stale daemon lock with a reused pid"
  assert_not_contains "$out" "another mx-supervise-daemon is already running" "daemon singleton lock still trusted the reused pid"
  pass "mx-afk-start.sh reclaims stale daemon locks whose live pid identity no longer matches"
}

test_daemon_state_root_uses_mx_home() {
  local dir home override out
  dir=$(make_supercase daemon-mx-home)
  home="$dir/broker-home"
  override="$dir/override-state"
  mkdir -p "$home" "$override"

  out=$(MX_HOME="$home" MX_STATE_OVERRIDE='' _state_root)
  [ "$out" = "$home/state" ] || fail "daemon state root ignored MX_HOME: $out"

  out=$(MX_HOME="$home" MX_STATE_OVERRIDE="$override" _state_root)
  [ "$out" = "$override" ] || fail "daemon state root ignored MX_STATE_OVERRIDE: $out"

  pass "supervise daemon state root is scoped by MX_HOME"
}
...
test_classify_routine_signal_self() {
  ...
  out=$(MX_STATE_OVERRIDE="$state" classify_signal "$state/foo-x1.status" "$state")
...
test_classify_terminal_signal_escalates() {
  ...
  for kw in "done: PR https://x/y/pull/1" "needs-decision: pick A" "blocked: no perms" \
            ...
            "ready in branch mx/t1" "merged"; do
    printf 'working\n%s\n' "$kw" > "$state/t.status"
    out=$(MX_STATE_OVERRIDE="$state" classify_signal "$state/t.status" "$state")
    case "$out" in escalate\|*) ;; *) fail "maintainer verb did not escalate ($kw): $out" ;; esac
  done
  pass "maintainer-relevant status verbs escalate"
}
...
test_stale_transient_self_records_marker() {
  ...
  stale_marker_record "sess:mx-qux-w4" "$state"
  out=$(MX_STATE_OVERRIDE="$state" classify_stale "sess:mx-qux-w4" "$state")
  case "$out" in self\|*) ;; *) fail "transient stale did not self-handle: $out" ;; esac
  key=$(printf '%s' "$(window_to_task "sess:mx-qux-w4")" | tr ':/.' '___')
...
test_stale_terminal_escalates() {
  ...
  state="$dir/state"
  printf 'done: ready in branch mx/t1\n' > "$state/fin-t5.status"
  out=$(MX_STATE_OVERRIDE="$state" classify_stale "sess:mx-fin-t5" "$state")
  case "$out" in escalate\|*) ;; *) fail "terminal stale did not escalate: $out" ;; esac
  mx_write_meta "$state/herdr-t5.meta" "window=default:w1:p2" "backend=herdr"
  printf 'done: ready in branch mx/herdr\n' > "$state/herdr-t5.status"
  out=$(MX_STATE_OVERRIDE="$state" classify_stale "default:w1:p2" "$state")
...
test_stale_paused_classifies_pause() {
  ...
  status_is_maintainer_relevant "$pause_reason" && fail "pause reason phrases made the status maintainer-relevant"
  printf '%s\n' "$pause_reason" > "$state/held-w9.status"
  out=$(MX_STATE_OVERRIDE="$state" classify_stale "sess:mx-held-w9" "$state")
  case "$out" in pause\|*) ;; *) fail "declared pause did not classify as pause: $out" ;; esac
  pass "paused reasons with maintainer phrases remain pause-classified"
}
...
test_handle_wake_paused_records_pause_marker() {
  ...
  MX_STATE_OVERRIDE="$state" handle_wake "stale: $win" "$state"
  [ -e "$state/.subsuper-paused-$key" ] || fail "pause marker not recorded by handle_wake"
  [ ! -e "$state/.subsuper-stale-$key" ] || fail "wedge marker not cleared when the actors declared a pause"
...
test_handle_wake_paused_signal_records_pause_marker() {
  ...
  state="$dir/state"
  win="sess:mx-held-w10-signal"
  printf 'window=%s\nkind=delivery\n' "$win" > "$state/held-w10-signal.meta"
  printf 'paused: awaiting the vendor rate-limit reset\n' > "$state/held-w10-signal.status"
  key=$(printf '%s' "held-w10-signal" | tr ':/.' '___')
  date +%s > "$state/.subsuper-stale-$key"
  MX_STATE_OVERRIDE="$state" handle_wake "signal: $state/held-w10-signal.status" "$state"
...
test_handle_wake_terminal_signal_clears_pause_tracking() {
  ...
  MX_STATE_OVERRIDE="$state" handle_wake "signal: $state/held-w10-terminal.status" "$state"
...
test_housekeeping_migrates_watcher_unpaused_marker_to_clear() {
  ...
  state="$dir/state"
  win="sess:mx-held-w10-migrate-unpaused"
...
test_housekeeping_paused_resurfaces_and_resets() {
  ...
  win="sess:mx-held-w11"; pane="$dir/pane.txt"
  printf 'paused: holding for the upstream tool release\n' > "$state/held-w11.status"
  printf 'idle prompt $\n' > "$pane"
  key=$(printf '%s' "held-w11" | tr ':/.' '___')
  echo $(( $(date +%s) - 5000 )) > "$state/.subsuper-paused-$key"
  PATH="$fakebin:$PATH" MX_FAKE_TMUX_WINDOW="$win" MX_FAKE_TMUX_CAPTURE="$pane" \
    MX_STATE_OVERRIDE="$state" MX_PAUSE_RESURFACE_SECS=240 housekeeping "$state"
...
# A pause whose pane became busy again (the actors resumed) drops its marker without
# escalating, exactly like a resumed wedge.
test_housekeeping_paused_resumed_cleared() {
  local dir state fakebin win pane key
  dir=$(make_supercase paused-resumed)
  state="$dir/state"; fakebin="$dir/fakebin"
  win="sess:mx-held-w12"; pane="$dir/pane.txt"
...
# A pane still idle but whose status is no longer a pause (the actors changed state
...
test_housekeeping_paused_unpaused_cleared() {
  ...
  state="$dir/state"; fakebin="$dir/fakebin"
  win="sess:mx-held-w13"; pane="$dir/pane.txt"
  ...
  [ ! -s "$state/.subsuper-escalations" ] || fail "an actor that left its pause was re-surfaced as a pause"
  pass "housekeeping clears a paused marker once the actor is no longer declaring the pause"
}

test_housekeeping_stale_marker_transitions_to_pause() {
  local dir state fakebin win pane key
  dir=$(make_supercase stale-to-paused)
  state="$dir/state"; fakebin="$dir/fakebin"; win="sess:mx-held-w14"; pane="$dir/pane.txt"
  ...
  PATH="$fakebin:$PATH" MX_FAKE_TMUX_WINDOW="$win" MX_FAKE_TMUX_CAPTURE="$pane" \
    MX_STATE_OVERRIDE="$state" MX_STALE_ESCALATE_SECS=240 housekeeping "$state"
...
test_housekeeping_pause_marker_transitions_to_clear() {
  ...
  state="$dir/state"; fakebin="$dir/fakebin"; win="sess:mx-held-w15"; pane="$dir/pane.txt"
  printf 'working: upstream landed, resuming\n' > "$state/held-w15.status"
  printf 'idle prompt $\n' > "$pane"
  key=$(printf '%s' "held-w15" | tr ':/.' '___')
  date +%s > "$state/.subsuper-paused-$key"
  PATH="$fakebin:$PATH" MX_FAKE_TMUX_WINDOW="$win" MX_FAKE_TMUX_CAPTURE="$pane" \
    MX_STATE_OVERRIDE="$state" MX_PAUSE_RESURFACE_SECS=999999 housekeeping "$state"
  [ ! -e "$state/.subsuper-paused-$key" ] || fail "pause marker remained after the actors resumed"
  ...
  pass "housekeeping clears tracking when an actor leaves pause"
}
...
test_housekeeping_herdr_persistent_stale_resolves_meta() {
  ...
  state="$dir/state"
  mx_write_meta "$state/herdr-w7.meta" "window=default:w1:p2" "backend=herdr"
  ...
  (
    ...
    mx_backend_busy_state() {
      [ "$1" = herdr ] || fail "expected herdr busy backend, got $1"
      [ "$2" = "default:w1:p2" ] || fail "expected herdr busy target, got $2"
      printf 'idle'
    }
    mx_backend_capture herdr default:w1:p2 40 >/dev/null
    [ "$(mx_backend_busy_state herdr default:w1:p2)" = idle ] || fail "herdr busy stub did not report idle"
    MX_STATE_OVERRIDE="$state" MX_STALE_ESCALATE_SECS=240 housekeeping "$state"
...
test_housekeeping_herdr_idle_busy_footer_clears_stale() {
  ...
  state="$dir/state"
  mx_write_meta "$state/herdr-footer.meta" "window=default:w1:p4" "backend=herdr"
  ...
  (
    ...
    mx_backend_capture herdr default:w1:p4 40 >/dev/null
    [ "$(mx_backend_busy_state herdr default:w1:p4)" = idle ] || fail "herdr busy stub did not report idle"
    MX_STATE_OVERRIDE="$state" MX_STALE_ESCALATE_SECS=240 housekeeping "$state"
...
test_housekeeping_herdr_resumed_stale_cleared() {
  ...
  state="$dir/state"
  mx_write_meta "$state/herdr-busy.meta" "window=default:w1:p3" "backend=herdr"
  ...
  (
    ...
    mx_backend_capture herdr default:w1:p3 40 >/dev/null
    [ "$(mx_backend_busy_state herdr default:w1:p3)" = busy ] || fail "herdr busy stub did not report busy"
    MX_STATE_OVERRIDE="$state" MX_STALE_ESCALATE_SECS=240 housekeeping "$state"
...
test_escalate_batches_into_one_digest() {
  ...
  afk_enter "$state"
  PATH="$fakebin:$PATH" MX_FAKE_TMUX_PANE_ALIVE=1 MX_FAKE_TMUX_SENT="$sent" \
    MX_FAKE_TMUX_CAPTURE="$capture" MX_ESCALATE_BATCH_SECS=0 escalate_flush "$state" \
    || fail "escalate_flush failed"
  grep -F 'MULTPLX_OP: v1 away-supervisor: ' "$sent" >/dev/null \
...
test_escalate_batch_age_uses_first_append() {
  ...
  afk_enter "$state"
  PATH="$fakebin:$PATH" MX_FAKE_TMUX_PANE_ALIVE=1 MX_FAKE_TMUX_SENT="$sent" \
    MX_FAKE_TMUX_CAPTURE="$capture" MX_ESCALATE_BATCH_SECS=90 MX_HOUSEKEEPING_TICK=0 \
    housekeeping "$state"
  grep -F 'event A: done: PR 1 | event B: done: PR 2' "$sent" >/dev/null \
...
test_handle_wake_routes_self_and_escalate() {
  ...
  MX_STATE_OVERRIDE="$state" handle_wake "signal: $state/h-routine.status" "$state"
  [ -s "$state/.subsuper-escalations" ] && fail "routine signal was escalated by handle_wake"
  printf 'done: PR 1\n' > "$state/h-done.status"
  MX_STATE_OVERRIDE="$state" handle_wake "signal: $state/h-done.status" "$state"
  [ -s "$state/.subsuper-escalations" ] || fail "maintainer signal was not buffered by handle_wake"
  pass "handle_wake routes routine->self and maintainer->escalate"
}
...
test_inject_skip_forces_self() {
  ...
  state="$dir/state"
  printf 'done: PR 1\n' > "$state/s1.status"
  MX_STATE_OVERRIDE="$state" MX_INJECT_SKIP="signal" handle_wake "signal: $state/s1.status" "$state"
  [ -s "$state/.subsuper-escalations" ] && fail "INJECT_SKIP=signal did not force self-handle"
  pass "INJECT_SKIP forces self-handle, bypassing maintainer-relevant classification"
}
...
test_is_wake_reason_distinguishes_status_stdout() {
  ...
  is_wake_reason "stale: s:mx-x" || fail "stale: not recognized as wake"
...
test_signal_escalate_marks_seen_no_catchall_refire() {
  ...
  MX_STATE_OVERRIDE="$state" handle_wake "signal: $state/sig-t8.status" "$state"
  [ -s "$state/.subsuper-escalations" ] || fail "maintainer signal was not escalated"
  key=$(printf '%s' "sig-t8" | tr ':/.' '___')
  [ "$(cat "$state/.subsuper-seen-status-$key" 2>/dev/null || true)" = "done: PR https://x/y/pull/8" ] \
    || fail "maintainer signal escalate did not write the seen-status marker"
  : > "$state/.subsuper-escalations"
  rm -f "$state/.subsuper-last-scan"
  MX_STATE_OVERRIDE="$state" housekeeping "$state"
  [ ! -s "$state/.subsuper-escalations" ] || fail "catch-all scan re-fired an already-escalated signal"
  pass "maintainer signal escalate marks seen so the catch-all scan does not re-fire"
}
...
test_afk_absent_daemon_does_not_inject() {
  ...
  if PATH="$fakebin:$PATH" MX_FAKE_TMUX_PANE_ALIVE=1 MX_FAKE_TMUX_SENT="$sent" \
    MX_FAKE_TMUX_CAPTURE="$capture" MX_ESCALATE_BATCH_SECS=0 escalate_flush "$state"; then
...
test_busy_guard_defers_when_supervisor_busy() {
  ...
  # pane shows a busy signature (broker mid-turn)
  printf 'esc to interrupt\n' > "$capture"
  escalate_add "$state" "done: PR 1"
  afk_enter "$state"
  if PATH="$fakebin:$PATH" MX_FAKE_TMUX_PANE_ALIVE=1 MX_FAKE_TMUX_SENT="$sent" \
...
test_marker_detection() {
  local marker_hex
  marker_hex=$(printf '%s' "$MX_INJECT_MARK" | od -An -tx1 | tr -d ' \n')
  [ "$marker_hex" = e281a3 ] \
    || fail "MX_INJECT_MARK must use terminal-safe U+2063 bytes, got $marker_hex"
  [ "$MX_OPERATIONAL_PREFIX" = "${MX_INJECT_MARK}MULTPLX_OP: " ] \
    || fail "away-mode operational prefix drifted from the shared maintainer-boundary marker"
  # message_is_injection: marker present -> injection; absent -> real message
  message_is_injection "${MX_OPERATIONAL_PREFIX}Supervisor escalate: done" \
    || fail "operationally-prefixed message not detected as injection"
  message_is_injection "${MX_INJECT_MARK}Supervisor escalate: done" \
...
test_afk_turn_exemption() {
  ...
  # a non-/afk skill invocation DOES exit (the maintainer is actively working)
  should_exit_afk "$state" "/deep-review" \
...
test_should_exit_afk_when_afk_inactive() {
  ...
  should_exit_afk "$state" "${MX_INJECT_MARK}test" \
...
test_strip_injection_marker() {
  ...
  mx_operational_input_encode away-supervisor "Supervisor escalate: done" encoded \
  ...
  stripped=$(strip_injection_marker "${MX_OPERATIONAL_PREFIX}Supervisor escalate: done")
  [ "$stripped" = "Supervisor escalate: done" ] \
    || fail "landed untyped operational prefix not stripped: '$stripped'"
  stripped=$(strip_injection_marker "${MX_INJECT_MARK}Supervisor escalate: done")
  [ "$stripped" = "Supervisor escalate: done" ] \
  ...
  # Only marker → empty.
  stripped=$(strip_injection_marker "$MX_INJECT_MARK")
...
test_pane_input_pending_detects_partial_input() {
  ...
  printf 'line one\nline two\nhuman draft text\n' > "$capture"
  PATH="$fakebin:$PATH" MX_FAKE_TMUX_CAPTURE="$capture" MX_FAKE_TMUX_CURSOR_Y=2 \
...
# The safety fix at the tmux classifier (task mx-composer-shellglyph-safety): a
...
test_tmux_composer_state_bare_shell_is_unknown() {
  ...
  for g in '$' '%' '#' '>'; do
    printf 'output\noutput\n%s \n' "$g" > "$capture"
    out=$(PATH="$fakebin:$PATH" MX_FAKE_TMUX_CAPTURE="$capture" MX_FAKE_TMUX_CURSOR_Y=2 \
      mx_tmux_composer_state "fakepane")
    [ "$out" = unknown ] \
      || fail "bare shell prompt '$g' must classify unknown (dead shell, unsafe), got '$out'"
  done
  pass "mx_tmux_composer_state: a bare shell prompt (\$/%/#/>) reads unknown, never empty (dead-shell injection safety)"
}
...
test_tmux_composer_state_bordered_and_agent_rows_are_empty() {
  ...
  fakebin="$dir/fakebin"; capture="$dir/pane.txt"
  printf '%s\n' "│ >                     │" > "$capture"
  out=$(PATH="$fakebin:$PATH" MX_FAKE_TMUX_CAPTURE="$capture" MX_FAKE_TMUX_CURSOR_Y=0 \
    mx_tmux_composer_state "fakepane")
  ...
  pass "mx_tmux_composer_state: a bordered composer box and bare agent glyphs (❯/›) still read empty"
}
...
test_tmux_composer_state_requires_matching_box_borders() {
  ...
  pass "mx_tmux_composer_state: only matching edge borders form a composer box"
}
...
test_pane_input_pending_honors_idle_override_after_border_strip() {
  ...
  capture="$dir/pane.txt"
  printf '│ custom idle> │\n' > "$capture"
  PATH="$fakebin:$PATH" MX_FAKE_TMUX_CAPTURE="$capture" MX_FAKE_TMUX_CURSOR_Y=0 \
    MX_COMPOSER_IDLE_RE='^custom idle>$' pane_input_pending "fakepane" \
    && fail "MX_COMPOSER_IDLE_RE was not applied after border stripping"
  pass "pane_input_pending honors MX_COMPOSER_IDLE_RE after border stripping"
}
...
test_classify_signal_dedup_against_scan() {
  ...
  out=$(MX_STATE_OVERRIDE="$state" classify_signal "$state/dup-s9.status" "$state")
...
test_classify_stale_dedup_against_signal() {
  ...
  out=$(MX_STATE_OVERRIDE="$state" classify_stale "sess:mx-dup-s10" "$state")
...
test_afk_nonterminal_working_merged_keeps_wedge_aging() {
  ...
  out=$(MX_STATE_OVERRIDE="$state" classify_stale "$win" "$state")
  case "$out" in
    self\|*transient*) ;;
...
test_afk_genuine_done_still_terminal_stale() {
  ...
  state="$dir/state"
  printf 'done: PR https://example.com/pull/76 checks green; stage 1 of 4 ready for broker merge\n' \
    > "$state/stage1-w2.status"
  out=$(MX_STATE_OVERRIDE="$state" classify_stale "sess:mx-stage1-w2" "$state")
...
test_pane_input_pending_bordered_idle_not_pending() {
  ...
  for line in \
    ...
    "│                                              │"; do
    printf '%s\n' "$line" > "$capture"
    if PATH="$fakebin:$PATH" MX_FAKE_TMUX_CAPTURE="$capture" MX_FAKE_TMUX_CURSOR_Y=0 \
...
test_pane_input_pending_bordered_with_text_is_pending() {
  ...
  # still read as pending so the daemon defers (and the maintainer-return race is
  # still protected).
  local dir state fakebin capture
...
test_submit_ack_confirms_on_bordered_empty_composer() {
  ...
  verdict=$(PATH="$fakebin:$PATH" MX_FAKE_COMPOSER="$dir/composer" MX_FAKE_SENT="$sent" \
    mx_tmux_submit_core "win" "the digest" 3 0.05 0.05)
...
test_submit_ack_reports_pending_on_persistent_swallow() {
  ...
  # reported as "pending" — the daemon keeps the buffer, mx-send exits non-zero —
  ...
  touch "$dir/.swallow"
  verdict=$(PATH="$fakebin:$PATH" MX_FAKE_COMPOSER="$dir/composer" MX_FAKE_SENT="$sent" \
    MX_FAKE_SWALLOW="$dir/.swallow" MX_FAKE_PERSIST_SWALLOW=1 \
    mx_tmux_submit_core "win" "the digest" 3 0.05 0.05)
...
test_max_defer_empty_swallow_types_once_and_alarms() {
  ...
  afk_enter "$state"
  PATH="$fakebin:$PATH" MX_FAKE_COMPOSER="$dir/composer" MX_FAKE_SENT="$sent" \
    MX_FAKE_SWALLOW="$dir/.swallow" MX_FAKE_PERSIST_SWALLOW=1 MX_INJECT_CONFIRM_SLEEP=0.05 \
    MX_ESCALATE_BATCH_SECS=99999 MX_MAX_DEFER_SECS=60 housekeeping "$state"
...
test_max_defer_flushes_empty_idle_pane() {
  ...
  afk_enter "$state"
  PATH="$fakebin:$PATH" MX_FAKE_COMPOSER="$dir/composer" MX_FAKE_SENT="$sent" \
    MX_ESCALATE_BATCH_SECS=99999 MX_MAX_DEFER_SECS=60 MX_INJECT_CONFIRM_SLEEP=0.05 \
    housekeeping "$state"
...
test_normal_flush_clears_stale_wedge_marker() {
  ...
  afk_enter "$state"
  PATH="$fakebin:$PATH" MX_FAKE_COMPOSER="$dir/composer" MX_FAKE_SENT="$sent" \
    MX_INJECT_CONFIRM_SLEEP=0.05 escalate_flush "$state" \
...
test_below_max_defer_does_nothing() {
  ...
  afk_enter "$state"
  PATH="$fakebin:$PATH" MX_FAKE_TMUX_PANE_ALIVE=1 MX_FAKE_TMUX_SENT="$sent" \
    MX_FAKE_TMUX_CAPTURE="$capture" MX_FAKE_TMUX_CURSOR_Y=0 \
    MX_ESCALATE_BATCH_SECS=99999 MX_MAX_DEFER_SECS=300 housekeeping "$state"
...
# ACTIVE alert channel must reach the maintainer even when the wedged pane and its
...
# the MX_WEDGE_ALARM_EXEC seam, which tests/wake-helpers.sh forces to a recorder
# ($MX_WEDGE_ALARM_LOG logs "<channel>\t<summary>"); the daemon also defaults
...
test_wedge_alarm_library_mode_defaults_to_discard() {
  ...
  # MX_WEDGE_ALARM_EXEC to "discard", so a sourced context (every test) cannot
  # fire a real notification even if it forgets to stub. Checked in a clean
  # subshell that first unsets this harness's recorder.
  local out
  # shellcheck disable=SC2016  # $1/$MX_WEDGE_ALARM_EXEC must expand in the child, not here
  out=$(env -u MX_WEDGE_ALARM_EXEC bash -c '. "$1"; printf "%s" "${MX_WEDGE_ALARM_EXEC:-UNSET}"' _ "$DAEMON")
  [ "$out" = discard ] \
    || fail "sourcing the daemon did not default the notifier seam to discard (got: $out)"
  pass "library mode: sourcing the daemon defaults MX_WEDGE_ALARM_EXEC to discard (no test can fire a real notification)"
}
...
test_wake_helpers_replace_inherited_notifier_override() {
  ...
  cat > "$unsafe" <<'SH'
#!/usr/bin/env bash
printf '%s\n' invoked >> "${MX_WEDGE_ALARM_UNSAFE_LOG:?}"
SH
  chmod +x "$unsafe"
  MX_WEDGE_ALARM_EXEC="$unsafe" MX_WEDGE_ALARM_UNSAFE_LOG="$unsafe_log" \
    MX_WEDGE_ALARM_LOG="$alert_log" MX_WEDGE_ALARM_CHANNEL=herdr \
...
test_wedge_alarm_discard_seam_fires_nothing() {
  ...
  PATH="$dir/fakebin:$PATH" MX_WEDGE_ALARM_LOG="$log" MX_WEDGE_ALARM_EXEC=discard \
    MX_WEDGE_ALARM_CHANNEL=$'herdr\n'"$channel" \
...
test_wedge_alarm_direct_notifiers_honor_discard_seam() {
  ...
  PATH="$dir/fakebin:$PATH" MX_WEDGE_ALARM_REAL_LOG="$real_log" MX_WEDGE_ALARM_EXEC=discard \
    wedge_alarm_via_herdr "away-mode WEDGED 900s"
  MX_WEDGE_ALARM_EXEC=discard wedge_alarm_via_command "$command" "away-mode WEDGED 900s"
...
test_wedge_alarm_herdr_channel_selected() {
  ...
  MX_WEDGE_ALARM_LOG="$log" MX_WEDGE_ALARM_CHANNEL=herdr \
...
test_wedge_alarm_command_channel_receives_summary() {
  ...
  MX_WEDGE_ALARM_EXEC='' MX_WEDGE_ALARM_CHANNEL="$chan" \
    wedge_alarm_notify "away-mode WEDGED 900s" "/s/.marker"
  ...
  pass "command channel runs the maintainer command with the summary on \$1 and on stdin"
}
...
test_wedge_alarm_command_failure_hides_configured_command() {
  ...
  LOG="$daemon_log" MX_WEDGE_ALARM_EXEC='' MX_WEDGE_ALARM_CHANNEL="command:exit 73 # $secret" \
    wedge_alarm_notify "away-mode WEDGED 900s" "/s/.marker"
  rc=$?
...
test_wedge_alarm_unknown_channel_hides_configured_directive() {
  ...
  secret="https://alerts.example.invalid/hook?token=private-wedge-token"
  LOG="$daemon_log" MX_WEDGE_ALARM_CHANNEL="webhook:$secret" \
    wedge_alarm_notify "away-mode WEDGED 900s" "/s/.marker"
  rc=$?
...
test_wedge_alarm_off_disables_active_alert_regardless_of_position() {
  ...
  for directives in $'herdr\noff' $'off\nherdr'; do
    : > "$log"
    MX_WEDGE_ALARM_LOG="$log" MX_WEDGE_ALARM_CHANNEL="$directives" \
      wedge_alarm_notify "away-mode WEDGED 900s" "/s/.marker"
...
test_wedge_alarm_auto_darwin_has_no_os_channel() {
  ...
  PATH="$dir/fakebin:$PATH" MX_WEDGE_ALARM_LOG="$log" MX_FAKE_UNAME=Darwin MX_WEDGE_ALARM_CHANNEL=auto \
    wedge_alarm_notify "away-mode WEDGED 900s" "/s/.marker"
...
test_wedge_alarm_auto_non_darwin_has_no_os_channel() {
  ...
  PATH="$dir/fakebin:$PATH" MX_WEDGE_ALARM_LOG="$log" MX_FAKE_UNAME=Linux MX_WEDGE_ALARM_CHANNEL=auto \
    wedge_alarm_notify "away-mode WEDGED 900s" "/s/.marker"
...
test_wedge_alarm_config_file_multi_channel() {
  ...
  MX_WEDGE_ALARM_LOG="$log" MX_CONFIG_OVERRIDE="$cfgdir" \
...
test_wedge_alarm_failing_channel_degrades_gracefully() {
  ...
  MX_WEDGE_ALARM_LOG="$log" MX_WEDGE_ALARM_FAIL=herdr \
    MX_WEDGE_ALARM_CHANNEL=$'herdr\ncommand:true' \
    wedge_alarm_notify "away-mode WEDGED 900s" "/s/.marker"
  rc=$?
...
test_wedge_alarm_hung_channel_times_out_and_falls_through() {
  ...
  start=$SECONDS
  LOG="$daemon_log" MX_WEDGE_ALARM_EXEC='' MX_WEDGE_ALARM_TIMEOUT_SECS=1 \
    MX_WEDGE_ALARM_CHANNEL=$'command:sleep 30\n'"$channel" \
    wedge_alarm_notify "away-mode WEDGED 900s" "/s/.marker"
...
SH
  chmod +x "$blocker"
  start=$SECONDS
  LOG="$daemon_log" MX_WEDGE_ALARM_EXEC="$blocker" MX_WEDGE_ALARM_TIMEOUT_SECS=1 \
    MX_WEDGE_ALARM_CHANNEL=$'herdr\ncommand:true' \
    wedge_alarm_notify "away-mode WEDGED 900s" "/s/.marker"
  elapsed=$((SECONDS - start))
...
test_inject_wedge_alarm_fires_active_alert_on_non_tmux_backend() {
  ...
  MX_WEDGE_ALARM_LOG="$log" MX_STATE_OVERRIDE="$state" \
    MX_WEDGE_ALARM_CHANNEL=herdr MX_SUPERVISOR_BACKEND=herdr \
...
test_inject_wedge_alarm_throttles_when_marker_cannot_be_written() {
  ...
  chmod u-w "$state"
  WEDGE_ALARM_LAST_EPOCH=0
  LOG="$daemon_log" MX_WEDGE_ALARM_LOG="$log" MX_MAX_DEFER_SECS=600 \
    MX_WEDGE_ALARM_CHANNEL=herdr MX_SUPERVISOR_BACKEND=herdr \
    inject_wedge_alarm "$state" 30600
  LOG="$daemon_log" MX_WEDGE_ALARM_LOG="$log" MX_MAX_DEFER_SECS=600 \
    MX_WEDGE_ALARM_CHANNEL=herdr MX_SUPERVISOR_BACKEND=herdr \
...
test_mx_send_exits_nonzero_on_confirmed_swallow() {
  # mx-send.sh must exit NON-ZERO when a steer's Enter is positively swallowed
  # (text left in the composer), so broker learns the instruction did not land
  ...
  # Clean submit -> exit 0.
  PATH="$fakebin:$PATH" MX_HOME="$dir" MX_STATE_OVERRIDE="$dir/state" MX_FAKE_COMPOSER="$dir/composer" \
    MX_SEND_SLEEP=0.05 "$ROOT/bin/mx-send.sh" sess:win 'route this work' >/dev/null 2>"$err" \
    || fail "mx-send exited non-zero on a clean submit: $(cat "$err")"
  # Persistent swallow -> exit non-zero with a clear message.
  printf '│ > │\n' > "$dir/composer"
  touch "$dir/.swallow"
  if PATH="$fakebin:$PATH" MX_HOME="$dir" MX_STATE_OVERRIDE="$dir/state" MX_FAKE_COMPOSER="$dir/composer" \
    MX_FAKE_SWALLOW="$dir/.swallow" MX_FAKE_PERSIST_SWALLOW=1 MX_SEND_SLEEP=0.05 \
    "$ROOT/bin/mx-send.sh" sess:win 'fix findings 1 and 3, skip 2' >/dev/null 2>"$err"; then
    fail "mx-send exited zero despite a swallowed Enter (silent unsubmitted instruction)"
  fi
  grep -F 'not submitted' "$err" >/dev/null || fail "mx-send did not explain the swallowed submit: $(cat "$err")"
  pass "mx-send exits non-zero on a confirmed swallow, zero on a clean submit"
}

test_mx_send_exits_nonzero_on_initial_send_failure() {
  local dir fakebin err
  dir=$(make_bordered_case send-type-failure)
  fakebin="$dir/fakebin"; err="$dir/send.err"
  if PATH="$fakebin:$PATH" MX_HOME="$dir" MX_STATE_OVERRIDE="$dir/state" MX_FAKE_COMPOSER="$dir/composer" \
    MX_FAKE_SEND_FAIL=1 MX_SEND_SLEEP=0.05 \
    "$ROOT/bin/mx-send.sh" sess:win 'route this work' >/dev/null 2>"$err"; then
    fail "mx-send exited zero despite initial tmux send-keys failure"
  fi
  grep -F 'text not sent' "$err" >/dev/null || fail "mx-send did not explain initial send failure: $(cat "$err")"
  pass "mx-send exits non-zero when initial text send fails"
}

# --- herdr backend-awareness (mx-turnend-guard-h6-adjacent transport fix) ----
...
test_discover_supervisor_backend_precedence() {
  local out
  out=$(MX_SUPERVISOR_BACKEND=herdr TMUX_PANE='%9' HERDR_ENV=1 HERDR_PANE_ID=w1:p1 discover_supervisor_backend)
  [ "$out" = herdr ] || fail "explicit MX_SUPERVISOR_BACKEND override was not honored: $out"

  out=$(MX_SUPERVISOR_BACKEND='' TMUX_PANE='%9' HERDR_ENV=1 HERDR_PANE_ID=w1:p1 discover_supervisor_backend)
  [ "$out" = tmux ] || fail "TMUX_PANE should win over HERDR_ENV (tmux nested in herdr resolves to tmux): $out"

  out=$(MX_SUPERVISOR_BACKEND='' TMUX_PANE='' HERDR_ENV=1 HERDR_PANE_ID=w1:p1 discover_supervisor_backend)
  [ "$out" = herdr ] || fail "HERDR_ENV=1 with HERDR_PANE_ID present should resolve to herdr: $out"

  if out=$(MX_SUPERVISOR_BACKEND='' TMUX_PANE='' HERDR_ENV='' HERDR_PANE_ID='' discover_supervisor_backend); then
...
test_discover_supervisor_target_herdr() {
  local out
  out=$(MX_SUPERVISOR_TARGET=explicit:target TMUX_PANE='' HERDR_ENV=1 HERDR_PANE_ID=w1:p9 discover_supervisor_target)
  [ "$out" = "explicit:target" ] || fail "explicit MX_SUPERVISOR_TARGET override was not honored: $out"

  out=$(MX_SUPERVISOR_TARGET='' TMUX_PANE='%3' HERDR_ENV=1 HERDR_PANE_ID=w1:p9 discover_supervisor_target)
  [ "$out" = '%3' ] || fail "TMUX_PANE should win over herdr markers: $out"

  out=$(MX_SUPERVISOR_TARGET='' TMUX_PANE='' HERDR_ENV=1 HERDR_PANE_ID=w1:p9 HERDR_SESSION='' discover_supervisor_target)
  [ "$out" = "default:w1:p9" ] || fail "herdr target should default HERDR_SESSION to 'default': $out"

  out=$(MX_SUPERVISOR_TARGET='' TMUX_PANE='' HERDR_ENV=1 HERDR_PANE_ID=w1:p9 HERDR_SESSION=iso1 discover_supervisor_target)
  [ "$out" = "iso1:w1:p9" ] || fail "herdr target should use an explicit HERDR_SESSION: $out"

  if out=$(MX_SUPERVISOR_TARGET='' TMUX_PANE='' HERDR_ENV='' HERDR_PANE_ID='' discover_supervisor_target); then
    fail "bare fallback should return non-zero"
  fi
  [ "$out" = "broker:0" ] || fail "bare fallback should still print broker:0: $out"

  pass "discover_supervisor_target: override > TMUX_PANE > herdr '<session>:<pane-id>' composition > broker:0 fallback"
}

test_pane_is_busy_herdr_native_busy_state() {
  (
    mx_backend_busy_state() { [ "$1" = herdr ] && [ "$2" = "default:w1:p2" ] || fail "unexpected busy_state args: $1 $2"; printf 'busy'; }
    mx_backend_capture() { fail "capture should not be consulted when busy_state is conclusive"; }
...
test_pane_is_busy_herdr_falls_back_to_capture_regex() {
  (
    mx_backend_busy_state() { printf 'unknown'; }
    mx_backend_capture() { [ "$1" = herdr ] && [ "$2" = "default:w1:p2" ] || fail "unexpected capture args: $1 $2"; printf 'esc to interrupt\n'; }
...
test_pane_is_busy_herdr_idle_falls_back_to_capture_regex() {
  (
    mx_backend_busy_state() { printf 'idle'; }
    mx_backend_capture() { [ "$1" = herdr ] && [ "$2" = "default:w1:p2" ] || fail "unexpected capture args: $1 $2"; printf 'esc to interrupt\n'; }
...
test_pane_is_busy_defaults_to_tmux_when_backend_omitted() {
  ...
  fakebin="$dir/fakebin"; capture="$dir/pane.txt"
  printf 'esc to interrupt\n' > "$capture"
  PATH="$fakebin:$PATH" MX_FAKE_TMUX_CAPTURE="$capture" pane_is_busy "fakepane" \
...
test_pane_input_pending_herdr_dispatch() {
  (
    mx_backend_composer_state() { [ "$1" = herdr ] && [ "$2" = "default:w1:p2" ] || fail "unexpected composer_state args: $1 $2"; printf 'pending'; }
  ...
  (
    mx_backend_composer_state() { printf 'empty'; }
  ...
  pass "pane_input_pending: dispatches through mx_backend_composer_state for backend=herdr"
}
...
test_inject_msg_herdr_busy_guard_defers() {
  ...
  afk_enter "$state"
  (
    mx_backend_target_exists() { [ "$1" = herdr ] && [ "$2" = "default:w1:p2" ] || fail "unexpected target_exists args: $1 $2"; return 0; }
    mx_backend_busy_state() { printf 'busy'; }
    mx_backend_capture() { fail "capture should not run when busy_state is conclusive"; }
    mx_backend_composer_state() { fail "composer_state should not be consulted once the busy-guard already deferred"; }
    mx_backend_send_text_submit() { fail "send_text_submit should not run when the busy-guard defers"; }
    if MX_SUPERVISOR_BACKEND=herdr MX_SUPERVISOR_TARGET="default:w1:p2" inject_msg "hello" "$state"; then
...
test_inject_msg_herdr_composer_guard_defers() {
  ...
  afk_enter "$state"
  (
    mx_backend_target_exists() { return 0; }
    mx_backend_busy_state() { printf 'idle'; }
    mx_backend_capture() { printf 'idle prompt\n'; }
    mx_backend_composer_state() { [ "$1" = herdr ] && [ "$2" = "default:w1:p2" ] || fail "unexpected composer_state args: $1 $2"; printf 'pending'; }
    mx_backend_send_text_submit() { fail "send_text_submit should not run when the composer-guard defers"; }
    if MX_SUPERVISOR_BACKEND=herdr MX_SUPERVISOR_TARGET="default:w1:p2" inject_msg "hello" "$state"; then
...
test_inject_msg_herdr_pane_gone_defers() {
  ...
  afk_enter "$state"
  (
    mx_backend_target_exists() { return 1; }
    mx_backend_busy_state() { fail "busy_state should not be consulted once the pane-exists check already failed"; }
    mx_backend_send_text_submit() { fail "send_text_submit should not run when the pane does not exist"; }
    if MX_SUPERVISOR_BACKEND=herdr MX_SUPERVISOR_TARGET="default:w1:gone" inject_msg "hello" "$state"; then
...
test_inject_msg_herdr_submits_through_backend_dispatch() {
  ...
  (
    ...
    MX_SUPERVISOR_BACKEND=herdr MX_SUPERVISOR_TARGET="default:w1:p2" inject_msg "hello" "$state" \
...
# Safety-critical (task mx-composer-shellglyph-safety): the away-mode injector
...
test_inject_msg_defers_on_dead_shell_unknown() {
  ...
  (
    mx_backend_target_exists() { return 0; }
    mx_backend_busy_state() { printf 'idle'; }
    mx_backend_capture() { printf '$ \n'; }
    mx_backend_composer_state() { printf 'unknown'; }
    mx_backend_send_text_submit() { fail "send_text_submit must NOT run when the composer is a dead shell (unknown)"; }
    if MX_SUPERVISOR_BACKEND=herdr MX_SUPERVISOR_TARGET="default:w1:p2" inject_msg "hello" "$state"; then
