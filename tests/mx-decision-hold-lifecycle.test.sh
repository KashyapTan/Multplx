...
EOF
  ...
  mx_fake_exit0 "$fakebin" tmux gh
  printf '%s\n' "$home"
}

run_status() {  # <home>
  local home=$1
  PATH="$home/fakebin:$PATH" MX_HOME="$home" MX_STATUS_NOW=2026-07-14T12:00:00Z \
    "$STATUS" --json
}

run_teardown() {  # <home> <id>
  local home=$1 id=$2
  PATH="$home/fakebin:$PATH" MX_ROOT_OVERRIDE="$ROOT" MX_HOME="$home" \
    MX_STATE_OVERRIDE="$home/state" MX_DATA_OVERRIDE="$home/data" \
    MX_CONFIG_OVERRIDE="$home/config" "$TEARDOWN" "$id"
}

# Reproduces the loss exactly with privacy-safe synthetic names: the investigation
# and visual review have ended, the only genuine unresolved decision is report prose,
# no held backlog item or open status exists, and the authoritative Catchup view
...
The maintainer still needs to choose route north or route south before follow-up work starts.
EOF

  json=$(run_status "$home") || fail "Catchup failed for unresolved-decision regression"
  printf '%s' "$json" | jq -e '
...
A separate recommendation is already resolved and requires no maintainer action.
EOF

  if run_decisions "$home" complete "$id" route access > "$home/early-complete.out" 2> "$home/early-complete.err"; then
    fail "completion succeeded before unresolved decisions had maintainer holds"
  fi
  ...
  route_hold=$(run_decisions "$home" hold "$id" route \
    --title "Choose the sample route" --reason "maintainer route choice pending" --repo sample) \
  ...
  run_decisions "$home" hold "$id" route \
    --title "Choose the sample route" --reason "maintainer route choice pending" --repo sample >/dev/null \
  ...
  access_hold=$(run_decisions "$home" hold "$id" access \
    --title "Choose the sample access level" --reason "maintainer access choice pending" --repo sample) \
  ...
  open=$(bash -c '. "$1"; status_open_decisions "$2"' _ \
    "$ROOT/bin/mx-classify-lib.sh" "$home/state/$id.status")
  [ -z "$open" ] || fail "maintainer-held transfer did not close duplicate live status decisions: $open"

  before=$(shasum -a 256 "$home/data/backlog.md" | awk '{print $1}')
  json=$(run_status "$home") || fail "Catchup failed with maintainer-held decisions"
  after=$(shasum -a 256 "$home/data/backlog.md" | awk '{print $1}')
  [ "$before" = "$after" ] || fail "Catchup mutated the authoritative backlog"
  printf '%s' "$json" | jq -e --arg route "$route_hold" --arg access "$access_hold" '
    (.decisions_open | any(.id == $route and .verb == "maintainer-hold" and .owner == "(main)"))
      and (.decisions_open | any(.id == $access and .verb == "maintainer-hold" and .owner == "(main)"))
      and (.gates | any(.id == $route or .id == $access) | not)
  ' >/dev/null || fail "Catchup did not surface structured maintainer holds: $json"
  ...
  json=$(run_status "$home") || fail "Catchup failed after source teardown and archival"
  printf '%s' "$json" | jq -e --arg route "$route_hold" --arg access "$access_hold" '
    (.decisions_open | any(.id == $route and .verb == "maintainer-hold"))
      and (.decisions_open | any(.id == $access and .verb == "maintainer-hold"))
      and (.in_flight | any(.id == "sample-systems-review") | not)
  ' >/dev/null || fail "teardown or archival erased a maintainer-held decision: $json"

  tasks_in "$home" add sample-route-implementation "Apply the selected sample route" \
    --kind delivery --repo sample >/dev/null \
  ...
  if run_decisions "$home" resolve "$id" route --decision-file "$home/route-decision.txt" \
    ...
    fail "maintainer hold closed before dependent work had a durable routing edge"
  fi
  ...
  if run_decisions "$home" resolve "$id" route --decision-file "$home/changed-route-decision.txt" \
    ...
    fail "partial resolution retry accepted a different maintainer decision"
  fi
  ...
  if run_decisions "$home" resolve "$id" route --decision-file "$home/changed-route-decision.txt" \
    ...
    fail "resolution retry accepted a different maintainer decision"
  fi
  if run_decisions "$home" resolve "$id" route --decision-file "$home/route-decision.txt" \
  ...
  assert_contains "$show" "Resolution recorded by mx-decision-hold" "resolved hold lost the decision record"
  show=$(tasks_in "$home" show sample-route-implementation --full)
  assert_contains "$show" "blocked: no" "recorded decision did not release dependent work"
  json=$(run_status "$home") || fail "Catchup failed after decision resolution"
  printf '%s' "$json" | jq -e --arg route "$route_hold" --arg access "$access_hold" '
  ...
  pass "maintainer holds are idempotent, distinct, teardown-safe, Catchup-visible, and durably routed before close"
}
...
test_visual_review_uses_shared_completion_owner() {
  ...
  printf '# Sample board investigation\n\nThe initial findings need no maintainer choice.\n' > "$home/data/$id/report.md"
  ...
  json=$(run_status "$home") || fail "Catchup failed after the ended visual review"
  printf '%s' "$json" | jq -e --arg hold "$hold" '
    .decisions_open | any(.id == $hold and .verb == "maintainer-hold")
  ' >/dev/null || fail "ended visual review did not leave its durable Maintainer Call: $json"
...
The recommendation is informational and needs no maintainer action.
EOF
  run_decisions "$home" complete "$id" --none >/dev/null \
    || fail "explicit no-decision inventory failed"
  json=$(run_status "$home") || fail "Catchup failed for no-decision inventory"
  printf '%s' "$json" | jq -e '
...
test_terminal_single_owner_status_decision_does_not_block_empty_inventory() {
  ...
  printf 'needs-decision [key=default]: choose route A or route B\ndone: report complete\n' \
    > "$home/state/$id.status"
  printf '# Terminal sample review\n\nNo unresolved maintainer choice remains.\n' > "$home/data/$id/report.md"
  open=$(bash -c '. "$1"; status_open_decisions "$2"' _ \
    "$ROOT/bin/mx-classify-lib.sh" "$home/state/$id.status")
  ...
  write_origin_meta "$home" "$daemon" daemon
  printf 'needs-decision [key=route]: choose route A or route B\ndone: heartbeat complete\n' \
    > "$home/state/$daemon.status"
  if run_decisions "$home" complete "$daemon" --none \
    > "$home/daemon-terminal.out" 2> "$home/daemon-terminal.err"; then
    fail "daemon terminal status decision was incorrectly cleared"
  fi
...
EOF
  ...
  printf '# Sample daemon review\n\nOne maintainer choice remains.\n' > "$mate/data/$origin/report.md"
  hold=$(run_decisions "$mate" hold "$origin" release \
    --title "Choose the sample release" --reason "maintainer release choice pending" --repo sample) \
    || fail "daemon-owned hold creation failed"
  run_decisions "$mate" complete "$origin" release >/dev/null \
    || fail "daemon-owned completion failed"
  run_teardown "$mate" "$origin" >/dev/null 2> "$mate/teardown.err" \
    || fail "daemon investigation teardown failed: $(cat "$mate/teardown.err")"
  tasks_in "$mate" "done" "$origin" --report "data/$origin/report.md" --keep 0 >/dev/null

  printf -- '- sample-mate - synthetic scope (home: %s; scope: sample reviews; projects: sample; added 2026-07-14)\n' \
    "$mate" > "$parent/data/daemons.md"
  mx_write_daemon_meta "$parent/state/sample-mate.meta" "$mate" \
    "broker:mx-sample-mate" sample
  json=$(run_status "$parent") || fail "parent Catchup could not read daemon hold"
  printf '%s' "$json" | jq -e --arg hold "$hold" '
    .decisions_open | any(.owner == "sample-mate" and .verb == "maintainer-hold" and (.id | endswith($hold)))
  ' >/dev/null || fail "daemon maintainer hold did not surface with authoritative owner: $json"
  assert_no_grep "$hold" "$parent/data/backlog.md" "daemon hold leaked into the main backlog"
  assert_grep "$hold" "$mate/data/backlog.md" "daemon hold left its authoritative backlog"
...
test_resolve_matches_quoted_blocked_by_edges() {
  ...
  hold_first=$(run_decisions "$home" hold "$origin" edge-first \
    --title "First edge decision" --reason "maintainer first pending" --repo sample) \
    || fail "could not register first-edge hold"
  hold_mid=$(run_decisions "$home" hold "$origin" edge-mid \
    --title "Middle edge decision" --reason "maintainer mid pending" --repo sample) \
    || fail "could not register mid-edge hold"
  hold_last=$(run_decisions "$home" hold "$origin" edge-last \
    --title "Last edge decision" --reason "maintainer last pending" --repo sample) \
    || fail "could not register last-edge hold"
  hold_absent=$(run_decisions "$home" hold "$origin" edge-absent \
    --title "Absent edge decision" --reason "maintainer absent pending" --repo sample) \
    || fail "could not register absent-edge hold"

  tasks_in "$home" add pad-a "Pad A" --kind delivery --repo sample >/dev/null \
    || fail "could not create pad-a blocker"
  tasks_in "$home" add pad-b "Pad B" --kind delivery --repo sample >/dev/null \
    || fail "could not create pad-b blocker"

  tasks_in "$home" add dep-first "Dep first position" --kind delivery --repo sample >/dev/null \
  ...
  tasks_in "$home" add dep-mid "Dep mid position" --kind delivery --repo sample >/dev/null \
  ...
  tasks_in "$home" add dep-last "Dep last position" --kind delivery --repo sample >/dev/null \
  ...
  tasks_in "$home" add dep-absent "Dep absent control" --kind delivery --repo sample >/dev/null \
