...
HERDR_LAB_HELPER=${HERDR_LAB_HELPER:-$ROOT/bin/mx-herdr-lab.sh}
...
HERDR_LAB_SESSION=$(PATH="$HERDR_ORIGINAL_PATH" \
  "$HERDR_LAB_HELPER" name mx-herdr-presentation-projection)
...
mkdir -p "$HOME_DIR/state" "$HOME_DIR/config" \
  ...
  "$HOME_DIR/data/order-fail" "$HOME_DIR/data/mx-hibit-resume-r1" \
...
# Keep one ordinary primary task live so the durable broker workspace is
...
MULTPLX_WSID=$(grep '^herdr_workspace_id=' "$ANCHOR_META" | cut -d= -f2-)
[ -n "$MULTPLX_WSID" ] || fail "anchor metadata did not record the broker workspace"
...
SECOND_ONE_OUT=$(lab workspace create --cwd "$PROJECT_DIR" --label daemon-alpha --no-focus) \
  || fail "could not create the first daemon presentation fixture"
SECOND_TWO_OUT=$(lab workspace create --cwd "$PROJECT_DIR" --label daemon-bravo --focus) \
  || fail "could not create the focused daemon presentation fixture"
...
[ -n "$SECOND_ONE_WSID" ] && [ -n "$SECOND_TWO_WSID" ] && [ -n "$SECOND_TWO_TAB" ] && [ -n "$SECOND_TWO_PANE" ] \
  || fail "daemon presentation fixtures returned incomplete IDs"
SECOND_ORDER_BEFORE=$(printf '%s\n%s\n' "$SECOND_ONE_WSID" "$SECOND_TWO_WSID")
MAINTAINER_FOCUS="$SECOND_TWO_WSID/$SECOND_TWO_TAB"
assert_focus_is "$MAINTAINER_FOCUS" "focused daemon fixture"
...
printf '%s' "$PROJECTED_TABS" | jq -e --arg tab "$PROJECTED_TAB" \
  '.result.tabs[0].tab_id == $tab and .result.tabs[0].label == "mx-shape"' >/dev/null 2>&1 \
  || fail "projected workspace's only tab was not the normal mx-shape task tab"
...
SECOND_TWO_INFO=$(lab workspace get "$SECOND_TWO_WSID") || fail "focused daemon disappeared during projected create"
[ "$(printf '%s' "$SECOND_TWO_INFO" | jq -r '.result.workspace.focused')" = true ] \
  || fail "projected create or workspace.move stole focus from the maintainer's current space"
...
grep -F "target is the maintainer's active tab" "$TMP_ROOT/active-seeded.err" >/dev/null 2>&1 \
...
lab tab focus "$SECOND_TWO_TAB" >/dev/null || fail "could not restore the captured maintainer tab after the active seeded-tab fixture"
assert_focus_is "$MAINTAINER_FOCUS" "active seeded-tab fixture restoration"
...
assert_focus_is "$MAINTAINER_FOCUS" "active seeded-tab fixture cleanup"
assert_cleanup_focus_preserved "$ACTIVE_SEEDED_CLEANUP_FOCUS_START" "$ACTIVE_SEEDED_PANE" "$MAINTAINER_FOCUS"
...
[ "$LOCK_CONTENTION_WSID" = "$MULTPLX_WSID" ] \
  || fail "bounded lock contention did not use the ordinary flat broker workspace"
...
assert_focus_is "$MAINTAINER_FOCUS" "bounded presentation lock flat fallback"
...
assert_focus_is "$MAINTAINER_FOCUS" "bounded presentation lock flat fallback teardown"
...
assert_focus_is "$MAINTAINER_FOCUS" "concurrent projected spawns"
...
SECOND_ORDER_AFTER=$(printf '%s' "$ORDER_LIST" | jq -r '.result.workspaces[] | select(.label | startswith("daemon-")) | .workspace_id')
[ "$SECOND_ORDER_AFTER" = "$SECOND_ORDER_BEFORE" ] \
  || fail "primary workspace ordering changed daemon relative order"
...
MX_BACKEND_HERDR_WORKSPACE_MOVER="$FAIL_MOVER" \
...
assert_focus_is "$MAINTAINER_FOCUS" "failed presentation ordering"
...
assert_focus_is "$MAINTAINER_FOCUS" "concurrent post-create abort cleanup"
assert_cleanup_focus_preserved "$ABORT_FOCUS_START" "$ABORT_A_PANE" "$MAINTAINER_FOCUS"
assert_cleanup_focus_preserved "$ABORT_FOCUS_START" "$ABORT_B_PANE" "$MAINTAINER_FOCUS"
...
lab pane get "$SECOND_TWO_PANE" >/dev/null 2>&1 \
  || fail "projected teardown affected the focused daemon workspace"
[ ! -e "$JOURNAL" ] || fail "confirmed projected teardown did not retire its presentation journal"
pass "real Herdr lab: exact task-pane close restores the exact maintainer workspace/tab after Herdr's raw focus steal"
...
assert_focus_is "$MAINTAINER_FOCUS" "concurrent projected teardowns"
...
assert_focus_is "$MAINTAINER_FOCUS" "failed-order projection teardown"
...
# ------------------------------------------------------------------
# Multi-home topology: real daemon MX_HOME spawn paths, inheritance,
# concurrent cross-home waves, and session-scoped lock contention.
# ------------------------------------------------------------------
SECOND_HOME_A="$TMP_ROOT/home-daemon-alpha"
SECOND_HOME_B="$TMP_ROOT/home-daemon-bravo"
...
printf 'alpha\n' > "$SECOND_HOME_A/.mx-daemon-home"
printf 'bravo\n' > "$SECOND_HOME_B/.mx-daemon-home"
touch "$SECOND_HOME_A/state/.last-watcher-beat" "$SECOND_HOME_B/state/.last-watcher-beat"
# Ensure the daemon homes look like gitignored Multplx homes so inheritance
...
git -C "$SECOND_HOME_A" -c user.name='Multplx Tests' -c user.email='tests@example.invalid' commit -qm init
git -C "$SECOND_HOME_B" -c user.name='Multplx Tests' -c user.email='tests@example.invalid' commit -qm init
mkdir -p "$SECOND_HOME_A/bin"
printf '# Multplx daemon fixture\n' > "$SECOND_HOME_A/AGENTS.md"
printf 'Daemon alpha charter.\n' > "$SECOND_HOME_A/data/charter.md"

# Primary flag only; real inheritance must push presence into both daemon homes.
...
[ ! -e "$SECOND_HOME_A/config/herdr-presentation-spaces" ] \
  || fail "daemon A unexpectedly had the presentation flag before inheritance"
[ ! -e "$SECOND_HOME_B/config/herdr-presentation-spaces" ] \
  || fail "daemon B unexpectedly had the presentation flag before inheritance"
SECOND_SPAWN_LOG_START=$(log_line_count)
spawn_daemon_task alpha "$SECOND_HOME_A" > "$TMP_ROOT/alpha.out" 2> "$TMP_ROOT/alpha.err" \
  || fail "daemon alpha spawn failed: $(cat "$TMP_ROOT/alpha.err")"
[ -f "$SECOND_HOME_A/config/herdr-presentation-spaces" ] \
  || fail "daemon spawn did not inherit the presentation flag"
[ ! -e "$HOME_DIR/state/alpha.herdr-presentation" ] \
  || fail "daemon spawn published a presentation journal"
SECOND_META="$HOME_DIR/state/alpha.meta"
[ "$(grep '^kind=' "$SECOND_META" | cut -d= -f2-)" = daemon ] \
  || fail "daemon spawn did not record kind=daemon"
...
[ "$SECOND_LABEL" = daemon-alpha ] \
  || fail "daemon spawn did not use its flat parent workspace: $SECOND_LABEL"
[ -z "$(projection_labels_from_log "$SECOND_SPAWN_LOG_START")" ] \
  || fail "daemon spawn created a corner projection workspace"
if sed -n "$((SECOND_SPAWN_LOG_START + 1)),\$p" "$HERDR_CALL_LOG" \
  | grep -E $'^(workspace\tmove|session\tlist)' >/dev/null 2>&1; then
  fail "daemon spawn attempted presentation ordering"
fi
# shellcheck source=/dev/null
. "$ROOT/bin/mx-config-inherit-lib.sh"
propagate_inheritable_config "$HOME_DIR/config" "$SECOND_HOME_A/config" \
  || fail "inheritance into daemon A failed"
propagate_inheritable_config "$HOME_DIR/config" "$SECOND_HOME_B/config" \
  || fail "inheritance into daemon B failed"
[ -f "$SECOND_HOME_A/config/herdr-presentation-spaces" ] \
  || fail "primary presentation flag did not reach daemon A"
[ -f "$SECOND_HOME_B/config/herdr-presentation-spaces" ] \
  || fail "primary presentation flag did not reach daemon B"
pass "real Herdr lab: primary presentation opt-in inherits into real daemon homes"

# Keep the pre-existing daemon-alpha/bravo workspaces as owning parents and maintainer focus.
assert_focus_is "$MAINTAINER_FOCUS" "multi-home maintainer focus"
...
assert_focus_is "$MAINTAINER_FOCUS" "multi-home sequential spawns"
...
case "$A1_LABEL" in $'└ a1 · p:'*) ;; *) fail "daemon A a1 label wrong: $A1_LABEL" ;; esac
case "$A2_LABEL" in $'└ a2 · p:'*) ;; *) fail "daemon A a2 label wrong: $A2_LABEL" ;; esac
case "$B1_LABEL" in $'└ b1 · p:'*) ;; *) fail "daemon B b1 label wrong: $B1_LABEL" ;; esac
case "$B2_LABEL" in $'└ b2 · p:'*) ;; *) fail "daemon B b2 label wrong: $B2_LABEL" ;; esac
...
pass "real Herdr lab: primary and two daemon homes each own a top-level contiguous child block"
...
assert_focus_is "$MAINTAINER_FOCUS" "cross-home concurrent wave"
...
[ "$AFLAT_LABEL" = daemon-alpha ] \
  || fail "cross-home lock contention did not use the ordinary daemon home workspace: $AFLAT_LABEL"
[ ! -e "$SECOND_HOME_A/state/aflat.herdr-presentation" ] \
  || fail "cross-home lock contention published a projection journal"
assert_focus_is "$MAINTAINER_FOCUS" "cross-home lock contention flat fallback"
...
pass "real Herdr lab: session lock contention from a daemon home falls back flat with no journal"

# Same-identity recovery replaces only one exact agent-free husk in its
# original projected workspace.
# Exercise both the leading mx- identity style seen in Hi Bit work and the
# project-name identity style used by Wheelhouse work.
for RESTART_ID in mx-hibit-resume-r1 wheelhouse-healing-r1; do
  ...
  EXPECTED_CONCISE=${RESTART_ID#mx-}
...
# A daemon child binds and reclaims only inside its own home and parent.
...
[ "$CROSS_BOUND_HOME" = "$(cd "$SECOND_HOME_A" && pwd -P)" ] \
  || fail "cross-home restart journal did not bind the daemon's exact home"
...
[ "$CROSS_NEW_WSID" = "$CROSS_OLD_WSID" ] && [ "$CROSS_NEW_PANE" != "$CROSS_OLD_PANE" ] \
  || fail "cross-home reclaim did not replace one pane inside the same daemon child workspace"
...
printf 'Concurrent daemon recovery fixture.\n' > "$SECOND_HOME_B/data/$BRAVO_WAVE_ID/brief.md"
...
spawn_task "$BRAVO_WAVE_ID" "$SECOND_HOME_B" "$PROJECT_DIR" > "$TMP_ROOT/bravo-wave-first.out" 2> "$TMP_ROOT/bravo-wave-first.err" \
  || fail "daemon recovery-wave fixture failed: $(cat "$TMP_ROOT/bravo-wave-first.err")"
...
wait "$BRAVO_WAVE_PID" || fail "concurrent daemon recovery failed: $(cat "$TMP_ROOT/bravo-wave-resume.err")"
...
LEGACY_OUT=$(lab workspace create --cwd "$PROJECT_DIR" --label "broker/legacy-seed · p:AbCdEfGhIjKlMnOpQrStUv" --no-focus) \
...
FLAT_TAB_OUT=$(lab tab create --workspace "$(lab workspace list | jq -r '.result.workspaces[] | select(.label == "daemon-alpha") | .workspace_id' | head -1)" --cwd "$PROJECT_DIR" --label mx-flat-legacy-tab --no-focus) \
  || fail "could not seed a flat daemon child tab"
...
[ "$(lab workspace get "$LEGACY_WSID" | jq -r '.result.workspace.label')" = "broker/legacy-seed · p:AbCdEfGhIjKlMnOpQrStUv" ] \
  || fail "correction renamed or moved the seeded legacy projection"
lab tab get "$FLAT_TAB_ID" >/dev/null 2>&1 \
  || fail "correction removed the seeded flat daemon child tab"
pass "real Herdr lab: legacy projection labels and flat daemon tabs are left unmigrated"
...
MISSING_JOURNAL=$(mx_backend_herdr_projection_journal_path "$MISSING_STATE" missing1)
START=$(log_line_count)
mx_backend_herdr_projection_recovery_allows_flat "$HERDR_LAB_SESSION" "$MISSING_JOURNAL" missing1 \
...
RENAMED_TOKEN=$(mx_backend_herdr_projection_journal_create "$RENAMED_STATE" renamed1)
RENAMED_JOURNAL=$(mx_backend_herdr_projection_journal_path "$RENAMED_STATE" renamed1)
RENAMED_OUT=$(lab workspace create --cwd "$PROJECT_DIR" --label "broker/renamed1 · p:$RENAMED_TOKEN" --no-focus)
...
START=$(log_line_count)
mx_backend_herdr_projection_recovery_allows_flat "$HERDR_LAB_SESSION" "$RENAMED_JOURNAL" renamed1 \
...
DUP_TOKEN=$(mx_backend_herdr_projection_journal_create "$DUP_STATE" duplicate1)
DUP_JOURNAL=$(mx_backend_herdr_projection_journal_path "$DUP_STATE" duplicate1)
DUP1=$(lab workspace create --cwd "$PROJECT_DIR" --label "broker/duplicate1 · p:$DUP_TOKEN" --no-focus)
...
START=$(log_line_count)
mx_backend_herdr_projection_recovery_allows_flat "$HERDR_LAB_SESSION" "$DUP_JOURNAL" duplicate1 \
...
lab pane report-agent "$DUP1_PANE" --source mx-projection-e2e --agent test-agent --state idle >/dev/null \
  || fail "could not register the duplicate-live-agent risk fixture"
START=$(log_line_count)
if mx_backend_herdr_projection_recovery_allows_flat "$HERDR_LAB_SESSION" "$DUP_JOURNAL" duplicate1; then
