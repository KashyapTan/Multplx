...
# Catchup before Multplx owned remediation. The shared script now stops,
...
[ "${1:-}" = stop ] || exit 2
printf 'stop\n' >> "$MX_HOME/stop.log"
rm -f "$MX_HOME/state/.afk"
if [ -e "$MX_HOME/state/.fail-terminal-stop-once" ]; then
  rm -f "$MX_HOME/state/.fail-terminal-stop-once"
  exit 1
fi
rm -f "$MX_HOME/state/.afk-daemon-terminal"
SH
  cat > "$dir/bin/mx-wake-drain.sh" <<'SH'
#!/usr/bin/env bash
file="$MX_HOME/state/.fake-drain"
[ -f "$file" ] && cat "$file"
: > "$file"
SH
  chmod +x "$dir/bin/"*.sh
...
run_return() {  # <case-dir> <mode>
  local dir=$1 mode=$2
  MX_HOME="$dir/home" MX_STATE_OVERRIDE="$dir/home/state" "$dir/bin/mx-afk-return.sh" "$mode" 2>&1
}

seed_live_blocker() {  # <case-dir> <backend> <key>
  local dir=$1 backend=$2 key=$3 target
  case "$backend" in
    tmux) target='synthetic:mx-repair-task' ;;
    herdr) target='mx-lab-synthetic:w1:p2' ;;
  esac
...
kind=delivery
EOF
  printf 'blocked [key=%s]: broker can refresh the synthetic token\n' "$key" > "$dir/home/state/repair-task.status"
}

test_return_gate_orders_catchup_before_status() {
  ...
  # The exact incident regression: Catchup is an ordinary request and must
  # refuse before reading/rendering while this shared gate remains open.
  set +e
  out=$(MX_HOME="$dir/home" MX_STATE_OVERRIDE="$dir/home/state" "$ROOT/bin/mx-status-snapshot.sh" --json 2>&1)
  rc=$?
  set -e
  [ "$rc" -eq 3 ] || fail "Catchup should refuse behind the return gate (rc=$rc): $out"
  assert_contains "$out" 'return catch-up is pending' "Catchup refusal did not point to the shared return owner"
  ...
  pass "return catch-up precedes Catchup, owns live blocker remediation, preserves evidence once, and clears idempotently"
}
...
test_maintainer_decision_does_not_masquerade_as_broker_blocker() {
  local dir out
  dir="$TMP_ROOT/maintainer-decision"
  install_runner "$dir"
  cat > "$dir/home/state/decision-task.meta" <<'EOF'
window=synthetic:mx-decision-task
backend=tmux
kind=delivery
EOF
  printf 'needs-decision [key=api-shape]: maintainer must choose the synthetic API shape\n' > "$dir/home/state/decision-task.status"
  date +%s > "$dir/home/state/.afk"
...
test_away_reentry_refuses_pending_return_gate() {
  ...
  set +e
  out=$(MX_HOME="$dir/home" MX_STATE_OVERRIDE="$dir/home/state" "$ROOT/bin/mx-afk-launch.sh" start-native 2>&1)
  rc=$?
  set -e
