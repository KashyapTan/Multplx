...
# shellcheck source=/dev/null
. "$ROOT/bin/mx-check-lib.sh"

PR_CHECK="$ROOT/bin/mx-pr-check.sh"
PR_MERGE="$ROOT/bin/mx-pr-merge.sh"
MIGRATE="$ROOT/bin/mx-pr-check-migrate.sh"
POLL="$ROOT/bin/mx-pr-poll.sh"
...
make_case() {
  ...
  cat > "$fake_root/bin/mx-guard.sh" <<'SH'
#!/usr/bin/env bash
printf 'guard\n' >> "$MX_TEST_GUARD_LOG"
SH
  chmod +x "$fake_root/bin/mx-guard.sh"
  cat > "$fakebin/gh" <<'SH'
#!/usr/bin/env bash
printf '%s\n' "$*" >> "$MX_TEST_GH_LOG"
case " $* " in
  ...
  *" state "*)
    ...
    [ "${MX_TEST_GH_SLEEP:-0}" = 0 ] || sleep "$MX_TEST_GH_SLEEP"
    printf '%s\n' "${MX_TEST_GH_STATE:-OPEN}"
    ;;
esac
SH
...
write_task_meta() {
  ...
  mx_write_meta "$dir/home/state/$id.meta" \
    "window=mx-$id" \
    "worktree=$dir/wt" \
    "project=$dir/project" \
    "kind=delivery" \
...
write_manual_poll_pair() {
  ...
  mx_pr_url_parse "$url" || fail "manual poll fixture URL was invalid"
...
start_ambiguous_pending_repair() {
  ...
  set +e
  MX_HOME="$dir/home" PATH="$BASE_PATH" "$MIGRATE" >/dev/null 2>/dev/null
  rc=$?
  set -e
...
write_watcher_lock() {
  ...
  identity=$(MX_STATE_OVERRIDE="$state" bash -c '. "$1"; mx_pid_identity "$2"' _ "$ROOT/bin/mx-wake-lib.sh" "$pid")
  [ -n "$identity" ] || fail "could not capture fake older-watcher identity"
  printf '%s\n' "$pid" > "$state/.watch.lock/pid"
  printf '%s\n' "$home" > "$state/.watch.lock/mx-home"
...
assert_valid_migration_marker() {
  ...
  grep -qxF mx-pr-check-migration-v1 "$marker" || fail "migration marker bytes were not exact"
...
assert_valid_scan_marker() {
  ...
  grep -qxF mx-pr-check-migration-scan-v1 "$marker" || fail "migration scan marker bytes were not exact"
...
test_parser_matrix() {
  ...
  while IFS='|' read -r url owner repo number; do
    [ -n "$url" ] || continue
    mx_pr_url_parse "$url" || fail "parser rejected canonical URL"
    [ "$MX_PR_URL" = "$url" ] || fail "parser changed canonical URL"
    [ "$MX_PR_OWNER" = "$owner" ] || fail "parser returned wrong owner"
    [ "$MX_PR_REPO" = "$repo" ] || fail "parser returned wrong repository"
    [ "$MX_PR_NUMBER" = "$number" ] || fail "parser returned wrong PR number"
  done <<'EOF'
...
EOF
  mx_pr_url_parse https://github.com/a/b/pull/1 || fail "parser rejected canonical URL"
  [ "$MX_PR_PROVIDER" = github ] || fail "parser did not tag a pull request URL as github"
  [ "$MX_PR_HOST" = github.com ] || fail "parser returned wrong GitHub host"
  [ "$MX_PR_PATH" = a/b ] || fail "parser returned wrong GitHub project path"
  for row in "${INVALID_URLS[@]}"; do
    ! mx_pr_url_parse "$row" || fail "parser accepted a rejected raw-byte URL class"
  done
  for id in -task task- task--a Task-a task_a task.a; do
    mx_pr_task_id_valid "$id" || fail "task ID validator rejected a safe lifecycle-compatible slug"
  done
  mx_task_id_creation_valid _noncanonical \
  ...
  mx_pr_task_id_valid "$id" || fail "operational validator rejected a path-safe legacy task ID"
  ! mx_task_id_creation_valid "$id" || fail "creation validator accepted an overlong task ID"
...
test_valid_recording_and_merge_derivation() {
  ...
  MX_TEST_GH_HEAD=$expected run_check_entry "$dir" task-a https://github.com/my-org/repo_name.with-dots/pull/37 \
  ...
  [ "$(mx_pr_file_link_count "$dir/home/state/task-a.check.sh")" = 1 ] \
    && [ "$(mx_pr_file_link_count "$dir/home/state/task-a.pr-poll")" = 1 ] \
    && [ "$(mx_pr_file_link_count "$dir/home/state/task-a.pr-poll-registration")" = 1 ] \
    || fail "published poll artifacts were not single-link files"
  mx_pr_poll_artifacts_valid "$dir/home/state" task-a "$POLL" \
  ...
  mx_pr_poll_artifacts_valid "$dir/home/state" Task_A.1 "$POLL" \
...
test_rejected_metacharacter_bytes_are_inert() {
  ...
  for family in "${families[@]}"; do
    ...
    mx_pr_poll_prepare "$dir/home/state" safe-check github https://github.com/o/r/pull/99 github.com o/r 99 "$POLL" \
      || fail "could not prepare bounded watcher poll"
    mx_pr_poll_publish_prepared || fail "could not publish bounded watcher poll"

    set +e
    MX_TEST_GH_STATE=MERGED run_watcher_bounded "$dir/home" "$dir/fakebin" > "$dir/watch.out" 2> "$dir/watch.err"
    rc=$?
    set -e
  ...
  MX_TEST_GH_STATE=OPEN run_check_entry "$dir" task-a https://github.com/o/r/pull/1 >/dev/null 2>/dev/null \
...
test_static_poll_contract() {
  ...
  make_poll_fixture "$dir"
  set +e
  out=$(MX_STATE_OVERRIDE="$dir/home/state" MX_CHECK_TIMEOUT=1 MX_TEST_GH_LOG="$dir/gh.log" \
    MX_TEST_GH_SLEEP=3 PATH="$dir/fakebin:$BASE_PATH" \
  ...
  mx_pr_poll_prepare "$dir/home/state" task-a github https://github.com/o/r/pull/1 github.com o/r 1 "$POLL" \
    || fail "could not prepare authenticated watcher poll"
  mx_pr_poll_publish_prepared || fail "could not publish authenticated watcher poll"
  rm -f "$dir/home/state/.last-check"
  set +e
  MX_TEST_GH_STATE=MERGED run_watcher_bounded "$dir/home" "$dir/fakebin" > "$dir/watch.out" 2> "$dir/watch.err"
  rc=$?
...
SH
  ...
  ! find "$dir/home/state" -name '.mx-pr-poll-*' -print | grep . >/dev/null \
...
SH
    ...
    direct_pid=$!
    i=0
    while [ "$i" -lt 100 ] && ! find "$dir/home/state" -name '.mx-pr-poll-check.*' -print | grep . >/dev/null; do
      sleep 0.01
      i=$((i + 1))
    done
...
test_migration_excludes_older_watcher_before_scan() {
  ...
  mx_write_meta "$state/task-a.meta" \
    'window=mx-task-a' \
...
SH
  ...
  set +e
  MX_HOME="$dir/home" PATH="$dir/fakebin:$BASE_PATH" "$MIGRATE" > "$dir/migrate.out" 2> "$dir/migrate.err"
  rc=$?
  set -e
...
test_migration_initializes_fresh_state() {
  ...
  set +e
  MX_HOME="$dir/home" PATH="$BASE_PATH" "$MIGRATE" > "$dir/migrate.out" 2> "$dir/migrate.err"
  rc=$?
  set -e
...
test_private_artifact_paths_refuse_symlinks_and_directories() {
  ...
  for artifact in task-a.pr-poll task-a.pr-poll-registration task-a.check.sh; do
    for kind in regular dangling directory; do
      ...
      state="$dir/home/state"
      mx_pr_poll_prepare "$state" task-a github https://github.com/o/r/pull/1 github.com o/r 1 "$POLL" \
        || fail "could not stage poll symlink refusal fixture"
      destination="$state/$artifact"
      make_private_symlink "$dir" "$destination" "$kind"
      if mx_pr_poll_publish_prepared; then
...
#!/usr/bin/env bash
last=${!#}
"${MX_TEST_REAL_MV:?}" "$@" || exit $?
[ "$last" = "${MX_TEST_FINAL_PATH:?}" ] || exit 0
case "${MX_TEST_FINAL_ACTION:?}" in
  type)
    rm -f -- "$last"
    ln -s "${MX_TEST_FAULT_LINK_TARGET:?}" "$last"
    ;;
  mode) "${MX_TEST_REAL_CHMOD:?}" 0644 "$last" ;;
  content) printf 'faulted final bytes\n' > "$last" ;;
  device) : > "${MX_TEST_FAULT_GATE:?}" ;;
  *) exit 2 ;;
esac
SH
...
#!/usr/bin/env bash
last=${!#}
if [ "$last" = "${MX_TEST_FINAL_PATH:-}" ] && [ -e "${MX_TEST_FAULT_GATE:-/nonexistent}" ]; then
  case " $* " in
    *" %d "*) printf '%s\n' 999999; exit 0 ;;
  esac
fi
exec "${MX_TEST_REAL_STAT:?}" "$@"
SH
...
test_postrename_poll_validation_revokes_and_retries() {
  ...
  for artifact in data registration check; do
    for action in type mode device content; do
      ...
      mx_pr_poll_publish_prepared || fail "could not publish prior poll"
      write_poll_meta "$state" task-a https://github.com/o/r/pull/2
      mx_pr_poll_prepare "$state" task-a github https://github.com/o/r/pull/2 github.com o/r 2 "$POLL" \
      ...
      if MX_TEST_FINAL_PATH="$destination" MX_TEST_FINAL_ACTION="$action" \
        MX_TEST_FAULT_LINK_TARGET="$link_target" MX_TEST_FAULT_GATE="$gate" \
        MX_TEST_REAL_MV="$REAL_MV" MX_TEST_REAL_STAT="$REAL_STAT" MX_TEST_REAL_CHMOD="$REAL_CHMOD" \
        PATH="$dir/fakebin:$BASE_PATH" mx_pr_poll_publish_prepared; then
      ...
      PATH="$BASE_PATH" mx_pr_poll_publish_prepared || fail "poll retry did not recover after final validation fault"
      mx_pr_poll_artifacts_valid "$state" task-a "$POLL" || fail "poll retry did not publish a valid pair"
    done
  done
...
for arg in "$@"; do
  case "$arg" in
    *"${MX_TEST_MV_MATCH:?}"*) matched=1 ;;
  esac
done
if [ "$matched" -eq 1 ]; then
  case "${MX_TEST_MV_ACTION:?}" in
    fail) exit 1 ;;
    signal)
      kill -TERM "$PPID"
      sleep 0.1
...
test_marker_and_diagnostic_rename_fail_closed() {
  ...
  for action in fail signal; do
    ...
    set +e
    MX_TEST_MV_MATCH=.mx-pr-check-migration. MX_TEST_MV_ACTION="$action" MX_TEST_REAL_MV="$REAL_MV" \
      MX_HOME="$dir/home" PATH="$dir/fakebin:$BASE_PATH" "$MIGRATE" > "$dir/migrate.out" 2> "$dir/migrate.err"
    rc=$?
    set -e
    ...
    ! find "$state" -name '.mx-pr-check-migration.*' -print | grep . >/dev/null \
    ...
    install_mv_fault "$dir"
    set +e
    MX_TEST_MV_MATCH=.mx-pr-check-log. MX_TEST_MV_ACTION="$action" MX_TEST_REAL_MV="$REAL_MV" \
      MX_HOME="$dir/home" PATH="$dir/fakebin:$BASE_PATH" "$MIGRATE" > "$dir/migrate.out" 2> "$dir/migrate.err"
    rc=$?
    set -e
    ...
    ! find "$state" -name '.mx-pr-check-log.*' -print | grep . >/dev/null \
...
test_postrename_marker_and_diagnostic_validation_retries() {
  ...
  for artifact in marker diagnostic obligation; do
    for action in type mode device content; do
      ...
      install_final_publication_fault "$dir"
      set +e
      MX_TEST_FINAL_PATH="$destination" MX_TEST_FINAL_ACTION="$action" \
        MX_TEST_FAULT_LINK_TARGET="$link_target" MX_TEST_FAULT_GATE="$gate" \
        MX_TEST_REAL_MV="$REAL_MV" MX_TEST_REAL_STAT="$REAL_STAT" MX_TEST_REAL_CHMOD="$REAL_CHMOD" \
        MX_HOME="$dir/home" PATH="$dir/fakebin:$BASE_PATH" "$MIGRATE" > "$dir/migrate.out" 2> "$dir/migrate.err"
      rc=$?
...
last=${!#}
case "$last" in
  ${MX_TEST_CHMOD_MATCH:?}) exit 0 ;;
esac
exec "${MX_TEST_REAL_CHMOD:?}" "$@"
SH
...
test_quarantine_validation_and_retry_contract() {
  ...
  set +e
  MX_TEST_CHMOD_MATCH="$state/.pr-check-quarantine" MX_TEST_REAL_CHMOD="$REAL_CHMOD" \
    MX_HOME="$dir/home" PATH="$dir/fakebin:$BASE_PATH" "$MIGRATE" >/dev/null 2>/dev/null
  rc=$?
  set -e
  ...
  install_chmod_noop_fault "$dir"
  set +e
  MX_TEST_CHMOD_MATCH="$state/.pr-check-quarantine/task-a.check.*" MX_TEST_REAL_CHMOD="$REAL_CHMOD" \
    MX_HOME="$dir/home" PATH="$dir/fakebin:$BASE_PATH" "$MIGRATE" >/dev/null 2>/dev/null
  rc=$?
  set -e
...
"${MX_TEST_REAL_MV:?}" "$@" || exit $?
case "$last" in
  */.pr-check-quarantine/task-a.check.*) : > "${MX_TEST_FAULT_GATE:?}" ;;
esac
SH
  cat > "$dir/fakebin/stat" <<'SH'
#!/usr/bin/env bash
...
SH
  chmod +x "$dir/fakebin/mv" "$dir/fakebin/stat"
  set +e
  MX_TEST_REAL_MV="$REAL_MV" MX_TEST_REAL_STAT="$REAL_STAT" MX_TEST_FAULT_GATE="$dir/device-fault" \
    MX_HOME="$dir/home" PATH="$dir/fakebin:$BASE_PATH" "$MIGRATE" >/dev/null 2>/dev/null
  rc=$?
  set -e
...
SH
  chmod +x "$dir/fakebin/mv"
  set +e
  MX_TEST_REAL_MV="$REAL_MV" MX_TEST_REAL_CP="$REAL_CP" \
    MX_HOME="$dir/home" PATH="$dir/fakebin:$BASE_PATH" "$MIGRATE" >/dev/null 2>/dev/null
  rc=$?
  set -e
...
SH
  chmod +x "$dir/fakebin/mv"
  set +e
  MX_TEST_REAL_MV="$REAL_MV" MX_TEST_FAULT_LINK_TARGET="$external" \
    MX_HOME="$dir/home" PATH="$dir/fakebin:$BASE_PATH" "$MIGRATE" >/dev/null 2>/dev/null
  rc=$?
  set -e
...
test_ambiguous_failure_accepts_validated_replacement() {
  ...
  mx_pr_poll_artifacts_valid "$state" task-a "$POLL" \
    || fail "replacement registration did not publish a valid poll pair"

  MX_HOME="$dir/home" PATH="$BASE_PATH" "$MIGRATE" > "$dir/migrate-retry.out" 2> "$dir/migrate-retry.err" \
  ...
  for case_name in copied-pair copied-registration metadata-mismatch task-mismatch forged-registration partial-publication; do
    ...
    case "$case_name" in
      ...
      copied-registration)
        ...
        mx_pr_poll_prepare "$donor" task-a github https://github.com/o/r/pull/10 github.com o/r 10 "$POLL" \
          || fail "could not prepare donor registration fixture"
        mx_pr_poll_publish_prepared || fail "could not publish donor registration fixture"
        ...
        ;;
      metadata-mismatch)
        mx_pr_poll_prepare "$state" task-a github https://github.com/o/r/pull/10 github.com o/r 10 "$POLL" \
          || fail "could not prepare metadata-mismatch fixture"
        mx_pr_poll_publish_prepared || fail "could not publish metadata-mismatch fixture"
        write_poll_meta "$state" task-a https://github.com/o/r/pull/11
        ;;
      task-mismatch)
        mx_pr_poll_prepare "$state" task-a github https://github.com/o/r/pull/10 github.com o/r 10 "$POLL" \
          || fail "could not prepare task-mismatch fixture"
        mx_pr_poll_publish_prepared || fail "could not publish task-mismatch fixture"
      ...
      forged-registration)
        write_manual_poll_pair "$state"
        printf '%s\n%s\n%s\n%s\n%s\n%s\n%s\n%s\n%s\n%s\n%s\n' \
          mx-pr-poll-registration-v2 task-a github https://github.com/o/r/pull/10 github.com o/r 10 \
    ...
    set +e
    MX_HOME="$dir/home" PATH="$BASE_PATH" "$MIGRATE" > "$dir/retry.out" 2> "$dir/retry.err"
    rc=$?
    set -e
...
test_complete_single_link_validation() {
  ...
  set +e
  MX_HOME="$dir/home" "$REGISTER" custom > "$dir/register.out" 2> "$dir/register.err"
  rc=$?
  set -e
...
test_failed_outcomes_block_every_retry_until_repaired() {
  ...
  for classification in canonical ambiguous; do
    ...
    set +e
    MX_HOME="$dir/home" PATH="$BASE_PATH" "$MIGRATE" > "$dir/migrate-1.out" 2> "$dir/migrate-1.err"
    rc=$?
    set -e
    ...
    printf '%s\n' mx-pr-check-migration-v1 > "$state/.pr-check-migration-v1"
    chmod 0600 "$state/.pr-check-migration-v1"

    set +e
    MX_HOME="$dir/home" PATH="$BASE_PATH" "$MIGRATE" > "$dir/migrate-2.out" 2> "$dir/migrate-2.err"
    rc=$?
    set -e
    ...
    rmdir "$state/task-a.pr-poll"
    MX_HOME="$dir/home" PATH="$BASE_PATH" "$MIGRATE" > "$dir/migrate-3.out" 2> "$dir/migrate-3.err" \
    ...
    if [ "$classification" = canonical ]; then
      ...
      mx_pr_poll_artifacts_valid "$state" task-a "$POLL" || fail "canonical repaired retry did not arm a valid poll pair"
    else
...
test_canonical_publication_failure_recovers_only_on_retry() {
  ...
  set +e
  MX_TEST_FINAL_PATH="$destination" MX_TEST_FINAL_ACTION=mode \
    MX_TEST_FAULT_LINK_TARGET="$link_target" MX_TEST_FAULT_GATE="$gate" \
    MX_TEST_REAL_MV="$REAL_MV" MX_TEST_REAL_STAT="$REAL_STAT" MX_TEST_REAL_CHMOD="$REAL_CHMOD" \
    MX_HOME="$dir/home" PATH="$dir/fakebin:$BASE_PATH" "$MIGRATE" > "$dir/migrate-1.out" 2> "$dir/migrate-1.err"
  rc=$?
  set -e
  ...
  mx_pr_poll_artifacts_valid "$state" task-a "$POLL" || fail "canonical publication retry did not arm a valid pair"
  assert_valid_migration_marker "$state/.pr-check-migration-v1"
  [ ! -e "$pending" ] && [ ! -L "$pending" ] \
...
test_obligation_namespace_compatibility() {
  ...
  mx_write_meta "$state/_noncanonical.meta" \
    'window=mx-_noncanonical' \
    "worktree=$dir/missing-worktree" \
    "project=$dir/project" \
    'kind=delivery' \
    'mode=local-only'
...
SH
  ...
  MX_HOME="$dir/home" "$MIGRATE" > "$dir/migrate.out" 2> "$dir/migrate.err" \
  ...
  printf 'legacy quarantined bytes\n' \
    > "$state/.pr-check-quarantine/_noncanonical.check.abc123"
  printf 'mx-pr-check-migration-scan-v1\n' > "$state/.pr-check-migration-scan-v1"
  printf 'mx-pr-check-migration-v1\n' > "$state/.pr-check-migration-v1"
  chmod 0600 "$state/.pr-check-quarantine/"* \
    "$state/.pr-check-migration-scan-v1" "$state/.pr-check-migration-v1"
  MX_HOME="$dir/home" "$MIGRATE" --checks-safe > "$dir/migrate.out" 2> "$dir/migrate.err" \
  ...
  state="$dir/home/state"
  mx_write_meta "$state/foo.diagnostic.bar.meta" \
    'window=mx-foo.diagnostic.bar' \
  ...
  mx_pr_poll_artifacts_valid "$state" foo.diagnostic.bar "$POLL" \
...
test_nonexecuting_migration() {
  local dir state marker snap_before snap_after rc
  ...
  mx_pr_poll_artifacts_valid "$state" task-a "$POLL" || fail "canonical migration did not leave a validated armed poll"
  assert_valid_migration_marker "$state/.pr-check-migration-v1"
  ...
  MX_HOME="$dir/home" "$MIGRATE" > "$dir/migrate-2.out" 2> "$dir/migrate-2.err" \
  ...
  snap_before=$(state_snapshot "$state")
  MX_HOME="$dir/home" "$MIGRATE" >/dev/null 2>/dev/null || fail "completed migration rerun failed"
  snap_after=$(state_snapshot "$state")
  [ "$snap_after" = "$snap_before" ] || fail "completed migration changed a later custom check"

  dir=$(make_case migration-ambiguous)
  state="$dir/home/state"
  mx_write_meta "$state/task-b.meta" \
    'window=mx-task-b' \
    'pr=https://github.com/o/r/pull/10' \
  ...
  assert_valid_migration_marker "$state/.pr-check-migration-v1"
  pass "migration never executes legacy checks, quarantines ambiguity, and is idempotent"
}
...
test_bootstrap_migrates_before_other_mutations() {
  ...
  MX_HOME="$dir/home" MX_ROOT_OVERRIDE="$ROOT" PATH="$dir/fakebin:$BASE_PATH" \
    "$ROOT/bin/mx-bootstrap.sh" > "$dir/bootstrap.out" 2> "$dir/bootstrap.err" \
...
test_bootstrap_isolates_incomplete_poll_migration() {
  local dir state fakebin system_marker rc
  dir=$(make_case bootstrap-migration-isolation)
  state="$dir/home/state"
  fakebin="$dir/fakebin"
  system_marker="$dir/system-ran"
  mx_write_meta "$state/task-a.meta" \
    'window=mx-task-a' \
    'pr=https://github.com/o/r/pull/12'
  printf 'legacy bytes\n' > "$state/task-a.check.sh"
  ...
  mx_pr_poll_prepare "$state" z-healthy github https://github.com/o/r/pull/13 github.com o/r 13 "$POLL" \
    || fail "could not prepare healthy poll for migration isolation"
  mx_pr_poll_publish_prepared || fail "could not publish healthy poll for migration isolation"
  mx_write_meta "$state/daemon-a.meta" \
    'window=broker:mx-daemon-a' \
    'kind=daemon' \
    'harness=codex' \
    'backend=tmux'
  mkdir -p "$dir/home/projects"
  mx_fake_exit0 "$fakebin" curl jq
  cat > "$fakebin/tmux" <<'SH'
#!/usr/bin/env bash
case " $* " in
  *' list-windows '*) printf 'mx-daemon-a\n' ;;
  *' display-message '*) printf 'node\n' ;;
esac
SH
  cat > "$dir/root/bin/mx-system-sync.sh" <<'SH'
#!/usr/bin/env bash
: > "${MX_TEST_SYSTEM_MARKER:?}"
printf 'alpha: recovered: continued after isolated migration failure\n'
SH
  chmod +x "$fakebin/tmux" "$dir/root/bin/mx-system-sync.sh"

  set +e
  MX_HOME="$dir/home" MX_ROOT_OVERRIDE="$dir/root" MX_TEST_SYSTEM_MARKER="$system_marker" \
    PATH="$fakebin:$BASE_PATH" "$ROOT/bin/mx-bootstrap.sh" > "$dir/bootstrap.out" 2> "$dir/bootstrap.err"
  rc=$?
  set -e
  ...
  assert_grep 'SYSTEM_SYNC: alpha: recovered: continued after isolated migration failure' "$dir/bootstrap.out" \
    "continued system refresh was not operator-visible"
  printf '%s\n' '#!/usr/bin/env bash' "printf '%s\\n' replacement-ran" > "$state/a-replaced.check.sh"
  chmod 0600 "$state/a-replaced.check.sh"
  set +e
  MX_HOME="$dir/home" MX_ROOT_OVERRIDE="$dir/root" \
    MX_TEST_GH_STATE=MERGED MX_POLL=0 MX_CHECK_INTERVAL=0 MX_SIGNAL_GRACE=0 \
  ...
  rm -f "$state/a-replaced.check.sh" "$state/.last-check"
  printf '%s\n' '#!/usr/bin/env bash' "printf '%s\\n' custom-ready" > "$state/b-custom.check.sh"
  chmod 0700 "$state/b-custom.check.sh"
  MX_HOME="$dir/home" "$REGISTER" b-custom > "$dir/register.out" \
  ...
  set +e
  MX_HOME="$dir/home" MX_ROOT_OVERRIDE="$dir/root" \
    MX_TEST_GH_STATE=OPEN MX_POLL=0 MX_CHECK_INTERVAL=0 MX_SIGNAL_GRACE=0 \
...
test_custom_snapshot_cleanup_on_signal() {
  ...
  printf '%s\n' '#!/usr/bin/env bash' 'trap "" TERM' \
    'printf "%s\n" "$$" > "$MX_TEST_CUSTOM_CHILD_PID"' 'while :; do sleep 1; done' \
    > "$state/custom.check.sh"
  chmod 0700 "$state/custom.check.sh"
...
SH
  ...
  MX_HOME="$dir/home" MX_ROOT_OVERRIDE="$ROOT" MX_POLL=0 MX_CHECK_INTERVAL=0 \
    MX_SIGNAL_GRACE=0 MX_TEST_CUSTOM_CHILD_PID="$child_pid_file" \
...
SH
    ...
    MX_HOME="$dir/home" MX_ROOT_OVERRIDE="$ROOT" MX_POLL=0.1 MX_CHECK_INTERVAL=999999 \
      MX_CHECK_TIMEOUT=10 MX_HEARTBEAT=999999 MX_SIGNAL_GRACE=0 \
      MX_CHECK_FORCE_FALLBACK="$force_fallback" MX_TEST_DESCENDANT_READY="$ready" \
      MX_TEST_DESCENDANT_SENTINEL="$sentinel" MX_TEST_DESCENDANT_PID="$child_pid_file" \
      MX_TEST_DIRECT_DONE="$direct_done" PATH="$fakebin:$BASE_PATH" "$WATCH" \
      > "$dir/watch.out" 2> "$dir/watch.err" &
    watcher_pid=$!
    i=0
    while [ "$i" -lt 200 ]; do
...
test_teardown_removes_poll_artifacts() {
  ...
  fakebin="$dir/fakebin"
  mx_write_meta "$dir/home/state/task-a.meta" \
    'window=mx-task-a' \
    "worktree=$dir/missing-worktree" \
    "project=$dir/project" \
    'kind=delivery' \
...
SH
  ...
  mx_pr_poll_snapshot_capture "$dir/home/state" task-a "$POLL" \
    || fail "could not snapshot teardown receipt fixture"
  mx_pr_poll_retirement_publish "$dir/home/state" task-a "$POLL" merged \
...
SH
  ...
  fakebin="$dir/fakebin"
  mx_write_meta "$dir/home/state/invalid.meta" \
    'window=mx-invalid' \
    "worktree=$dir/missing-worktree" \
    "project=$dir/project" \
    'kind=delivery' \
...
seed_canonical_poll() {
  ...
  state="$dir/home/state"
  mx_pr_url_parse "$url" || fail "retirement fixture URL was invalid"
  provider=$MX_PR_PROVIDER
  host=$MX_PR_HOST
  path=$MX_PR_PATH
  number=$MX_PR_NUMBER
  mx_pr_poll_prepare "$state" "$id" "$provider" "$url" "$host" "$path" "$number" "$template" \
    || fail "could not prepare retirement fixture"
  mx_pr_poll_publish_prepared || fail "could not publish retirement fixture"
  printf '%s\n' mx-pr-check-migration-scan-v1 > "$state/.pr-check-migration-scan-v1"
  printf '%s\n' mx-pr-check-migration-v1 > "$state/.pr-check-migration-v1"
  chmod 0600 "$state/.pr-check-migration-scan-v1" "$state/.pr-check-migration-v1"
}
...
add_stop_custom_check() {
  ...
  MX_HOME="$dir/home" "$REGISTER" z-stop >/dev/null \
...
test_merged_poll_retires_once() {
  ...
  set +e
  MX_TEST_GH_STATE=MERGED run_watcher_bounded "$dir/home" "$dir/fakebin" > "$dir/watch-1.out" 2> "$dir/watch-1.err"
  rc=$?
  set -e
  ...
  rm -f "$state/.last-check"
  set +e
  MX_TEST_GH_STATE=MERGED run_watcher_bounded "$dir/home" "$dir/fakebin" > "$dir/watch-2.out" 2> "$dir/watch-2.err"
  rc=$?
  set -e
...
test_persistent_daemon_retirement_is_poll_only() {
  ...
  dir=$(make_case merged-retirement-daemon)
  state="$dir/home/state"
  mx_write_meta "$state/domain.meta" \
    'window=session:mx-domain' \
    "worktree=$dir/daemon-home" \
    "project=$dir/project" \
    'kind=daemon' \
    'mode=daemon' \
    'backend=tmux' \
    "home=$dir/daemon-home" \
    'pr=https://github.com/o/r/pull/2' \
    'pr_head=0123456789abcdef0123456789abcdef01234567'
  mkdir -p "$dir/daemon-home"
  printf 'working: persistent endpoint remains healthy\n' > "$state/domain.status"
  printf -- '- domain | scope: test | home: %s\n' "$dir/daemon-home" > "$dir/home/data/daemons.md"
  ...
  registry_before=$(shasum -a 256 "$dir/home/data/daemons.md")
  ...
  rc=$?
  set -e
  [ "$rc" -eq 0 ] || fail "persistent daemon merged watcher failed: $(cat "$dir/watch.err")"
  assert_poll_absent "$state" domain
  [ "$(shasum -a 256 "$state/domain.meta")" = "$meta_before" ] || fail "retirement changed daemon metadata"
  [ "$(shasum -a 256 "$state/domain.status")" = "$status_before" ] || fail "retirement changed daemon status"
  [ "$(shasum -a 256 "$dir/home/data/daemons.md")" = "$registry_before" ] || fail "retirement changed daemon registry"
  [ "$(shasum -a 256 "$dir/endpoint-sentinel")" = "$endpoint_before" ] || fail "retirement changed daemon endpoint evidence"
  [ -d "$dir/daemon-home" ] || fail "retirement removed the persistent daemon home"
  pass "merged poll retirement preserves every persistent daemon lifecycle artifact"
}
...
test_retirement_crash_recovery() {
  ...
  MX_STATE_OVERRIDE="$state" bash -c '. "$1"; mx_wake_append check "$2" "$3"' _ \
    "$ROOT/bin/mx-wake-lib.sh" "$state/task-a.check.sh" "check: $state/task-a.check.sh: merged" \
  ...
  mx_pr_poll_snapshot_capture "$state" task-a "$POLL" || fail "could not snapshot receipt crash fixture"
  mx_pr_poll_retirement_publish "$state" task-a "$POLL" merged || fail "could not publish crash receipt"
  add_stop_custom_check "$dir"
  set +e
  MX_TEST_GH_STATE=MERGED run_watcher_bounded "$dir/home" "$dir/fakebin" > "$dir/restart.out" 2> "$dir/restart.err"
  rc=$?
  set -e
  ...
  mx_pr_poll_snapshot_capture "$state" task-a "$POLL" || fail "could not snapshot partial removal fixture"
  mx_pr_poll_retirement_publish "$state" task-a "$POLL" merged || fail "could not publish partial removal receipt"
  rm -f "$state/task-a.check.sh"
  add_stop_custom_check "$dir"
  set +e
  MX_TEST_GH_STATE=MERGED run_watcher_bounded "$dir/home" "$dir/fakebin" > "$dir/restart.out" 2> "$dir/restart.err"
  ...
  assert_poll_absent "$state" task-a
  mx_pr_poll_retirement_recover_one "$state" task-a "$POLL" || fail "completed retirement was not idempotent"

  dir=$(make_case retirement-after-registration-removal)
  state="$dir/home/state"
  write_poll_meta "$state" task-a https://github.com/o/r/pull/5
  seed_canonical_poll "$dir" task-a https://github.com/o/r/pull/5
  mx_pr_poll_snapshot_capture "$state" task-a "$POLL" || fail "could not snapshot sidecar-removal fixture"
  mx_pr_poll_retirement_publish "$state" task-a "$POLL" merged || fail "could not publish sidecar-removal receipt"
  ...
  write_poll_meta "$state" task-a https://github.com/o/r/pull/5
  seed_canonical_poll "$dir" task-a https://github.com/o/r/pull/5
  mx_pr_poll_snapshot_capture "$state" task-a "$POLL" || fail "could not snapshot receipt-only fixture"
  mx_pr_poll_retirement_publish "$state" task-a "$POLL" merged || fail "could not publish receipt-only fixture"
  ...
  state="$dir/home/state"
  historical_poll="$dir/historical-mx-pr-poll.sh"
  ...
  mx_pr_poll_snapshot_capture "$state" task-a "$historical_poll" \
    || fail "could not snapshot pre-update retirement fixture"
  mx_pr_poll_retirement_publish "$state" task-a "$historical_poll" merged \
...
test_external_merge_transition_retires_only_terminal_poll() {
  ...
  for label in open-green open-red closed-unmerged forge-error malformed; do
    ...
    case "$label" in
      open-green|open-red)
        MX_TEST_GH_STATE=OPEN run_watcher_bounded "$dir/home" "$dir/fakebin" > "$dir/$label.out" 2> "$dir/$label.err"
        ;;
      closed-unmerged)
        MX_TEST_GH_STATE=CLOSED run_watcher_bounded "$dir/home" "$dir/fakebin" > "$dir/$label.out" 2> "$dir/$label.err"
        ;;
      forge-error)
        MX_TEST_GH_FAIL=1 run_watcher_bounded "$dir/home" "$dir/fakebin" > "$dir/$label.out" 2> "$dir/$label.err"
        ;;
      malformed)
        MX_TEST_GH_STATE=NOT_A_FORGE_STATE run_watcher_bounded "$dir/home" "$dir/fakebin" > "$dir/$label.out" 2> "$dir/$label.err"
        ;;
    esac
    rc=$?
    set -e
  ...
  set +e
  MX_TEST_GH_STATE=MERGED run_watcher_bounded "$dir/home" "$dir/fakebin" > "$dir/merged.out" 2> "$dir/merged.err"
  rc=$?
  set -e
...
test_retirement_refuses_replacement_and_nonterminal_results() {
  ...
  mx_pr_poll_snapshot_capture "$state" task-a "$POLL" || fail "could not snapshot replacement fixture"
  write_poll_meta "$state" task-a https://github.com/o/r/pull/7
  seed_canonical_poll "$dir" task-a https://github.com/o/r/pull/7
  before=$(state_snapshot "$state")
  mx_pr_poll_retirement_publish "$state" task-a "$POLL" merged \
  ...
  mx_pr_poll_artifacts_valid "$state" task-a "$POLL" || fail "replacement poll was not left canonical"

  mx_pr_poll_snapshot_capture "$state" task-a "$POLL" || fail "could not snapshot nonterminal fixture"
  for result in OPEN CLOSED ERROR 'merged extra'; do
    mx_pr_poll_retirement_publish "$state" task-a "$POLL" "$result" \
  ...
  mx_pr_poll_snapshot_capture "$state" task-a "$POLL" || fail "could not snapshot rearm-race fixture"
  mx_pr_poll_retirement_publish "$state" task-a "$POLL" merged || fail "could not publish rearm-race receipt"
  ...
  mx_pr_poll_retirement_recover_one "$state" task-a "$POLL" || fail "stale receipt did not yield to a canonical replacement"
  ...
  mx_pr_poll_artifacts_valid "$state" task-a "$POLL" || fail "replacement poll lost canonical provenance"

  dir=$(make_case retirement-template-update-rearm-race)
  state="$dir/home/state"
  historical_poll="$dir/historical-mx-pr-poll.sh"
  current_poll="$dir/current-mx-pr-poll.sh"
  cp "$POLL" "$historical_poll"
  ...
  replacement_registration=$(shasum -a 256 "$state/task-a.pr-poll-registration")
  replacement_meta=$(shasum -a 256 "$state/task-a.meta")
  mx_pr_poll_retirement_recover_one "$state" task-a "$current_poll" \
  ...
  mx_pr_poll_artifacts_valid "$state" task-a "$current_poll" || fail "updated replacement poll lost canonical provenance"

  dir=$(make_case custom-merged-not-retired)
  state="$dir/home/state"
  printf '#!/usr/bin/env bash\nprintf "merged\\n"\n' > "$state/custom.check.sh"
  chmod 0700 "$state/custom.check.sh"
  MX_HOME="$dir/home" "$REGISTER" custom >/dev/null || fail "could not register merged custom check"
  set +e
...
test_retirement_queue_failure_and_receipt_tampering() {
  ...
  mx_pr_poll_snapshot_capture "$state" task-a "$POLL" || fail "could not snapshot receipt tamper fixture"
  mx_pr_poll_retirement_publish "$state" task-a "$POLL" merged || fail "could not publish receipt tamper fixture"
  printf 'extra\n' >> "$state/task-a.pr-poll-retirement"
  before=$(state_snapshot "$state")
  mx_pr_poll_retirement_recover_one "$state" task-a "$POLL" \
