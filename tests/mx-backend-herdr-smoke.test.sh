#!/usr/bin/env bash
# tests/mx-backend-herdr-smoke.test.sh - real herdr smoke test for the herdr
# session-provider adapter (bin/backends/herdr.sh), P2 of
# data/mx-backend-design-d7 (herdr-addendum.md), extended for the P3
# workspace-per-home pass (AGENTS.md task herdr-sm-spaces-k4). Mirrors
# tests/mx-backend-tmux-smoke.test.sh's structure: every other suite fakes the
...
# maintainer's real herdr usage. Skips cleanly when herdr (or jq) is not
...
# that command killed the maintainer's live default herdr server twice in
...
mx_backend_herdr_version_check || fail "version_check failed against the real installed herdr"
pass "real herdr: version_check accepts the installed binary's protocol"

# mx_backend_herdr_container_ensure now echoes
...
# single tab character; only mx_backend_herdr_create_task is ever allowed to
# act on the seeded tab id, and only for the container that just created it.
CONTAINER_RAW=$(mx_backend_herdr_container_ensure /tmp) || fail "container_ensure failed"
...
# a tab as prunable.
CONTAINER2_RAW=$(mx_backend_herdr_container_ensure /tmp) || fail "second container_ensure failed"
...
LABEL="mx-smoke1"
TASK_IDS=$(mx_backend_herdr_create_task "$CONTAINER" "$LABEL" /tmp "$SEEDED_TAB_ID") || fail "create_task failed"
...
# exactly one clean mx-<id> task tab.
...
pass "real herdr: create_task prunes the freshly-created workspace's seeded default tab, leaving exactly one clean mx-<id> task tab"
...
# server restart, and a restored mx-<id> task tab comes back a HUSK: a dead
...
LIVE_DUP_LABEL="mx-smoke-livedup"
LIVE_DUP_IDS=$(mx_backend_herdr_create_task "$CONTAINER" "$LIVE_DUP_LABEL" /tmp) || fail "could not create the live-duplicate scenario's tab"
...
herdr pane report-agent "$LIVE_DUP_PANE_ID" --source mx-smoke-test --agent mx-smoke-live-agent --state idle --session "$SESSION" >/dev/null 2>&1 \
  || fail "could not register a live agent on the live-duplicate scenario's pane"
if mx_backend_herdr_create_task "$CONTAINER" "$LIVE_DUP_LABEL" /tmp >/dev/null 2>&1; then
...
mx_backend_herdr_kill "$SESSION:$LIVE_DUP_PANE_ID"

# 2. A husk (no registered agent at all - the restored-plain-shell shape)
#    must be CLOSED AND REPLACED instead of refused.
HUSK_LABEL="mx-smoke-husk1"
HUSK_IDS=$(mx_backend_herdr_create_task "$CONTAINER" "$HUSK_LABEL" /tmp) || fail "could not create the husk-simulation tab"
...
REPLACED_IDS=$(mx_backend_herdr_create_task "$CONTAINER" "$HUSK_LABEL" /tmp) \
...
mx_backend_herdr_kill "$SESSION:$NEW_HUSK_PANE_ID"

# --- workspace-per-home: a daemon-shaped home gets its OWN space --------
...
SM_SCRATCH=$(mktemp -d "${TMPDIR:-/tmp}/mx-herdr-smoke-sm.XXXXXX")
SM_HOME="$SM_SCRATCH/daemon-home"
mkdir -p "$SM_HOME"
printf 'smoketest-sm1\n' > "$SM_HOME/.mx-daemon-home"

SM_CONTAINER_RAW=$(MX_HOME="$SM_HOME" mx_backend_herdr_container_ensure /tmp) || fail "daemon-shaped container_ensure failed"
SM_CONTAINER=${SM_CONTAINER_RAW%%$'\t'*}
SM_SEEDED_TAB_ID=${SM_CONTAINER_RAW#*$'\t'}
case "$SM_CONTAINER" in
  "$SESSION":w*) : ;;
  *) fail "daemon container_ensure returned an unexpected shape: $SM_CONTAINER" ;;
esac
[ "$SM_CONTAINER" != "$CONTAINER" ] || fail "a daemon-shaped home must get a DIFFERENT workspace than the primary's, got the same: $SM_CONTAINER"
[ -n "$SM_SEEDED_TAB_ID" ] || fail "the daemon-shaped home's container_ensure must CREATE its own workspace and report its seeded default tab id"
pass "real herdr: a daemon-shaped home (.mx-daemon-home) gets its OWN herdr workspace, distinct from the primary's, in the SAME session"
...
if [ -z "$SM_TAB_ID" ] || [ -z "$SM_PANE_ID" ]; then
  fail "daemon create_task did not return tab/pane ids"
fi
pass "real herdr: a task spawned into the daemon-shaped home lands as a tab inside the daemon's OWN workspace"

# list_live for each home must never see the OTHER home's task.
PRIMARY_LIVE=$(mx_backend_herdr_list_live "$SESSION")
case "$PRIMARY_LIVE" in
  *"$SM_TASK_LABEL"*) fail "the primary home's list_live must not see a daemon-shaped home's task"$'\n'"$PRIMARY_LIVE" ;;
esac
SM_LIVE=$(MX_HOME="$SM_HOME" mx_backend_herdr_list_live "$SESSION")
case "$SM_LIVE" in
  *"$SM_TASK_LABEL"*) : ;;
  *) fail "the daemon-shaped home's list_live did not see its own task"$'\n'"$SM_LIVE" ;;
esac
case "$SM_LIVE" in
  *"$LABEL"*) fail "the daemon-shaped home's list_live must not see the primary's task ($LABEL)"$'\n'"$SM_LIVE" ;;
esac
...
mx_herdr_lab_stop "$SESSION" >/dev/null 2>&1 \
...
[ "$POST_SM_PANE" = "$SM_PANE_ID" ] || fail "the daemon task's pane id did not survive the restart: before=$SM_PANE_ID after=$POST_SM_PANE"
...
mx_backend_herdr_kill "$SESSION:$SM_PANE_ID"

# --- send_text_line (atomic run) ---------------------------------------------

mx_backend_herdr_send_text_line "$TARGET" "echo maintainer-on-deck-line" \
  || fail "send_text_line failed"
sleep 0.5
out=$(mx_backend_herdr_capture "$TARGET" 20) || fail "capture failed after send_text_line"
case "$out" in
  *maintainer-on-deck-line*) : ;;
...
mx_backend_herdr_send_literal "$TARGET" 'echo literal-then-key-maintainer' \
  || fail "send_literal failed"
sleep 0.2
mx_backend_herdr_send_key "$TARGET" Enter || fail "send_key Enter failed"
sleep 0.5
out=$(mx_backend_herdr_capture "$TARGET" 20) || fail "capture failed after send_literal+send_key"
case "$out" in
  *literal-then-key-maintainer*) : ;;
...
mx_backend_herdr_send_text_line "$TARGET" "cd /tmp"
sleep 0.3
p=$(mx_backend_herdr_current_path "$TARGET") || fail "current_path failed"
case "$p" in
  */tmp) : ;;
...
if [ "${MX_HERDR_SMOKE_REAL_CLAUDE:-0}" = 1 ] && command -v claude >/dev/null 2>&1; then
  mx_backend_herdr_send_literal "$TARGET" "CLAUDE_CODE_ENABLE_PROMPT_SUGGESTION=false claude --dangerously-skip-permissions --print 'say the word HERDRSMOKEOK and nothing else'"
  sleep 0.2
  mx_backend_herdr_send_key "$TARGET" Enter
  found_working=0
  for _ in $(seq 1 20); do
    bs=$(mx_backend_herdr_busy_state "$TARGET" 2>/dev/null)
  ...
  out=$(mx_backend_herdr_capture "$TARGET" 30)
  case "$out" in
...
elif [ "${MX_HERDR_SMOKE_REAL_CLAUDE:-0}" != 1 ]; then
  echo "note: MX_HERDR_SMOKE_REAL_CLAUDE=1 not set; skipping the real-agent busy_state check" >&2
else
...
mx_backend_herdr_kill "$TARGET" || fail "kill on an already-dead target must stay best-effort (never fail)"
pass "real herdr: kill removes the pane and is idempotent/best-effort"

# --- list_live (label-based recovery discovery) ------------------------------

# Real broker spawns always re-run container_ensure immediately before
# create_task (bin/mx-spawn.sh), never reusing a container reference from an
...
# workspace.
CONTAINER_RAW=$(mx_backend_herdr_container_ensure /tmp) || fail "container_ensure for the second task failed"
CONTAINER=${CONTAINER_RAW%%$'\t'*}
SEEDED_TAB_ID=${CONTAINER_RAW#*$'\t'}
...
LABEL2="mx-smoke2"
TASK_IDS2=$(mx_backend_herdr_create_task "$CONTAINER" "$LABEL2" /tmp "$SEEDED_TAB_ID") || fail "second create_task failed"
read -r _TAB_ID2 PANE_ID2 <<EOF
$TASK_IDS2
EOF
live=$(mx_backend_herdr_list_live "$SESSION")
...
pass "real herdr: list_live discovers a live task tab by mx-<id> label"

mx_backend_herdr_kill "$SESSION:$PANE_ID2"
