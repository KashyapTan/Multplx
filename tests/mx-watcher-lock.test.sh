#!/usr/bin/env bash
# tests/mx-watcher-lock.test.sh - watcher singleton + lock-primitive races +
...
WATCH_ARM="$ROOT/bin/mx-watch-arm.sh"
DRAIN="$ROOT/bin/mx-wake-drain.sh"
LIB="$ROOT/bin/mx-wake-lib.sh"

TMP_ROOT=$(mx_test_tmproot mx-watcher-lock-tests)
...
test_singleton_start() {
  ...
  PATH="$fakebin:$PATH" MX_STATE_OVERRIDE="$state" MX_POLL=5 MX_SIGNAL_GRACE=1 MX_CHECK_INTERVAL=999999 MX_HEARTBEAT=999999 "$WATCH" > "$out1" &
  pid1=$!
  PATH="$fakebin:$PATH" MX_STATE_OVERRIDE="$state" MX_POLL=5 MX_SIGNAL_GRACE=1 MX_CHECK_INTERVAL=999999 MX_HEARTBEAT=999999 "$WATCH" > "$out2" &
  pid2=$!
  i=0
  while [ "$i" -lt 50 ]; do
    live=0
...
test_stale_watch_lock_reclaimed() {
  ...
  PATH="$fakebin:$PATH" MX_STATE_OVERRIDE="$state" MX_POLL=5 MX_SIGNAL_GRACE=1 MX_CHECK_INTERVAL=999999 MX_HEARTBEAT=999999 "$WATCH" > "$out" &
  pid=$!
  i=0
  live=0
  lock_pid=
...
test_live_stale_watch_lock_is_actionable() {
  ...
  status=0
  PATH="$fakebin:$PATH" MX_STATE_OVERRIDE="$state" MX_GUARD_GRACE=1 MX_POLL=5 MX_SIGNAL_GRACE=1 MX_CHECK_INTERVAL=999999 MX_HEARTBEAT=999999 "$WATCH" > "$out" 2> "$err" || status=$?
...
test_guard_warnings() {
  ...
  # MX_ROOT_OVERRIDE points the worktree-tangle check at a non-git dir so it stays
  ...
  CLAUDECODE=1 PI_CODING_AGENT='' MX_ROOT_OVERRIDE="$dir" MX_STATE_OVERRIDE="$state" MX_GUARD_GRACE=1 "$ROOT/bin/mx-guard.sh" 2> "$err" >/dev/null || fail "guard failed"
  ...
  # Non-git MX_ROOT keeps the worktree-tangle check inert so "fresh watcher ->
  # total silence" stays a pure assertion about watcher state.
  MX_ROOT_OVERRIDE="$dir" MX_STATE_OVERRIDE="$state" MX_GUARD_GRACE=300 "$ROOT/bin/mx-guard.sh" 2> "$err" >/dev/null || fail "guard failed"
...
test_lock_single_winner_under_concurrency() {
  ...
  i=1
  while [ "$i" -le 40 ]; do
    MX_STATE_OVERRIDE="$state" bash -c '
      . "$1"
      if mx_lock_try_acquire "$2"; then
        printf "%s\n" "$$" >> "$3"
  ...
  pass "concurrent mx_lock_try_acquire yields exactly one winner"
}
...
test_lock_steals_dead_pid_lock() {
  ...
  printf '%s\n' "$dead" > "$lockdir/pid"
  rc=0
  newpid=$(MX_STATE_OVERRIDE="$state" bash -c '
    . "$1"
    if mx_lock_try_acquire "$2"; then cat "$2/pid"; else exit 7; fi
  ' _ "$LIB" "$lockdir") || rc=$?
...
test_lock_live_steal_mutex_is_not_reclaimed() {
  ...
  MX_STATE_OVERRIDE="$state" bash -c '
    . "$1"
    mx_lock_try_acquire "$2.steal" || exit 7
    printf "%s\n" "${BASHPID:-$$}" > "$3"
    sleep 2
    mx_lock_release "$2.steal"
  ...
  out=$(MX_LOCK_STALE_AFTER=0 MX_STATE_OVERRIDE="$state" bash -c '
    . "$1"
    if mx_lock_try_acquire "$2"; then rc=0; else rc=1; fi
    printf "rc=%s held=%s lockpid=%s stealpid=%s\n" "$rc" "${MX_LOCK_HELD_PID:-}" "$(cat "$2/pid" 2>/dev/null || true)" "$(cat "$2.steal/pid" 2>/dev/null || true)"
  ' _ "$LIB" "$lockdir")
...
test_lock_does_not_steal_live_lock() {
  ...
  mkdir "$lockdir"
  printf '%s\n' "$live" > "$lockdir/pid"
  out=$(MX_STATE_OVERRIDE="$state" bash -c '
    . "$1"
    if mx_lock_try_acquire "$2"; then rc=0; else rc=1; fi
    printf "rc=%s held=%s\n" "$rc" "${MX_LOCK_HELD_PID:-}"
  ' _ "$LIB" "$lockdir")
  ...
  case "$out" in
    *"held=$live"*) ;;
    *) fail "live holder pid not reported via MX_LOCK_HELD_PID: $out" ;;
  esac
...
test_lock_late_claim_loses_after_recreate() {
  ...
  out=$(MX_LOCK_STALE_AFTER=0 MX_STATE_OVERRIDE="$state" bash -c '
    . "$1"
    owner1=$(mx_lock_owner_dir "$2") || exit 20
    ln -s "$owner1" "$2" || exit 21
    touch -h -t 200001010000 "$2" 2>/dev/null || sleep 2
    if ! mx_lock_try_acquire "$2"; then exit 22; fi
    before=$(cat "$2/pid" 2>/dev/null || true)
    if mx_lock_claim "$2" "$owner1"; then late=won; else late=lost; fi
...
test_lock_paused_mid_acquire_claim_fails_during_steal() {
  ...
  out=$(MX_LOCK_STALE_AFTER=0 MX_STATE_OVERRIDE="$state" bash -c '
    . "$1"
    owner=$(mx_lock_owner_dir "$2") || exit 20
    ln -s "$owner" "$2" || exit 21
    mx_lock_try_acquire "$2.steal" || exit 22
    steal_owner=${MX_LOCK_OWNER_DIR:-}
    if mx_lock_claim "$2" "$owner"; then late=won; else late=lost; fi
    if mx_lock_try_create "$2" "$steal_owner"; then stealer=won; else stealer=lost; fi
...
test_watch_restart_rejects_reused_pid() {
  ...
  printf '%s\n' "$dir" > "$state/.watch.lock/mx-home"
  printf '%s\n' "$WATCH" > "$state/.watch.lock/watcher-path"
  printf '%s\n' "stale watcher identity" > "$state/.watch.lock/pid-identity"
  PATH="$fakebin:$PATH" MX_HOME="$dir" MX_POLL=5 MX_SIGNAL_GRACE=1 MX_CHECK_INTERVAL=999999 MX_HEARTBEAT=999999 "$WATCH_ARM" --restart > "$out" &
  pid=$!
...
test_watch_restart_attaches_to_healthy_peer() {
  ...
  mark_pr_check_migration_complete "$state"
  node -e 'process.on("SIGTERM", () => {}); console.log("ready"); setTimeout(() => {}, 300000)' > "$peer_ready" &
  peer=$!
  i=0
  while [ "$i" -lt 50 ]; do
    grep -qF ready "$peer_ready" 2>/dev/null && break
    sleep 0.02
    i=$((i + 1))
  done
  grep -qF ready "$peer_ready" 2>/dev/null || fail "TERM-resistant peer did not report readiness"
  identity=$(MX_STATE_OVERRIDE="$state" bash -c '. "$1"; mx_pid_identity "$2"' _ "$LIB" "$peer") || fail "could not identify peer pid"
  mkdir "$state/.watch.lock"
  ...
  touch "$state/.last-watcher-beat"
  PATH="$fakebin:$PATH" MX_HOME="$dir" MX_POLL=5 MX_SIGNAL_GRACE=1 MX_CHECK_INTERVAL=999999 MX_HEARTBEAT=999999 MX_ARM_ATTACH_POLL=0.1 MX_ARM_CONFIRM_TIMEOUT=1 "$WATCH_ARM" --restart > "$out" &
  armpid=$!
  i=0
  while [ "$i" -lt 80 ]; do
...
test_watcher_self_evicts_on_lock_takeover() {
  ...
  fakebin="$dir/fakebin"
  out="$dir/watch.out"
  PATH="$fakebin:$PATH" MX_STATE_OVERRIDE="$state" MX_POLL=0.2 MX_SIGNAL_GRACE=1 MX_CHECK_INTERVAL=999999 MX_HEARTBEAT=999999 "$WATCH" > "$out" &
  pid=$!
  i=0
  while [ "$i" -lt 80 ]; do
...
test_arm_self_eviction_is_loud_without_successor() {
  ...
  armout="$dir/arm.out"
  mark_pr_check_migration_complete "$state"
  PATH="$fakebin:$PATH" MX_STATE_OVERRIDE="$state" MX_POLL=0.2 MX_SIGNAL_GRACE=1 MX_CHECK_INTERVAL=999999 MX_HEARTBEAT=999999 MX_ARM_CONFIRM_TIMEOUT=1 "$WATCH_ARM" > "$armout" &
  armpid=$!
  i=0
  while [ "$i" -lt 80 ]; do
...
test_arm_attaches_and_waits_for_live_fresh_watcher() {
  ...
  PATH="$fakebin:$PATH" MX_STATE_OVERRIDE="$state" MX_ARM_ATTACH_POLL=0.1 MX_ARM_CONFIRM_TIMEOUT=1 "$WATCH_ARM" > "$armout" &
  armpid=$!
  i=0
  while [ "$i" -lt 80 ]; do
...
test_arm_starts_and_self_heals() {
  ...
  for row in clean dead-pid; do
    ...
    PATH="$fakebin:$PATH" MX_HOME="$dir" MX_POLL=5 MX_SIGNAL_GRACE=1 MX_CHECK_INTERVAL=999999 MX_HEARTBEAT=999999 "$WATCH_ARM" > "$armout" &
    armpid=$!
    i=0
    while [ "$i" -lt 80 ]; do
      grep -qF 'watcher: started pid=' "$armout" 2>/dev/null && break
...
test_arm_hup_cleans_child_and_temp_output() {
  ...
  fakebin="$dir/fakebin"
  armout="$dir/arm.out"
  PATH="$fakebin:$PATH" MX_HOME="$dir" MX_STATE_OVERRIDE="$state" MX_POLL=5 MX_SIGNAL_GRACE=1 MX_CHECK_INTERVAL=999999 MX_HEARTBEAT=999999 "$WATCH_ARM" > "$armout" &
  armpid=$!
  i=0
  while [ "$i" -lt 80 ]; do
    grep -qF 'watcher: started pid=' "$armout" 2>/dev/null && break
...
SH
  ...
  MX_STATE_OVERRIDE="$state" "$ROOT/bin/mx-check-register.sh" task >/dev/null \
    || fail "could not register immediate-wake custom check"
  rc=0
  PATH="$fakebin:$PATH" MX_STATE_OVERRIDE="$state" MX_GUARD_GRACE=0 MX_POLL=5 MX_SIGNAL_GRACE=1 MX_CHECK_INTERVAL=0 MX_HEARTBEAT=999999 "$WATCH_ARM" > "$armout" || rc=$?
...
test_arm_waits_for_peer_beacon_after_child_stands_down() {
  ...
  printf '%s\n' "$WATCH" > "$state/.watch.lock/watcher-path"
  printf '%s\n' "$identity" > "$state/.watch.lock/pid-identity"
  PATH="$fakebin:$PATH" MX_HOME="$dir" MX_POLL=5 MX_SIGNAL_GRACE=1 MX_CHECK_INTERVAL=999999 MX_HEARTBEAT=999999 MX_ARM_CONFIRM_TIMEOUT=1 MX_ARM_ATTACH_POLL=0.1 "$WATCH_ARM" > "$armout" &
  armpid=$!
...
test_arm_fails_loud_when_no_fresh_watcher_confirmable() {
  ...
  printf '%s\n' "$live" > "$state/.watch.lock/pid"
  touch -t 200001010000 "$state/.last-watcher-beat"
  PATH="$fakebin:$PATH" MX_STATE_OVERRIDE="$state" MX_POLL=5 MX_SIGNAL_GRACE=1 MX_CHECK_INTERVAL=999999 MX_HEARTBEAT=999999 MX_ARM_CONFIRM_TIMEOUT=3 "$WATCH_ARM" > "$armout" &
  armpid=$!
...
SH
  ...
  PATH="$fakebin:$PATH" MX_STATE_OVERRIDE="$state" MX_GUARD_GRACE=0 MX_POLL=5 MX_SIGNAL_GRACE=1 MX_CHECK_INTERVAL=0 MX_HEARTBEAT=999999 "$WATCH_ARM" > "$armout" &
  first_arm=$!
  ...
  PATH="$fakebin:$PATH" MX_STATE_OVERRIDE="$state" MX_WATCH_PREDECESSOR_ARM_PID="$first_arm" MX_POLL=5 MX_SIGNAL_GRACE=1 MX_CHECK_INTERVAL=999999 MX_HEARTBEAT=999999 "$WATCH_ARM" > "$armout" &
  successor_arm=$!
  i=0
  while [ "$i" -lt 80 ]; do
    grep -qF 'watcher: started pid=' "$armout" 2>/dev/null && break
  ...
  while [ "$iteration" -lt 6 ]; do
    ...
    PATH="$fakebin:$PATH" MX_STATE_OVERRIDE="$state" MX_WATCH_CYCLE_LOG_MAX_BYTES=1400 MX_WATCH_CYCLE_LOG_KEEP_LINES=2 MX_POLL=5 MX_SIGNAL_GRACE=1 MX_CHECK_INTERVAL=999999 MX_HEARTBEAT=999999 "$WATCH_ARM" > "$armout" &
    successor_arm=$!
    i=0
    while [ "$i" -lt 80 ]; do
      grep -qF 'watcher: started pid=' "$armout" 2>/dev/null && break
...
test_stopped_watcher_is_live_but_stale_then_exit_is_classified() {
  ...
  touch -t 200001010000 "$state/.last-watcher-beat"
  MX_STATE_OVERRIDE="$state" bash -c '. "$1"; mx_pid_alive "$2"' _ "$LIB" "$watcher_pid" \
    || fail "SIGSTOP watcher was not classified as a live pid"
  if MX_HOME="$dir" MX_STATE_OVERRIDE="$state" bash -c '. "$1"; mx_watcher_healthy "$2" "$3" 300 "$4"' _ "$LIB" "$state" "$WATCH" "$dir"; then
...
test_pid_identity_is_locale_invariant() {
  ...
  # mx_pid_identity, so its output must be byte-identical regardless of the caller's
  ...
  baseline=$(MX_PROC_ROOT_OVERRIDE="$no_proc" LC_ALL=C bash -c '. "$1"; mx_pid_identity "$2"' _ "$LIB" "$live" 2>/dev/null)
  via_lc_all=$(MX_PROC_ROOT_OVERRIDE="$no_proc" LC_ALL=ko_KR.UTF-8 bash -c '. "$1"; mx_pid_identity "$2"' _ "$LIB" "$live" 2>/dev/null)
  via_lc_time=$(MX_PROC_ROOT_OVERRIDE="$no_proc" LC_TIME=ko_KR.UTF-8 bash -c 'unset LC_ALL; . "$1"; mx_pid_identity "$2"' _ "$LIB" "$live" 2>/dev/null)
  kill "$live" 2>/dev/null || true
  wait "$live" 2>/dev/null || true
  [ -n "$baseline" ] || fail "mx_pid_identity produced no baseline identity under LC_ALL=C"
  [ "$via_lc_all" = "$baseline" ] || fail "mx_pid_identity varied with exported LC_ALL (got '$via_lc_all', want '$baseline')"
  [ "$via_lc_time" = "$baseline" ] || fail "mx_pid_identity varied with exported LC_TIME (got '$via_lc_time', want '$baseline')"
  pass "mx_pid_identity is locale-invariant across LC_ALL/LC_TIME"
}
...
write_fake_proc_identity() {
  ...
  printf 'bash\0/path with spaces/mx-watch.sh\0--flag\0' > "$proc_root/$pid/cmdline"
}
...
test_linux_pid_identity_ignores_wall_clock_and_detects_pid_reuse() {
  ...
  before=$(MX_PROC_ROOT_OVERRIDE="$proc_root" MX_STATE_OVERRIDE="$state" bash -c '. "$1"; mx_pid_identity "$2"' _ "$LIB" "$pid") \
    || fail "could not read initial fake Linux process identity"
  printf 'btime 1784094016\n' > "$proc_root/stat"
  after_time_jump=$(MX_PROC_ROOT_OVERRIDE="$proc_root" MX_STATE_OVERRIDE="$state" bash -c '. "$1"; mx_pid_identity "$2"' _ "$LIB" "$pid") \
  ...
  after_pid_reuse=$(MX_PROC_ROOT_OVERRIDE="$proc_root" MX_STATE_OVERRIDE="$state" bash -c '. "$1"; mx_pid_identity "$2"' _ "$LIB" "$pid") \
