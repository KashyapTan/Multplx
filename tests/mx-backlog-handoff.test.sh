#!/usr/bin/env bash
# tests/mx-backlog-handoff.test.sh - full item-block handoff (header + indented body).
#
# The happy single-line path and broad safety refusals live in the daemon
...
setup_homes() {
  ...
  seed_daemon_home_marker "$subhome" "$id"
  local sub_abs
...
EOF
  ...
  MX_HOME="$home" "$ROOT/bin/mx-backlog-handoff.sh" design body-item >/dev/null \
...
## Done
- [x] old-task - delivered - local main (merged 2026-07-01)
EOF

  local expected_block
  expected_block=$(extract_item_block "$home/data/backlog.md" section-tail)

  MX_HOME="$home" "$ROOT/bin/mx-backlog-handoff.sh" design section-tail >/dev/null \
...
test_body_moves_when_last_lines_of_file() {
  ...
  MX_HOME="$home" "$ROOT/bin/mx-backlog-handoff.sh" design eof-item >/dev/null \
...
test_eof_body_before_seeded_destination_section_keeps_boundary() {
  ...
  MX_HOME="$home" "$ROOT/bin/mx-backlog-handoff.sh" design seeded-eof-item >/dev/null \
...
test_untouched_eof_line_preserves_terminator() {
  ...
  MX_HOME="$home" "$ROOT/bin/mx-backlog-handoff.sh" design move-item >/dev/null \
...
EOF
  ...
  MX_HOME="$home" "$ROOT/bin/mx-backlog-handoff.sh" design idem-item >/dev/null \
  ...
  local out
  out=$(MX_HOME="$home" "$ROOT/bin/mx-backlog-handoff.sh" design idem-item 2>&1) \
  ...
  [ "$dest_after" = "$(cat "$sub/data/backlog.md")" ] \
    || fail "idempotent re-run mutated the daemon backlog"
...
EOF
  ...
  if out=$(MX_HOME="$home" "$ROOT/bin/mx-backlog-handoff.sh" design malformed-body 2>&1); then
  ...
  cmp -s "$destination_before" "$sub/data/backlog.md" \
    || fail "noncanonical-continuation refusal changed the daemon backlog"
...
EOF
  ...
  MX_HOME="$home" "$ROOT/bin/mx-backlog-handoff.sh" design ha-codex-fast-default-4e >/dev/null \
...
EOF
  ...
  MX_HOME="$home" "$ROOT/bin/mx-backlog-handoff.sh" design multi-para >/dev/null \
  ...
  dest_after=$(cat "$sub/data/backlog.md")
  local out
  out=$(MX_HOME="$home" "$ROOT/bin/mx-backlog-handoff.sh" design multi-para 2>&1) \
...
EOF
  ...
  MX_HOME="$home" "$ROOT/bin/mx-backlog-handoff.sh" "$id" paren-item >/dev/null \
...
test_registry_home_missing_field_fails_cleanly() {
  ...
  mkdir -p "$home/data" "$home/state"
  seed_daemon_home_marker "$sub" "$id"
...
EOF
  ...
  local out rc=0
  out=$(MX_HOME="$home" "$ROOT/bin/mx-backlog-handoff.sh" "$id" orphan-item 2>&1) || rc=$?
  ...
  [ ! -f "$sub/data/backlog.md" ] || ! grep -q 'orphan-item' "$sub/data/backlog.md" 2>/dev/null \
    || fail "item appeared in daemon backlog despite missing home"
