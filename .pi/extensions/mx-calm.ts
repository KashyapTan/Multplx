// Multplx's home-persistent Pi transcript presentation toggle.
//
...
import { installCalmAssistantLayout } from "./lib/mx-calm-assistant-layout.ts";
import { installCalmOperationalUserLayout } from "./lib/mx-calm-operational-user-layout.ts";
import {
  calmPresentationHides,
  calmPresentationIsActive,
  MULTPLX_CALM_PRESENTATION_EVENT,
  registerMultplxSyntheticPresentation,
  setCalmPresentation,
...
export default function (pi: ExtensionAPI) {
  ...
  const configDirectory = process.env.MX_CONFIG_OVERRIDE || resolve(fmHome, "config");
  ...
  const publishPresentationState = (): void => {
    pi.events.emit(MULTPLX_CALM_PRESENTATION_EVENT, {
      active: calmPresentationIsActive(),
      stockExportRendering: exportRendering,
    });
  };

  registerMultplxSyntheticPresentation(pi);
  ...
  function registerBuiltIn<TParams extends TSchema, TDetails, TState>(
    ...
    if (!originalRenderCall || !originalRenderResult) {
      throw new Error(`Multplx calm mode requires both render slots for Pi built-in tool ${original.name}`);
    }
  ...
  pi.registerCommand("calm", {
    description: "Toggle Multplx's supported conversation-only transcript presentation.",
