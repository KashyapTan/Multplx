...
function runSessionstartNudge(): string {
  const result = spawnSync(`${root}/bin/mx-sessionstart-nudge.sh`, [], { encoding: "utf8" });
  if (result.status !== 0) return "";
  return result.stdout.trim();
}
...
function runGuard(): Promise<{ code: number; stderr: string }> {
  return new Promise((resolveResult) => {
    const child = spawn(`${root}/bin/mx-turnend-guard.sh`, {
...
// PreToolUse seatbelts (bin/mx-arm-pretool-check.sh, docs/arm-pretool-check.md;
// bin/mx-cd-pretool-check.sh, docs/cd-guard.md). Both piggyback on this same
...
function runPretoolCheck(command: string): Promise<{ code: number; stderr: string }> {
  return runChecker("mx-arm-pretool-check.sh", command);
}

function runCdCheck(command: string): Promise<{ code: number; stderr: string }> {
  return runChecker("mx-cd-pretool-check.sh", command);
}
