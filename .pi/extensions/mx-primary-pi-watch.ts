...
import {
  ...
  MULTPLX_CALM_PRESENTATION_EVENT,
} from "./lib/mx-calm-visibility.ts";
...
const fmHome = process.env.MX_HOME || process.env.MX_ROOT_OVERRIDE || root;
const fmRoot = process.env.MX_ROOT_OVERRIDE || root;
...
const retryMaxMs = positiveInteger("MX_WATCH_REARM_RETRY_MAX_MS", 4000);
const retryLimit = positiveInteger("MX_WATCH_REARM_RETRY_LIMIT", 5);
const armReadyTimeoutMs = positiveInteger("MX_PI_ARM_READY_TIMEOUT_MS", 12000);
const armRetireTimeoutMs = positiveInteger("MX_WATCH_ARM_RETIRE_TIMEOUT_MS", 1000);
const repairOnlyHint = "call mx_watch_arm_pi again only after a later notification says the cycle is missing, failed, or unhealthy";
...
function classifyClose(stdout: string, stderr: string, code: number | null, signal: NodeJS.Signals | null): CloseClassification {
  ...
  if (code && code !== 0) {
    return {
      kind: "failure",
      message: `watcher: FAILED - mx-watch-arm.sh exited ${code}${combined ? `\n${combined}` : ""}`,
    };
  }
  return {
    kind: "failure",
...
export default function (pi: ExtensionAPI) {
  ...
  pi.events?.on?.(MULTPLX_CALM_PRESENTATION_EVENT, (data) => {
  ...
  async function sendWake(message: string): Promise<void> {
    const content = encodeMultplxOperationalInput(
      "watcher",
  ...
  function startArm(predecessorArmPid = ""): ArmResult {
    ...
    if (ownership === "missing") {
      return {
        ok: false,
        message: "watcher: not armed - no live session holds the lock; run bin/mx-session-start.sh to reclaim it, then call mx_watch_arm_pi to re-arm",
      };
    }
    markLoaded();
    if (child) {
    ...
    const env = {
      ...process.env,
      MX_HOME: fmHome,
      MX_ROOT_OVERRIDE: fmRoot,
      MX_CONFIG_OVERRIDE: config,
      MX_WATCH_ARM_SCRIPT: armScript,
      MX_WATCH_PREDECESSOR_ARM_PID: predecessorArmPid,
    };
    const armChild = spawn("bash", ["-lc", "exec \"$MX_WATCH_ARM_SCRIPT\" --restart"], {
      cwd: fmRoot,
      env,
  ...
  pi.registerTool?.({
    ...
    renderCall: (_args, theme, context) => {
      ...
      if (calmPresentation.stockExportRendering) {
        return new Text(theme.fg("toolTitle", theme.bold("mx_watch_arm_pi")), 0, 0);
      }
      const state = context.state as WatchToolShellState;
      state.call = new Text(theme.fg("toolTitle", theme.bold("mx_watch_arm_pi")), 0, 0);
