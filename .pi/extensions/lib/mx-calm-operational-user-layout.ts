...
import { calmPresentationHides } from "./mx-calm-visibility.ts";
import { classifyMultplxCurrentOperationalText } from "./mx-operational-input.ts";
...
export function installCalmOperationalUserLayout(): void {
  ...
  const isOperationalInput = (text: string): boolean => {
    ...
    return (
      classifyMultplxCurrentOperationalText(text) !== undefined ||
  ...
  if (typeof originalAddMessageToChat !== "function") {
    throw new Error("Multplx Calm requires Pi InteractiveMode.addMessageToChat");
  }
