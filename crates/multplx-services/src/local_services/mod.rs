...
pub const VIZ_HELP: &str = "Start, inspect, and stop the disposable Multplx dashboard.\n\nUsage:\n  mx-viz.sh serve\n  mx-viz.sh status\n  mx-viz.sh stop\n  mx-viz.sh --help\n\n`serve` is singleton and idempotent per MX_HOME. It binds loopback only,\ntries MX_VIZ_PORT (default 4890) plus 19 upward ports, prints the URL only,\nand never opens a browser. The server exits after MX_VIZ_IDLE_SECS (default\n1800) without a request. Snapshot polling defaults to MX_VIZ_POLL_MS=2500.\nMX_VIZ_REFRESH_SECS=2 controls the shared single-flight snapshot cache, and\nMX_VIZ_COMMAND_TIMEOUT_MS=10000 bounds snapshot, doctor, and timeline readers.\nStale last-good state remains available while refresh work runs or fails;\n`/api/meta` exposes cache/refresh counters and bounded latency measurements.\n\nRun record contract, state/.viz/server.run, mode 0600:\n  version=1\n  home=<canonical MX_HOME>\n  state=<state directory>\n  port=<bound loopback port>\n  pid=<server pid>\n  pid_identity=<portable process identity>\n  token=<random cleanup binding, never served>\n  started_at=<UTC ISO-8601 time>\n`stop` signals only a live identity-matched process. A dead or reused PID\ncauses record cleanup without signaling.\n";

pub const VPLAN_HELP: &str = "Create, serve, inspect, and stop explicitly requested one-shot vplan review artifacts.\nAn HTML artifact alone never starts vplan, and vplan is not a startup dependency.\n\nUsage:\n  mx-vplan.sh new <file> [--project-root <dir>]\n  mx-vplan.sh review <file> [--project-root <dir>] [--task <id> --brief-revision <n> [--attempt <id>]]\n  mx-vplan.sh comments <file> [--project-root <dir>]\n  mx-vplan.sh stop <file> [--project-root <dir>]\n  mx-vplan.sh --self-check\n  mx-vplan.sh --help\n\n`new` copies the vendored seed template and rewrites only its Mermaid asset\npath relative to the destination. `review` validates optional assets lazily,\nrequires an artifact inside the selected project root, binds comments to the\nartifact hash present at review start, starts the loopback-only Rust service,\nrecords its PID identity under state/.vplan/, and prints the bound URL.\n`--task` requires a canonical current allocation and exact accepted brief;\n`--attempt` may be omitted to derive the current attempt. The first port is\nMX_VPLAN_PORT (default 4870), with 19 upward fallbacks. Confirm or the idle\ntimeout (MX_VPLAN_IDLE_SECS, default 1800) removes the run record and exits.\n`comments` prints the persisted #vplan-comments array as formatted JSON.\n`stop` signals only a live process whose PID identity and review token still\nmatch the artifact's run record; stale records are cleaned without signaling.\n";
...
fn record_live(record: &BTreeMap<String, String>, expected_home: Option<&Path>) -> bool {
    ...
    if !matches!(record.get("version").map(String::as_str), Some("1" | "2")) {
        return false;
    }
